...
//
//	撤掉:代价【有界】—— 花掉几个撤单池的名额(台账 #197 分池后撤单不再跟下单抢),
//	      外加走平。最坏就这些。
...
// 是在单边挂着 —— 那就不该继续挂。
//
// 撤单走【撤单池】+ gateClassCritical(可等名额、可退避重试、绕过熔断);并且
// cancelAll 的读挂单那一步也已经改到同一档(types.go CancelPathReader)——
// 它花的是查询池里留给撤单路径的那几个预留名额,否则站下动作本身会在熔断下空转。
// ─────────────────────────────────────────────────────────────────────────────
...
//
// 【为什么必须节流】探针是一发报价类的读(OpenOrders),吃查询池的名额。台账 #197
// 分池之前,那个池就是下单的 10/10s、报价类只看得见 7 个 —— 按 refresh(默认 1 秒)
// 每轮打一发,熔断一解除就是 1 发/秒,10 秒窗口里 7 个名额全被探针吃光。端到端跑出来
// 的结果就是:状态机宣布恢复了,quote() 的第一发 OpenOrders 却撞上"名额耗尽",
// 连着几轮报不出价 —— 恢复动作自己把恢复堵住了。
//
// 分池之后查询池宽得多(默认 200/窗口),这条节流已经不是"不得不",但保留:
// 它的另一半作用是别在熔断期间白打请求,而那半条与池子多大无关。代价是恢复的探测
// 粒度是一个窗口(默认 10s)—— 反正恢复本来就要等满 standUpHealthyWindows 个窗口的
// 连续健康,这点粒度落在同一个量级里。
