...
//
// 后果实测（2026-09-19T17:0xZ 上线后复量，1600 行窗口、threshold=3）：
//   - 失效面：191 个孪生对里 **47 对（24.6%）的 open_time 不相等** ⇒ 旧键只并掉了 3/4。
//   - 方向：**只有虚高**。计数被更正的 18 个（APT 5→3、NEAR 6→5、CROSS 4→3、STRK 5→4、
//     牛来 5→3、XTZ 5→4、USELESS 5→4、EVAA 5→4、LSK 4→3、AR 4→3 …）。
//   - **隔离名单不变（13 → 13，零净新增、零误挡）** ⇒ 这不是"漏挡保护"的修复，
//     是"日志里那个连亏=N 会误导排障"的修复。
//
// ⛔ 撤回一条我此前写在这里的错误结论：我曾写「虚低：AKE/DRIFT/PONS 报 2、真值 3，
// 净效果是漏判 3 个 symbol」。上线后逐条复量**复现不出来** —— AKE 旧键=2、新键=2 完全一致；
// 且全窗口 **正负异号的孪生对 = 0** ⇒ 旧键从未把一笔真亏损洗成盈利，不存在虚低通路。
// 那条结论是我在没有逐条对质前写下的推断，已作废。
//
// 修法：只跨**记账来源**配对合并（同一来源的两条行绝不合并，那是两次真实平仓），
// 且平仓时间相差在容差内。孪生对的 close_time 实测只差 3~7 秒，120s 留足余量。
