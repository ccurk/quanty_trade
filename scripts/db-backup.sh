#!/bin/bash
# ============================================================================
# ⚠️ 定性（2026-09-09 老徐，台账 #68 / #116）：**dump 那一半退役，异地那一半保留待接。**
#
#   退役的一半（Step 1~3）：它要求 host 上装 mysqldump，还要把 MySQL root 口令
#   落进 /etc/quanty-backup.env。现在真正在跑的 /usr/local/bin/quanty-backup.sh
#   走 `docker exec` + 容器自己的 env，**两样都不需要**（cron 每天 03:40，
#   产出 /var/backups/quanty/*.sql.gz，实测 34.4MB/份，保留 14 天）。
#   同一件事两套实现、其中一套还多要一份 root 口令，不是"修一修"的问题。
#
#   保留的一半（Step 4 + 清理 + TG 告警）：异地副本这件事本身还没有做（台账 #68 ②
#   零异地副本，盘挂了三份备份和数据一起没）。这段 rclone 逻辑是待办 #30/#31 的参考实现。
#   接的时候按 #31 的结论改两点：① 输入改成 quanty-backup.sh 已经产出的那份 .sql.gz，
#   不要自己再 dump 一遍；② 加密从 `gpg --symmetric` 改**非对称** —— 对称口令和备份
#   存在同一台机器上，机器没了口令一起没，机器被入侵两样一起被拿走。
#
#   本脚本不做修改、也不加拒绝执行的闸：它是只读的（dump + 上传），跑起来只会在
#   缺 rclone / 缺 env 时以明确的 fatal 停下，不会破坏任何东西。
#   （对比 db-restore.sh：那个会 DROP 生产库，所以那个加了硬拒绝。）
# ----------------------------------------------------------------------------
# QuantyTrade 加密备份脚本
# ----------------------------------------------------------------------------
