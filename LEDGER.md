...
| 2026-09-18 | **止损 ROI 4% 直令(live 17:5x-18:0x)**: owner 原话"止损在收紧 调整到 3%"→"变成 4%"→"不可能 我看现在的单子都是在 10% 亏损的时候才平掉"。**口径=ROI(UI 显示值)**: 17:59 前 22 笔饥饿止损 roi −8.8~−9.6%(键 0.075=7.5% ROI+10s 轮询过冲)=UI −9~−10%=CC 所说 0.75% 价×10 倍,同一事实两种单位。CC 17:56/17:58 误读为价格距离落 0.30/0.40(v57/v58)→owner 17:59:41Z 自落 **hunger_stop_loss_pct=0.04**(v59;admin PATCH),CC 18:03 登记(v60)。首两笔 CHIP roi −4.14%/SIREN −4.71%(过冲 0.1~0.7pp)。证据: 15:25→17:56 n41 −30.99 wr44%<be64% 饥饿SL 22/−54.72;p6 封段 n71 33 止损 −91.8 vs 31 赢 +61.8。**现货 1m 反事实(33/71 有数据,+0.15% 滑点,基差误差≤0.36% 与门槛同量级=只看方向)**: 0.4% 止损 −21.11 vs 实际 −2.62——亏单 −31.4→−22.1 但 8/18 赢单先回撤 0.4% 被杀(+14.5→−13.2,含 G +7.34)。已 TG: 目标"单笔别到 −10%"三条路 ①ROI 4%(现) ②pct 0.25→0.125(每笔亏减半不动胜率,CC 推荐) ③看 30 笔再定;pct 为 owner 键。预注册 _exp_cc.p7(段均净≥0;eval 00:00Z 或 n≥30;劣化只报警)。引擎钳 0.3/lev(strategy_position.go:485/884)与 4% ROI 无关(饥饿 0.4% 远在交易所 SL 之前)。 |

| 2026-09-19 | **金额止损直令(live 03:3x)**: owner 原话"重新搞一下止盈止损，百分比看起来有点问题，用亏损金额来算，止损设置为亏损金额不能超过 3-5u，止盈还是百分比 15～20，最大持仓时间不变"。引擎无金额止损键(resolveTPSLFromROI 价距=pct/lev; 饥饿/本地监控比 ROI)→03:34 临时按现仓位换算 tp0.15/sl0.30(交易所钳 0.3/lev)。**随即被第二条直令的反事实取代**(下行);金额帽转为仓位规则: 单笔止损额=名义×atr_sl_mult×ATR≤4U ⇔ 名义≤4U/(2×ATR)(现 pct0.0625 名义≈100U→≈1.3U 在帽内; pct0.25 超帽→须 max_initial_margin_usdt≈30 或 Go 风险定量 sizing CC-22)。 |
...
### 亏损侧
- **[新增 09-19 03:5x] 第一波=ATR 比例(owner 案例, 优化常备知识)**: n=206(09-18 04:00Z→09-19 03:27Z, 1m 永续代理 Bitget/Gate, 入场价 vs bar 收盘中位误差 0.17% p90 0.77%): 45m 内到 +1.5% 的赢单 111/206=54%, **到达前最大逆行 中位 0.61×ATR / p75 1.34×ATR / p90 2.43×ATR**(=−0.47/−1.10/−1.84% 价); 0.4% 刀杀掉其中 56%, 0.75% 35%, 1.0% 25%, 1.5% 18%, 2.0% 8%; 紧刀两段实际亏单 81 笔中 40% 被砍后 45m 内 MFE≥+1.5%; 首 5 分钟 MAE 中位 −0.53%(全体)/−0.90%(亏单). **推论: 止损≥2×ATR(1m,14) 才扛得过第一波; 固定%/固定金额止损不随币的波动缩放→多币池上全负(§6 网格).** 金额上限只能靠仓位实现(名义≤cap/(2×ATR)). 方法=cf3.py(scratch; 网格 SL×TP×45m, 同 bar 双触计 SL, 费 0.1%).
...
- **[新增 09-18 20:2x] 饥饿刀首查时点+DB 漏行+CYPH TradFi(日志定案)**: quick_trade_monitor 首查在 hunger_after 分钟整点后 10s 轮询→hunger_sl 0.04(0.4%价@10x)在入场后 1 分钟内的闪跌上直接过冲: M 19:53 roi −11.6%/1m03s、PIEVERSE 20:03 −7.8%/1m04s、11 次触发 roi −4.0~−11.6%(均过冲 +0.2~0.4pp,极端 +0.8pp)→ROI 口径的止损数字≠实际亏损上限;**DB closed 懒生成漏行再证**: income 1h by_symbol 有 STRKUSDT 7 fills −2.18/SAGAUSDT 12 fills −1.25,DB closed48 无行→逐笔 n 以 income by_symbol 补齐;**CYPH/USDT -4411 TradFi-Perps 首现(18:02/19:41 两次)→规则"任一现-4411即隔离"生效,已入 bl(v62)+rotate remove**。
...
| CC-11 | 巡检 | pct 配置下每轮报: 保证金占用峰值/最大单仓名义/最大单笔SL亏/6h,24h钱包%;DS breaker 若砍pct→按owner令处置并TG(裸改簿) | 09-18 13:1x | open(**读数@09-19 00:1x: 最大单仓名义 557U(pct0.25 期)/DS 砍后 275U;最大单亏 M −7.81;6h 并发峰 3;保证金峰≈161U(19:4x);6h income 净 −13.39(毛 +1.39 费 −14.78)=−8.1%(基 165.4 含入金 58.5);24h −49.34(毛 −8.34 费 −40.59)=−19.0%(基 259.8 含入金 142.06)/−41.9%(入金前基 117.8);钱包≈210.5**) |
...
| CC-9 | DS越界簿 | admin mcp 未声明改动=协议7越界: #1 09-16 22:10 10→6 / #2 09-17 13:15 6→2 / #3 09-18 06:30 10→4 / #4 09-18 15:22 10→6(随 breaker 砍 pct 0.25→0.125;CC 15:25 按 owner 令恢复 v56) / **#5 09-18 21:22:13Z lock-1789766533 砍 pct 0.25→0.125(声明,frozen)+mcp 10→6(未声明);why=净 −43.09U<−4U∧wr38%<45%**;裁决@09-19 00:1x=不硬抢(CC-9 既定'再犯=不硬抢';RECOVER 门关 6h/24h 双负;mcp6 不绑定:6h 并发峰 3;硬恢复=每 3h 被再砍乒乓)→TG owner 三选一 A 硬恢复 B 接受至 24h 净>−4U C owner 改 qt_breaker 阈值(钱包比例)后恢复;owner 令恢复即恢复+TG | 09-18 08:1x | open(候 owner A/B/C) |
...
| CC-21 | 评判 | **p9 ATR 括号**(_exp_cc 03:41:02Z; 重启 03:41:14Z 首单起): atr_sl 2.0×/atr_tp 2.5×/45m, tp,sl pct 0, hunger 0.30/0.30 兜底, trailing 关, BE 关, max_atr_pct 6; n≥40 或 12:00Z: 费后均净≥0 ∧ wr≥42% ∧ TP 命中≥35% ∧ 首5分钟SL占比<20%; worsen(只报警) n≥20∧费后≤−8U 或 首5分钟SL≥40%→备选 2×/3× 或 2.5×/2.5×; 回滚集 _exp_cc.rollback | 09-19 03:4x | open(**自查点 2/3 @04:36Z: 括号生效 9/9 fills 比 1.25, ATR 0.51~1.38%(3 笔>0.8=门已回滚); 段 n7 毛 −2.45 wr14%(G 多 TP +4.71; 6 空 SL, 首5分钟 SL 3/7) 费后 −0.44/笔 hold中位 6.2m 速率 184/日; 最大单亏 XTZ −2.17(1.85% 止损×117U 名义, 在帽内); 死法日志: max_hold 1; DS 0 PATCH; feed 276 ∩=∅; 429×7 无-1003; 1h income 毛 −5.81 费 −0.90; n<20 不裁; 空头 6/7 全 SL 与 §4 H-0919b 同向, 继续观察**) |
...
| **09-18 p5/p6/p7 终读** | p5 KEEP 3/3;p6 封段 n71 −34.75;**p7(hunger_sl 0.04, open≥17:59:41Z) 终读@09-19 00:1x n46 179笔/日 毛+4.26 均+0.093 wr52% be50% 赢均+2.09 亏均−2.09 hold中位2.4m 名义中位285U;饥饿SL 带 15/−26.26+首分钟过冲深亏 4/−17.06(M −14.3%/AKE −14.8%)=41%;费/笔0.32→费后 −0.23/笔 🔴 北极星 −41U/日;多35/−2.61 空11/+6.87;裁决=owner 值 KEEP(无回滚权)+病根定位 ATR>0.8% 子集→FIX max_atr_pct**;全文=git 5096ca1版§6 | 09-19 00:1x | 见下行交互表 |
| **09-19 ATR×固定价格刀 交互(四段,tpsl 日志 sl距/2.5 反推入场 1m ATR%)** | ATR>0.8% vs ≤0.8%: lev2 段(<08:12, 饥饿45m 2.5%价) n61 −4.70 wr57% vs n102 −31.85 wr39% ‖ P3 段(08:12→13:39, 饥饿45m+ATR SL) **n16 +22.30 wr69%** vs n36 +4.13 wr44% ‖ owner 紧出场段(13:39→, 饥饿自第1分钟 0.75%→0.4%价) **n35 −31.66 wr31%** vs n84 −3.31 wr51% ‖ p7 段(0.4%) **n16 −22.09 wr25%** vs n30 +26.35 wr67%(费后 +0.56/笔);阈值 0.7/0.9/1.0 方向一致;机制=固定刀<0.5×ATR=噪声穿刺;多头 ATR>0.8 n28 −30.11 wr29% 是『多头失血』的主体 | 09-19 00:1x | 行动=max_atr_pct 0.8(CC-20);owner 若改 ATR 缩放止损(CC-15 E)→放宽回 6;再证判据=新段 ATR≤0.8 wr≥55% |
| **09-19 03:5x 止损×止盈×45m 反事实网格(n206, 费后 0.1%, 实际名义, 1m 永续代理)** | **SL2.0×ATR/TP2.5×ATR +54.0(均+0.26 wr49% sl87/tp94/time25)** ‖ 2×/3× +37.0 ‖ 2.5×/2.5× +19.6 ‖ 2×/2× +14.5 ‖ 固定% 最好 SL1.0%/TP3.0% +11.7(wr32%) ‖ 参照 SL0.4%/TP1.5%(现刀) −43.6 ‖ SL3.0%/TP1.5%(03:34 临时) −73.9 ‖ 金额帽 cap3/4/5U+TP1.5% −59.6/−66.3/−39.8 ‖ 实际同批 毛 −8.8 费后 −54.0 wr48% ‖ 子窗≥12:00Z(n168) 同序 2×/3× +35.2, 2×/2× +13.7, 0.4/1.5 −43.8 ‖ 多 n136 2×/2× +36.4 vs 空 n70 −22.0 | 09-19 03:5x | 行动=p9(CC-21); 注: 无滑点/同bar双触计SL/代理噪声 0.17%→+54 为上界, 排序稳健 |
...
## 7. 运行日志（每轮一行，新行追加在表首）
| 2026-09-19 03:3x-03:5x | **交互轮·owner 两条直令(金额止损→第一波反事实)**: DS breaker #6 03:22 pct 0.125→0.0625(不硬抢, owner 在线裁 A/B/C);CC-20 n40 毛 −5.67 费后 −0.33/笔 FAIL→回滚 max_atr_pct 6;03:34 临时 tp0.15/sl0.30→owner 第二令: 226 仓+81 币 1m 永续行情(Bitget/Gate)→206 笔反事实: **赢单到 +1.5% 前 MAE 中位 0.6×ATR p90 2.4×ATR, 0.4% 刀杀 56%; 固定%/金额刀全负(3-5U 帽 −40~−66), SL2×ATR/TP2.5×ATR/45m +54 vs 实际 −54**→p9 落地(atr 2.0/2.5, tp/sl pct 0, hunger 0.30/0.30 兜底, trailing/BE 关, max_hold45, max_atr 6)+force 重启 03:41:14Z+feed 300→276 ∩=∅;6h income 净 −17.5(毛 −4.9 费 −12.7);金额帽→仓位规则(名义≤4U/(2×ATR));CC-22 Go 风险定量 sizing 候 owner ‖ DS: 1 PATCH(#6),lock 至 09-20 03:22,_ai_task_ds 空;下轮=CC-21 n≥40/12:00Z 评判+owner A/B/C+trailing 变体 |
| 2026-09-19 00:1x-00:4x | 定时轮·p7 评判+ATR 门 FIX+DS #5 记簿: 管道活(active DOS);钱包≈210.5(avail 152.8);**6h income 净 −13.39(毛 +1.39 费 −14.78)=−8.1% 报警;24h −49.34(毛 −8.34 费 −40.59)=−19.0%(含入金基)/−41.9%(入金前基)**→只 TG(owner 令);p7 终读 n46 毛+4.26 均+0.093 wr52% 饥饿SL 41% 费后 −0.23/笔 🔴;**病根(四段交互表 §6)=owner 0.4% 固定刀×ATR>0.8% 币(13:39 后 n35 −31.66 wr31%,同批币在 45m 饥饿段 +22.30 wr69%)**;执行①ROUTE LSK 16/−5.35+STRK 7/−5.21 bl35 ②max_atr_pct 6→0.8+force 重启 00:25:26Z(owner atr 2.0/1.5 同步生效, CC-12 结案)+feed 300→276 ∩=∅+_exp_cc CC-20 预注册;DS: 21:22 breaker 砍 pct0.125/mcp6(#5 未声明)→不硬抢(门关/不绑定/乒乓)TG owner A/B/C;费率病根 CC-19 maker 入场 M 候选;有效门槛 0.36✓;宏观 FGI71 BTC +6.0% 市值 +2.5%;频率 179/日 vs 38.7|均净 +0.09 vs 3×费 0.96 🔴|北极星 −41U/日 ‖ DS: 1 PATCH(越界 #5),lock 至 21:22,_ai_task_ds 空;下轮=CC-20 n≥30/06:00Z 评判+DS 04:xx 槽对账+owner A/B/C 与 D/E 回复 |
| 2026-09-18 20:1x-20:3x | 定时轮·止血未达成(owner 令不缩表)+ROUTE 7 币: 管道活(active M 空 462U);avail 168.3(3h 入金 +58.5);**6h income 净 −49.3U(毛 −25.8 费 −23.5)=−31% 基 159U;24h −43.6=−20.7%(含入金基 211.9)/−62%(入金前基 69.9);09-18 日≈−75U 净,入金 +142.06**;报警线双触→按 13:4x 令只 TG🆘;p7(hunger_sl 0.04) DB n15+3 漏行 毛≈−3.3 wr47% be45% 饥饿SL 11/−27.2(触发 roi −4~−11.6%,首查第 1 分钟整点=闪跌过冲) 赢 7/+24.4 全 trailing/BE 费/笔 0.38 🔴;病根=0.4% 价刀(0.25~0.8×ATR)+10s 过冲+0.5×ATR 追踪截赢→wr≈be 毛≈0,费 0.4U×170 笔/日;有效门槛 0.35✓;ROUTE v62 SYN/牛来/PIEVERSE/USELESS/AIN/MARSCOIN+CYPH(-4411)→bl33 feed274 ∩=∅;_exp_cc v63 留言板 v64;宏观 FGI56 BTC +5.9%(大盘涨、多头 wr37%=微结构非 regime);频率 p7 172/日 vs 38.7|均净 −0.18 vs 3×费 1.15 🔴|北极星 −197U/日(6h 口径) ‖ DS: 0 PATCH,lock 至 09-19 15:22,_ai_task_ds 空;下轮=p7 n≥30/00:00Z 评判+owner 对 D/pct 减半 回复+DS 22:10 槽(若写 cd1800/min_conf0.45→留言板+TG,owner 重启会静默生效) |
| 2026-09-18 17:5x-18:1x | 全文=git 2fcc0da版(交互轮·owner 止损 ROI 4% 直令;CC 误读价格距离 v57/58→owner 自落 0.04 v59;15:25→17:56 n41 −30.99 饥饿SL 22/−54.72;反事实 0.4% 止损 −21.11 vs 实际 −2.62) | | |
| 2026-09-18 15:1x-15:3x | 全文=git 2fcc0da版(定时轮·DS 熔断纠回 v56+CC-14 反事实否决+ROUTE NEAR/STAR v55) | | |
| 2026-09-18 14:1x-14:3x | 全文=git 58e5e5c版(自查点 P3/cc8→p6 worsen 首读+入金 83.56) | | |
| 2026-09-18 13:5x-14:0x | 全文=git 58e5e5c版(owner max_hold45+键表 v50/v51) | | |
| 2026-09-18 13:2x-13:5x | 全文=git fa4cd1d版(owner 出场收紧直令+prompt v5.3) | | |
| 2026-09-18 13:1x-13:3x | 全文=git fa4cd1d版(owner "#8傻逼"+"去掉了";owner_no_autocut) | | |
| 2026-09-18 13:0x-13:2x | 全文=git fa4cd1d版(owner UI 自改 pct0.25/mcp20+重启) | | |
