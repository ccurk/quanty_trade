...
|---|---|---|---|
| 后端DB层挂死watch(→§3@08-27 04:4x;**结案@08-27 20:2x**) | **已恢复:总冻结04:31→≈18:55Z≈14.4h(n=9段签名恒定)**;恢复=owner静默部署ed0c160(阶梯②:FF并main+部署;行为实证health/db 200+login1.3s@20:12,TG零回复);恢复时刻界定:18:15旧二进制(health/db 404)→18:59:41 ARIA开仓(新引擎已交易)→19:04:33首IDLE;IDLE缺口01:41→19:04含04:01-04:31复活窗零IDLE=引擎日志/信号链复活窗亦未恢复(与04:31 active=0互证);恢复首轮协议执行✓@20:1x-2x:active0×3✓/盲窗重建04:31→18:55零平仓行✓/恢复后仅ARIA0.75常规笔−0.186(硬超时60m,sid空=#72候部署形态,池归属main)/feed漏14热清→86✓/#74落地✓/旧fade未误启✓ | 08-27 20:2x | 复发判据:health/db非200或login>5s即报;readTimeout日志可定位卡死query;拖死源未定位(取证未做)=复发候;owner取证清单留档 |
| EXP提频0.600档·信号无成交watch(新@08-27 20:2x) | n=2零成交:恢复后PROM空18:59:29 conf0.6000+MVLL空20:06:33 conf0.6000均无closed/active行(1h窗复核✓);PROM可解释=择优攒批被12s后ARIA0.7500压过;MVLL原因不明;pre-hang档笔TAC00:17曾正常成交;附记:ARIA信号价0.04009 vs成交0.0354(−11.7%,n=1,疑重启warmup错位或追跌滑点) | 08-27 20:2x | 判据:档信号累计n≥4零成交→查下单层日志(q=币名)定位拒因;若档系统性无法成交→#74评期空转,_exp转诊断轨 |
