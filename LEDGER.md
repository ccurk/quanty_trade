...
| # | 类型 | 内容（含完整意图） | 登记轮 | 状态 |
|---|---|---|---|---|
| 81 | bl种子同步 | main symbol_blacklist 14→15(+"USELESS/USDT"斜杠形态)=blacklist_sync@09-02 21:18 plan(隔离13+TUT+USELESS);执行窗=下一个main自然重启/PATCH窗(bl=重启免疫互斥层;当前互斥由feed不相交+引擎闸兜底,trend种子已落库无缺口);**09-03 00:2x排自查点01:15Z抢窗**(T仓max_hold~01:07后可空;流程=验全账户空仓→stop main[poll stopped]→PATCH symbol_blacklist=15项(现14+USELESS/USDT)→start→复读bl15+running;≤3retry,未成留下轮) | 09-02 21:1x | open |
...
| 09-03 00:1x-00:3xZ | 例行轮(max) | −1.37(n1)/+0.22(n3)/+0.22(n3)/+2.36(n10 wr70.0 be57.9 +12.1pp)/−1.63(n20 wr60.0 be64.3 −4.3pp)(毛) | FGI65/BTC77194−0.2%/mcap−2.7%/trend∩池{ARB,ENA,LIT,UNI} | **HOLD原子包0(12h费覆🟢二连强化+wr−be+12.1pp=S31基线修复趋势成立,无新证据簇不动刀);ROUTE v1.5零差分十一连(plan空,aging=TUT+USELESS双advisory留池,bl_sync desired15=§5#81);#81排自查点01:15Z抢窗(main bl14→15+USELESS/USDT,T仓max_hold~01:07后账户可空,≤3retry)** | 钱包159.3U(avail145.5+T margin14.3+浮−0.45);T多00:06:34在飞(S31 verdict后第5笔);增量3笔+0.22=UAI空+1.50(13.7m回拉)/MAGMA空+0.09/BULLA空SL穿刺−1.37(9.1m);死法24h=7×SL穿刺(空4笔−5.05分散4币+多接刀3笔−3.91=#80族存量)+1×硬超时(AGT);费覆24h🔴十连(avg−0.081 gap−0.181@ntl33.2U;20笔/日)/12h🟢二连(avg+0.236 gap+0.134,n10);UAI 48h回拉−2.29/n5距隔离1.71;MAGMA网n2/−1.08距线2.92;#79 n=32(MAGMA22:22双行第8案);S31 verdict后多头4笔+1.96维持;S32/#80零新增(n3/反证n2);池ATR中位0.22(10/75过门)低波续暖;audit零无主(main1792/trend1793=上轮种子PATCH);margin Σ0.55✓;充值线未触(24h负,12h wr−be+12.1pp≥6pp两支已备) |
