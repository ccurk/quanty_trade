...
| main 通才(平台名 Meme_合约信号计算引擎_1) | 8eb182b6 | **@09-19 14:1x 代码被换: tpl1057 = Meme_v40_crypto_perp_20260919-133457(hash 13e56f4c@14:15Z→deb8795a@14:3xZ 模板仍在改; 运行体=14:11:22Z 版; 0 个 S 锚点; 疑 DS 作/owner 委托, 候认领 CC-32; 归档 confident-fermi strategies/quicktrade-8eb182b6/tpl1057_v40_*.py)**; 前代 tpl998(S4..S23+S30+S31+S33; hash 3e311e8b)=回滚目标 | symbols ""+max_price 1e12 ⇒ filter 路径 300 池(select_limit 300); feed 267(CC 14:3x rotate remove 32 + 15:20 remove LSK; bl∩feed=∅, ∩majors=∅; 每次重启后必 rotate) | running(runtime 1789827082759=14:11:22Z; 今日重启 11:08/13:42:49/14:04:47/14:11:22) | **14:3x 现值: lev5(DS 10:09, owner 值 10) pct0.125(DS 07:08, owner 值 0.25, lock 至 09-20 09:37Z) mcp3(DS 09:37, owner 令 10) tp,sl pct0(腿=信号 ATR 括号) max_hold45 cd180 min_conf0.35 atr 2.0/2.5 hunger off trailing/BE off bl45 sides 双向; v40 只读 min_confidence/tp_ratio/sl_ratio/atr_tp_mult/atr_sl_mult/cooldown_sec/warmup_bars/max_bars/cost_mult/fee_round_trip/long_bias/short_bias/max_candidates/log_every/log_trace(其余 Python 键=死键: max_atr_pct/min_atr_pct_for_trade/volume_ratio_min/breadth_*/best_pick_*/lcp/scp/atr_discount*/trend_confirm_bars/symbol_blacklist Python 侧)**; 300 池/tpl998 段(11:08→14:11) 终读 n14 −2.34 wr43%; v40 段(14:11 起) @15:2x n15 −1.28 wr53%(隔离币 USELESS/LSK −2.25; 去掉后 +0.97 wr62%); 模板 hash deb8795a 未再变, 运行体仍 14:11:22 版 |
| majors(平台名 Majors_BTC_ETH_BNB_SOL) | e725e31a | main fork → **tpl1056 = tpl998 + S35 bar聚合 15m**(CC apply 09-19 08:39:38Z, hash 766b576a; 13:44:29 重启后模板保持) | manual BTC/ETH/BNB/SOL/XRP(bl0; main 侧已 bl+rotate remove SOL/XRP 防撞) | **running@08:01:47Z(owner UI start); 重启 11:08:41(owner)/13:44:29(v40 批次, 归属候认领)** | 现值: lev10 pct0.125 mcp2 cd900 max_hold720 hunger off atr 2.0/2.5 min_atr 0.03 reentry 15m bar_agg 15 sl_ratio0.03 auto_optimize dry_run=true; 1m 段 3 空 SL −0.78; S35 有效段 n2 +1.37(SL 距中位 0.67%≥0.3✓), 腿 8/8✓; 12:46 后 0 开仓; 史=git 4d0e7da 版§1.5; CC-28 eval 09-20 08:40Z/n≥15 |
...
### 双执行体(09-17 起)
- **[新增 09-19 15:2x] 槽位分配丢弃置信度(源码)**: strategy_signal.go:491 rr=(tp−px)/(px−sl), :505 score=rr/penalty(penalty=1+0.35·近期+0.5·连开), :532-542 按 score→rr→连开→近期排序(sort.Slice 不稳定, 平局随机); v40 tp=1.25×sl 恒定 ⇒ 新鲜候选全部 rr=score=1.25 平局 ⇒ mcp3 槽位归属与 v40 置信度(成交 0.45-1.0, 中位 0.77)无关(14:23:05 批 龙虾(bl)>GWEI>ENA 0.739 实证; 跳过 7+候选失败 23/1.2h=闸口饱和); 杠杆=v40 侧 tp 倍率随 conf 或每批 top-k(CC-33), 或 Go score×conf(部署)。
- **[新增 09-19 15:2x] DB closed 行丢交易所腿平仓 + LSK SL 腿消失亚型**: ENA 14:20 开仓(腿✓ 14:20:02)→SL 腿成交(14:30-14:34 价 0.20016<sl 0.20061)无任何 '平仓' 日志且 closed48 无行, 仅 income by_symbol 有(−0.78); ZAMA 11:1x 2 笔 −2.13 亦从 closed48 消失(12:4x 曾在) ⇒ 逐笔 n/净 必须 DB∪income by_symbol; 交易所腿平仓在 /logs 无行(只有守护/超时/饥饿路径有)。**LSK 14:51**: SL 腿在触发前已不存在(平仓前撤单 -2011 Unknown order, found=2 canceled=1) → tpsl_monitor.go:262 ErrStopLossBreached → 守护市价平 −1.85%(括号 −1.33%, 超出 0.4-0.5%); 今日 '止损失效' 8 例(BSB/SAGA/MORPHO×2/C/EVAA×2/LSK ≈5% 成交); 假说 H-0919h。
- **[09-19 14:3x] DS 改动簿 #11 = v40 换码事实 + 三论据论证 + 隔离执行链断裂**: 要点=13:34:57 建模板→13:42:49/14:04:47/14:11:22 三次 apply 重启(零 audit; 13:57/14:04 admin log_level/log_every=脚本签名); 三论据 2/3 不成立(funding/ls 由 Go strategy_dataflow.go:479-480 注入 candle extra, tpl998 :1743 读 extra; LONG_BIAS 等键 tpl998 0 命中; 48h 多 −0.109 vs 空 −0.120/笔 无偏置); v40 机制=成本闸 1.25×ATR14≥0.2% + sl 2×ATR/tp 1.25×sl(盈亏平衡 wr≈67%) + OI/LS/taker 因子 每币 4 个 /futures/data 调用(≈150 调用/min, 429 风险) + 无黑名单/veto/regime; 隔离链断裂(硬边界#9): v40 不读 bl + 部署版 Go 无下单口闸 + symbols "" 重播种 ⇒ USELESS 14:11/LSK 14:23 实开 ⇒ 每次 apply/重启后必 rotate; 14:3x 模板再改(+signal.price 供 reanchorTPSLToFill, 仓库 0 命中=部署版≠仓库); /api/positions?strategy_id= 过滤无效 ⇒ 跨实例核对按池归属; 全文=git 3b411de 版§3。
...
- **[新增 09-19 08:4x] Python stdout 日志不出 API(部署版)**: `_log`(stdout JSON) 行——START/统计汇总/IDLE/收到首根/S35 种子——在 /logs 中 main/majors 均 n=0(09-16 起统计汇总缺失同因) ⇒ Python 侧只能由 Go 侧动作行(候选/置信度动态仓位/开仓/已设置止盈止损)反推; S35…; 全文=git 4d0e7da 版§3。
...
- **[新增 09-19 09:2x] 回测引擎非确定性实证**: 同请求 3 次 10/12/12 笔 −12.3/+3.6/+7.6, 首分歧=首入场时刻(根因 CC-29); 单次回测符号不可信; 全文=git 29d8478 版§3。…; 全文=git 4d0e7da 版§3。
...
- **[新增 09-19 05:5x] 空头无正期望取证(方法可复用)**: 现货 1m 对 24h 空单 n45 算入场前 r1..r30 与 ATR: 赢/输中位无差, BTC 平, 括号反事实 −16.2 vs 实际 −17.5 ⇒ 空头亏损=信号本身; 全文=git d19a237版§3。…; 全文=git 4d0e7da 版§3。
...
- **[新增 09-19 00:1x] 入场单类型=MARKET(taker)**: strategy_signal.go:745 price=0→binance.go:1372 仅 price>0 走 LIMIT GTC(无 GTX);双边 taker≈0.1% 来回(24h 手续费 40.59U/名义 40.5k 实证)→费率病根 M 候选 CC-19。…; 全文=git 4d0e7da 版§3。
...
- **现货引擎事实@09-17(源码)**: 全文=prompt v5.1【现货载具】节◆引擎事实/git b341694版§3(market进程级→第二进程;可用=市价买卖+本地TP/SL;不可用=饥饿/max_hold/ROI/交易所TPSL/追踪/保本/收养/余额sizing;amount静态→S34改名义/现价;funding非us…; 全文=git 4d0e7da 版§3。
...
| # | 类型 | 内容 | 依据 | 复现计数 | 状态 |
| **H-0919f 括号锚在信号价非成交价(追涨滑点偏斜)** | 09-19 12:4x | 机制=strategy_position.go:476 括号按信号价而非成交价; @13:1x n11 逆向滑点中位 +0.06%, >1% 仅 2/11 ⇒ 非系统性, 优先级降; v40 14:3x 模板已加 signal.price(CC-31 同意图, 需部署版 Go reanchorTPSLToFill); 全文=git 3b411de 版§4 | 门=n≥20 或 owner 令 |
| **H-0919h 交易所 SL 腿触发前消失(priceProtect EXPIRED?)** | 09-19 15:2x | 证据=LSK 14:51 撤单 -2011 found=2 canceled=1 + 今日 8 例 '止损失效'; 机制候选=binance.go:2728-2730 STOP_MARKET closePosition+MARK_PRICE+priceProtect=TRUE: 触发时 mark/last 偏离超币种阈值 → 单 EXPIRED → 仓位裸露至 15s 巡检; 代价=超出 SL 0.3-0.5%/例; 验证=owner 查 Binance 条件单历史 status=EXPIRED; 修=priceProtect=FALSE 或 巡检 15s→5s(部署) | 门=再现 ≥3 例/日 连续 2 日 → M 候选 |
...
| CC-9 | DS越界簿 | admin mcp 未声明改动=协议7越界 #1~#7 + lev 10→5 裸改 10:09; 裁决=不硬抢(RECOVER 门关/429 预算/乒乓); owner 一字令候(mcp 10/5/3, lev 10/5/阶梯); v40 下 mcp3=频率上限(跳过 7+候选失败 23/1.2h); 全文=git 4d0e7da 版§5 | 09-18 08:1x | open |
...
| CC-30 | owner 认领+裁决 | 11:06Z admin 批(main hunger off + symbols ""→300 池 + majors mcp2 + 双壳重启) 请 owner 认领; 有意回 300 池→建议 symbols 写显式币列表(重启键; 修 isAllowedSymbol 把 majors 仓计入 main 槽位 + 重启重播种隔离币两害); 无意→回 7 币列表+重启(CC 可执行, 空仓窗); 全文=git 3b411de 版§5 | 09-19 11:4x | open(@15:2x 仍候认领) |
| CC-31 | M候选(括号重锚) | **@14:3x v40 作者同意图: 模板 14:3x 版 signal 加 price 字段供 Go reanchorTPSLToFill(仓库 0 命中=仓库外改动)**; strategy_position.go:476 前按 (成交价−信号 entry) 平移 tp/sl(或 ×成交/信号比), 阈值 |滑点|>0.25×ATR 才平移; 依据 H-0919f; 部署=owner; 回滚=不平移(默认关) | 09-19 12:4x | open(候 n≥20 或 owner 令; **@13:1x n11 逆向滑点中位 +0.06%, >1% 仅 2/11(ZAMA/SWARMS 追涨) ⇒ 非系统性偏移, 优先级降**) |
| CC-32 | owner 认领+拍板(v40) | **main 代码被换为 v40(tpl1057, 13:42:49 起, 零 audit)**: 请 owner 认领(DS 作? owner 委托?); 拍板二选一: A 保留 v40 评判(CC 推荐, 附 3 项必修: ①v40 读 symbol_blacklist 或部署含下单口闸后端(8669ea1 在仓库) ②signal.price 重锚需部署版 Go 有 reanchorTPSLToFill(仓库 0 命中) ③REST 预算) / B 回滚 tpl998(S 谱系); 未拍板前 CC 按 _exp_cc EVAL 线执行(劣化→TG+rollback tpl998); 每次 apply/重启后 CC 必 rotate 隔离币 | 09-19 14:3x | open(@15:2x 仍候认领; v40 段 n15 −1.28 wr53%, 隔离币 −2.25, worsen 未触) |
| CC-33 | 策略代码(v40 下一档候选) | **槽位分配丢弃置信度**(§3 15:2x): v40 tp=1.25×sl ⇒ Go rr 恒 1.25 平局, mcp3 闸口随机选; 候选修=①v40 tp 倍率 1.0+0.5·conf(rr 1.22-1.5 携带 conf, 高 conf 同时放宽 TP) ②每批只发 top-k(k=空槽数) by conf ③Go score×conf(部署); 预注册门=v40 EVAL 封段(n≥30)后单独 _exp_cc, 劣化线同 EVAL; owner 拍板回滚 tpl998 则作废; 已写留言板请 DS 回应 | 09-19 15:2x | open(候 EVAL 封段 + CC-32 拍板) |
...
| 计数项 | 读数 | 更新轮 | 备注 |
| **09-19 15:2x 读数(DB∪income; income 括号)** | 1h n13 +0.23 wr62%(income −1.74=毛 −0.55 费 −1.19) / 3h n22 −1.16 wr55%(income −3.47) / 6h n29 +1.74 wr52%(income −0.28: 毛 +2.84 费 −3.12) 多 +1.61 空 +0.14 / 12h n67 −7.48 wr37%(income −37.3: 毛 −24.5 费 −12.9) 多 +12.1 空 −19.6 / 24h n165 −43.0 wr39%(income −100.5: 毛 −52.3 费 −48.2; 入金 +58.5 另列) 多 −25.9 空 −17.1; 24h 死法(main): SL<2m 36/−68.5, SL 2-5m 25/−36.5, SL 5-44m 33/−50.7, 45m 超时 10/+0.6, 赢 61/+112.1; **v40 段(14:11 起) n15 −1.28 wr53%: SL 2-5m 2/−1.41, SL 5-44m 5/−3.75(含 ENA), 赢 8/+3.87; 隔离币 USELESS/LSK −2.25(去掉后 n13 +0.97 wr62%); 成交 conf 0.45-1.0 中位 0.77; 笔/日≈300(mcp3 饱和)**; majors S35 段 n5 +0.62 wr60%; 48h 规则命中 7 币(SYN/XTZ/USELESS/AR/PIEVERSE/牛来/LSK)全在 bl; 14:3x 读数全文=git 3b411de 版§6 | 09-19 15:2x | 判据不变 |
...
## 7. 运行日志（每轮一行，新行追加在表首）
| 2026-09-19 15:1x-15:3x | 定时轮: 管道活(REST 200; 429 since v40 4: 守护跳过 1/查 TPSL 3; tpsl_setup_failed 0); 钱包≈avail 101.6+保证金≈58≈160(在飞 GWEI 空/RONIN 多/IOST 多 + majors BNB 多); income 1h −1.74/6h −0.28/12h −37.3(≈−23%)/24h −100.5(≈−50%; 入金 +58.5 另列)🆘(owner 令不缩表, 只报); v40 段 n15(DB 14+ENA income 补) −1.28 wr53%, 隔离币 −2.25, worsen 未触→OBS 继续(eval 09-20 02:00Z/n≥30); 止损完整性: main 18/18 腿✓(GWEI SL 0.50%/RONIN 1.35%/IOST 1.0%) majors 9/9✓(BNB 15:01 SL 0.88%) max_hold 45/720 ⇒ 两实例非裸奔; 隔离: rotate remove LSK 15:20 复读 feed 267 bl∩feed=∅ feed∩majors=∅ 无同 symbol 跨实例仓; DS: 14:04 后 0 PATCH(main/majors), lock 至 09-20 09:37, _ai_task_ds null; 改动: 0 交易键(EVAL 在飞 + RECOVER 门关 6h −0.28/24h −100 + owner 不缩表令); 新发现 ①Go 排序 rr 恒 1.25→置信度不参与槽位(§3/CC-33) ②DB 行丢交易所腿平仓 ENA/ZAMA(§3) ③LSK SL 腿触发前消失→守护超出 0.4%(H-0919h); config 与 §1.5 现值零漂移, 模板 hash deb8795a 未再变/运行体未重启; 宏观 FGI71 BTC +1.1% ETH +2.4% 市值 −1.4% trending∩feed=ZAMA/ZEC/ARB/PONS/UNI/ONDO/ENA; 留言板 6 条 7.9KB 复读✓, _exp_cc reads_1530 复读✓; 下轮=16:1x: DS 16:09 节拍对账(砍 pct/lev/mcp 只 TG); v40 n≥30 早读(DB∪income); 重启即 rotate; 止损失效计数 |
| 2026-09-19 14:1x-14:4x | 全文=git 3b411de 版§7(要点: v40 换码发现+三论据论证; 隔离链断裂→rotate remove 32; 腿 6/6+8/8; EVAL v40 登记; DS 13:57/14:04 非交易键) |
| 2026-09-19 13:1x-13:3x | 全文=git 4d0e7da 版§7(要点: 300 池段 n10 −1.62 刀死法 0; lev5 watch 终裁不执行; 腿 10/10+8/8; DS 0 PATCH) |
