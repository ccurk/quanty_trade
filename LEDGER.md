...
| main 通才(平台名 Meme_合约信号计算引擎_1) | 8eb182b6 | **@09-19 14:1x 代码被换: tpl1057 = Meme_v40_crypto_perp_20260919-133457(hash 13e56f4c@14:15Z→deb8795a@14:3xZ 模板仍在改; 运行体=14:11:22Z 版; 0 个 S 锚点; 疑 DS 作/owner 委托, 候认领 CC-32; 归档 confident-fermi strategies/quicktrade-8eb182b6/tpl1057_v40_*.py)**; 前代 tpl998(S4..S23+S30+S31+S33; hash 3e311e8b)=回滚目标 | symbols ""+max_price 1e12 ⇒ filter 路径 300 池(select_limit 300); feed 268(CC 14:3x rotate remove 32; bl∩feed={LSK 持仓拒}, ∩majors=∅) | running(runtime 1789827082759=14:11:22Z; 今日重启 11:08/13:42:49/14:04:47/14:11:22) | **14:3x 现值: lev5(DS 10:09, owner 值 10) pct0.125(DS 07:08, owner 值 0.25, lock 至 09-20 09:37Z) mcp3(DS 09:37, owner 令 10) tp,sl pct0(腿=信号 ATR 括号) max_hold45 cd180 min_conf0.35 atr 2.0/2.5 hunger off trailing/BE off bl45 sides 双向; v40 只读 min_confidence/tp_ratio/sl_ratio/atr_tp_mult/atr_sl_mult/cooldown_sec/warmup_bars/max_bars/cost_mult/fee_round_trip/long_bias/short_bias/max_candidates/log_every/log_trace(其余 Python 键=死键: max_atr_pct/min_atr_pct_for_trade/volume_ratio_min/breadth_*/best_pick_*/lcp/scp/atr_discount*/trend_confirm_bars/symbol_blacklist Python 侧)**; 300 池/tpl998 段(11:08→14:11) 终读 n14 −2.34 wr43%; v40 段(14:11 起) n4 +0.28 wr75% |
...
### 双执行体(09-17 起)
- **[新增 09-19 14:3x] DS 改动簿 #11 = v40 换码事实 + 三论据论证 + 隔离执行链断裂**: 13:34:57Z 建模板 Meme_v40_crypto_perp_20260919-133457 → 13:42:49/14:04:47/14:11:22 三次 apply 重启(零 audit=apply/start 不写 audit; 13:57:33 admin log_level=info 同秒写 main+majors、14:04:41 log_every=5=脚本签名; 13:42 apply 的 stop 强平 CAP −0.10/HEI +0.01; majors 13:44:29 重启 tpl1056 未变); 归属=疑 DS(owner 07:2x "我让 deepseek 在做,一会你检查一下"), 候 owner 认领(CC-32). **论证**: ①"funding/ls 是钉死常量" 错—strategy_dataflow.go:479-480 把 bx.FundingRate/LSRatio(binance.go:1031/1038 premiumIndex+globalLongShortAccountRatio) 注入 candle extra, tpl998 :1743-1744 读 extra 非 config(作者查了 config 键) ②tpl998 无外部行情调用 属实但数据由 Go 供 ③LONG_BIAS_FACTOR 1.15/SHORT_RESTRICTION_FACTOR 0.7/MIN_SHORT_CONFIDENCE 0.68 在 tpl998 grep 0 命中=不存在; 48h DB 多 n187 −0.109/笔 vs 空 n127 −0.120/笔 ⇒ 无多头偏置证据 ⇒ 动机 2/3 不成立. **v40 机制**: 成本闸 expected=1.25×ATR14(1m)≥2×0.1% ⇔ ATR≥0.16%(替代 min_atr_pct 0.5); sl=2×ATR14 tp=1.25×sl(实测 SL 0.33-0.63% TP 0.27-0.79%, 盈亏平衡 wr≈67%); 外部 OI/LS/topLS/taker 因子(每币 4 个 /futures/data 调用, premiumIndex 全宇宙 30s; 首 11.6min ok=1748≈150 调用/min, 与下单同 IP=CC-23 429 风险); 无黑名单/无 S18-S22 veto/无 S31 regime/无 volume/breadth 门; [v40] 日志经 Go 出 API(与 tpl998 stdout 不同). 14:3x 模板再改(+signal.price 供 Go reanchorTPSLToFill 重锚=CC-31 同意图; **仓库 grep reanchorTPSLToFill 0 命中 ⇒ 该 Go 改动在仓库外(宿主直改?)=部署版≠仓库 再证**; 运行体未重启=未生效). **隔离执行链断裂(硬边界#9)**: 唯一有效黑名单层曾是 tpl998 Python("黑名单拦截" 日志 09-16); 仓库 Go 下单口闸 strategy_position.go:72(8669ea1@09-08) 日志史 0 条 ⇒ 部署版无; symbols "" ⇒ 每次重启重播种 300 含 bl 33 ⇒ USELESS 14:11:49(−0.73)/LSK 14:23:02 实开; 处置=rotate remove 32(LSK has_open_position 拒, 下轮补); 每次 apply/重启后必须 rotate 直到根修(CC-32 三选一). **/api/positions?strategy_id= 过滤无效**(main/majors 同返回) ⇒ 跨实例持仓核对按 symbol 池归属(majors 5 币).
...
| CC-9 | DS越界簿 | admin mcp 未声明改动=协议7越界 #1~#7 + lev 10→5 裸改 10:09; 裁决=不硬抢(RECOVER 门关/429 预算/乒乓); owner 一字令候(mcp 10/5/3, lev 10/5/阶梯); 全文=git 4d0e7da 版§5 | 09-18 08:1x | open(**@14:3x v40 信号洪流下 mcp3 跳过 96 次/9min(14:11-14:20)=频率上限仍是 mcp3; 429 14:11 后 1 次**) |
...
| CC-30 | owner 认领+裁决 | **11:06Z admin 批(main hunger off + symbols ""→300 池 + majors mcp2 + 双壳重启)**: 请 owner 认领(若非 owner=未知 admin 写路径🆘); 若有意回 300 池: 建议 symbols 写显式 265 币列表(重启键)使 isAllowedSymbol 不再把 majors 仓计入 main 槽位; 若无意: 回 7 币 symbols 列表+重启(CC 可执行, 空仓窗) | 09-19 11:4x | open(@14:3x 仍候认领; symbols "" 第二个代价=每次重启重播种隔离币(v40 无 Python 黑名单后成硬边界#9 破口, 14:11 USELESS/LSK 实开) ⇒ 显式 symbols 列表优先级↑)|
...
| 计数项 | 读数 | 更新轮 | 备注 |
| **09-19 14:3x 读数(DB 去重; income 括号)** | 1h n4 −0.72 / 3h n16 −0.97 / 6h n27 +1.90 wr41%(income: 毛 +1.90 费 −2.59 = −0.69) 多 +2.11 空 −0.21 / 12h n81 −26.1 wr33%(income −39.6: 毛 −26.0 费 −13.6) 多 +4.7 空 −30.8 / 24h n217 −48.3 wr42%(income −99.5: 毛 −48.8 费 −50.8; 入金 +58.5 另列) 多 −29.0 空 −19.4; 24h 死法(main n209): SL<2m 43/−87.3, SL 2-5m 34/−58.1, SL 5-43m 35/−62.3, 45m 超时 9/−8.9, 赢<5m 45/+78.0, 赢≥5m 43/+90.4; **300 池/tpl998 段(11:08→14:11 封段) n14 −2.34 wr43%: SL<5m 3/−2.17, SL 5-43m 2/−3.66, 超时 3/−3.12, 赢 6/+6.60; hold<5m 亏单 3/8**; **v40 段(14:11 起) n4 +0.28 wr75%(USELESS −0.73 隔离币/SSV +0.38/ROBO +0.22/AVAX +0.41), SL 距 0.46-0.62%**; 48h 规则命中 9 币全已在 bl; watch M n3/−9.05, VVV n3/−5.71, OP n2/−5.36; 13:1x 读数全文=git 4d0e7da 版§6 | 09-19 14:3x | 判据不变 |
...
## 7. 运行日志（每轮一行，新行追加在表首）
| 2026-09-19 14:1x-14:4x | 定时轮: 管道活(REST 200; Rate limited 14:11 后 1); 钱包≈avail 113.4+保证金≈167(在飞 ENA/LSK/ONDO 多); income 6h −0.69/12h −39.6(≈−20%)/24h −99.5(≈−50% 起始钱包; 入金 +58.5 另列)🆘(owner 令不缩表, 只报); **主发现: main 代码被换为 v40(tpl1057; 13:34:57 建模板, 13:42:49/14:04:47/14:11:22 三次 apply 重启, 零 audit, 疑 DS 作 owner 委托), S 谱系 0 锚点; 隔离执行链断裂→USELESS 14:11:49/LSK 14:23:02 实开(硬边界#9)→ROUTE rotate remove 32 复读 feed 268 bl∩feed={LSK 持仓}✓**; v40 三论据论证 2/3 不成立(§3 #11); 300 池/tpl998 段封段 n14 −2.34; v40 段 n4 +0.28 wr75%; 止损完整性: main 14:11 后 6/6 腿✓ majors 12:46 后无新仓(13:44:29 重启) 8/8✓; 隔离: feed∩majors=∅, 无同 symbol 跨实例仓(positions?strategy_id 过滤无效, 按池归属); DS: 13:57 log_level(双壳同秒)/14:04 log_every 非交易键 合规, _exp lock 至 09-20 09:37, _ai_task_ds null; 改动: 0 交易键; _exp_cc=EVAL v40(n≥30 费后≥0∧wr≥60%; 劣化 段净≤−4U∨n≥10∧wr<45%∨429 裸奔≥2→rollback tpl998) 复读✓; 留言板 6 条 7.4KB 复读✓; 宏观 FGI71 BTC +1.3% ETH +3.1% trending∩feed=ZAMA/ENA/PONS; 下轮=15:15: LSK 平仓后 rotate remove; v40 hash 是否再变/再重启(重启即 rotate); n≥10 早读 |
