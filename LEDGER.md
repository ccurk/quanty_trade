...
| 载具 | id | 原型 | 池 | 状态 | 门/备注 |
|---|---|---|---|---|---|
| main 通才 | 8eb182b6 | 通才(S18-S23栈) | auto·**feed93@08-24 15:3x(ROUTE#36 purge7[12:43计划重启+14:29:50无端重启各重播种100];COTI出trend回main=孤儿态在feed待bl释放;purge史#34/#33/#29见git)** | running | pct0.08 sides=[buy,sell] **long_conf_premium=0.35(#61c rollback@08-19,门0.90=多头事实关闸)** tpl567;**trailing(act1.0/cb1.2%)+BE(1.0)=基线几何(#49KEEP)**;**_exp=closed rollback(#61c;史§4#61/#63+git 0940c62)**;RECOVER刹线6h空≤−8U继承;#11降级观察(§4) |
| qt-trend-long | 827ffe8c | 趋势动量多 | **TUT单币**(COTI出池@08-24 15:3x ROUTE#36:48h nL14净−1.06触头注线[窗滚动08-22赢单出窗];种子同步stop-PATCH-start@15:35✓;COTI回main孤儿态待bl释放;ACE隔离@08-22;史git f43756c) | running | **tpl575(S24)** pct0.05 mcp3 buy trailing on;**_exp=open·#69三值包重申请@08-23 09:18Z**(adt0.5+lcp0.149+mces off;终裁二元=新段净>0→keep否则回滚[adt1.0/lcp0.15/mces3];eval=n≥16或08-25 09:18Z;劣化线n≥8∧≤−4U;**进度@08-24 15:2x段n10净+0.68**(12h新4笔:COTI+0.067/TUT−0.809饥饿/COTI−0.198超时/COTI+0.386 8m;劣化线远;COTI出池后段增量=TUT单引擎,判据不变段净含COTI既往笔);否决窗空置+程序史git 492779a);原三值段终裁rollback+ex-ACE归因全文git 5df52d2;**常驻保险=滚动6h净≤−5U→stop(继承)**;三值包史git 8a1d995/f71d825 |
| qt-fade-short-v2 | 7583727a | 冲高回落空(**tpl887/S28+S29**;谱系tpl576/S25→823) | HANA/COLLECT/MELANIA/BTR(feed4=种子4✓@08-24 15:2x;**MELANIA出池线触发但deferred@15:3x ROUTE#36**:nS7净−0.53证据全pre-S28[tpl806/823时代,S28段n0]=#57病理证据不可移植+移出=撤走S28/S29主标的;defer至_exp终裁重跑对账,风险界=保险6h−6U+劣化线;BTR晋升#35;MELANIA晋升08-22) | running | **S28衰竭签名直入上线@08-23 21:24Z**(#66修复,机制全文§2 S28行+git;附随#65 epsilon永久化;apply三复检✓);**_exp=open·EXP:S28**(二元:段净[Σpnl−名义×0.1%]>0→keep否则rollback回tpl576;劣化线段n≥8∧≤−4U即回;评期段n≥16或08-25 21Z;崩溃立即回滚;**S29 amend@08-24 12:40Z(tpl887)**;段n0@15:2x(约18h;S29后2.6h无完整签名bar,MELANIA 0.35-0.36被拒行=非签名bar=域限定按设计实证);评期/劣化线/二元不变;终裁rollback=rollback×2或apply归档tpl576码;**烟雾task32死于12:40 apply载具级重启(基建非代码,同task30→31型),task33重发@15:2x**);低波三值包ROLLBACK@08-23(组级知识:低波解锁不付费;git a2e845f);保险=6h净≤−6U停(继承) |
...
| # | 类型 | 内容（含完整意图） | 登记轮 | 状态 |
|---|---|---|---|---|
| 70 | bl_sync | main symbol_blacklist 19→18: **移COTI/USDT出bl**(COTI已出trend池回main,现孤儿态;MELANIA保留在bl[fade池,出池deferred]);动作=main空仓窗stop-poll-PATCH-start;若执行轮对账器desired有变以新desired为准 | 08-24 15:3x | open(阻塞=UAI在押) |
...
| closed平仓行消失观测(≤48h短窗) | 瞬时缺行反转@08-23(拉取瞬态);零盈亏行分型@08-24 03:2x(SCRT案:毛=0无REALIZED_PNL income→行无pnl字段,真实成交佣金在income侧;本轮48h窗仍含该行=同例持续非新增);大规模掉行@08-24 12:3x(长窗重建丢中段行再证);**纪律=段级判定必须16h内短窗+income双拉**;全文git b505652系 | 08-24 12:3x | 判据:瞬时缺行复现≥3例→读源码找竞态;零盈亏亚型再现1例→升§3定案;滤行纪律维持(毛=0不失真)但其佣金只在income侧,费覆计算记得含 |
...
| S19顺涨拒空veto | 裁决KEEP@08-06终态;+ONG非应拦例@08-21(n=1);+VELVET 08-24 14:36**非应拦**(入场前bar序列全跌RSI37,+2.00%挤压bar在入场后;14:38该bar上7b正确拦再入[2.00>0.6×2.12]=正拦实证;死因归#30已结案族追跌空反弹,n=1轶事) | 08-21 | 新增应拦型(顺涨bar+快SL)即报警复核;ATR膨胀语境同型≥3例再议门用min(ATR,cap) |
...
| 08-24 15:1x-15:4xZ | 例行轮(max) | 1h−0.91(2)/3h−1.11(3)/6h毛−0.69净−0.86(7)/12h毛+1.71净+1.45(11,wr72.7/be60.5=+12.2pp)/24h毛+1.87净+1.43(18,wr66.7/be57.0=+9.7pp) | FGI73/BTC+3.2%/市值+0.6%无降压;trending∩main=AERO,ENA,LIT,PUMP,VELVET | **0原子包(两_exp在飞未到门;#58门48h费覆🔴不过);ROUTE#36=trend出COTI(nL14净−1.06触线,种子同步✓_exp三值保留✓)+main purge7→feed93+fade出MELANIA deferred(_exp冲突:证据全pre-S28=#57病理);trend#69段n10净+0.68;v2段n0@18h(S29后无签名bar,域限定实证);task32死因=apply载具重启(基建),task33重发;**无端重启新例14:29:50(main-only)**;费覆连3轮🟢组+0.104≥0.081(main+0.144/trend+0.082/fade+0.088全🟢,48h🔴锁扩张);充值三条件连3轮成立;VELVET−1.30快SL=非S19应拦(§6) | 钱包≈170(avail155.13+UAI仓);死法24h:W12+5.55/中亏4−1.58/饥饿1−0.81/快SL1−1.30 |
| 08-24 12:2x-12:5xZ | 例行轮(max) | 1h−0.81(1)/3h+0.34(5)/6h+2.66(12,wr92/be71)/12h+1.85(13,wr84.6/be70.4=+14.2pp)/24h净+2.60(26,毛+2.98佣−0.41资+0.03,wr80.8/be64.0=+16.8pp) | FGI73/BTC+1.7%/市值−1.6%无降压;trending∩池=LIT,PUMP(main) | **原子包1=S29@fade-v2(tpl887,详§2/§1.5)**:签名域ATR地板0.5→0.30;依据=08:44-47Z MELANIA ss0.80被ATR地板4bar全灭+census378/791;_exp amend判据全不变;复检三✓;ROUTE#35=BTR晋升fade+main移出+bl_sync19落库(main重播种100=预期漂移);apply作用域n=2定案§3;trend#69段n8净+0.49劣化线远;crash loop静默4.2h;费覆连2轮🟢+0.115≥0.043;充值条件连2轮成立;烟雾task32在跑 | 钱包171.0;掉行再证(§6):段判定纪律=16h短窗+income双拉 |
