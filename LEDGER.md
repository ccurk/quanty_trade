...
| 载具 | id | 原型 | 池 | 状态 | 门/备注 |
|---|---|---|---|---|---|
| main 通才 | 8eb182b6 | 通才(S18-S23栈) | auto·feed88(COTI晋升移出@08-13 12:5x;黑名单18在库,期望bl15=旧14+COTI[bl缺COTI,main重启窗需补PATCH];重播种观测无第3次;孤儿4[KAITO/DODOX/BLUAI/AIO]留feed待种子同步) | **running@08-12 22:20Z(全组复飞令;[buy]latch维持)** | pct0.08 sides=[buy](刹车latched) tpl567;段(自22:20)**n3/+3.13 wr67**(HOME+0.62超时+COTI−0.55超时+COTI+3.06饥饿TP45m mv+5.45%);US多单12:32在飞;**S23已裁=证据不足·结案@08-13 00:33Z**(详§6;重开判窗=#37;_exp槽名义空,清写入#45) |
...
- 互斥不变式: feed层✓(overlap=0全组两两,08-13 12:5x COTI晋升后复核)>种子层blacklist18(期望bl15,缺COTI待main重启窗补)>⚠️引擎互斥闸未部署,部署升急;**对账器v1.3本轮运行:plan=COTI晋升main→brk已执行**(churn1≤4),aging_watch=trend3+fade2+brk2 advisory
...
| #43(冻结@02:38Z停机直令) | RECOVER执行包 | 门=组逐笔6h≥0∧24h≥0(**≥0含0,n=0视为0**——latch下零成交是结构性上限,止血即达门;此解释先于数据登记防移门柱;组口径承12:3x轮先例)→步=main sides=[buy]→[buy,sell]空仓窗stop→PATCH→start原子,同PATCH写回刹线:unlatch后6h main空侧净≤−8U(≈钱包3.1%,镜像brake_20260809原触发型短burst)→重latch sides=[buy]续用同键;执行轮先复核无新刹车触线∧宏观降压未触发 | 08-10 18:2x | open,冻结维持;门读数: 12:38Z首读→18:14Z第2读→08-12 00:17Z第3读→06:26Z第4读→**12:34Z连续第5读达标**(第4/5读6h=0✓∧24h=0✓[双窗n=0=预注册视0];第3读6h=0✓∧24h=+0.35✓;无移门柱)→**18:2x第6读不达标**(6h−3.19<0=canary段亏,达标链重断,unlatch候双转正重来)→**22:19复飞令到冻结解除,执行前复核(第7读)FAIL**(24h−3.19<0)→**08-13 00:3x第8读FAIL**→06:3x第9读FAIL→**12:4x第10读FAIL**(6h+3.06✓∧24h−1.11✗;COTI+3.06饥饿TP入窗拉近,TUT−2.26/BICO−0.93今14:34/15:22滚出后24h料转+2.08→**下轮大概率达门**,变数=US在飞单与新平仓);执行窗=门成立∧全组空仓窗(=main空仓窗,回声行自愈已证);执行包同窗顺带:main _exp槽清写s23_closure(#45 main部分)+回刹线预注册 |

| #45(缩@08-13 06:3x) | _exp补写×3(main/fade/brk) | 四载具config._exp补写resume_note(评期重锚累计口径;trend另含canary_insurance):trend原PATCH 13:54:27Z被500拒(running锁,audit1717载荷留痕);fade/brk/main 22:20复飞后同样running锁。**重锚权威=§1直令条(08-12 22:19+补录)+§1.5各行**,约束力不受config缺载影响;**main payload追加@08-13 00:3x**: s23_closure速记(证据不足结案00:33Z)+_exp槽清空;**trend部分已落地@08-13 06:35✓;fade部分已落地@08-13 12:45(评期v2 PATCH同窗)✓;余main(US持仓锁,并#43执行包)+brk(候明日12:36Z评判窗)** | 08-12 18:2x扩22:4x缩06:3x | open,阻塞=三载具running;落地窗=自然stopped窗或eval verdict动作同PATCH带上(fade评期今12:28Z=下轮首选窗) |
