...
|---|---|---|---|
| 后端DB层挂死watch(平台事故→已升§3@08-27 04:4x) | **连续挂死中@08-27 12:37,自04:31复发起≥8h06m(n=6观测段)**: 03:14首发→03:5x重启→04:01复活~30min→04:31复发;观测段06:28四层+06:30-07:12×16探+09:14-09:20四层+10:38回访+12:37四层(例行轮)=签名恒定(鉴权401/0.9s+路由404/1.3s=进程活;login×3挂+templates 15s挂=DB端点死;health/db 404);owner零动作三证@12:4x(main头09f76cf不变+ed0c160未并+TG getUpdates空);mm线已排除 | 08-27 09:2x | 判据不变:部署#73后再现→health/db秒判+readTimeout日志定位卡死query;owner待办=重启前SHOW FULL PROCESSLIST/INNODB STATUS/df -h;恢复后首轮必做=active核对+closed since04:00盲窗重建+feed漂移复核 |
