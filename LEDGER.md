...
| 2026-09-20 10:1x-10:3x(v6.0 #13) | 上轮(09:28 板#1960; TG 10835/10836)= [攻-1] DS mcp 1→2 / [攻-2] DS take_profit_pct 0→0.10; owner: A 改 deepseek_optimize.py 3 行事实 或 B UI 直改 mcp2/tp0.10 | **DS 0 交易键改动(第 10 轮)**: audit admin 最新仍 #1954 06:22:16(breaker _exp); 09:37/09:52/10:07 三拍在门开期(6h pair_count 31≥30, 板 #1960 在 config 内可读)仍无 admin 写; 10:17Z config main lev10 pct0.25 mcp1 tp0 sl0 hunger off max_hold90 max_mult0.8 不变; majors lev10 pct0.25 mcp2 max_hold720 bl BNB/SOL/XRP 不变; tpl1057 hash 6c087f2c/tpl1056 766b576a 未变. **owner 未 UI 改**(无 config 变化, 无非 cron audit). 结构否决第二轮实锤: deepseek_optimize.py:298 费占比句/:327 事实#4「mcp 不是瓶颈 7-8/10」/:329 事实#5「TP 0 命中 调它无意义」+ validate MAX_DRIFT 2.0. 门预计 ~10:46Z 关(ROSE 对 04:46 close 滑出→29), main 锁死不产新对 ⇒ DS 死锁至 ETH 出场+main 攒 30 对(≥6h). 我方写: 板 #1961 10:26:27 仅 _ai_task_cc(2 条 6054B; 08:26 条按 8KB 裁掉; 复读 mcp1/tp0/sl0 未变 ✓) | **效果**: 无改动 ⇒ 无前后. 逐笔(DB 10:18Z 全账户): 1h 0/0 · 3h 2/+1.06 · 6h 4/−0.66 wr75% · 12h 13/−0.08 wr77% · 24h 106/−9.80 wr48%(多 80/−13.49 空 26/+3.69); 交易所配对 6h 31 对 毛−0.66 费−0.96 净−1.61 rr0.44(多 14/−2.04 wr14% 空 17/+0.38 wr100%) / 24h income 毛−9.80 费−7.48 资金+0.05 净−17.2(pairs 197 incomplete); avail 130.54(钱包≈149; ETH 保证金 18). **北极星**: 15m 段(21:12Z 起) main 13 笔 23.8/日×−0.196=−4.66U/日 税前(300U 单 费 0.30/笔: BIGTIME .299/ON .298/CTR .295), wr 9/13 均赢 +0.408 均亏 −1.552(0 TP 腿; 赢单全钟出; 收盘≥+1% 仅 BANK+2.03/POWR+1.07/ROSE+1.11); **1h 段 0 笔 ⇒ 北极星=0**. **病根**: main 09:16:40 ON 平后 0 开仓 70min; 09:46 52/10:01 102/10:16 120 共 274 信号(多 117 空 157; conf1.0 48) 0 开, 09:16:43「已持仓1个 达到最大并发仓位1」(logMaxPosSkipOnce 仅一行) ⇒ 占槽者=majors ETH 空 08:16:31 180U tp2561.87(−0.64%)/sl2592.81(+0.56%), OKX 1m 08:17-10:22 hi2586.2 lo2568.26 ⇒ MFE+0.39%/MAE−0.30% 皆未触, 钟 20:16Z. 源码: strategy_signal.go:660-676 FetchPositions 循环计入一切 isAllowedSymbol 持仓(main symbols="" ⇒ manager.go 恒 true), otherHeld(:631-640)已算却未用于排除 ⇒ 跨实例吃槽=代码缺陷, 一行修=循环内 `if _,ok:=otherHeld[key]; ok {continue}`. 止损核对: main 0 仓(上笔 ON 腿 07:46:36 ✓ 钟 90 ✓); majors ETH 腿 08:16:32 ✓(order 4000001912590445) 钟 720 ✓; 无裸奔; 无同币撞车. 宏观: FGI 71 Greed; BTC 80340(−1.1%) ETH 2573(−2.5%) SOL 108(−3.4%); global Throttled; trending FIRO/AKE/BP/STRK/ONE/PONS/NEAR/SUI/HYPE/INJ. **本轮建议(板 #1961; TG 10837)**: [攻-1] owner UI main mcp 1→2(Go 热键; Σ0.5≤0.75; owner 直令值 10, 现 1=DS 23:39 改) | 274 信号/0 开, ETH 占唯一槽最坏至 20:16Z; mcp2 ⇒ 笔/日 0→≥16 并重开 DS 门 | 验: 11:01Z 批 main 开仓≥1·下轮 1h n≥1·6h pair_count 回≥30; 根治备选=strategy_signal.go otherHeld 跳过(需部署). [攻-2] 同次 UI 落 take_profit_pct 0→0.10(1.0% 价@lev10, 只换 TP 腿) | 13 笔 0 TP 腿 均赢 +0.41; 7/13 盘中到过 +1%(3 收盘+4 回放) ⇒ +3U/笔 vs 费 0.30 | 验: 新腿距 1.00%·5 笔内 TP 成交≥30%·均净≥+0.15; 单落 mcp2 不落 tp=失血加速(−0.196−0.30/笔). DS 松绑: deepseek_optimize.py:327-329 事实 #4/#5 改现算或删, :298 加 n<10∨净>−2U 不适用. 🎓 期望=0.69×0.41−0.31×1.55=−0.20 ≈ 实测 −0.196; 均亏/均赢 3.8 > wr/(1−wr) 2.2 ⇒ 必亏. 下轮验: 11:01/11:16 批开仓数; audit admin/owner 写 mcp/tp; ETH 腿; pair_count. |
