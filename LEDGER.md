...
| 载具 | id | 原型 | 池 | 状态 | 门/备注 |
|---|---|---|---|---|---|
| main 通才 | 8eb182b6 | 通才(S18-S23栈) | auto·feed91(H隔离@08-14 21:26热移除✓;overlap0) | running | pct0.08 sides=[buy,sell] **long_premium=0.35(FIX@08-14 18:47Z)** tpl567;_exp=**FIX长溢价0.15→0.35**(long_thr0.70→0.90;证据=48h长侧n20 wr30/−11.79过20笔门,病根=逆势栈0.70恰等长门,S12先例;keep=08-15 19:00Z长侧24h净≥−2U∧组均净较−0.216改善;劣化=长侧≤−6U或组净恶化→回0.15再议[sell]latch;全文config._exp)+**继承RECOVER刹线**(6h空侧≤−8U→重latch[buy];21:4x读−3.48未触);**FIX段早读@T+2.5h**:长侧n1/−1.82(AKE 0.90门高置信仍接刀n=1;流量≈13笔/24h→1笔/2.5h↓符合预期),空侧n5/−6.30wr0(AIO/DOLO=#30型+ALICE=S20边界+LAB/H超时);⚠️劣化线'长24h≤−6U'现读−12.68=滚窗伪影(13长中12笔pre-FIX)被改路径自身未崩→不提前回滚,评估点24h窗=纯post-FIX自然对齐(解释注记@21:4x非事后换指标);24h n37/−13.36wr32:长n13/−12.68wr15 vs 空n24/−0.68wr42;sid伪影纪律照旧(§3,AKE→brk第3例) |
| qt-trend-long | 827ffe8c | 趋势动量多 | TUT/BICO/ACE/COTI/CAP(种子=feed=5✓;aging_watch=TUT/BICO/CAP advisory) | running | **tpl575(S24)** pct0.05 mcp3 buy trailing on;_exp v3常驻:**累计n22/−1.16**(CAP 2笔−1.88:快SL@18:04+中段SL@20:47);keep门净>0✗∧费覆🟢24h+0.084 vs 0.074=1.1×薄;劣化−5U/canary−5U(现6h−1.88)未触;**终裁08-16 06:35Z当值净>0=keep**;ACE 48h+3.44组最佳∧trending;全史git+config._exp |
| qt-fade-short | 21519f1b | 冲高回落空 | 1000CAT/ARC(种子✓;aging_watch全2币) | running@08-13 12:45Z | **tpl576(S25)** pct0.04 mcp2 sell;_exp v2续:累计n8/−2.63零增(现池零成交**第8轮**,want.fade=[]连续;08-14多币冲高但S净证据不足格,非载具故障);评期=累计n≥15或**08-16 12:50Z**;自动保险6h≤−6U(0✓) |
...
- 互斥不变式: feed层✓(overlap=0三running:main91/trend5/fade2,08-14 21:26 H隔离后复核)>种子层bl实13=隔离6+trend5+fade2,desired14(+H)候重启窗PATCH(feed层已防,H不可交易)>⚠️引擎互斥闸未部署;COTI晋trend=v1.3 plan首个跨态再路由;aging=trend3/fade2 advisory
...
> 维护注 08-13 19:0xZ：#43 RECOVER unlatch落地结案@19:03(30s原子窗,[buy,sell]+bl18→15+刹线_exp继承;全文见git 313676f前史§5)。
...
| main无端重启重播种观测 | **1@08-13(22:5x-00:18窗feed89→100;rotate源码排除非重启回加路径[strategy_symbols.go:feedSymbols仅rotate/start改写]→必经进程重启;窗内零apply零start API;audit盲区=start/stop不写;领先假设=owner后端重启/部署BOOT RESTORE)** | 08-13 00:2x | 复现→升§3候证平台事实+评估counter清零对评期锚影响;黑名单闸在防=零实仓风险;每轮复核feed数(12:3x复核89;08-14 21:2x复核92→91=有主H隔离,无新无主重启) |
...
| 08-14 18:05-18:2xZ | 告警响应(max) | 2h close −3.19(8):18:01-07波动窗三连SL(US/H/CAP) | 单一行情脉冲 | 3条engine error核定=平仓撤单竞态无害类,定案入§3+§6;零处置 | 全文见git 313676f;刹车线全远 |
...
| 08-14 06:41-07:1xZ | 例行轮(max) | close: 1h+2.25(5)/3h+1.29(8)/6h−2.77(19)/12h−9.09(23)/24h−7.05(29,wr41) | FGI29无降压 | ①**brk评判FAIL→预注册回滚stop@06:50**(段n15/−3.23≤−3U;诊断=trail0.8%帽截赢payoff倒挂;S27候选#47) ②ROUTE:COTI晋trend(churn1) ③#42/#45结案+#46登记 ④1原子包 | 全文见git 313676f前史§7;详情存brk _exp+§1.5+#47 |
| 08-14 00:32-00:5xZ | 例行轮(max) | close: 1h0(0)/3h−2.44(1)/6h−6.32(4)/12h−7.35(9)/24h−5.27(13,wr31) | FGI29无降压 | ①ROUTE:v1.0误plan弃用→v1.3权威副本入台账ops/(修根)+AKE/EDEN→brk执行 ②unlatch首夜main空侧4连亏−6.32 wr0(n4轶事登记) ③0原子包 | 全文见git 313676f前史§7 |
