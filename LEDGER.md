...
| 2026-08-29 | **提频直令(live会话)**: "不是不是,现在需要提升频率了"——现场覆盖08-03费覆红线"未覆盖=只修不扩"锁;红线降级为评判/回滚指标(非前置门)。同轮追问确认方向="不影响准确性的提频:改策略+按行情动态判多空"。执行=当轮2原子包(EXP多头闸重开+FLEET breakout复活);既定序是否永久调整待owner后示,默认=一次性提频授权+此后按_exp评判劣化即回滚 |

## 1.5 策略组注册表（08-09 起;池归属与载具状态权威节;roster 有变当轮必更）

| 载具 | id | 原型 | 池 | 状态 | 门/备注 |
|---|---|---|---|---|---|
| main 通才 | 8eb182b6 | 通才(S18-S23栈) | auto·**feed83@08-29 15:2x(ROUTE v1.5摘BAS,NIL→孤儿态待挂brk;前史feed85@14:5x=apply重播种热清15/15,HANA回池,4币→breakout池)** | running | pct0.08 sides=[buy,sell] **bl20@08-29 16:4x force窗(+BAS,NIL=desired20落地;隔离13+TUT+brk池4+BAS+NIL)** feed94@16:4x(BTR残留被幻影副本卡,强平后移) **scp=0.05@08-28 09:33(EXP提频终裁rollback)** **long_conf_premium=0.15门0.70@08-29(owner提频直令+S30疫苗伴随;劣化回0.35;史#61c关闸0.90)** tpl972(+S30)@08-29;**trailing(act1.0/cb1.2%)+BE(1.0)=基线几何(#49KEEP)**;**_exp=open·EXP多头闸重开+S30疫苗@08-29 14:52(评08-31 15Z或多头n≥10;劣化线=多头段净≤−3U或磁铁型≥3→lcp回0.35,S30代码保留;前史ROLLBACK提频0.600档git 0940c62)**;RECOVER刹线6h空≤−8U继承;#11降级观察(§4) |
...
| 计数项 | 读数 | 更新轮 | 备注 |
| 幻影行post-fix观察 | +1@08-25 00:2x(SCRT空回声指纹);**窗内0行@08-29 15:1x(closed48全41行带realized_pnl,连观,累计n=1)** | 08-28 18:1x | 再现累计;n≥3或含实亏→升§4 |
...
| S31候选·regime动态方向偏置(owner 08-29问句登记) | 设计意向=按行情自适应judge多/空:宏观宽度(S13已有骨架)+BTC动量联动动态premium(跌行情自动加多头premium/涨行情加空头premium),替代静态墙 | 08-29 15:1x | 门=多头闸重开后多头段n≥30实测数据到位再设计(校准数据当前=0,先攒);现有三层已动态(EMA±0.30/S13宽度门/S18-S20方向veto),增量=premium级联动 |
...
| 08-28 21:1x-21:4xZ | 例行轮(max) | 24h净+4.10(wr63/be50=+13pp🟢) | FGI73 | **v2终裁auto-C执行(签名域全窗n0→stop+RETIRED占位audit1780;段毛−0.317净≈−0.70=rollback轨,物理回滚不执行=占位闸);ROUTE bl_sync采纳bl19→14(audit1781,重播种漏9热清9/9→feed91);#36/#62-64defer结案;充值信号第5轮形式续触owner拍板悬** | stub@08-29 09:x;全文git链(00:1x轮push版) |
| 08-28 18:1x-18:4xZ | 例行轮(max) | 24h净+3.51(wr61/be50=+11pp🟢) | FGI73 | **HOLD;评判1=旧fade壳FIX 18Z终裁PASS(sid新行0→API层收养向量占位封闭,#72降纵深防御);ROUTE#64 BTR差分三现defer;v2评期未到不提前裁;充值信号第4轮形式续触(集中度告警,owner拍板悬)** | stub@08-29 06:x;全文git链(21:1x轮push版) |
