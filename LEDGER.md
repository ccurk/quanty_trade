...
| main 通才(平台名 Meme_合约信号计算引擎_1) | 8eb182b6 | **tpl1057 = Meme_v40_crypto_perp_20260919-133457(hash deb8795a 自 14:3x 未变; 0 个 S 锚点; 候认领 CC-32; 归档 confident-fermi strategies/quicktrade-8eb182b6/tpl1057_v40_*.py)**; 前代 tpl998(S4..S23+S30+S31+S33; hash 3e311e8b)=回滚目标; 史=git 063b9e8 版§1.5 | symbols ""+max_price 1e12 ⇒ filter 路径 300 池; feed 268(16:2x rotate remove 32; bl∩feed=∅, ∩majors=∅; **每次重启后必 rotate**: 20 项截断已修但 symbols "" 重播种仍含 bl) | running(runtime 1789833852457=16:04:12Z **owner 部署 e07f3d7+3e1e467 全实例"开机自恢复"重启**; 今日重启 11:08/13:42/14:04/14:11/16:04) | **16:3x 现值: lev4(DS 16:09 裸改, 前 5@10:09, owner 值 10) pct0.125(DS 07:08, owner 值 0.25, lock-1789833131 至 09-20 15:52Z) mcp3(DS 09:37, owner 令 10; 被 majors 仓吃槽=实际 1-2) tp,sl pct0(腿=信号 ATR 括号, 部署后锚到成交价) max_hold45 cd180 reentry_cd0(owner 15:36) min_conf0.35 atr 2.0/2.5 hunger off trailing/BE off bl46(+BNB@16:20) sides 双向; v40 只读 min_confidence/tp_ratio/sl_ratio/atr_tp_mult/atr_sl_mult/cooldown_sec/warmup_bars/max_bars/cost_mult/fee_round_trip/long_bias/short_bias/max_candidates/log_every/log_trace**; v40 段 @16:3x n≈21 −1.97 wr55%(DB∪income; 费后≈−3.9) |
| majors(平台名 Majors_BTC_ETH_BNB_SOL) | e725e31a | main fork → **tpl1056 = tpl998 + S35 bar聚合 15m**(CC apply 09-19 08:39:38Z, hash 766b576a; 13:44:29/16:04:12 重启后模板保持) | manual BTC/ETH/BNB/SOL/XRP(bl0; main 侧 bl SOL/XRP/BNB+rotate 防撞; BTC/ETH 未入 main bl=靠组内互斥兜底) | **running@08:01:47Z(owner UI start); 重启 11:08:41(owner)/13:44:29(v40 批次)/16:04:12(owner 部署)** | 现值: lev10 pct0.125 mcp2 cd900 reentry_cd0(owner 15:36) max_hold720 hunger off atr 2.0/2.5 min_atr 0.03 bar_agg 15 sl_ratio0.03 auto_optimize dry_run=true; S35 段 @16:3x DB n3 +0.28(XRP TP +0.88/BTC +0.03/SOL SL −0.63)+income BNB −0.47(16:1x SL)+BNB +0.49(14:3x 读) ⇒ n≈5 ≈+0.3, SL 距中位 ≥0.67%✓, 腿 ETH 15:46✓; 史=git 063b9e8 版§1.5; CC-28 eval 09-20 08:40Z/n≥15 |
...
### 双执行体(09-17 起)
- **[新增 09-19 16:3x] backend 部署 e07f3d7+3e1e467(owner black 15:44/15:53Z, 分支 claude/hopeful-lamport-0iwne3, 服务器改为 git 拉取本地构建) → 16:04:12Z 全实例"开机自恢复"重启**: ①reanchorTPSLToFill(strategy_signal.go:139, strategy_position.go:376/381 日志 "止盈止损已锚到成交价"; ZIL 滑点 −0.030%/XMR +0.007% 实证; CC-31/H-0919f 结案) ②排序 strategy_signal.go:585-630 改 score=rr(×lastEntryWinBoost 1.25 若上一笔平仓盈利, loadLastEntryPnL 跳过 pnl_source 缺失行), 删"连开少/近期少/开得早"三平局规则, 兜底 symbol 名(确定性; rr 仍恒 1.25 ⇒ CC-33 仍成立) ③manager.go parseSymbolsValue `len>20→[:20]` 静默截断→500: **H-0919g 隔离链断裂真根因**(bl 第 21~45 项从未生效: GPRO/H/HK0992/KODEX200/LSK/MARSCOIN/MYX/NEAR/ONE/PIEVERSE/POWER/SOXS/STAR/STRK/SYN/TST/USELESS/XNY/XTZ/哈基米/牛来/龙虾/SOL/XRP/PROM; 提交正文列 24h 口径 10 币 −79.8U) ④UpdateStrategyConfig running 态放行+Config()/setConfig 快照(config_hotreload_test 6 例)=部署版≠仓库版之谜结案(仓库现含) ⑤symbol_reentry_cooldown_minutes 15:36 admin→0(提交正文自述 owner 直令"不再冷却"; Go 热键 :264); 旁证: 重启后 150 批排序 0 个 bl 候选(部署前 USELESS 14:11/LSK 14:23 实开); **仍在**: strategy_signal.go:691 FetchPositions 循环按 isAllowedSymbol(symbols ""⇒恒 true) 把 majors 仓计入 main openCount ⇒ 16:16:09 ZIL 开仓即 3/3(ZIL+majors ETH/BNB), 16:18-16:23 "跳过本批信号" ≥100 次(CC-30); 重启重播种 300 含 bl 32 → rotate remove 32 复读 feed 268。
...
| CC-23 | 管道(owner/M) | 429 级联=唯一裸奔路径; REST 预算补丁 claude/dev-rest-budget 61631b0 候部署=第一优先; v40 自带 REST 拉取≈150 调用/min(首 12min)与下单同 IP; 全文=git 063b9e8 版§5 | 09-19 05:5x | open(候 owner; @16:3x since v40 5: 守护跳过 2/查 TPSL 3; +SOLV 16:09:37 tpsl_setup_failed 429→开仓事务回滚(非裸奔; 部署重启后 v40 首 12min REST 洪峰再现)) |
...
| CC-30 | owner 认领+裁决 | 11:06Z admin 批(main hunger off + symbols ""→300 池 + majors mcp2 + 双壳重启) 请 owner 认领; symbols "" ⇒ isAllowedSymbol 恒 true ⇒ majors 仓计入 main 槽位 + 重启重播种隔离币; 全文=git 063b9e8 版§5 | 09-19 11:4x | open(@16:3x 部署后仍在: strategy_signal.go:691 循环未改; 16:16 ZIL 开仓即 3/3=ZIL+majors ETH/BNB, 16:18-16:23 跳过本批 ≥100 次 ⇒ 20 项上限已修, symbols 300 显式列表可写(重启键)+mcp 回 owner 10 二选一请 owner 令) |
| CC-31 | M候选(括号重锚) | 全文=git 063b9e8 版§5 | 09-19 12:4x | **closed@16:3x: owner 部署 e07f3d7 reanchorTPSLToFill(ZIL 滑点 −0.03%/XMR +0.007% 实证)** |
| CC-32 | owner 认领+拍板(v40) | main 代码被换为 v40(tpl1057, 13:42:49 起, 零 audit): 请 owner 认领+拍板 A 保留 v40 评判(CC 推荐) / B 回滚 tpl998; 未拍板前按 _exp_cc EVAL 线执行(劣化→TG+rollback tpl998); 全文=git 063b9e8 版§5 | 09-19 14:3x | open(@16:3x v40 段 n≈21 −1.97 wr55% 费后≈−3.9; worsen 未触; 必修①bl 闸已由 3e1e467 部署解决, ②reanchor 已部署, ③REST 预算仍候) |
...
| CC-34 | owner 认领 | 三项 admin/无 audit 动作请 owner 认领: ①16:04 部署 e07f3d7+3e1e467(本分支) ②15:36 symbol_reentry_cooldown_minutes→0(main+majors) ③16:20 main bl +BNB; 附建议: BTC/ETH 一并入 main bl(现靠 strategy_signal.go 组内互斥兜底) | 09-19 16:3x | open |
...
| 计数项 | 读数 | 更新轮 | 备注 |
| **09-19 16:3x 读数(income 口径; DB 只作下界)** | 1h −1.6(毛 −1.2 费 −0.4) / 3h −4.6(毛 −2.5 费 −2.0) / 6h −7.9(毛 −4.6 费 −3.3 ≈−5%) / 12h −32.2(毛 −20.0 费 −12.2) / 24h −85.9(毛 −42.9 费 −43.1; 入金 +58.5 另列 ≈−50%); DB 24h(main) n127 −50.6 wr40% 多 75/−14.4 空 52/−36.2, 死法 SL<2m 21/−41.5, SL2-5m 23/−36.9, SL5-45m 26/−37.5, 45m 超时 6/−4.6, 赢 51/+70.0; **v40 段(14:11 起) n≈21 −1.97 wr55%(DB 14: SL2-5m 2/−1.41, SL5-45m 5/−4.23, 赢 7/+3.63; income-only RONIN −1.12/WAL −0.50/MITO +0.20/ALCH +0.24/ZK +0.41/ZIL +0.57/ROBO +0.22), 费≈−1.9 ⇒ 费后≈−3.9(−0.19/笔); 成交 conf n44 0.45-1.0 中位 0.77(<0.5:1/0.5-0.7:12/≥0.7:31); 笔/日≈130-150(24h DB 127+丢行) vs 基线 38.7 但均净<0=无效频率; 重启后 16:18-16:23 跳过本批 ≥100 次**; majors S35 段 n≈5 ≈+0.3; 15:2x 读数全文=git 063b9e8 版§6 | 09-19 16:3x | 判据不变 |
...
## 7. 运行日志（每轮一行，新行追加在表首）
| 2026-09-19 16:1x-16:3x | 定时轮: 管道活(REST 200); 钱包≈avail 121.8+保证金≈25≈147(在飞 XMR 空/DASH 多/STEEM 多 + majors ETH 多); income 1h −1.6/6h −7.9(≈−5%)/12h −32.2/24h −85.9(≈−50%; 入金 +58.5 另列)🆘 只报(owner 令不缩表); **16:04:12Z backend 重启=owner 部署 e07f3d7+3e1e467**(锚到成交价上线/排序改盈利优先/bl 20 项截断修=H-0919g 真根因) → 重播种 300 含 bl 32 → rotate remove 32(feed 268, bl∩feed=∅, ∩majors=∅); v40 段 n≈21 −1.97 wr55% 费后≈−3.9, worsen 未触→EVAL 继续(09-20 02:00Z/n≥30); 止损完整性: main 14/14+重启后 4/4✓(ZIL/XMR/DASH/STEEM), majors ETH✓, max_hold 45/720 ⇒ 两实例非裸奔; 429 since v40 5+SOLV 事务回滚 1; 止损失效 0 新例; 隔离: 无同 symbol 跨实例仓; DS: 15:52 续锁/16:09 lev 5→4 裸改→watch_ds(eval 22:00Z)/15:36 reentry_cd 0=owner/16:20 bl+BNB; 槽位: 16:16 ZIL 开仓即 3/3(含 majors ETH/BNB), 16:18-16:23 跳过本批 ≥100 次=mcp3 实际 1-2 槽(CC-30); 改动: 0 交易键; 宏观 FGI71 BTC +1.0% ETH +1.9% 市值 −1.3% trending∩feed=ZAMA/ZEC/PONS/AVAX/SUI/TAO/PRL(NEAR/AR 已隔离); 留言板 5 条 6.9KB 复读✓(本条 1035 字超 600 限, 下轮收紧), _exp_cc reads_1630/deploy_1604/watch_ds.leverage 复读✓; 下轮=17:1x: lev4 watch 早读; v40 n≥30; DS 19:09 节拍; CC-34 认领 |
