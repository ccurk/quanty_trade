...
| qt-trend-long | 827ffe8c | 趋势动量多 | **COTI/TUT**(种子2=feed2✓#25重启后重建正确;ACE隔离@08-22 12:22;aging=TUT;破荒史git f43756c) | running | **tpl575(S24)** pct0.05 mcp3 buy trailing on;**三值包_exp终裁@08-23 06:2x=ROLLBACK执行**(段n18≥16门到:毛+0.12净−0.40≤0→预注册二元回滚;现值=adt1.0+lcp0.15+mces3;程序stop[poll1]-PATCH-start✓running✓feed2✓);**归因:亏损全源ACE 4笔毛−4.40(已隔离),ex-ACE n14毛+4.52净+4.11 wr86%毛均+0.323费覆🟢,隔离后子段n10净+1.65 wr80%→#69重申请候选(§4队首,owner否决窗至09:1x轮)**;#65浮点修复随三值忠实回退(lcp0.15,重申请时恢复0.149);**常驻保险=滚动6h净≤−5U→stop(继承)**;三值包+延评全史git 8a1d995/f71d825;S24 keep史git f43756c;§4#55已被吸收史 |
...
- 隔离区(规则≤−4U∧n≥4)【13】: 4/CYS/TST/龙虾/BMT/BTW/H/APR/AIO/BICO/BEAT/XNY/**ACE**(+ACE@08-22 12:22[n4/−4.40闪崩滑点主因;榜上仍隔离=失血压过信号;详git 8a797ec])(+BEAT[n9/−4.19混合血统,main era仅−0.76 rehab须知]+XNY[n6/−4.23全main空]@08-17 13:0x;+BICO[n8/−4.05资金费磁铁§6结案]@08-17 03:2x;+AIO/APR/H等史git;逐条全文git bb3b883)。**池列值=快照,权威=每轮对账器输出**。**对账器现版v1.4权威副本=quanty-ledger分支ops/route_pools.py@08-22**(v1.4=roster遮蔽修复:ROSTER同角色双壳时stopped旧壳[21519f1b]覆盖running新壳[7583727a]的running态→fade差分v2上线以来静默跳过,MELANIA晋升条达标无plan实证@15:18;修复=running壳优先;头注阈值零改动;谱系v1.3@08-14←61b1bb4←377c567←f970704;v1.0默认分支禁跑;演进史git bb2e)。**arg4输入纪律(08-15实证):隔离表必须X/USDT形态**(裸名norm不match feed键→漏移)。出池只认头注规则或隔离,失格=aging_watch advisory;brk/lowvol出池阈值未注册(#42)前无自动出口。最近跑@08-23 06:21(v1.4): **plan=main.remove13(#25重播种泄漏:隔离9[4/ACE/APR/BEAT/BTW/CYS/H/TST/龙虾]+专家4[COTI/TUT/HANA/MELANIA])→rotate热清13/13执行✓feed100→87残留0**;aging_watch=trend.TUT+fade.HANA+fade.MELANIA✓;无新隔离;零差分连4跑史(03:1x/00:16/21:16/18:1x)止于#25;want.breakout=HANA/MELANIA/ONG/PUMP/TRUMP/TUT(#47锁);前跑00:16/21:16/18:1x零差分;15:18(v1.4首跑,MELANIA晋升churn1/4);12:20 ACE隔离热rotate✓(详史git 8a797ec);前史git d0e40d9/2ce0fb4/f623b98/044907d
- 保证金: Σ_running=main0.08×5+trend0.05×3+fadev2 0.04×2=**0.63≤0.75✓**(08-21 16:5x FLEET建壳重算;前史0.55见git)
- 互斥不变式: feed层@08-23 06:2x复核=main87/trend2/fadev2 3**两两∩∅**(热清#25后);种子层=main bl**18**=desired**18**零缺口✓;重播种**#25=08-23 03:18-06:21窗(无audit=非配置类重启,疑外部部署/崩溃;窗内main零成交=bl交易闸零违例;trend/v2种子重建完好;fadeV1未被误复活✓)清13/13;#24=08-22 18:2x计划内清11零挡;#23=07:54:47Z清10零挡;#20=08-19首例实证bl币可直漏feed('重启免疫'=交易闸级非feed级)**(#22及#8-#19史git);引擎互斥闸✓live;**fade壳seed5与main auto理论重叠=停机态无违例**(不变式限定running载具;若owner UI复活fade须先重划池,互斥闸+TG示警兜底);隔离币零成交✓(复核@08-21 09:2x,48h窗隔离12币零平仓行)
...
|---|---|---|---|---|---|
> 瘦身注(合并@08-23 06:4x): 08-22 04:1x行裁(owner优化直令轮,git 6283811系);08-22 03:1x行裁(git 6283811系);08-21 16:4x交互轮行裁(FLEET fade-v2建壳,git 9e896ed); 08-21 12:2x行裁(git f623b98); 08-19 21:13行裁(git 7e2b925);08-20 15:16转stub(git bf6c431);08-19 15:15裁(#61c全文§4+git 9d0dc94);08-19 18:14裁(git 82e6cf6);§3超长条压缩史git 0980231;旧注链在git历史。
