...
| 载具 | id | 原型 | 池 | 状态 | 门/备注 |
|---|---|---|---|---|---|
| main 通才 | 8eb182b6 | 通才(S18-S23栈) | auto·feed89(15:03重启重播种带回6隔离币→15:56热移除✓;bl13=对账器desired精确一致) | **running@08-14 15:03Z(#46窗重启,有主)** | pct0.08 sides=[buy,sell] tpl567;_exp=**RECOVER刹线常驻**(6h空侧净≤−8U→重latch[buy];本轮读−2.10✓(HEI为多单不入空侧口径;15:1x自纠更正);首评08-14 19:00Z);24h n24/−4.14 wr33:短侧+0.78 vs H空3败−3.89(48h差1笔到隔离线,观察)+HEI多SL−1.99;载具主死法=SL穿刺;⚠️sid伪影纪律照旧(§3) |
| qt-trend-long | 827ffe8c | 趋势动量多 | TUT/BICO/ACE/COTI/CAP(**种子5币✓@08-14 15:56** 空仓窗11s原子stop→PATCH→start,_exp/参数完好,5币K线回流实证) | running | **tpl575(S24)** pct0.05 mcp3 buy trailing on;_exp v3常驻监护:**累计首次转正 n20/+0.72**(台账n16/−1.85+段n4/+2.57;逐笔=COTI+0.51/ACE+1.51/ACE+1.72/ACE−1.17);keep门=累计净>0✓∧费覆✗(均净0.036<3×费0.12)→续观;劣化(累计≤−5U→rollback tpl567)与canary(6h≤−5U→stop)均远(6h+2.73);**终裁08-16 06:35Z**;ACE 24h n7/+3.44组最佳币∧trending榜;aging_watch=TUT/BICO;v1/v2全史git+config._exp |
| qt-fade-short | 21519f1b | 冲高回落空 | 1000CAT/ARC(种子✓;aging_watch全2币) | running@08-13 12:45Z | **tpl576(S25)** pct0.04 mcp2 sell;_exp v2续:累计n8/−2.63零增(现池零成交**第5轮**,对账器want.fade=[]=市况无冲高回落型,非载具故障);评期=累计n≥15或**08-16 12:50Z**;自动保险6h≤−6U(0✓) |
| qt-breakout-follow | 2111f5f9 | 突破追动量(S26) | 无(孤儿已释:AKE/BTR/COOKIE/EDEN回main feed✓@15:5x;DYM未被当前选集选中=场外自由身) | **stopped@08-14 06:50Z(评判FAIL,verdict-gated勿auto-start)** | _exp closed=段n15/−3.23≤−3U宽容线;诊断=trail0.8%回调帽截赢payoff倒挂(全文config._exp+git);复活=S27新原型周期(#47,门=组费覆🟢+新预注册+重ROUTE灌池;种子3币旧值勿直启);⚠️STAR空3闭+1飞行挂brk名=sid伪影(brk buy-only物理不可能开空=铁判据,实为main空侧;§3) |
...
- 互斥不变式: feed层✓(overlap=0三running:main89/trend5/fade2,08-14 15:5x隔离清除+种子同步后复核)>种子层blacklist13=隔离6+trend5+fade2(=对账器desired,#46+CAP种子双闭环@08-14 15:56)>⚠️引擎互斥闸未部署;COTI晋trend=v1.3 plan首个跨态再路由;aging=trend3/fade2 advisory
...
- **账户级position行sid=陈旧symbol→strategy映射伪影(已确认@08-14 09:xx)**: STAR空3笔挂brk名(brk buy-only物理不可能开空=铁判据)+BLUAI空挂fade名(fade feed仅2币无此币);实为main空侧(两币∈main feed87,45m hunger签名吻合,上轮空侧+2.04已含STAR=跨轮口径一致)。**归因权威=池归属纪律**,行sid仅参考。monitors(quick_trade/roi)DB驱动按行sid解析instance→错标仓由被标载具代管TP/SL(BLUAI由fade代管平仓−1.83@09:37实证=无害);风险注记=若载具间hunger/hold/tp/sl参数分叉,错标仓将按错参数被管——参数分叉改动前先核在飞仓sid。
...
|---|---|---|---|---|---|
| 08-14 18:05-18:2xZ | 告警响应(max) | 2h close −3.19(8): US空−2.94/H空−1.42/CAP多−1.10(18:01-07同窗三连SL) vs AKE+2.41/H+1.75/LAB+0.49 | 波动窗18:01-18:07单一行情脉冲 | 3条engine error核定=**平仓撤单竞态无害类**(定案入§3+§6计数):仓位全平✓零裸仓✓孤儿单双扫兜死✓CAP=trend开的sid伪影非互斥违规✓;零处置零config动作 | 现仅TAKE空1仓浮+0.64;RECOVER空侧6h≈−3.82远离−8U(首评19:00Z照常);trend canary 6h−2.27远离−5U;组刹车线全远;台账§3/§6/§7三处登记 |
| 08-14 15:49-16:0xZ | 例行轮(max) | close: 1h−1.99(1)/3h−3.69(2)/6h+0.69(10,wr50)/12h+2.73(26,wr58 be53=+5pp)/24h−6.03(43,wr47 be54=−7pp) | FGI29/BTC−1.0%/mcap−0.5%无降压;trending∩池=ACE+CAP(trend双热)+AKE/ETHFI/TAKE/UNI/VELVET(main5热) | ①ROUTE:15:03重启重播种带回6隔离币→main热移除(feed95→89,overlap0,残留0,bl13=desired✓)②**CAP种子同步落地**(§1.5待项;trend空仓窗11s原子15:56:14-22,symbols4→5,_exp/pct/mcp/sides/trailing全完好,重启后5币K线回流+信号评估实证)③0原子包(15:03后新增平仓=0=零新证据轮;RECOVER首评今19:00Z未到不动) | 钱包≈244.9全空仓;24h载具(时间感知归因):main n26/−8.40 wr31(**长侧12/−9.18 wr17=主失血** vs 空侧14/+0.78;n12<20登记不动)/trend n13/+2.85 wr69/fade 0成交第6轮(want.fade=[]=市况无型)/brk段残4/−0.48;死法:中段SL 13/−18.22主刀>快SL 2/−3.60,超时13/+4.97与饥饿TP 10/+8.41全正;H空48h n3/−3.89差1笔隔离线(对账器在看);费覆组🔴avg−0.140 vs 线0.121禁提频(trend 24h🟢0.219=5.2×);四保险全未触;audit零无主 |
| 08-14 14:38-15:1xZ | 例行轮(max) | close: 1h0(0)/3h−1.97(3)/6h+1.02(11,wr55 be50=+5pp)/12h**+3.87**(29,wr59=+8pp)/24h−4.68(44,wr48 be54=−6pp;真净−6.26=佣−1.78资费+0.20) | FGI29/BTC−1.89%/mcap−1.60%无降压;trending∩池=**ACE+CAP双热(trend)**+ETHFI(main),AKE孤儿在榜 | ①ROUTE v1.3:**CAP晋trend执行**(48h L n3/+1.52 wrL67达门;先main remove后trend add,main86/trend5/overlap0/churn1;无新隔离,H n3/−3.89差1笔到线观察)②**trend累计首次转正n20/+0.72**(keep门费覆未达0.036<0.12,v3续观) ③**#46落地@15:03**(HEI SL−1.99@15:02开真窗;抢窗器全序stop10s→PATCH bl13验证→start;重播种100→rotate−5自愈95;竞态教训立规一窗一执行器,详§5维护注) ④0原子包(RECOVER首评今19:00Z;trend终裁/fade评期08-16) | 钱包≈245.4全空仓收轮(HEI终局SL−1.99);组24h死法=SL穿刺12/−18.74 vs TP+8.02·超时+4.97·饥饿+1.07(其余机制全正);main 24h n23/−2.15;brk段账closed;fade零成交第5轮;费覆组🔴−0.106/笔vs线0.121禁提频(trend🟢0.219=5.2×);四保险全未触;audit零无主改动 |
| 08-14 09:28-09:5xZ | 例行轮(max) | close: 1h−1.66(2)/3h+0.75(8)/6h+2.04(16,wr62 be56=+7pp)/12h−4.45(28,wr50=−10pp)/24h−6.30(37,wr46 be55=−9pp) | FGI29/BTC−1.44%/mcap−0.94%无降压;trending∩池=ETHFI(main)+AKE(孤儿在榜) | ①**S24评期v2到点裁决(累计n16≥15先到)=keep未达(−1.85<0)∧劣化未触(>−5U)→S24保留转v3常驻监护+种子同步COTI入config**(原子窗09:41,15s;终裁08-16 06:35Z不后移) ②ROUTE v1.3 plan={}零动作(无晋降隔离;bl_sync与#46期望一致) ③#46三连试失败回队(main stop异步>30s入§3;3重启全有主) ④sid伪影机制确认入§3(STAR→brk/BLUAI→fade,实为main空侧;BLUAI由fade代管平仓实证无害) ⑤1原子包 | 钱包≈247(avail220.1+COTI/STAR两仓保证金≈27);main 24h n23/−6.48 wr30=空侧+1.05 vs 长侧−7.53,组24h主失血=SL穿刺11笔/−17.87(硬超时10/+1.34,饥饿域6/+7.85);trend累n16/−1.85(ACE 4W0L+1.38/TUT nL2/−3.43);fade零成交第4轮;费覆组🔴−0.17/笔禁提频(3×费≈0.12);组6h+2.04正,四保险全未触;RECOVER空侧6h+3.75,首评今19:00Z |
