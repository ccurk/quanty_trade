...
| CC-20 | 评判 | FIX max_atr_pct 6→0.8(_exp_cc 09-19 00:24:49Z;重启 00:25:26Z 首单起): n≥30 或 06:00Z: 均净(毛)≥+0.3 ∧ wr≥55% ∧ 费后均净≥0 ∧ 笔/日≥60;ATR 桶反推=tpsl 日志 sl距/1.5(新进程);worsen 段 n≥15∧净≤−4U 或 笔/日<38.7→回滚 6 或放宽 1.0;同段单独计 TP(2×ATR) 命中与被封赢单(owner 2.0/1.5 同时生效=混杂) | 09-19 00:2x | open(**自查点 1/3 @01:18Z: 门生效实证=重启后 16/16 入场 ATR 0.50~0.76%(反推 sl距/1.5, tp/sl 比 1.33=owner 2.0/1.5 生效),重启前 35% >0.8 → 0/16 偶然概率≈0.1%;拒绝原因只写 detail.hard_filter_reject(code:1155)不落后端日志,'ATR%过高'查询 0 行不代表未生效;段 n15 毛 −1.87 wr47% 费后 −0.33/笔 速率 404/日(重启后速开仓 burst,§6 n=7);1h income 毛 −0.52 费 −3.42;DS 0 PATCH;feed 276 ∩=∅;-1003 IP 2400 req/min 触 01:11:37→ORDER 开仓回滚(300 币 REST 回退+监控量,报 owner);n<30 不裁**) |
> 结案集@09-19 00:2x: CC-12(owner atr 2.0/1.5 经 CC force 重启 00:25:26Z 生效)/CC-13(p6 封段 17:56Z n71 −34.75)/CC-17(p7 裁决见 §6)/CC-18(ROUTE 7 币落地 20:2x) 终态全文=git 5096ca1版§5;更早结案集=git 2fcc0da版§5。
