...
| 80 | 代码候选(main多头;S33编号未消耗) | **设计轮证伪@09-05——双子签名不apply**(死赢全域重叠+三批满门枯竭=定义级过拟合禁发货;论证全文+残差=ops/exp_archive/s33_calibration_20260905.json+batch2/3细目;量表史git ab10f1d→0097bd7链) | 全表在archive | 24案=15死9赢 | **退回假设库攒证据**:重启线=新特征族≥15死+≥8赢精测分离≥80%∧不误杀top2赢例;出路唯一化=新特征族(orderflow taker_buy占比/资费截面)在24案表回测达线才发货;旧量表逐例精测停止,T+1死例登记继续;**batch4闭环@09-10**:赢例对照4例入库ops/exp_archive/batch4_wins_20260910/(pre15m: HEMI+0.769→0.397/MIRA 0.475/VVV 0.531/MARSCOIN 0.450 vs 死例0.500-0.529)→赢例带完全罩住死例带且方向反直觉(最大赢=卖压中入场),**taker_buy pre15m退出候选**;族出路=资费截面或新死例批 |
...
| # | 类型 | 内容（含完整意图） | 登记轮 | 状态 |
|---|---|---|---|---|
| #88 | PATCH | main bl种子16→22(+6 TradFi币,格式照现值config口径复核后写):断连解除(ctx fetch_error消失)∧main空仓窗→stop→PATCH{symbol_blacklist单键,勿动_exp}→start→复读;意图=TradFi隔离重启免疫(rotate系内存态,重播种会放回) | 09-10 04:1x | open·等断连解除(03:1x cron直证仍SERVFAIL≥107.0h同串(#33);desired_bl22 45连零一致;+6币斜杠形态照bl16口径已复核;落地窗=恢复瞬间全账户空仓stop→PATCH bl22单键勿动_exp→start→复读;probe史归并git adfc233) |
...
| 计数项 | 读数 | 更新轮 | 备注 |
| backend→Binance REST DNS断连 | **n=1(episode): 09-09 16:12Z起持续,09-14 03:12仍在(≥107.0h;probe史63miss+cron直证廿五轮=git链→f8cf733→b27911f→3f6d7a6;03:1x直证=active positionRisk+main/trend ctx income同串fapi SERVFAIL+balance伪零双载具;ws活证main SNXX评估@03:12:03+trend USELESS@03:12:00 recv3/3;feed183/3无重播种=无重启=#89未部署;S31宽度0.792四连ON=窗新高)** | 09-14 03:1x | 特征=fapi SERVFAIL(resolver 100.100.100.100=Tailscale MagicDNS,修复在宿主机);伪影族=ctx balance/income伪零+skip文案+closed窗过滤失效(close_time过滤免疫)+active回error(全谱git ecec3b0);**已升§3复发向量n=2+#89补丁c49c555候部署(§4)**;本轮re-arm×3@03:57/04:57/05:57→恢复即落#88;恢复后复核:①income补记完整性②feed重播种③断连窗内重启(ws也死须立报);⚠️断连窗禁stop/start(重订阅可能失败) |
...
| #79·closed行sid按symbol旧绑定继承(#77族新亚型) | 两形态:①**双行**(main行+末代壳行同笔PnL双写,壳行tp/sl null裸奔,出场治理权=两行落库序竞态,08-30 03:34壳饥饿档45m真接管收割−0.61实锤;判别特征=末代持仓币BTR/NIL/COLLECT类,泛池币牛来/USELESS零双行)②**空sid**=binance_only视图伪影(全量视图sid=main齐);binance_only对双行币dedup保壳行=池归因陷阱 | 09-01 12:5x | 归因纪律:sid冲突以载具日志开仓链为准,逐笔归因用全量视图+日志链;机制修复并入#77 ownership单一化(候owner部署),部署后复验消失;计数史n38→63全链+逐案细目=git ed40296→933abbd→dd4326e→f9c8407→c6a9c02版行(**末读n=63@09-09 12:3x;断连窗零增**);归因纪律升级(疑异常→全量视图对拆)git e6f5b0f/47753ae |
| S31·regime动态方向偏置(**结案KEEP@09-02 06:3x**) | v1谱=宽度三档(EMA-up<0.35 OFF多+0.10/0.35-0.60 N/>0.60 ON多−0.05空+0.05)阈值内联+False-guard,上线09-01 10:02=tpl988;评判KEEP@09-02(段n22/15W7L毛−0.11,ON档多n3均+0.73 vs N/OFF n5净−1.99,劣化线双绿);verdict全文ops/exp_archive/s31_verdict_20260902.json+git 1417e31/eea7cb7/558c235系 | 08-29 17:0x | 常规监控;复核判据:多头累计≤−5U或ON档均净转负→议关guard;verdict后多头4笔+1.96@09-02判据远离；+09-05 09:2x宽度首直采=0.65(138币eval行EMA-up90/138)=ON档实证@−3.2%大盘日；门反证@09-05(死亡簇非OFF窗产物,S31非失血向量)=git f9c8407;宽度直采纪律入轮程;翻转速度=单日内;直采史链git ff561cc→f9c8407;**末读0.792(145/183)ON档@09-14 03:1x(四连ON 0.639→0.617→0.639→0.792;带上界0.639→0.792扩张=断连窗新高;此前单日0.268OFF→0.639ON全档穿越;全链史git 9e8dc48→…→3f6d7a6)**,日内振荡常态确认+3h内OFF→ON全幅翻转实证,恢复窗首批信号档位须复核(现值ON=多头premium−0.05),判据(多头累计≤−5U或ON档均净转负)远离;断连零成交=零live影响 |
| funding磁铁·他币再现watch(重开@08-26;BICO条引用) | era史n2/−2.83+BICO/ACE史=git 9b9c39d/86e771d/8a797ec/bb3b883;**S30深档减半实证@09-02(+0.85)** | 08-27 00:2x | **S30已上线@08-29(owner直令跳磁铁门)**;watch=疫苗效果验证;疫苗期细目+源码定案git 8729d22/ad3c8d4版行;**下修议案+升级门预注册全文git 8917d4e/47753ae(已执行@09-08,劣化线活在config._exp)**;**多头cohort史git ad3c8d4/e6f5b0f**;**空侧镜像(资金费率>0.0003:+0.20;判据=空侧决胜累计n≥6∧净≤−3U→议短侧入S33包)**:逐轮决胜扫描纪律=新入场评分行含资费bonus且减bonus后<门即计入;批史与逐案细目git 933abbd/e6f5b0f/18ebd9a/8917d4e/47753ae;**S33下修执行@09-08 09:3x(tpl998):触发门n10/−5.82达标;扫描标签自tpl998起=S33前缀,旧S30停产;执行全案(SOPH决胜#10/0.700恰门存疑不计纪律/标签清单)=git dd4326e/f9c8407版行**;**镜像n38/+3.30@09-09 03:1x(批细目git f9c8407/0809b7a)**;S33段员=open_time>apply 09-08 09:35Z判(定纪律@12:3x,平仓跨轮不漂移);**S33段读数史(n8/−0.755→n8/−0.388漂移三证→n12/−0.334去重)全链=git 0097bd7→c6a9c02→805a115版行;终读@09-10 04:1x:多头n12/−0.334均−0.028 vs 基线−0.512改善向(n<15门未达;空头段n27/−2.71);断连窗cohort冻结;行集回摆=懒生成再证→评判权威=git n12读数** |
...
## 7. 运行日志（每轮一行，新行追加在表首）
| 2026-09-14 05:5x | 注记: 断连probe#3 miss(同串SERVFAIL@05:58,≥109.8h,末次;ws活证main评估行@05:58:04);session probe预算尽,交接06:1x cron |
| 2026-09-14 04:5x | 注记: 断连probe#2 miss(同串SERVFAIL@04:57,≥108.75h;ws活证main评估行@04:58:04);probe#3@05:57候(末次) |
| 2026-09-14 03:5x | 注记: 断连probe#1 miss(同串SERVFAIL@03:57,≥107.75h;ws活证main评估行@03:57:04);probe#2@04:57候 |
| 2026-09-14 03:1x | 断连轮#33(≥107.0h,锚09-09 16:12Z): 三直证同串@03:12(active positionRisk fapi SERVFAIL on 100.100.100.100+main/trend ctx income同串+balance伪零双载具);ws活证(main SNXX评估@03:12:03+IDLE recv=183/183+trend USELESS评估@03:12:00 recv=3/3);feed183/3/0无重播种=无重启=#89(c49c555)未部署;audit尾1808/1807零无主;config零漂移双载具(main cs五键+lcp0.15/scp0.05/pct0.08/trailing1/1.2/be1/bl16/etw""/ao=false,trend lcp0.149/pct0.05/mcp3/buy/池TUT,USELESS,CAP);真窗n0(839实行全≤09-09 14:56,幻影2008/2847零增);ROUTE v1.6 plan={}45连零/quarantine19/desired_bl22=#88一致/aging trend三币;**S31宽度0.792(145/183)ON档=四连ON(0.639→0.617→0.639→0.792),带上界0.639→0.792扩张=断连窗新高**;S33冻结KEEP-armed零动;宏观FGI57 Greed/BTC77774+0.65%/mcap−2.43%回升未触;trending∩main池={PONS,PENGU,LIT,ARB}(BTW上榜但隔离在位✓);刹车未触(零成交);保证金0.55✓;决策=HOLD 0原子包;probe×3 re-arm 03:57/04:57/05:57(恢复即落#88+复核三项:income补记/feed重播种/窗内重启,交接06:1x cron);TG=续催宿主resolver+#89部署(≥107h) |
| 2026-09-14 00:1x | 断连轮#32(≥104.1h,锚09-09 16:12Z): 三直证同串@00:12-15(active positionRisk fapi SERVFAIL on 100.100.100.100×4+main/trend ctx income fetch_error+balance伪零双载具);ws活证(main VELVET评分@00:13:07+trend IDLE recv=3/3@00:15:33+CAP评估@00:15:03);feed183/3/0无重播种=无重启=#89(c49c555)未部署;audit尾1808/1807零无主;config零漂移双载具(main cs五键true/21/0.6/1.4/0.55+lcp0.15/scp0.05/pct0.08/trailing1/1.2/be1/bl16/etw""/ao=false,trend lcp0.149/pct0.05/mcp3/buy/池TUT,USELESS,CAP);真窗n0(839实行全≤09-09 14:56,幻影2008/2847零增);ROUTE v1.6 plan={}44连零/quarantine19/desired_bl22=#88一致/aging trend三币;S31宽度0.639(117/183)ON档=三连ON(0.639→0.617→0.639)带0.268-0.639上界复触;S33冻结KEEP-armed零动;宏观FGI57 Greed/BTC76912−0.45%/mcap−4.18%未触;trending∩main池={LIT,PENGU,PONS,PRL,PUMP};刹车未触(零成交);保证金0.55✓;决策=HOLD 0原子包;probe×3 re-arm 00:57/01:57/02:57(恢复即落#88+复核三项:income补记/feed重播种/窗内重启,交接03:1x cron);TG=续催宿主resolver+#89部署(≥104h) |
> 瘦身注(指针@09-14 03:1x归并): 断连#31(21:1x)全文+#32后probe#1-3注记(00:5x/01:5x/02:5x)=git 3f6d7a6版行。
> 瘦身注(指针@09-14 00:1x归并): 断连#30(18:1x)全文+#31后probe#1-3注记(21:5x/22:5x/23:5x)=git b27911f版行。
...
> 瘦身注(指针@09-13 09:1x归并): 断连#25(03:1x)全文+#25后probe#3注记(05:5x)+#26后probe#1-3注记(06:5x/07:5x/08:5x)=git 11361fb版行。
> 瘦身注(指针@09-13 06:1x归并): 断连#23(09-12 21:1x)/#24(00:1x)全文+probe#1/#2注记=git 94e9b88版行；断连#22系(前归并@09-13 00:1x): 断连#22(18:1x全文+probe×3注记)=git e40f8cf版行;断连#19(09:1x)/#20(12:1x)=git ecec3b0版行;#21(15:1x)+probe#1/#2注记=git 981f7ed版行;#11=git fc2c6d4;#12=git 0207b9f;#13(15:1x)=git 1e6270a版行;#14(18:1x)/#15(21:1x)=git 9e8dc48版行;#16(00:1x)=git 01d79a0版行;#17(03:1x)=git dc081bf版行;#18(06:1x)=git f4090af版行。
