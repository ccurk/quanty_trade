...
| 载具 | id | 原型 | 池 | 状态 | 门/备注 |
|---|---|---|---|---|---|
| main 通才 | 8eb182b6 | 通才(S18-S23栈) | auto·**feed89@08-23 21:3x稳定(#29热清生效实为18:50-51两步100→94→89=purge tick滞后;TUT在押放行反例→§6观察)**(#28/#27史git f171d49/f63a060) | running | pct0.08 sides=[buy,sell] **long_conf_premium=0.35(#61c终裁rollback@08-19 15:3x,门0.90=多头事实关闸,#61族基线)** tpl567;**trailing(act1.0/cb1.2%)+BE(1.0)=基线几何(#49KEEP)**;**_exp=closed rollback(#61c;史§4#61/#63+git 0940c62)**;RECOVER刹线6h空≤−8U继承;#11降级观察(§4) |
| qt-trend-long | 827ffe8c | 趋势动量多 | **COTI/TUT**(种子2=feed2✓#26重启后重建正确;ACE隔离@08-22 12:22;aging=COTI+TUT;破荒史git f43756c) | running | **tpl575(S24)** pct0.05 mcp3 buy trailing on;**_exp=open·#69三值包重申请@08-23 09:18Z**(adt0.5+lcp0.149+mces off;终裁二元=新段净>0→keep否则回滚[adt1.0/lcp0.15/mces3];eval=n≥16或08-25 09:18Z;劣化线n≥8∧≤−4U;**进度@08-24 00:2x段n2毛+0.02**(COTI超时−0.06/TUT快胜+0.08;+COTI 00:08L在飞;feed2在流);否决窗空置+程序史git 492779a);原三值段终裁rollback(n18净−0.40)+ex-ACE归因(n14净+4.11 wr86%)全文git 5df52d2+config史;**常驻保险=滚动6h净≤−5U→stop(继承)**;三值包全史git 8a1d995/f71d825;S24 keep史git f43756c |
| qt-fade-short-v2 | 7583727a | 冲高回落空(**tpl823/S28**;谱系tpl576/S25) | HANA/COLLECT/MELANIA(feed3=种子3✓;MELANIA晋升@08-22;#57-ROUTE病理活测第3例看护) | running | **S28衰竭签名直入上线@08-23 21:24Z**(#66修复:short∧RSI>75∧价≥布林上轨∧末bar涨<0.3×ATR%仅旁路EMA方向+趋稳一门,S19/S20/RSI<32/量能/ATR带/追空ext全保留;附随#65 epsilon永久化[ss≥thr−1e-9;0.60精确组合RSI0.30+BB0.15+MACD0.15=S28主通道解锁];apply字节复检✓零config泄漏✓feed3✓烟雾MELANIA18h task30;载具级重启实证→§6);**_exp=open·EXP:S28**(二元:段净[Σpnl−名义×0.1%]>0→keep否则rollback回tpl576;劣化线段n≥8∧≤−4U即回;评期段n≥16或08-25 21Z;崩溃立即回滚;**进度@08-24 00:2x段n0**=2.8h管道健康[评估流活/0Traceback/衰竭条件未现]);低波三值包终裁ROLLBACK@08-23 18:2x(低波解锁不付费=组级知识;全文git a2e845f+config史);保险=6h净≤−6U停(继承) |
...
- **[升格v2@08-23 15:2x] 平台级crash loop确认并加速(非apply非start API路径)**: recv计数回落法(main IDLE行top计数跌回~202=重启+回填200bar签名)33h窗计15次重启: 08-22×3→08-23 15h内×12,最近2h 4次(13:14/13:36/14:44/15:07)≈28min间隔,趋势=恶化;方法学修正=旧'IDLE symbols转换法'只见有rotate间隔的重启(12:1x轮报2.7-4.8h系探测偏差,真频率高5倍);已知#3/#4例(08-22 05:49/07:54 fade2 START精锚)交叉验证✓,#1/#2例史见git。影响=清内存态(CB/统计/rotate态/择优攒批窗)不清DB(config/种子/持仓/LastEntryAt);跨重启完好实证: hunger计时DB open_time锚(COLLECT 11:33开跨12:02重启12:18准点45m收割)+交易所挂单TP/SL存续+bl交易闸连8窗零违例。残余: ①main feed长期泄漏态(热清寿命≈下次重启≤1h) ②重启后≤2min突发开仓n=2(§6) ③评期内存计数断链(#34/#37域)。TG已连2轮报owner;修复权=owner(硬边界#6);**降温@08-24 00:2x轮:16:05Z后至00:17连续8.2h零重启(top695连续)=冷却确立,观察续**
...
| 计数项 | 读数 | 更新轮 | 备注 |
|---|---|---|---|
| main断流观测(三闸交集关门) | **断流结束@08-23 18:24**(UAI空开仓,总时长23.0h[BTR 08-22 19:20→UAI 18:24];该笔60m超时−0.56;三闸交集设计性关门定性维持,18:1x首登全文git a2e845f);本轮main笔数=2(UAI−0.56超时/TAC+1.13饥饿W) | 08-24 00:2x | 判据不变:再现6h+零笔且池ATR中位≥0.5→查管道;每轮记main笔数 |
...
| 重启后速开仓观测(crash loop伴生) | **n=2维持@08-23 18:1x**(15:22/16:05两重启后无≤2min开仓:15:39 MELANIA=+17min不计,16:05后首开18:02=+117min;史14:44→63s/15:07→106s);机制候选=重启清cd_sec态+回填后立即重评 | 08-23 18:1x | ≥5例且亏单≥3→议重启后静默期;n<5只计数 |
| closed平仓行消失观测(≤48h短窗) | **首例反转@08-23 03:1x: 00:12消失的2行(COLLECT 05:24−0.00/06:56−0.56)在03:1x现拉完整重现**→定性改判=closed API 48h窗存在瞬时缺行(单pull缺行≠删除),收养吸收假说排除(吸收应不可逆);行未消失只是拉取瞬态 | 08-23 03:1x | 判据改:瞬时缺行复现≥3例(不同轮)→读positions查询路径源码找竞态;纪律不变=一切终裁以裁决时现拉为准;与§3'>120h长窗丢行'分立 |
...
| 08-24 00:1x-00:3xZ | 例行轮(max) | 1h0/3h+1.10(1)/6h毛+0.65净+0.57(3)/12h净+0.02(7)/24h毛+0.70净+0.27(16,wr62/be57=+5pp转正) | FGI73/BTC+0.7%/市值−1.8%无降压 | **HOLD 0原子包**;ROUTE=#31对账plan={}零差分连2(aging=fade.COLLECT+HANA;want全在位;bl_sync18无漂移);两_exp未到门[trend#69段n2毛+0.02+COTI 00:08L在飞·v2 S28段n0管道健康:评估流活/0Traceback/衰竭条件未现(池ATR%0.22-0.37 RSI44-67)];刹车全绿载具保险绿;费覆🔴毛均+0.044<3×费0.081(改善自−0.076) | crash loop降温=16:05Z后8.2h零重启(top695);ACE隔离复核✓48h窗4笔全在12:22隔离令前;互斥main89/trend2/v2 3∩∅✓;TRANSFER−50=已知出金;钱包169.0(avail157.1+COTI保证金11.9);audit零无主;幻影0/71;trending∩main=LIT/PUMP/ENA |
| 08-23 21:1x-21:4xZ | 例行轮(max) | 1h0/3h−0.48(2)/6h毛−0.21净−0.31(5)/12h−1.00(7)/24h毛−1.30净−1.53(17,wr53/be64=−11pp🔴) | FGI66/BTC+0.8%/市值−2.5%无降压 | **S28衰竭签名直入apply@v2=唯一原子包**(tpl576→823;_exp open二元[段净>0]评期n≥16或08-25 21Z;#65 epsilon附随;字节复检✓零泄漏✓烟雾task31完成不崩✓[18h窗0成交=冷启动已知;API事实:backtest body的hours字段被忽略,须显式start_time/end_time否则默认168h窗——task30=168h误发无害在跑]);**ROUTE=#30对账plan={}零差分**(aging=fade.COLLECT+HANA;trend双币脱watch);刹车全绿载具保险绿;费覆🔴毛均−0.076 | main断流23h结束=UAI 18:24空60m超时−0.56(本轮唯一平仓,管道自证活);crash loop降温确认16:05后5h08m零重启;#29热清实际生效18:50-51两步100→94→89(purge tick滞后=前轮"18:3x生效"校正);apply载具级重启实证→§6新行;trend#69段n2毛+0.02(COTI−0.06超时/TUT+0.08);钱包168.09=出金后新基线(刹线6h13.4U/24h33.6U);audit零无主;幻影0/70 |
...
| 08-23 03:1x-03:3xZ | 例行轮(max) | 24h毛−0.65净−2.31(57) | FGI66 | **HOLD 0原子包;两_exp未到门[trend14/16·v2 11/16];ROUTE零差分连4;刹车全绿;费覆🔴** | stub;全文git 8a1d995 |
| 08-23 00:1x-00:3xZ | 例行轮(max) | 24h净−2.22(53) | FGI66 | **HOLD 0原子包;ROUTE零差分;§6消失2行首例观测;刹车全绿;费覆🔴** | stub;全文git 87615b8 |
|---|---|---|---|---|---|
