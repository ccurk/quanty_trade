...
|---|---|---|---|
| 2026-09-20 06:1x-06:3x(v6.0 #9) | 上轮(05:31)= [攻-1] owner UI hunger 三键 true/0.10/0.20 / [攻-2] owner mcp 1→3(第 5 次)+DS 门 MIN_TRADES 30→10 或 QT_WINDOW_HOURS 24 | **DS 0 交易键改动(第 6 轮)**: audit admin 03:31:57 后仅 **06:22:16 #1954 = qt_breaker 写 _exp lock-1789885335-breaker(pct 0.25→0.25 空改, 冻 order_amount_pct 至 09-21 06:22Z)**; mcp 锁 05:39Z/max_hold 锁 05:54Z 到期仍不动 ⇒ 样本门实锤: ctx 6h trades_window.count=21(按 fills 计: 6 仓 21 fills)<MIN_TRADES 30 ⇒ [skip]; 现值 lev10 pct0.25 mcp1 tp/sl pct0 hunger false(sl 0.04/tp 0.30) max_hold90; majors lev10 pct0.25 mcp2 max_hold720 | **效果**: 无改动 ⇒ 北极星无前后对比. 逐笔 06:17Z: 1h 1/−2.13 3h 2/−1.72 6h 6/−2.85 wr67% 12h 48/−5.07 24h 127/−14.48(ctx 24h income −1003 限频 income_complete=false 弃用); avail 117.4(钱包≈147); 持仓 1: main CTR 空 06:16:23 296U(conf 0.859 mult0.8) 腿 tp −1.53%(+4.5U)/sl +1.23%(−3.6U) 90 钟→07:46; majors 0 仓. 首笔 lev10 BIGTIME 04:46→06:16 90 钟 −0.71% 价=−2.13U(301U). mcp1 段(23:39Z 起 6.6h) main 5 开=18/日, 槽 3s 回填(06:16:20 平→06:16:23 开; 同批 BULLA conf1.00 适配 1.496/IOST 0.944 被跳). **病根(ATR 分层, 腿反推 ATR=SL距/2)**: 15m 段 main n11: ATR≤2.5% 8 笔 均 +0.52% 价(POWR+1.07/ATOM+0.99/ROSE+1.11/SUI+0.79/VANA+0.50/PUNDIX+0.48/NIL−0.04/BIGTIME−0.71) vs ATR>2.5% 3 笔(全 BANK 3.3-3.9%, 腿 6.6-7.8%) 均 −2.89%(−6.75/−3.94/+2.03) ⇒ 300U 下 CTR 腿 −3.6U vs BANK 型 −20U=6 倍风险差, 零金额帽. 48h hold 桶: <15m n291 −45.6 / 15-44m n50 +9.2(1m 时代 48 笔 +11.4 多 26/+3.7 空 22/+7.7) / 45m n21 −2.9 / 47-89 n4 +0.8 / 90m n5 −1.1 / >92 n7 −3.2. 止损核对: 两实例腿✓ 无裸奔; 隔离无撞车(CTR 仅 main; majors 有效池 BTC/ETH). **本轮建议(板 06:25:42Z; 首写 9560B 超 8KB ⇒ 二次裁为 2 条 6644B; TG msg 10826)**: [攻-1] 重发第 3 次 owner UI hunger_mode_enabled=true + hunger_tp 0.10 + hunger_sl 0.20(ROI; lev10=+1%/−2% 价=+3U/−6U@300U; DS BOUNDS (0,0) 禁) | 帽只咬 ATR>1% 币(2×ATR>2%)=亏损来源: BANK 3 笔 −8.66%→−1.97%, 段(38U) −3.30→−0.76; tp 0.10 把 ATOM(34m 到 +1%)/SUI(47m)/VANA(83m) 提前腾槽; 备选 sl 0.15=4.5U 合 3-5U 金额令, 代价 PUNDIX 型 MAE −1.29% 贴线(owner 拍板) | 验: 饥饿模式触发 tp=10.00% sl=20.00% 行·亏单 ≥−6.5U·中位 <60m·段均净 ≥+0.15. [攻-2] 重发第 6 次 owner UI mcp 1→3(先 攻-1 后落, 否则 3×300U 撞 BANK 型腿单波 −60U) | 笔/日 18→~54; 副作用 6h fills 21→~60 过样本门=DS 自解锁; 备选 宿主 MIN_TRADES 30→10(6h 全在 15m 时代, 比 QT_WINDOW_HOURS=24 混入 1m 时代干净) | 验: audit admin 新行(伴 _exp lock-*-ds)·开仓/h ≥2·候选开仓失败 归零. 0 交易键写入(两次 PATCH diff=[_ai_task_cc]). FGI 71 Greed; 市值 24h −4.5%; BTC 80485(−0.6%) ETH 2581(−1.6%); dom 59%; trending∩池 ZAMA/ENA/ZEC/PONS/PENGU/ARB/INJ/CELR/AVAX(ONE/AKE/NEAR bl) |
