...
|---|---|---|---|
| 后端DB层挂死watch(平台事故→已升§3@08-27 04:4x) | **n=2@08-27 04:32**: 03:5x重启后仅~30min复挂(04:01复活→04:08后某刻再挂→04:32回访确认同签名;04:37仍挂)=系统性根因非偶发;判据'再现→升§3+M通道候修'到点,双双执行(§3新条+§4#73 db-timeouts补丁已推候部署) | 08-27 04:4x | 后续判据: 部署#73后再现→靠/api/health/db秒判+readTimeout错误日志定位卡死query;owner侧待办=二次重启前SHOW FULL PROCESSLIST/INNODB STATUS/df -h(TG7749清单) |
