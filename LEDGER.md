...
| 载具 | id | 原型 | 池 | 状态 | 门/备注 |
|---|---|---|---|---|---|
| main 通才 | 8eb182b6 | 通才(S18-S23栈) | auto·**feed91@08-24 18:4x(ROUTE#37:bl18重启后泄漏9热清;COTI已释放入feed可交易;purge史#36及前见git)** | running | pct0.08 sides=[buy,sell] **bl18@08-24 18:4x(#70落地移COTI)** **long_conf_premium=0.35(#61c rollback@08-19,门0.90=多头事实关闸)** tpl567;**trailing(act1.0/cb1.2%)+BE(1.0)=基线几何(#49KEEP)**;**_exp=closed rollback(#61c;史§4#61/#63+git 0940c62)**;RECOVER刹线6h空≤−8U继承;#11降级观察(§4) |
| qt-trend-long | 827ffe8c | 趋势动量多 | **TUT单币**(COTI出池@08-24 15:3x ROUTE#36:48h nL14净−1.06触头注线[窗滚动08-22赢单出窗];种子同步stop-PATCH-start@15:35✓;COTI回main孤儿态待bl释放;ACE隔离@08-22;史git f43756c) | running | **tpl575(S24)** pct0.05 mcp3 buy trailing on;**_exp=open·#69三值包重申请@08-23 09:18Z**(adt0.5+lcp0.149+mces off;终裁二元=新段净>0→keep否则回滚[adt1.0/lcp0.15/mces3];eval=n≥16或08-25 09:18Z;劣化线n≥8∧≤−4U;**进度@08-24 18:2x段n11净−0.35**(新TUT 17:33平−0.758快SL[17:18重启后~75s开仓=§6速开仓第5例];劣化线−4U远;评期明晨09:18Z或n16,到点仍负→按预注册回滚[adt1.0/lcp0.15/mces3]);否决窗空置+程序史git 492779a);原三值段终裁rollback+ex-ACE归因全文git 5df52d2;**常驻保险=滚动6h净≤−5U→stop(继承)**;三值包史git 8a1d995/f71d825 |
| qt-fade-short-v2 | 7583727a | 冲高回落空(**tpl887/S28+S29**;谱系tpl576/S25→823) | HANA/COLLECT/MELANIA/BTR(feed4=种子4✓@08-24 15:2x;**MELANIA出池线触发但deferred@15:3x ROUTE#36**:nS7净−0.53证据全pre-S28[tpl806/823时代,S28段n0]=#57病理证据不可移植+移出=撤走S28/S29主标的;defer至_exp终裁重跑对账,风险界=保险6h−6U+劣化线;BTR晋升#35;MELANIA晋升08-22) | running | **S28衰竭签名直入上线@08-23 21:24Z**(#66修复,机制全文§2 S28行+git;附随#65 epsilon永久化;apply三复检✓);**_exp=open·EXP:S28**(二元:段净[Σpnl−名义×0.1%]>0→keep否则rollback回tpl576;劣化线段n≥8∧≤−4U即回;评期段n≥16或08-25 21Z;崩溃立即回滚;**S29 amend@08-24 12:40Z(tpl887)**;段n0@15:2x(约18h;S29后2.6h无完整签名bar,MELANIA 0.35-0.36被拒行=非签名bar=域限定按设计实证);评期/劣化线/二元不变;终裁rollback=rollback×2或apply归档tpl576码;**烟雾task33死于16:3x-17:18重启波(4连死,且发现其误用7d窗违≤18h规程);task34单币12h窗重发@18:4x=queued**);低波三值包ROLLBACK@08-23(组级知识:低波解锁不付费;git a2e845f);保险=6h净≤−6U停(继承) |
...
| 收养错归属亚型(#57族;§4#72) | **n=1@08-24 18:12**(VELVET行sid=21519f1b详§4#72;缓解=退役壳自bl@18:4x) | 08-24 18:4x | 判据:自bl后再现sid错归属行→#72升部署主诉求;每轮扫closed行sid=21519f1b计数 |
| closed平仓行消失观测(≤48h短窗) | 瞬时缺行反转@08-23(拉取瞬态);零盈亏行分型@08-24 03:2x(SCRT案:毛=0无REALIZED_PNL income→行无pnl字段,真实成交佣金在income侧;本轮48h窗仍含该行=同例持续非新增);大规模掉行@08-24 12:3x(长窗重建丢中段行再证);+18:1x同窗10min两拉31→40行差9=重建抖动再证;**纪律=段级判定必须16h内短窗+income双拉**;全文git b505652系 | 08-24 12:3x | 判据:瞬时缺行复现≥3例→读源码找竞态;零盈亏亚型再现1例→升§3定案;滤行纪律维持(毛=0不失真)但其佣金只在income侧,费覆计算记得含 |
...
| 08-24 18:1x-19:0xZ | 例行轮(max) | income:1h−0.80/3h−1.03/6h−2.22/12h+0.42/24h+0.20(rows n21,wr62/be63≈平;rows净−0.91=重建分叉,income为准) | FGI73/BTC+2.2%/市值−0.3%无降压;trending∩main=AERO,PUMP,INJ,LIT,VELVET | **原子包①=退役壳自bl5种子封收养错归属向量(18:12 VELVET案:main开的仓行sid=21519f1b,机制源码定位§4#72,守护管理正常实害≈0;_exp FIX评08-27)**;ROUTE#37=main stop-PATCH bl18-start(#70落地)+泄漏9热清feed100→91+MELANIA defer继续+无新隔离;crash loop复燃4连16:31-17:18(§6);task34烟雾12h窗重发(task33死于重启+曾误用7d窗);trend#69段n11净−0.35(评明晨09:18Z仍负→预注册回滚);v2段n0@21h(HANA RSI78触上轨ATR0.13<0.30地板=设计边界工作中,评明晚21Z);费覆24h🔴(组均毛负vs3×费0.086)锁#58/#47;充值三条件破(12h wr−be −1pp);无刹车无保险触发 | 钱包168.55(干净读@18:20,150.14系VELVET保证金在押假象);死法24h:fastSL5−4.34(VELVET×3/TUT×2)/hunger3/timeout3/W8+5.0;VELVET一币−3.09=当日主失血(trending榜上=榜首逆信号又一例,距隔离线0.9U) |
