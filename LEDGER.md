...
| # | 类型 | 内容（含完整意图） | 登记轮 | 状态 |
|---|---|---|---|---|
| #88 | PATCH | main bl种子16→22(+6 TradFi币,格式照现值config口径复核后写):断连解除(ctx fetch_error消失)∧main空仓窗→stop→PATCH{symbol_blacklist单键,勿动_exp}→start→复读;意图=TradFi隔离重启免疫(rotate系内存态,重播种会放回) | 09-10 04:1x | open·等断连解除(probe史19:45→05:42十miss,末=probe#2@05:42Z双err直证[≥38.8h];probe史:06:2x session三枚尽;09:1x cron session:probe#1@10:30=miss/#2@11:30:25=miss(DNS同串直证≥43.3h),末枚留备不自续→交接12:1x cron轮接棒;落地窗=恢复瞬间全账户空仓stop→PATCH bl22单键→start;bl22值=对账器blacklist_sync期望态06:2x再复核一致(22连零)) |
...
| 计数项 | 读数 | 更新轮 | 备注 |
| backend→Binance REST DNS断连 | **n=1: 09-09 16:12Z起持续,09-11 09:2x仍在(≥41.2h;probe史14:07→08:29共12miss+cron直证03:1x/06:2x/09:1x;ws双载具存活09:14:07/09:14:03直证;被挡开仓史19行/13币,09:2x发信号0/0无新增且下单行0)**;特征=fapi SERVFAIL(resolver 100.100.100.100=Tailscale MagicDNS,修复在宿主机);ws评分循环存活(main183/183+trend3/3),active端点返回DNS错误串=直证;skip文案'不支持该交易对'=误导;ctx balance=0+income{}=伪影 | 09-11 09:2x | 恢复后复核:①income补记完整性②feed有无重播种③若平台重启于断连窗=ws也死须立报;再现→升§3;⚠️断连窗禁stop/start(重订阅可能失败) |
...
| 09-11 06:2x stub: 断连轮#10(≥38.1h);真窗n11/-2.15零新亏;ROUTE 22连零;S31宽0.661ON;S33裁决框架预登记;probe#1-3排定;全文git 06df9f1版行 |
| 09-11 03:1x stub: 断连轮#9(≥36.3h);closed hours参数失效新证(2847行回灌,v1.6自证);真窗n15/-3.45;ROUTE 21连零;HOLD;probe 04:44/05:42双miss;全文git 371479b/65e468a版行 |
...
> 瘦身注(指针集@09-11 03:1x归并): 断连轮#6[git 1d34ef4]/#7[git 7d83418]双stub并入git 7eb36e6版行。前集@00:1x: #4[git 2596a9a]/#5[git 967d54b]并入git 7d83418版行。前集@09-10 21:2x: 09-10 09:1x/04:0x双stub并入[git 805a115]。前集@18:2x: 09-09 15:1x[BOOK硬超时门预注册,git 522dfdc]/12:2x[batch4三死例,git 0097bd7]双stub并入。更早指针集(09-05..09-10各轮)=git 65e468a版§7注原文。
|---|---|---|---|---|---|
