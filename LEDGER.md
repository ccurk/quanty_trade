...
| main断流观测(三闸交集关门) | 本轮main笔数=15/24h@09-02 00:1x(末close20:17;00:07新开SKR=管道活性直证;判据未触) | 08-30 00:1x | 判据不变:再现6h+零笔且池ATR中位≥0.5→查管道;每轮记main笔数 |
...
| #79·closed行sid按symbol旧绑定继承(#77族新亚型) | 宕机前段08-30 02:04-06:38共6笔全=main自开(main日志完整触发/下单链:KOMA/BTR×2/COLLECT/ZKC/SKR),但DB closed行归属=该币末代载具(BTR→brk 3b64,COLLECT→fade-v2 7583)或空sid(KOMA/ZKC/SKR无前史);n=6/6一致零反例→sid解析疑走symbol历史绑定非下单载具;+空sid亚型恢复后8例(ONG/MARSCOIN×3/牛来/FF/USELESS/STAR@09-01,均无前史币零绑定→空sid,main日志开仓链全实锤,累计n=14;MARSCOIN三现同型) | 09-01 12:5x | 归因纪律即刻生效:sid与池归属冲突时以载具日志(触发开仓信号行)为准;机制修复并入#77 ownership单一化(候部署);#77部署后复验此亚型消失;**双行升级@09-01 18:1x(#77级实锤)**:全量closed视图(无source过滤)证era BTR×2(08-30 02:06/03:34)与新例BTR 09-01 17:10(main空conf0.75正常链+0.70)均=DB双行——main行tp/sl齐+壳行tp/sl null,同笔PnL双写两行,非单行错标;17:10壳行为live开行(壳日志17:10:44-17:11:59连报'仓位缺少有效止盈止损配置'=strategy_tpsl_monitor.go L138,resolveTPSLFromROI对停机壳解析0/0→该行全程裸奔);**08-30 03:34案=#72'风险有界'首兑现:壳饥饿档45m1s真接管main仓收割−0.61(EXIT_AUDIT reason=hunger_sl落壳日志;main行SL远未及)——出场治理权=两行DB主键序竞态,谁先落库谁管仓**;今17:12 main行胜出trailing收+0.70良性;空sid=binance_only视图伪影确证(同10笔全量视图sid=main齐全n=10),且binance_only对双行币dedup保壳行弃main行=池归因陷阱→逐笔归因改用全量视图+日志链;牛来(前brk池币非遗仓)今交易零双行→向量判别特征疑=末代持仓币(BTR/NIL/COLLECT类)非池成员泛化;n=15;**+4空sid@09-01 21:1x(UAI首/MARSCOIN四现/牛来/USELESS,main日志开仓链全实锤=n19)**;牛来/USELESS再交易零双行=判别特征续证;**+4@09-02 03:1x=n23:MAGMA第2币种双行实锤(00:38 S main日志conf0.700开仓链实锤,全量视图同笔−1.17双行main+壳3b64,binance_only又dedup保壳行;另见MAGMA era双行2例15:17+0.05/18:21−0.31=绑定早在场)+3空sid伪影续(SKR×2/USELESS,全量视图sid=main齐)** |
| S31·regime动态方向偏置(**升队首候选@08-29 17:0x owner动态化直令**) | v1谱:宽度regime三档(池内EMA-up占比<0.35=RISK_OFF/0.35-0.60=中性/>0.60=RISK_ON)→动态premium加减(OFF:多头prem+0.10;ON:多头−0.05空头+0.05),阈值内联防漂移,S31_ENABLED False-guard回滚,叠加于config prem之上(config回滚路径保留);main先行 | 08-29 17:0x | **已上线@09-01 10:02=tpl988**(patcher断言过=基底零漂移/编译+6符号+锚8组✓/apply字节复检✓/零模板泄漏✓);_exp评09-04 09Z或S31段n≥20,到点n<5顺延48h一次(49h宕机后市场冷启动条款);劣化线=S31段多头净≤−3U或组wr−be劣化≥5pp@n≥20→S31_ENABLED=False;草案+验证史=ops/drafts/+git b12d7d9版行;**段累积表n11/9W2L@09-01 18:1x(毛+3.30;多n4净−0.49/空n7净+3.79;判别笔仍仅ONG+0.37)**:①-⑧全链git 497cdf2版行(要点:①ONG L conf0.650 w0.656 ON降门放行=唯一判别+0.37;④FF L conf恰0.70非判别边界;⑦STAR S N档SL穿刺−1.42);⑨-⑪细目git 20ecf1d版行(要点:⑨FF边界笔SL−1.30;⑪BTR OFF+0.70;81分钟ON→N→OFF全滑);**n15/11W4L@09-01 21:1x(毛+1.49;多n5净−0.045/空n10净+1.54)**:⑫UAI S18:23 conf0.700 w0.183 OFF −1.32 SL穿刺30m⑬MARSCOIN S18:42 conf0.750 w0.161 OFF −1.17 SL穿刺16m(18:53-58空头双SL=宽度翻转squeeze窗)⑭牛来S18:59 conf0.800 w0.301 OFF +0.23⑮USELESS L19:34 conf0.750 w0.742 ON非判别(静态0.70亦放行)+0.45;71分钟OFF0.161→ON0.742剧烈翻转=churn观察n3;判别笔仍仅ONG;劣化线双查:多头−0.045≫−3U✓wr73.3−be68.1=+5.2pp✓;宽度重建法:logs q=日期T分钟→评估行EMA up占比/93(不带日期=跨天污染);评期用本累积表;**n19/13W6L@09-02 03:1x(毛+0.57;多n8净+0.21/空n11净+0.37)**:⑯SKR L00:07 conf0.750 w0.591 N非判别(#16结案)−1.29 SL穿刺15m(入场1min后1bar−1.85%闪跌,S30 veto晚一步)⑰USELESS L00:23 w0.538 N +0.16硬超时60m⑱MAGMA S00:38 conf0.700 w0.505 N非判别−1.17 SL穿刺14m(追空反弹;壳双行见#79)⑲SKR L02:32 conf0.750 w0.710 ON非判别(静态0.70亦放行)+1.38 trailing收割(峰roi10.8%;平后RSI超买/放量/S30闪跌三滤拦追涨再入);重建法回验00:07=0.591与#16预登记值一致✓;判别笔仍仅①ONG;**ON档多头n3(①⑮⑲)均净+0.73 vs N/OFF档多头n5净−1.99=expect方向性首证(样本瘦,ON放行笔本身非S31判别)**;劣化线双查:多头+0.21≫−3U✓wr68.4−be66.8=+1.6pp(自n15的+5.2pp滑坡,劣化条款@n≥20未达judgment点,差1笔) |
...
| 09-02 03:1x-03:4xZ | 例行轮(max) | +1.38(n1)/−0.92(n4)/−0.92(n4)/−2.43(n12 wr58.3 be69.6 −11.3pp)/+0.57(n19 wr68.4 be66.8 +1.6pp)(毛) | FGI63/BTC77504−1.2%/mcap−3.8%/trend∩池{UNI,PUMP,ARB} | **HOLD原子包0;ROUTE v1.5零差分六连(plan空,bl14期望态,TUT aging第8日,want.brk涨至6币退役壳屏蔽);S31段+4→n19/13W6L差1笔到评判门,劣化线双绿(多+0.21/wr−be+1.6pp),重建法回验00:07 w0.591=#16值✓,ON档多头n3均+0.73 vs N/OFF多头n5净−1.99=expect方向性首证;MAGMA网复挂(n1/−1.17);#79 MAGMA第2币种双行实锤(n23)** | 钱包161.05U零持仓;死法新4:SL穿刺2(⑯SKR N档入场1min闪跌/⑱MAGMA N档追空反弹)+硬超时1(⑰USELESS+0.16)+trailing收割1(⑲SKR+1.38峰roi10.8%,平后三滤拦追涨);费覆🔴三连恶化(段均毛+0.030 vs 3×费0.104@名义34.5U差−0.073;≈25.6笔/日;段毛+0.57≈费后打平)→TG明示风险;audit零无主(main1791/trend1790);feed93/1零漂移;sizing31.8-36.1U✓;充值线未触(12h净负) |
| 09-02 00:1x-00:4xZ | 例行轮(max) | 0/0/−1.81(n4)/+1.12(n14 wr71.4 be67.3 +4.1pp)/+1.49(n15 wr73.3 be68.1 +5.2pp)(毛) | FGI63/BTC77363−1.7%/mcap−3.8%/trend∩池{ARB,PUMP,USELESS} | **HOLD原子包0;零新平仓(20:17后3.8h安静窗=证据零增量);ROUTE v1.5零差分五连(plan空,bl14期望态,TUT aging第7日,want.brk5币退役壳屏蔽);S31段n15不变(评期09-04T09Z或n≥20未到);#16预登记=SKR多00:07开@w0.591 N档conf0.75非判别(在飞浮−0.51);费覆🔴二连=同段读数无新笔(均毛+0.0996 vs 3×费0.1032差−0.0036)** | 钱包161.3U(avail143.7+SKR保证金18.1−浮0.5)持仓1;死法:无新笔;#79 n19零新增(SKR活跃行空sid=已知伪影,main日志00:07:33触发→filled实锤归属);trend TUT n0第7日;audit零无主(main1791/trend1790);feed93/1零漂移;宏观无降压;充值线未触(12h+4.1pp<6pp) |
