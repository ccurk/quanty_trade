...
| 计数项 | 读数 | 更新轮 | 备注 |
| backend→Binance REST DNS断连 | **n=1(episode): 09-09 16:12Z起持续,09-14 12:24仍在(≥116.2h;probe史69miss+cron直证廿八轮+09-14probe×3miss(09:57/10:57/11:57)=git链→19130f2→9137f71;12:2x直证=active positionRisk fapi SERVFAIL同串+main ctx income同串+balance伪零;ws活证main recv LDO序号9008@12:21:05+trend USELESS序号10269@12:21:01双评估行;feed183/3无重播种=无重启=#89未部署;S31宽度0.519回升仍N)** | 09-14 12:2x | 特征=fapi SERVFAIL(resolver 100.100.100.100=Tailscale MagicDNS,修复在宿主机);伪影族=ctx balance/income伪零+skip文案+closed窗过滤失效(close_time过滤免疫)+active回error(全谱git ecec3b0);**已升§3复发向量n=2+#89补丁c49c555候部署(§4)**;本轮re-arm×3@06:57/07:57/08:57→恢复即落#88;恢复后复核:①income补记完整性②feed重播种③断连窗内重启(ws也死须立报);⚠️断连窗禁stop/start(重订阅可能失败) |
...
## 7. 运行日志（每轮一行，新行追加在表首）
| 2026-09-14 13:5x | 注记: 断连probe#2 miss(同串SERVFAIL@13:57,≥117.8h;active+ctx双证balance伪零;ws活证main 评估行@13:58);probe#3@14:57候(末次) |
| 2026-09-14 12:5x | 注记: 断连probe#1 miss(同串SERVFAIL@12:57,≥116.8h;active+ctx income双证,balance伪零;ws活证main PROM评估@12:58:01);probe#2@13:57候 |
