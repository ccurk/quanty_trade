...
- 隔离区(规则≤−4U∧n≥4)【13】: 4/CYS/TST/龙虾/BMT/BTW/H/APR/AIO/BICO/BEAT/XNY/ACE(逐条血统/日期全文git bb3b883/8a797ec/bb2e系)。**池列值=快照,权威=每轮对账器输出**。**对账器v1.5权威副本=quanty-ledger分支ops/route_pools.py@08-29**(v1.5=roster增补breakout复活壳3b646bf4,阈值零改动;谱系v1.4←v1.3←61b1bb4;v1.0默认分支禁跑)。**arg4隔离表必须X/USDT形态**(08-15实证:裸名norm不match feed键→漏移)。出池只认头注规则或隔离,失格=aging_watch advisory;brk/lowvol出池阈值未注册(#42)前无自动出口。**MAGMA网:解除@09-04 03:1x(复挂判据=磁铁型死法再现;史git bf52ca3);MARSCOIN watch@09-05 18:2x:48h n8/+1.00(24h正,隔离线n≥4∧≤−4U远离;解除→复挂史git 8729d22);KOMA watch@09-05 18:2x:48h n3/−2.68(距隔离线再1笔≤−1.32即触n≥4∧≤−4;新币震荡双面收割签名史git 6a3383a;CB连败2);PORTAL watch:48h n3/−2.21维持(再亏1笔≥−1.79触线);牛来watch@09-05 18:2x:48h n4/+0.08(现空单在飞18:15@空侧磁铁镜像决胜,让策略信号处置;规则线远离)**;对账史(09-01清扫/08-30/08-29链)=git b12d7d9版行;#51-#58史git 9957041/1ae847c/91b3276/830a46c/188af33/66048a9
...
| # | 类型 | 内容（含完整意图） | 登记轮 | 状态 |
|---|---|---|---|---|
| 84 | ROUTE配套 | main symbol_blacklist 加 BULLA/USDT(BULLA晋升trend后重启免疫层;当前bl16→desired17,斜杠形态;PATCH需main空仓窗:stop→PATCH→start;缺此则平台无端重启重播种时BULLA漂回main feed=互斥违例,热rotate仅内存态) | 09-05 18:2x | open(待main空仓窗) |
...
|---|---|---|---|
| main断流观测(三闸交集关门) | 本轮main笔数=28/24h@09-05 18:2x(判据未触) | 09-05 06:2x | 判据不变:再现6h+零笔且池ATR中位≥0.5→查管道;每轮记main笔数 |
...
| 09-05 18:1x-18:4xZ | 例行轮(max) | +0.19(n1)/+3.25(n6)/+1.33(n13 wr54)/−1.13(n22 wr50 be53 −2.8pp)/−3.05(n32 wr50 be55 −5.5pp)(毛) | FGI73 BTC80017+0.5% MCAP−1.31% trending∩池=USELESS,MARSCOIN,DASH | **ROUTE差分执行=BULLA晋升trend(对账器规则A:48h L净+3.05/nL5/wrL80%达标);main rotate remove11(BULLA晋升配套+TUT/USELESS互斥+隔离残留8热清)feed150→139;trend add BULLA→4币;互斥三层复核trend池币∩main=∅∧隔离∩main=∅✓**;账户持牛来short1仓(18:15开@conf0.75W-bar,空侧资费磁铁镜像决胜在飞);**空侧镜像cohort闭仓n7/+0.05(+KOMA1707+0.19硬超时+踏马1512−0.47饥饿;赢4死3)→方向不对称续(空+0.05 vs 多−4.37;判据n≥6∧≤−3U空侧远离)**;多头磁铁维持n6/−4.37;#80退回假设库攒证据(死赢不可分);#79维持n41;S31宽0.400(60/150)N档三连;死法SL穿刺13/−13.88vsTP14/+11.19;多n18+0.03/空n14−3.08 | 钱包≈145;费覆main🔴27连(均毛−0.092vs3×费0.075)trend🔴n4(−0.118vs0.058);充值未达(12h/24h双负);刹车/保险全绿(6h组+1.33/main6h空−0.91线−8U/trend6h+0.96线−5U);宏观降压门未触;待落1(main bl加BULLA重启免疫待空仓窗);CB连败≥2=AKE/BULLA/KOMA;audit1802尾零新增 |
