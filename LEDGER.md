...
| 载具 | id | 原型 | 池 | 状态 | 门/备注 |
|---|---|---|---|---|---|
| main 通才 | 8eb182b6 | 通才(S18-S23栈) | auto·feed87(重播种第5例@12:1x热清13自愈;种子bl13=期望态✓,APR/BEAT候重启窗入bl) | running | pct0.08 sides=[buy,sell] **long_premium=0.35(FIX@08-14 18:47Z)** tpl567;_exp=**FIX长溢价0.15→0.35**(long_thr0.70→0.90;证据=48h长侧n20 wr30/−11.79过20笔门,病根=逆势栈0.70恰等长门,S12先例;keep=08-15 19:00Z长侧24h净≥−2U∧组均净较−0.216改善;劣化=长侧≤−6U或组净恶化→回0.15再议[sell]latch;全文config._exp)+**继承RECOVER刹线**(6h空侧≤−8U→重latch[buy]);**FIX段读@T+17.5h(12:1x轮)**:长n2/−1.44零增~18h(0.90门节流在效),空n20/−15.13wr25=段主刀不变;段∩24h组均净−0.753劣于−0.216(恶化源=空侧非FIX触及路径;19:00Z评估轮按侧拆臂、以纯段24h窗当值);12:1x RECOVER读6h空−6.25收敛未触−8U;sid伪影纪律照旧(§3) |
| qt-trend-long | 827ffe8c | 趋势动量多 | ACE/TUT/BICO/COTI(feed4=种子4✓;aging_watch=TUT/BICO/COTI advisory@12:1x对账) | running | **tpl575(S24)** pct0.05 mcp3 buy trailing on;_exp v3常驻:**累计n27/−0.12**(+ACE11:38快TP+0.28;48h窗ACE n8/+3.71 wr88扛旗∧trending);早结门净>0✗;劣化−5U/canary−5U(现6h−0.20)未触;费覆24h转🔴−0.252vs0.087(窗口滚动:昨午ACE大赢出窗,非新恶化);**终裁08-16 06:35Z**当值:净>0=keep/−5U<净≤0=证据不足结案S24留用(现值−0.12落此格,一笔胜即翻正);全史git+config._exp |
| qt-fade-short | 21519f1b | 冲高回落空 | 1000CAT/ARC/**BEAT**(+BEAT@08-15 12:26 ROUTE补灌:48h nS3/+2.98 wr67达fade入池条,want.fade 13轮首次非空;预热160bar实证;种子2币候重启窗写回3币) | running@08-13 12:45Z | **tpl576(S25)** pct0.04 mcp2 sell;_exp v2续:累计n8/−2.63(池含BEAT起可产样本;前12轮零成交根因=want.fade空,非载具故障);评期=累计n≥15或**08-16 12:50Z**(池补灌已先行,到点若仍n<15按证据不足格裁,停驶取舍届时议);自动保险6h≤−6U(0✓) |
...
- 互斥不变式: feed层✓overlap0@08-15 12:2x(main87/trend4/fade3)>**种子层=期望态✓@08-15 03:20**(main bl13含H无CAP;trend种子4币;CAP释回main发现漏斗;重启免疫达成)>⚠️引擎互斥闸未部署;aging=trend2/fade2 advisory;重播种规律第5例(无端型第3例@08-15 09:3x-12:18窗,audit零无主/config零丢失,trend/fade种子免疫✓):feed播回auto100含bl币(信号层bl在防零交易风险)→热rotate自愈;种子同步后专家池重启免疫实证(trend4/fade2零漂移);pre-restart IDLE行93 vs API91=频道层订阅残留伪影(重启自清,观测勿当违规)
...
| 15 | M通道 | 脱管仓 sweeper：monitor 增"币安实仓∩DB 无 open 行→重收养/TG 告警"扫描 | KOMA 29h 漂移 −15.13（n=1）+源码 L48 机制已裁决+HEAD 1292 修复疑同根因（部署态未知） | 0/2·**FOLKS watch@08-15 12:1x**(active行amount0.1≈$0.2名义空,sid="",open11:24Z,48h无FOLKS closed行=来源不明非正常下单尺寸;|upnl|<0.01经济nil;下轮复核在管/消失/漂移再定性) | 候选（n=1 轶事登记；再现 1 次或部署确认前再有漂移 → 升执行；08-03 US"第二例"证伪=重建伪影非真漂移，n=1 维持） |
| 18 | M通道 | closedPositionsFromBinance 窗口边界播种修复：拉 fills 前先取窗口起点在持仓态（或多拉 2h 缓冲），链起点=平仓 fill 时正确接续而非误开新链 | §3 08-03 重建洞条目（机制已落源码行为=可行动级）；US 双窗实证；影响=归因主武器完整性+前端 closed 历史正确性；**08-08 18:1x清晰标本**:BTW 6真短链间5幻影long填隙(无realized_pnl字段/qty198/487/入场价=前链出场价);48h文件10幻影行,24h窗3行以0额混入(51→真48);分析纪律=逐笔先滤无pnl字段行;**08-09 18:4x 镜像行新形态**(open期双记账,详§3新条;幻影家族第3形态);**+1镜像例@08-15 12:2x**(ROBO 09:17→10:17整60m,entry=exit=0.01784,qty2101,无pnl字段,滤除✓) | n/a | 候选（deploy 队列已积压 3 项未部署[logs-retention/回测v2/HEAD]，先候部署窗口再推第 4 支，分析侧缓解已在用） |
...
| 30 | S21类(代码gate) | 追跌空反弹拒空veto候选(gate7e,暂缓中): short且跌势陈旧拒;draft单维300m阈值族**证伪@08-07**(误杀24%>10%门,收紧复审仍12%,−10~−14带无分离力);下一迭代=加第二判别维(距60m低点反弹幅/自低点时间) | 追跌型累计10例/−24.28+晨午簇候审15(vision T+1定标待做)〔审计全史见git 121e975 #30行〕 | **n=17/−38.94**+候审16·暂缓(+1确认@08-15 12:3x APR#2 11:16空CG5m:低121m前∧自低+3.29%∧距3h高仅−3.64%→入场后+6.63%轧空;APR#1 08:51入候审[陈旧跌势特征在(低81m前+2.70%)但CG入场后60m逆行−0.93%与饥饿−1.33矛盾=源分辨率存疑不强标];+2@09:2x ROBO−3.33/APR−1.93;+1@06:2x VELVET−1.77;前例AIO×2/DOLO全文git 42e4fc8;APR已隔离但跨币同型US/AIO/ROBO/VELVET在列=系统性非个币) | 执行门=确认型≥20或owner核准;影子通道S23 WS vs AS已随#37冻结;误杀审计先行原则保留 |
...
> 瘦身注 08-15(12:4x轮更新)：更老运行日志行随轮裁剪(08-15 00:14轮CAP出trend要点存§1.5;08-14 18:14轮FIX登记+RECOVER unlatch要点存main _exp+§1.5;08-14 21:15轮H隔离+#30定标要点存§1.5隔离区+§4;更早各轮要点见前注与git),全史见 git 历史。