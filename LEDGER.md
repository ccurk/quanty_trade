...
| 计数项 | 读数 | 更新轮 | 备注 |
| 幻影行post-fix观察 | +1@08-25 00:2x(SCRT空08-24T01:43→01:54,入=出0.01744,无realized_pnl空sid=回声指纹);**0新增@18:2x(48h窗仍同一SCRT行,累计n=1)** | 08-25 18:2x | 再现累计;n≥3或含实亏→升§4 |
| epsilon边界放行观测(v2域;#65永久化伴生) | **n=2/类净−0.757@08-25 18:2x**(+HANA空#2 15:24conf恰0.600[MACD0.15+资费0.20+EMA0.30+量0.10=0.75×折0.8],RSI37.1布林中部=追跌空非fade型,48.8m饥饿收割毛−0.542;资费0.00032恰过0.0003=双边界;15:28第三信号被S22 emit-veto拦✓;#1详史15:2x行) | 08-25 18:2x | 边界类累计n≥5∧类净≤−2U→复议;**⚠️premium加距已源码判死@18:2x:tpl887 L806 short_thr为常规+S28共用门,S28最小组合(衰竭0.30+上轨0.15+死叉0.15=0.60)与常规0.75×0.8简并于0.600,任何加距同杀S28主通道→类修复须代码层路径感知(S30候选,门=本计数达标)** |
|---|---|---|---|
| main断流观测(三闸交集关门) | 本轮main笔数=7/24h@18:2x(6h 1笔VELVET;末笔14:06后4.1h静默=门内[池ATR中位<0.5低波];15:2x读数10/24h史) | 08-25 18:2x | 判据不变:再现6h+零笔且池ATR中位≥0.5→查管道;每轮记main笔数 |
...
| main行零归属(空sid;§3 closed回填缺口) | **16/16@08-25 18:2x**(48h窗main池成交行全空sid持续,窗滑动;trend/v2行归属正常;始08-23 18:24 UAI;**+新亚例:TUT空22.9h长仓08-24 17:19平[=17:18重启窗]sid亦空=trend池首例,重启窗close-sync同缺口,COTI/TUT它行归属正常**) | 08-25 18:2x | 无P&L实害(income/守护/池互斥归因全旁路);判据:crash loop根因修复后仍空sid→升M通道close-sync候修;每轮扫main池closed行空sid计数 |
...
| 08-25 12:2x-12:5xZ | 例行轮(max) | 24h净−1.69(n14 wr57.1/be63.8;组🔴) | FGI74无降压 | **0原子包HOLD;ROUTE#43零差分(连7);v2评期21Z三选项TG呈owner(推荐B)** | stub@18:3x;全文git 830a46c |
| 08-25 09:1x-09:4xZ | 例行轮(max) | 24h净−0.10(n16) | FGI74无降压 | **原子包①=ROLLBACK#69终裁(trend三值回滚落库✓);ROUTE#42零差分(连6)** | stub@18:3x;全文git 830a46c |
