...
| 2026-09-18 | **止损 3%→4% 直令(live 17:5x)**: owner 原话"止损在收紧 调整到 3%"→"变成 4%"。读法=价格距离(与 13:39 "0.5% 太噪声"及 CC 报告单位一致;0.03 ROI 读法已 TG 请否认,未否认)。执行 Go 热 v57 hunger_stop_loss_pct 0.075→0.30 → v58 0.40 复读✓;其余键不动。**引擎事实(源码 strategy_position.go:485/494/884/893)**: 交易所 SL 钳 entry×(1∓0.3/lev) → 10x 最远 3% 价 ⇒ 有效止损=min(2.5×ATR[重启后 1.5×],3%),饥饿 4% 排其后不触;4% 成真只有 A) lev 10→5(钳 6%,ROI 键换算 hunger_sl0.20/tp0.15,名义减半) 或 B) use_exchange_tpsl=false(不推荐)——TG 候 owner,不回复=保持 10x。证据: 15:25→17:56 n41 −30.99 wr44%<be64% 饥饿SL 22/−54.72;13:39 起 n71 33 止损 −91.8 vs 31 赢 +61.8。风险陈述已 TG: 3%×260~460U=−8~−14U/次;10 仓同向 −55U;DS −4U 熔断首笔即触→拉锯。预注册 _exp_cc.p7_sl3(止损占比<25%∧段均净≥0;eval 00:00Z 或 n≥30;劣化只报警)。 |
...
| CC-17 | 评判 | p7_sl3/4(hunger_sl 0.40,有效 3%) 段(open≥17:56:55Z): 止损占比<25%(p6 48%)∧段均净≥0∧最大单亏≤3.5%×名义;eval 09-19 00:00Z 或 n≥30;劣化(段净≤−10U 或 24h≤−20%)只 TG 建议回 0.075/改 ATR 倍数;DS 砍 pct→按 owner 值恢复 | 09-18 17:5x | open |
...
## 7. 运行日志（每轮一行，新行追加在表首）
| 2026-09-18 17:5x-18:0x | 交互轮·owner 止损直令: "止损在收紧 调整到 3%"→"变成 4%" → hunger_stop_loss_pct 0.075→0.30(v57)→0.40(v58) 复读✓(读作价格距离,0.03 读法已 TG 请否认);源码核实 SL 钳 0.3/lev(strategy_position.go:485/884)→10x 有效止损 3%,4% 需 lev5(ROI 键换算)或关交易所 TPSL,候 owner;逐笔: 15:25→17:56 n41 −30.99 wr44%<be64% 饥饿SL 22/−54.72 多 −27.57/空 −3.43, 35/41 <10m;p6 封段 n71 −34.75 止损 33/−91.8;报警线 6h income 净 −36.6(−18%,过 8% 线只 TG)/24h −31.96(−16%);avail 112.4 持仓 3(SIREN/CHIP/SENT 多);DS: 15:25 后 0 PATCH, lock 至 09-19 15:22, 留言板#15(勿动 hunger_*,阈值改钱包比例);预注册 _exp_cc.p7_sl3(CC-17);TG 两条(读法+引擎钳+代价+🎓 盈亏平衡算术);下轮=p7 首读+DS 是否砍+owner 对 lev5/D 的回复 |
