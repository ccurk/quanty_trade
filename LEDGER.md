...
- **[观察中] 15-60m 持仓桶曾为失血主体**（−8.08 @07-21）→ 07-22 已翻正（+9.48/38 对 wr42.1%），疑与 E2 同源（边际空单多落此桶），待跨轮复现确认后可结案。
...
| # | 类型 | 内容 | 依据 | 复现计数 | 状态 |
|---|---|---|---|---|---|
| 1 | 排队动作 | L2→L1 恢复升档：lev3 / conf0.40 / pct0.25 + `short_conf_premium` 0.05→0.20（保 short_thr=0.60 不变，长门槛 0.60→0.45），一次原子 PATCH | 07-22 08:11Z 恢复条件全满足；候 22:22Z 预算窗 + E2 评判通过 + 空仓窗 | n/a | 待 23:11Z 运行 |
