...
| 载具 | id | 原型 | 池 | 状态 | 门/备注 |
|---|---|---|---|---|---|
| main 通才 | 8eb182b6 | 通才(S18-S23栈) | auto·**feed89@18:24**(bl12种子=9隔离+trend3,重播种#14+heatclean-v9) | running | pct0.08 sides=[buy,sell] long_conf_premium=0.35 tpl567;**trailing(act1.0/cb1.2%)+BE(1.0)=基线几何(#49终裁KEEP@08-16 18:2x)**;_exp=closed(verdict 628字存config._exp.verdict+git);**实验槽空闲**;#49终数=段n30/−3.81/wr63/均净−0.127[vs−0.73收敛5.7×]/SL率20%[vs40%]/均亏−1.385[vs−1.84]/赔率0.43<1.2按15:30预登记框架裁=统计失真(13微赢≤0.7灌稀分子+6实质赢均+1.152=右尾+80%,EXIT_AUDIT棘轮直证);保守n29口径(BICO08-15T18:17留trend账)同结论;段亏全额=BICO长4笔−3.85(§6计数,入场病与几何正交),ex-BICO段n26/+0.04;RECOVER刹线6h空≤−8U继承;#11降级观察@18:2x(§4);后续实验以本段为基线 |
| qt-trend-long | 827ffe8c | 趋势动量多 | ACE/TUT/COTI(feed3=种子3✓复核@06:36;aging_watch=COTI/TUT advisory[对账器@06:30],ACE凭24h 4W+1.73脱离aging) | running | **tpl575(S24)** pct0.05 mcp3 buy trailing on;**归因修正@09:2x**:BICO 04:02(+0.57)/07:57(−1.90)两行盖trend sid实为main开仓(trend日志零开仓行,main日志全链;S24 keep判据'36h零新笔'修正后仍真);正牌08-16=ACE+0.02@06:46+COTI多在飞@09:30;**_exp=closed@06:35:12Z: S24终裁keep结案**(预注册当值格'净>0=keep':链n33/+0.024>0;末笔08-15 18:17 BICO−0.91,终裁前36h零新笔=03:11后零笔复核@06:35;程序=真空仓窗stop[poll1沉降]-PATCH-start,种子3币完好);**常驻保险(继承canary)=滚动6h净≤−5U→stop本载具**;实验槽空闲;候选EXP降prem=§4#55(费覆红线内只登记) |
| ~~qt-fade-short~~ | 21519f1b | **停机@08-16 12:26Z 劣化线**(预注册'累计n≥10∧净≤−5U'触发:n12/−5.44;终裁=config._exp verdict=degraded_stop;stop舍rollback四由+逐笔链全文git 766c7d9后继) | — | stopped(**gated劣化,勿auto-start**) | 要点:原型病非仅选币病(池内挣钱行为全在main几何下,VELVET nS5/+7.03);5币已回main(bl17→12);tpl576+S25存git,复活=新壳走FLEET;壳留seed5+auto_symbols=false硬停;自动保险随停机失效 |
| ~~qt-breakout-follow~~ | 2111f5f9 | owner删除@08-15 15:5x(评判FAIL史+S26+tpl577存git;#47复活=新壳FLEET) | — | deleted | 幽灵运行时00:1x发现→检疫+heatclean固化;**幽灵仍活实证@08-16 21:2x**:18:25新开STAR空(19:25硬超时+0.32,删除后累计n2/+0.13;API404复核不可控;"沉寂"=信号荒假象);**#53修复fa93736候部署**(RemoveStrategy无条件Kill+stopping压制复活;同分支#56 closed归因order_id反查),owner部署+重启即根治=当前唯一通路;详史git 766c7d9+§3 |

- 隔离区(规则≤−4U∧n≥4): 4/CYS/TST/龙虾/BMT/BTW/H/APR/**AIO**(+AIO@08-15 21:2x,48h n4/−6.53全空单,bl16v2种子同步=重启免疫当轮闭环;APR@12:2x n5/−7.54;H隔离史存git)。**池列值=快照,权威=每轮对账器输出**。**对账器现版v1.3权威副本=quanty-ledger分支ops/route_pools.py@08-14(=334dae1内容;谱系61b1bb4←377c567←f970704;默认分支工作树只有v1.0初版——禁直接跑,实证2例错误plan)**:出池只认头注规则或隔离(失格币aging_watch);隔离表arg4;add互斥只排他专家池(v1.2误把main feed入排除集=晋升封死,COTI案实证修复);晋升币同轮移出main(封v1.1单轮重叠窗,执行序=先main remove后专家add);brk/lowvol出池阈值未注册前无自动出口(#42)。**arg4输入纪律(08-15 12:2x实证):隔离表必须传X/USDT形态——裸名norm后不match feed键,5隔离币漏出remove计划,当轮补刀rotate清除;代码零改动,属输入格式纪律**。最近跑@08-16 15:2x: **plan={}**,零新隔离候选,desired bl12=现值逐项一致✓,aging_watch=COTI/TUT advisory不变,want.fade=SPORTFUN/VELVET仅存档(载具停机);12:30读数同plan={}
- 保证金: Σ_running=main0.08×5+trend0.05×3=**0.55≤0.75✓**(08-16 12:3x fade停机后重算;fade0.04×2出式)
- 互斥不变式: feed层@18:24=main89/trend3(fade停机无live feed),main∩trend=∅✓ main∩bl12=∅✓;种子层=main bl**12**(9隔离+trend3)与对账器desired逐项一致✓重启免疫;重播种**#14=18:22 #49终裁窗重启100→heatclean-v9清11[trend3+隔离8;TST已出百强]→89**(#8-#13史git);引擎互斥闸✓live;**fade壳seed5与main auto理论重叠=停机态无违例**(不变式限定running载具;若owner UI复活fade须先重划池,互斥闸+TG示警兜底);隔离币零成交✓(复核@12:1x)
...
- **[速记·压缩@08-16 15:5x] DELETE不杀进程竞态→幽灵载具(brk 2111f5f9实证)**: DeleteStrategy先删DB行,RemoveStrategy仅Status==running才Kill→竞态窗进程存活成幽灵(API全404不可控,仍收feed开仓,三层出场+互斥闸仍罩);杀灭唯一通路=**后端重启**;幽灵账目单列不入载具归因(行带sid=2111可滤);#53修复fa93736候部署即根治;机制全文(源码行号+时间线)git 766c7d9系〔00:2x初登〕**复活实证@08-16 21:2x**: 18:25新开STAR空(18:22 main重启窗+3min巧合,非因果——ghost不受main生命周期影响;19:25硬超时+0.32,三层出场仍罩✓);"00:11后自然沉寂18h"=信号荒假象非死亡,幽灵是活运行时信号来即开;删除后累计n2/+0.13;API 404复核@21:2x仍不可控;TG已升级催部署
...
| # | 类型 | 内容 | 依据 | 复现计数 | 状态 |
|---|---|---|---|---|---|
| 34 | S23候选 | S23影子观测(逆bar) | **结案@08-16 12:39**:A组n25≥20门,AL/WL分离0pp<15pp→enforcement不采纳;S23零交易路径留场;副产物=重置点法揭2次台账外重启;全文git 766c7d9系 | 25·结案 | 结案 |
...
| 15 | M通道 | 脱管仓sweeper(实仓∩DB无行→重收养/告警) | KOMA 29h/−15.13+源码L48;FOLKS粉尘watch史git 26eaf24 | 0/2 | **结案@08-16 21:2x**——08-15部署以来零新漂移(≥5轮:无孤儿仓/无收养冲突/FOLKS粉尘消失);复现即重开 |
| 18 | M通道 | closed窗口边界播种修复(幻影行族根因) | 幻影三形态全史+标本链见git 26eaf24 #18行(BTW隙5幻影/镜像行/ROBO整60m例) | n/a | **修复部署@08-15**(open_key唯一索引+entry去重入f4848f8)→预期归零;观察第3轮@08-16 21:2x:0新增,窗内唯一幻影仍=ROBO 08-15 09:17旧例(明日~09:17自然出窗)→出窗后零新增即结案撤滤行纪律 |
...
| 8 | 观测性 | `stats_log_interval_sec` 600→60（config-only，零交易路径影响） | 全史git e767b9b | n/a | **结案@08-16 21:2x**——巡检发现main/trend现值已=60,audit实锤"仪表捎带(#8并入)"随早前PATCH落地未销账;陈账销;与#37(?q&limit)合流=统计行观测债体系性解决 |
| 9 | E7 类 | short盈利机制固化(白名单小改) | 判据链git 121e975 #9行 | 0/2·过时@08-13 | short主战场归fade(S25);main短侧复议前提=#43 unlatch后新证据 |

| 11 | E8 类 | short 方向跨币修补·**设计定稿@08-15 15:3x**: `breadth_max_for_short` null(引擎默认0.70)→**0.50**(config-only零代码;S13宽度门L1713既有veto路径:池内EMA上行占比>0.50拒空;与长侧breadth_min_for_long=0.50对称=regime分区;回滚=set null;_exp草案metric=main空侧24h n/net/wr+对照长侧,keep线=空侧24h净≥−8U∧均净改善于−0.541,劣化=≤−12U或n<5/24h窒息→回null) | 08-01登记0/2→**2/2@08-09**(24h short−15.20 n51跨16币同窗burst;门槛链git);15:12轮增证:FIX段空n23/−18.51 wr22跨8币在0.70门下全数放行=门未起regime保护作用(main统计行结构性不可观测,拦截计数不可直证;0.50收紧效果即_exp所测) | 2/2达成→病灶消退 | **降级观察@08-16 18:2x(#49终裁重算义务履行)**:终裁段空侧n26/+0.04打平(24h main空+4.30 wr77),08-09依据(24h空−15.20 n51 burst)在#49基线几何下未再现→0.50收紧包撤出候选队列,设计存档本行;**重升门=main空侧24h≤−8U复现**(届时以#49段为基线重算keep线再上_exp) |
...
| 55 | E7类(trend) | **EXP:trend long_conf_premium 0.15→0.10**(gate 0.70→0.65;config-only零代码;回滚=回0.15) | 分数墙源码级:ACE多头0.650×9h+顶门0.70反复拒发(03:21实证);S24终裁链n33/+0.024=36h零笔载具空转;ACE 24h 4W+1.73=池内信号质量在场,门下0.05徘徊非无信号 | 机制级(gate) | 候选@06:35;**执行门=费覆红线解禁(组24h均净≥3×费)或owner裁量**(降prem=提频,红线内只登记@03:11轮既定);执行时走EXP _exp预注册(trend槽已空闲) |
...
| 53 | 引擎候修(M通道) | **DeleteStrategy斩草除根**:RemoveStrategy去Status==running守卫改无条件Kill;与#52同族(生命周期竞态) | §3 08-16幽灵条(manager.go L1496守卫+行为实证23:11Z);zombie 48h n15/−1.56 | n=1(brk) | **修复已推@08-16 09:5x**=claude/dev-closed-attrib-delete-kill(fa93736):无条件Kill+置stopping压制复活;候owner部署(部署自带重启=幽灵即灭);startCh drain未做(getStartableStrategy按注册表查=天然挡,部署后观察);#52仍独立候修 |
| 56 | 引擎候修(M通道) | **closed视图归因order_id精确反查**:closedPositionsFromBinance归因主路径改入场腿order_id→strategy_orders(exchange_order_id)精确定位下单载具,±5min时间窗降级兜底(enrich跳过已归因行) | §3归因条陷阱③(BICO 08-16双笔实证:main开仓被trend镜像行抢归因;污染载具账目/保险线/评判段) | n=2笔 | **修复已推@08-16 09:5x**(与#53同commit fa93736);候owner部署;部署后验证=closed48重拉BICO类行sid=开仓者,观察2轮结案;部署前修正纪律见§3③ |
| 49 | 组共病(出场几何) | **出场几何倒挂重构**:main trailing(act1.0/cb1.2%)+BE(1.0);目标=实现赔率0.88→≥1.3 | 72h审计n=96基线:SL率40%/均亏−1.84/TP仅12%均+0.64/实现0.88:1 vs 设计1.8:1〔全文git 191b049前〕 | n=30段 | **结案KEEP@08-16 18:2x n=30门先触**(段n30/−3.81/均净−0.127[5.7×]/SL率20%/均亏−1.385[−25%]/实质赢6笔均+1.152[+80%];赔率0.43<1.2按15:30预登记框架裁=13微赢灌稀分子的统计失真,三无失真维度全同向改善;回滚线[赔率<1∧净≤−10]远未触;保守n29口径同结论;verdict全文config._exp+git;trailing+BE转main基线几何) |
...
> 维护注 08-16 06:2xZ：#54 **落地结案删行**——06:23真空仓窗程序v2(stop→poll1即stopped→PATCH bl17→start),后验bl17含BULLA✓/_exp#49完好/running;重启重播种100→**heatclean-v7**(热清17币=bl在册14+僵尸3)→feed83残留0 overlap全绿;BULLA种子级重启免疫闭环,对账器desired=actual逐项一致。
...
> 维护注 08-15 19:2xZ：#50 **落地结案删行**——19:1x金丝雀唤醒轮真空仓窗执行程序v2(stop→紧循环PATCH try1即中→start),后验bl16/VELVET在bl/_exp#49完好/running;heatclean-v5(重播种#10:100→87 bl残留0)。
> 维护注 08-14：#46/#42/#45 结案,全史git。
...
| BICO资金费磁铁长侧 | **n3/−2.94@08-16**(04:02+0.57/07:57−1.90/13:32−1.61;三入全为funding−1.1%级→'资金费率<−0.0003:+0.20'bonus推过0.70长门;#49终裁确认=段唯一净亏源;**21:2x复核:13:32后7.7h零新笔**;income直证磁铁结构=BICO 24h收funding+0.292但价亏−2.94=吃小费亏大价) | 08-16 21:2x | 判据:累计≥6笔或净≤−6U→候选化(选项=funding bonus降权/该币CB提前);n3轶事级只记账;归因注记:08-15T18:17 BICO−0.91按池归属应归main(trend feed无BICO)但S24已闭链留trend账,两侧verdict双稳(#49含/不含同结论,S24链n32/+0.93仍keep),不重开 |
...
| 08-16 21:12-21:4xZ | 例行轮(max) | @21:12: 1h0/3h+0.32(1=幽灵)/6h+0.68(4仓,wr75)/12h+1.05(12仓,wr67 be62=+4.4pp)/24h−1.42(30仓,wr63 be67) | FGI34/BTC−0.05%/市值−0.14%无降压;trending∩main池=BEAT/AKE/CHIP,BTW隔离bl在防 | ①**幽灵复活实证**:brk 18:25新开STAR空19:25硬超时+0.32(删除后累计n2/+0.13;API404复核不可控;"沉寂18h"=信号荒假象;§3/§1.5已更;TG升级催fa93736=唯一根治) ②0原子包HOLD(费覆24h🔴锁#55/#47;三_exp全closed实验槽双空闲;无刹车无待落) ③ROUTE 21:2x plan={}(bl12逐项一致/零隔离候选/aging COTI·TUT advisory不变) ④陈账清理:#8结案(audit实锤"仪表捎带并入"现值已60)+#15结案(零漂移≥5轮)+#18第3轮0新增 ⑤刹线全绿(ex-ghost 6h+0.36/24h−1.55 vs −17.8/−44.4;trend6h0>−5;main空6h+0.36>−8;Σ0.55≤0.75) | 死法24h(ex-ghost n28):SL穿刺7/−9.88主刀(main4/−6.90=BICO×2−3.51+BEAT−1.72+SPFN−1.68;fade遗留3/−2.98已止血✓),WIN18/+10.50;载具拆分=**main n23/+2.25(wr74,空20/+5.19,多3/−2.94全BICO)**/trend n2/−0.82(COTI饥饿−0.85)/fade遗留−2.98;BICO§6 n3不变13:32后7.7h零新笔;费覆24h组−0.047 vs 0.118🔴禁提频(main毛+0.098 vs ~0.117边缘🔴;6h名义🟢被幽灵+0.32灌注,ex-ghost+0.120 vs 0.132边缘🔴);钱包222.13空仓;main18:24重启后3h零笔=信号荒非故障(21:20 IDLE feed89/89✓TOWNS 0.52<0.55实证);⏭BICO门/24h费覆转正解锁#55/幻影ROBO出窗核 |
| 08-16 18:11-18:4xZ | 例行轮(max) | @18:11: 1h0/3h+0.36(7)/6h+1.78(15,wr87)/12h−2.62(29,wr55 be57)/24h−3.36(33仓位口径,wr61 be57) | FGI34/BTC+0.1%/市值+0.01%无降压;trending∩trend池=ACE;CYS上榜隔离在防 | ①**#49终裁=KEEP**@18:2x n=30门先触(段n30/−3.81/均净−0.127✓/SL率20%✓/赔率0.43<1.2按预登记框架裁失真[13微赢灌稀+6实质赢均+1.152右尾+80%+均亏−25%];保守n29同结论;流程=空仓窗stop poll1→PATCH _exp closed 628字→start→重播种#14 100→heatclean-v9清11→feed89全绿;trailing+BE转main基线) ②**#11降级观察**(段空n26/+0.04,burst病灶未再现;重升门=空24h≤−8U) ③ROUTE 18:1x plan={}零隔离候选bl12一致 ④0原子包(#55/#47费覆门未过) ⑤刹线全绿(6h+1.78/24h−3.36 vs −17.7/−44.4;trend6h0>−5;main空6h+3.13>−8) | 死法24h:fastSL7/−9.88主刀(BICO×2−3.51/BEAT−1.72/fade遗留3);最大失血机制=BICO磁铁(§6 n3未达门);费覆24h组−0.102 vs 0.113🔴禁提频(main+0.018回🔴;6h组+0.119首🟢待续);钱包221.86空仓;幽灵00:11后18h零新笔;fa93736未部署;⏭BICO门/24h费覆转正解锁#55#47/幻影观察第3轮 |
| 08-16 15:14-15:5xZ | 例行轮(max) | @15:25: 1h0/3h+1.42(4)/6h+0.37(8)/12h−1.13(18,wr61 be65)/24h−0.10(35,wr57 be57) | FGI34/BTC+0.1%/市值−0.04%无降压;trending∩main池=CHIP,H/CYS上榜但在隔离bl防 | ①**#49到点评判=continue**@15:30Z(段n26/−3.27/赔率0.39/均净−0.126/SL率19.2%;两读法收敛:终门=n≥30或08-17 15:32Z先到;裁由+右尾EXIT_AUDIT直证全文§1.5) ②0原子包(HOLD:唯一大改在飞不混段;#11重算义务登§4;#55/#47费覆门未过) ③ROUTE对账15:2x plan={} bl12逐项一致零隔离候选 ④归因新修正2行:BICO13:32→main(trend feed3铁证)+VELVET14:03→main(fade停机铁判据);组−0.10+幽灵−0.18=income −0.472精确对账✓ ⑤刹线全绿(6h+0.37/24h−0.10 vs −17.7/−44.3;trend6h−0.85>−5;main空6h≈0>−8) ⑥幽灵brk 00:11后零新笔15h✓ | 死法24h:SL穿刺6/−10.05主刀,WIN20/+17.93;最大失血机制=BICO长3笔−2.94(§6新计数,段空侧n23/−0.32已打平);费覆24h组−0.003 vs 0.114🔴禁提频,**main+0.176首🟢**,trend−0.60🔴(n4含2笔BICO遗留出窗中);钱包≈221.7(avail202.6+PTB仓~19);幻影0新增(ROBO旧例第2轮);fa93736未部署;⏭#49终门(n26→30,节奏~6-12h) |
| 08-16 12:18-13:0xZ | 例行轮(max) | @12:18: 1h+0.47(1)/3h−1.05(4)/6h−4.41(8)/12h−2.84(19,wr63 be74)/24h−2.77(41,wr54 be57) | FGI34/BTC+0.1%/市值−0.05%无降压;trending∩main池=ACU/CHIP/NIL | ①**fade劣化线触发→FLEET:stop**@12:26(累计n12/−5.44=承前8/−2.63+池内sid确证4笔−2.81,BEAT10:08−0.88落线,评期12:50Z前劣化线先至;stop舍rollback四由§1.5;_exp=degraded_stop) ②**ROUTE:5币回main**@12:50全序(空仓窗stop poll1→bl17→12→start;重播种#13→heatclean-v8清11+补种2→feed91;main∩trend=∅;Σ0.63→0.55) ③**#34结案**(S23bar重置点法回补:A组25≥20门,AL/WL分离0pp<15pp→enforcement不采纳,观测债清偿) ④#49段@12:50 **n21/−2.49赔率0.36 SL率14%均净−0.12**(回滚线未触续飞;尾笔SPORTFUN+0.62=BE+trail救−0.91水下单) ⑤对账器12:30 plan={} bl_sync=bl12独立一致零新隔离 ⑥刹线全绿(6h−4.41/24h−2.77 vs −17.6/−44;trend6h−0.82>−5;main空6h 0>−8) | 死法24h:SL穿刺9/−12.88主刀;WIN22/+17.4;**失血源=fade−2.98+trend−0.83,main24h+4.19(wr−be+7.9pp)独正**;费覆24h🔴组−0.068 vs 0.106(main+0.168🟢/trend−0.42🔴);钱包≈220.4;幻影1=ROBO旧例;fa93736未部署(feed83无重启证据,TG再请);⏭15:30 #49评判(段口径=池归属+VELVET17:18归main,order_id候fa93736) |
| 08-16 09:12-10:1xZ | 例行轮(max) | @09:26: 1h0/3h−3.36(4)/6h−1.50(10)/12h−2.47(18,wr61 be71)/24h−4.38(43,wr51 be57) | FGI34/BTC+0.2%无降压;trending∩池=CYS+H(均隔离区,bl在防) | ①**归因污染破案**:BICO 04:02(+0.57)/07:57(−1.90)盖trend sid实为main开仓(trend日志零开仓行/main日志全链实锤;机制=镜像行双记账×enrich±5min时间窗抢行,§3归因条+陷阱③;无幽灵trend,BICO评估日志21:1x正常截止=旧trend运行时确死) ②**1原子包=引擎修复推送**fa93736@claude/dev-closed-attrib-delete-kill:#56 closed归因order_id精确反查+#53 RemoveStrategy无条件斩杀(go build过;候owner部署,部署自带重启=幽灵brk即灭;TG已请) ③#49段n20/−5.36赔率0.34 wr60 SL率20%均净−0.268(回滚线未触;段读数已按修正归因) ④fade链n11/−4.56(+VELVET−0.82,距劣化线0.44U),评期12:50Z ⑤ROUTE对账器plan=空,aging=trend2/fade4 advisory,desired bl=现bl17逐项一致,零隔离候选 ⑥刹线全绿(6h−1.50/24h−4.38 vs −17.7/−44.3;trend6h+0.02/fade6h−0.82/main空6h+1.30) | 死法24h:SL穿刺9/−13.22主刀(main−9.83),硬超时7/−4.75,饥饿4/−3.58,慢亏1/−0.65,WIN22/+17.82;费覆24h🔴组均净−0.102 vs 0.118(main−0.043/trend+0.056🔴n8费0.082/fade−0.644🔴)禁提频;钱包~221.5,09:30起2仓(trend COTI多+main GUA空);幻影1=ROBO旧例(#18在防);⏭12:50 fade评(n11,大概率证据不足格)→15:30 #49评(用order_id归因口径) |
| 08-16 06:15-06:5xZ | 例行轮(max) | 24h−5.85(44,wr50 be57) | FGI34;trending∩池=ACE+COW | ①#54落地结案(真空窗v2+heatclean-v7,100→83残留0;BULLA种子免疫+三层互斥全同步闭环) ②S24终裁keep结案(链n33/+0.024;canary转常驻6h≤−5U;#55登§4) ③#49段n18/−2.88续飞 ④对账器plan空 ⑤刹线全绿〔全文git 2aa822b〕 | 死法24h:SL穿刺9/−15.33主刀;费覆24h🔴−0.133;钱包224.73空仓;0原子包;⏭12:50fade→15:30#49 |
|---|---|---|---|---|---|

> 瘦身注 08-16 18:4x：03:11/00:13两行随轮裁剪(BULLA晋升轮[fade已closed故moot]+幽灵发现轮,要点存§3幽灵条/§4#53,全文git 2aa822b)。〔前注 15:5x：08-15 21:13/19:05/18:12三行随轮裁剪(canary轮+机制发现轮,要点存§3/§4#52)〔前注 12:5x：15:12/16:06两行裁剪(#49上线轮+VELVET晋升轮,要点存§1.5/_exp)〔另 08-15 21:4x：12:18/14:3x/15:2x/15:1x四行裁剪〕(质疑轮结论=era对比部分证实部分证伪+owner择A打#49,全文git 7a6729c前后;其余要点存§1.5/§3/§4/_exp),全史见 git 历史。