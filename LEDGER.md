...
| 载具 | id | 原型 | 池 | 状态 | 门/备注 |
|---|---|---|---|---|---|
| main 通才 | 8eb182b6 | 通才(S18-S23栈) | auto·feed90(种子bl13=期望态✓;06:2x无端重启重播种100→rotate−10自愈;BICO残留has_open拒[持仓属trend,bl在防]下轮释;overlap1=BICO) | running | pct0.08 sides=[buy,sell] **long_premium=0.35(FIX@08-14 18:47Z)** tpl567;_exp=**FIX长溢价0.15→0.35**(long_thr0.70→0.90;证据=48h长侧n20 wr30/−11.79过20笔门,病根=逆势栈0.70恰等长门,S12先例;keep=08-15 19:00Z长侧24h净≥−2U∧组均净较−0.216改善;劣化=长侧≤−6U或组净恶化→回0.15再议[sell]latch;全文config._exp)+**继承RECOVER刹线**(6h空侧≤−8U→重latch[buy];03:2x读−1.29未触);**FIX段读@T+11.5h(06:2x轮)**:长侧n2/−1.44零增(节流在效),空侧n11/−8.88wr18(+VELVET#30型−1.77+FHE超时+0.49)=空侧段主刀不变;两臂现值双过(长24h−1.44≥−2U✓∧组均净−0.177优于−0.216✓,以19:00Z当值为准,按侧拆臂说理);06:2x RECOVER读6h空−1.51未触;sid伪影纪律照旧(§3) |
| qt-trend-long | 827ffe8c | 趋势动量多 | ACE/TUT/BICO/COTI(feed4=种子4✓@08-15 03:20 CAP种子卸载;aging_watch=TUT/BICO advisory) | running | **tpl575(S24)** pct0.05 mcp3 buy trailing on;_exp v3常驻:**累计n25/+0.07首转正**(+BICO快TP+0.68@08-15 04:09);随时keep早结门=净>0✓∧毛均净0.003<3×费0.096✗不早结;劣化−5U/canary−5U(现6h+1.09)未触;费覆🟢24h+0.167vs0.096=1.7×;**终裁08-16 06:35Z当值净>0=keep**(现值边际正,BICO多@05:51在飞待兑现);ACE 48h+3.44/COTI+3.56双扛旗∧ACE trending;全史git+config._exp |
| qt-fade-short | 21519f1b | 冲高回落空 | 1000CAT/ARC(种子✓;aging_watch全2币) | running@08-13 12:45Z | **tpl576(S25)** pct0.04 mcp2 sell;_exp v2续:累计n8/−2.63零增(现池零成交**第11轮**,want.fade=[]连续;非载具故障[06:19在评:1000CAT空0.48/ARC0.24<0.55门]);评期=累计n≥15或**08-16 12:50Z**(明日到点n8<15将再触证据不足格→届时议池补灌或停驶取舍);自动保险6h≤−6U(0✓) |
...
- 互斥不变式: feed层≈✓(overlap=1仅BICO残留@08-15 06:2x rotate后,main bl信号层在防,待trend平BICO下轮释;main90/trend4/fade2)>**种子层=期望态✓@08-15 03:20**(main bl13含H无CAP;trend种子4币;CAP释回main发现漏斗;重启免疫达成)>⚠️引擎互斥闸未部署;aging=trend2/fade2 advisory;重播种规律第4例(第2例无端型,升§3):feed播回auto100含bl币(信号层bl在防零交易风险)→热rotate自愈;种子同步后专家池重启免疫实证(trend4/fade2零漂移);pre-restart IDLE行93 vs API91=频道层订阅残留伪影(重启自清,观测勿当违规)
