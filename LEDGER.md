...
| 计数项 | 读数 | 更新轮 | 备注 |
| backend→Binance REST DNS断连 | **n=1(episode): 09-09 16:12Z起持续,09-12 21:15仍在(≥77.0h;probe史33miss+cron直证十五轮=git链→981f7ed→8fcded3;21:1x直证=active×2+双载具ctx income三点同串fapi SERVFAIL+balance伪零;ws活证main评批183×4+trend@21:12;feed183/3无重播种=无重启=#89未部署)** | 09-12 21:1x | 特征=fapi SERVFAIL(resolver 100.100.100.100=Tailscale MagicDNS,修复在宿主机);伪影族=ctx balance/income伪零+skip文案+closed窗过滤失效(close_time过滤免疫)+active回error(全谱git ecec3b0);**已升§3复发向量n=2+#89补丁c49c555候部署(§4)**;probe#3@21:30前session末次+本轮re-arm×3@21:57/22:57/23:57→恢复即落#88;恢复后复核:①income补记完整性②feed重播种③断连窗内重启(ws也死须立报);⚠️断连窗禁stop/start(重订阅可能失败) |
