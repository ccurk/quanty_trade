...
| CC-15 | owner裁决 | **p6 出场几何 D 方案(推荐)**: 只放宽赢单侧 trailing_callback_pct 0.5→1.2 / trailing_activation_atr 0.5→1.0 / breakeven_trigger_atr 0.5→1.0(亏单侧 hunger_sl0.075/max_hold45 不动);证据=6 笔可取赢单 45m 现货 MFE vs 实收: G +11.49/+2.07, VTHO +5.22/+2.39, LPT +3.72/+0.03(BE 出场), JUP +1.69/+0.08(BE), STRK +0.55/+0.14, HEI +0.93/+0.03;代价 STRK/HEI 转 0.75% 止损(MAE45 −1.21/−4.52);粗估 6 笔 +17→+35~40U(现货代理小样本);备选 A(反事实否决)/C 维持;owner 表键 CC 不擅动,每轮 TG 提醒 | 09-18 15:2x | open(**17:5x owner 裁了亏单侧=止损 4%(v58,见 §1);赢单侧 D 仍候;lev5/关交易所TPSL 二选一亦候**) |
| CC-17 | 评判 | p7(owner hunger_sl **0.04**=ROI 4%,v59@17:59:41Z) 段(open≥17:59:41Z): 段均净(毛)≥0 ∧ 止损占比(p6 54%)与单次均亏(预期 −1.2~−1.5U) ∧ 赢单是否被 0.4% 噪声提前杀(对照反事实 8/18) ∧ 6h/24h 钱包%;eval 09-19 00:00Z 或 n≥30;劣化(段净≤−10U 或 24h≤−20%)只 TG 建议 ②pct 减半或回 0.075;DS 砍 pct→按 owner 值恢复 | 09-18 18:0x | open |
...
| **09-18 p5(pct0.25) 终裁 + p6(紧出场) 读数** | @15:2x **p5 段(open≥13:09:22Z) n31 395笔/日 净+8.40 均+0.271 wr58% be53% 赢均+2.47 亏均−2.77 名义中位291U(max429) → KEEP 3/3(owner 值);诚实注记 G 2笔+21.62 撑段,去 G −13.2** ‖ **p6 段(open≥13:39:15Z) n23 全多 净−11.46 均−0.498 wr48% be58% 穿刺13%;饥饿SL 11/−28.7;微赢(<0.4%价)7/+2.95;跑赢 3/+18.29;conf0.44 桶 n10 −15.73**;24h 逐笔 n100 均+0.119 wr54% be49%;income 24h 毛+14.86 手续费−12.86(费吞 87%)净+2.19;6h +1.63;1h −3.88;入金 +83.56 已在 24h 窗;费覆 p5 0.271 vs 3×费0.87 🔴 | 09-18 15:2x | **p6 封段终读@17:56Z(open 13:39:15→17:56:55) n71 398笔/日 净−34.75 均−0.490 wr44% be55% 多51/−41.69 空20/+6.94;饥饿SL 33/−91.8 vs 赢 31/+61.8;hold<10m 60/71;conf0.44 桶 n25 −27.89, ≥0.56 n15 +19.46;15:25 后子段 n41 −30.99 饥饿SL 22/−54.72** → owner 令 hunger_sl 0.40(CC-17);D 方案候 owner(CC-15) |
...
## 7. 运行日志（每轮一行，新行追加在表首）
| 2026-09-18 17:5x-18:1x | 交互轮·owner 止损直令(ROI 口径): "3%"→"4%"→"不可能…都是在 10% 亏损才平";CC 误读价格距离落 0.30/0.40(v57/58)→owner 自落 0.04(v59)→CC 登记 v60+留言板更正 v61;源码核 SL 钳 0.3/lev(strategy_position.go:485/884)与 ROI 4% 无关;逐笔: 15:25→17:56 n41 −30.99 wr44%<be64% 饥饿SL 22/−54.72(roi −8.8~−9.6%=UI −10%) 多 −27.57/空 −3.43;p6 封段 n71 −34.75 止损 33/−91.8;首两笔 0.04 止损 CHIP −4.14%/SIREN −4.71% ROI;反事实 0.4% 止损 33 笔 −21.11 vs 实际 −2.62(8/18 赢单被杀);报警线 6h income 净 −36.6(−18%,只 TG)/24h −31.96(−16%);avail 164 空仓;DS: 0 PATCH, lock 至 09-19 15:22;TG 三条(读法/引擎钳/单位对质+三条路+🎓);下轮=p7 首读(n≥30 或 00:00Z)+owner 对 ②pct 减半/D 赢单侧 的回复+DS 是否砍 |
