...
| CC-2 | RECOVER门跟踪 | 门自09-18 06:2x只管lev升档;lev 按owner令10;**@09-19 00:1x 6h −8.1%/24h −19.0%(含入金基)=门关;下一档(lev12/20)非owner令不推,须双转正+预注册+max_atr同步** | 09-18 | open |
...
| CC-15 | owner裁决 | **p6 出场几何 D 方案(推荐)**: 只放宽赢单侧 trailing_callback_pct 0.5→1.2 / trailing_activation_atr 0.5→1.0 / breakeven_trigger_atr 0.5→1.0(亏单侧不动);证据=6 笔可取赢单 45m 现货 MFE vs 实收(G/VTHO/LPT/JUP/STRK/HEI 全文 git 5096ca1),粗估 +17→+35~40U;owner 表键 CC 不擅动 | 09-18 15:2x | open(**@09-19 00:1x 第 4 次提醒;新增备选 E=止损随 ATR 缩放(hunger_sl≈0.5×ATR×lev 或 hunger_after 1→≥10m):证据=同批 ATR>0.8% 币在 P3 段(饥饿45m+ATR SL) n16 +22.30 wr69%,在 0.4% 固定刀段 n16 −22.09 wr25%;CC 已用 max_atr_pct 0.8 匹配 owner 现出场,owner 若选 E 则放宽回 6**) |
...
| CC-19 | M候选(费率病根) | **maker 入场**: 24h 手续费 −40.59U vs 毛 −8.34U(费=毛的 5 倍);源码=入场 MARKET 单(strategy_signal.go:745 enqueueOrderForInstance price=0→binance.go:1372 price>0 才 LIMIT GTC,无 GTX/post-only)=双边 taker 0.1% 来回;方案=claude/dev-maker-entry: 入场改 LIMIT GTX 挂最优价,N 秒未成交撤单改市价(config entry_post_only/entry_maker_timeout_sec);预期费 0.1%→0.07%(单边 maker)~0.04%(双边);部署权 owner;先候 owner 一句话再开发 | 09-19 00:1x | open |
| CC-20 | 评判 | FIX max_atr_pct 6→0.8(_exp_cc 09-19 00:24:49Z;重启 00:25:26Z 首单起): n≥30 或 06:00Z: 均净(毛)≥+0.3 ∧ wr≥55% ∧ 费后均净≥0 ∧ 笔/日≥60;ATR 桶反推=tpsl 日志 sl距/1.5(新进程);worsen 段 n≥15∧净≤−4U 或 笔/日<38.7→回滚 6 或放宽 1.0;同段单独计 TP(2×ATR) 命中与被封赢单(owner 2.0/1.5 同时生效=混杂) | 09-19 00:2x | open |
> 结案集@09-19 00:2x: CC-12(owner atr 2.0/1.5 经 CC force 重启 00:25:26Z 生效)/CC-13(p6 封段 17:56Z n71 −34.75)/CC-17(p7 裁决见 §6)/CC-18(ROUTE 7 币落地 20:2x) 终态全文=git 5096ca1版§5;更早结案集(DS-1/DS-10/CC-3/CC-7/CC-8/CC-10/CC-16/CC-14/DS-11/CC-6)=git 2fcc0da版§5。
