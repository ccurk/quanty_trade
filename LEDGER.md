...
| 载具 | id | 原型 | 池 | 状态 | 门/备注 |
|---|---|---|---|---|---|
| main 通才 | 8eb182b6 | 通才(S18-S23栈) | auto·**feed88@08-17 03:26**(bl13种子=10隔离+trend3,重播种#15+heatclean-v10) | running | pct0.08 sides=[buy,sell] long_conf_premium=0.35 tpl567;**trailing(act1.0/cb1.2%)+BE(1.0)=基线几何(#49终裁KEEP@08-16 18:2x)**;_exp=closed(verdict 628字存config._exp.verdict+git);**实验槽空闲**;#49终数=段n30/−3.81/均净−0.127(5.7×)/SL率20%/赔率0.43裁=统计失真keep(裁由全文_exp+git+§4#49);段亏全额=BICO×4−3.85(§6),ex-BICO n26/+0.04;RECOVER刹线6h空≤−8U继承;#11降级观察(§4);后续实验以本段为基线;**BICO已隔离@08-17 03:2x**(§6结案;隔离后零新笔✓复核07:0x) |
| qt-trend-long | 827ffe8c | 趋势动量多 | ACE/TUT/COTI(feed3=种子3✓;**aging_watch=全池3币advisory@08-17 07:0x**,ACE 24h n2/−2.03 wr0重入列;S24 keep后漂移n3/−2.01观察) | running | **tpl575(S24)** pct0.05 mcp3 buy trailing on;**归因修正@09:2x**:BICO 04:02(+0.57)/07:57(−1.90)两行盖trend sid实为main开仓(trend日志零开仓行,main日志全链;S24 keep判据'36h零新笔'修正后仍真);正牌08-16=ACE+0.02@06:46+COTI多在飞@09:30;**_exp=closed@06:35:12Z: S24终裁keep结案**(预注册当值格'净>0=keep':链n33/+0.024>0;末笔08-15 18:17 BICO−0.91,终裁前36h零新笔=03:11后零笔复核@06:35;程序=真空仓窗stop[poll1沉降]-PATCH-start,种子3币完好);**常驻保险(继承canary)=滚动6h净≤−5U→stop本载具**;实验槽空闲;候选EXP降prem=§4#55(费覆红线内只登记) |
...
- 隔离区(规则≤−4U∧n≥4): 4/CYS/TST/龙虾/BMT/BTW/H/APR/AIO/**BICO**(+BICO@08-17 03:2x对账器v1.3:48h n8/−4.05全多头≤−4∧n≥4[24h口径n3=窗口差,08-15五笔在册],资金费磁铁§6结案,rotate热移+bl13种子同轮闭环;+AIO@08-15 21:2x,48h n4/−6.53全空单,bl16v2种子同步=重启免疫当轮闭环;APR@12:2x n5/−7.54;H隔离史存git)。**池列值=快照,权威=每轮对账器输出**。**对账器现版v1.3权威副本=quanty-ledger分支ops/route_pools.py@08-14(=334dae1内容;谱系61b1bb4←377c567←f970704;默认分支工作树只有v1.0初版——禁直接跑,实证2例错误plan)**:出池只认头注规则或隔离(失格币aging_watch);隔离表arg4;add互斥只排他专家池(v1.2误把main feed入排除集=晋升封死,COTI案实证修复);晋升币同轮移出main(封v1.1单轮重叠窗,执行序=先main remove后专家add);brk/lowvol出池阈值未注册前无自动出口(#42)。**arg4输入纪律(08-15 12:2x实证):隔离表必须传X/USDT形态——裸名norm后不match feed键,5隔离币漏出remove计划,当轮补刀rotate清除;代码零改动,属输入格式纪律**。最近跑@08-17 07:0x: **plan={}**,desired bl13=现值逐项一致✓,aging_watch=ACE/COTI/TUT(trend全池advisory),want.fade=SPORTFUN/VELVET·want.brk=ACE/BEAT/ROBO仅存档(载具停机/已删)
- 保证金: Σ_running=main0.08×5+trend0.05×3=**0.55≤0.75✓**(08-16 12:3x fade停机后重算;fade0.04×2出式)
- 互斥不变式: feed层@08-17 03:26=main88/trend3(fade停机无live feed),main∩trend=∅✓ main∩bl13=∅✓;种子层=main bl**13**(10隔离+trend3)与对账器desired逐项一致✓重启免疫;重播种**#15=03:24 BICO种子同步窗重启100→heatclean-v10清12[trend3+隔离9;TST出百强]→88**(#8-#14史git);引擎互斥闸✓live;**fade壳seed5与main auto理论重叠=停机态无违例**(不变式限定running载具;若owner UI复活fade须先重划池,互斥闸+TG示警兜底);隔离币零成交✓(复核@12:1x)
...
| funding磁铁扩展watch(ACE) | trend ACE 08-17 03:43 L conf0.85买RSI85.5顶10min −1.18;funding−0.0054触bonus+0.20但**非决定性**(裸分0.65仍过0.55门)≠BICO推门型;真实伤害=conf虚高→mult1.4放大仓位 | 08-17 | n=1轶事;再现(bonus决定性入场价亏 或 bonus致mult放大亏≥3例)→按BICO结案条重开候选(S24 funding bonus降权/极端负funding闸) |
...
| 08-17 06:40-07:3xZ | 例行轮(max) | @07:05: 1h+0.87(1)/3h+0.11(2)/6h−1.36(4,ex-ghost−1.07)/12h−0.80(6,wr50 be61)/24h−2.63(19,wr53 be60) | FGI31/BTC+0.9%/市值+0.7%无降压;trending∩池=ACE(trend)+VELVET/PORTAL/CAP/AKE(main);BTW隔离在防 | ①**#57发现+修复推送a451d32**(收养竞态双行双守护:main开BEAT 05:54[开仓链'已预留1/200'=mcp200指纹]落fade sid行,06:35-06:44 main/fade两日志流互撤重挂TP/SL['重复止盈止损单tp=2 sl=2撤销全部重建'每~5s],错config接管[fade BE1.2/act2.0盖main1.0/1.0,幸max_hold/hunger同值处置未变];BLUAI 03:48同型['2/200'指纹,fade sid 60m硬超时−0.76];根=roi_monitor tick快照落后入场行落库~3s+存在检查带strategy_id=只查本策略行;修=账户级任意行查重+每净仓单守护认领;推同分支=owner一趟部署带走#53#56#57) ②0策略原子包HOLD(费覆24h🔴avg−0.156 vs 0.116;12h main+0.117 vs 0.145边缘🔴;#55#47续锁;双_exp槽空闲无在飞) ③ROUTE plan={}(bl13逐项一致;aging_watch扩至trend全池ACE+COTI+TUT;无隔离候选) ④幽灵02:37后零新笔;刹线全绿(6h−1.36/24h−2.63 vs −17.7/−44.2,trend6h−1.18>−5,main空6h−0.76+0.87>−8,Σ0.55✓) | 死法24h:WIN9/+6.90,fastSL5/−7.28,hunger1/−0.85,timeout2/−1.43;载具拆分=main n14/+0.25(wr64,ex-BICO+3.76,空侧9W0L,BICO隔离后零新笔✓)/trend n2/−2.03(wr0;ACE 03:43 conf0.85买RSI85顶10min −1.18=funding−0.0054 bonus+0.20非决定性[裸0.65仍过门]但mult1.4放大仓位,§6登记)/fade遗留n1/−0.88/ghost n2/+0.02;钱包220.81空仓;misattr行账已按开仓者归位;⏭a451d32部署观察(重启→feed重播种+stopped复活+双行归零验证)/ROBO幻影10:17出窗/24h费覆转正解锁#55 |
| 08-17 03:12-03:4xZ | 例行轮(max) | @03:13: 1h−0.29(1=幽灵)/3h−0.29(1)/6h−0.05(2)/12h+0.63(6,wr67 be60)/24h−0.50(24,wr62 be64) | FGI31/BTC+0.4%/市值+0.4%无降压;trending∩main池=PORTAL;BTW上榜bl在防 | ①**ROUTE:BICO隔离**@03:2x(48h n8/−4.05全多≤−4∧n≥4 v1.3;24h只见n3=窗口差;8行全trend sid=#56镜像归main)=rotate热移89→88+空仓窗bl12→13种子同步(stop poll1即沉降,_exp628字完好)+重播种#15(100)+heatclean-v10清12→feed88 overlap0;隔离区9→10 ②0原子包HOLD(费覆24h🔴均净−0.021 vs 0.119锁#55#47;12h+0.105 vs 0.132回落🔴=上轮首绿未守,SPORTFUN赢单出窗;双_exp槽空闲;刹线全绿6h−0.05/24h−0.50 vs −17.8/−44.4,trend6h 0>−5,main空6h+0.24>−8,Σ0.55✓) ③**幽灵删除后第3笔**:STAR空02:37超时−0.29,累计n3/−0.15;fa93736唯一根治TG再催 ④audit零无主;隔离币隔离后零成交✓ | 死法24h:WIN11/+7.65,fastSL5/−6.92,timeout7/−0.38,hunger1/−0.85;载具拆分=main n17/+2.67(wr76,ex-BICO+5.61)/trend n2/−0.82(17h零新笔,#55费覆锁)/fade遗留n3/−2.37止血中/ghost n2/+0.02;钱包221.98空仓;⏭ROBO幻影09:17出窗核0/BICO隔离后main长侧净化/24h费覆转正解锁#55/fa93736部署观察(重启→查feed重播种+stopped复活) |
|---|---|---|---|---|---|

> 瘦身注 08-17 07:3x：08-16 21:12+00:18两行裁(幽灵复活轮+信号荒市场性轮,要点§3/§4#53)(幽灵复活实证轮,要点存§3幽灵条+§4#53;#8#15#18结案注存§4;全文git)。〔前注 00:4x：09:12/06:15两行随轮裁剪(归因破案轮+S24结案轮,要点存§3归因条/§1.5 trend行/§4#53#56,全文git 34a897e)。〕〔前注 18:4x：03:11/00:13两行随轮裁剪(BULLA晋升轮[fade已closed故moot]+幽灵发现轮,要点存§3幽灵条/§4#53,全文git 2aa822b)。〔前注 15:5x：08-15 21:13/19:05/18:12三行随轮裁剪(canary轮+机制发现轮,要点存§3/§4#52)〔前注 12:5x：15:12/16:06两行裁剪(#49上线轮+VELVET晋升轮,要点存§1.5/_exp)〔另 08-15 21:4x：12:18/14:3x/15:2x/15:1x四行裁剪〕(质疑轮结论=era对比部分证实部分证伪+owner择A打#49,全文git 7a6729c前后;其余要点存§1.5/§3/§4/_exp),全史见 git 历史。