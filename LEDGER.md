...
| 载具 | id | 原型 | 池 | 状态 | 门/备注 |
|---|---|---|---|---|---|
| main 通才 | 8eb182b6 | 通才(S18-S23栈) | auto·**feed90@08-23 15:2x(#28热清10/12;COLLECT/MELANIA被has_open_position拒[v2持仓非main仓=账户级判]下轮重试,双兜底=bl交易闸+互斥闸)**(#27清9;前史git f63a060) | running | pct0.08 sides=[buy,sell] **long_conf_premium=0.35(#61c终裁rollback@08-19 15:3x,门0.90=多头事实关闸,#61族基线)** tpl567;**trailing(act1.0/cb1.2%)+BE(1.0)=基线几何(#49KEEP)**;**_exp=closed rollback(#61c;史§4#61/#63+git 0940c62)**;RECOVER刹线6h空≤−8U继承;#11降级观察(§4) |
...
| qt-fade-short-v2 | 7583727a | 冲高回落空(tpl576/S25继承) | HANA/COLLECT/**MELANIA**(feed3=种子3@08-22 18:2x #68写回落库✓;MELANIA晋升自main@15:2x[48h S+2.29 nS3 wr100%];⚠️#57-ROUTE病理活测第3例[VELVET/BULLA先例],看护=隔离线/出池线/保险线既有三层) | running | 建壳@08-21 16:5x(tpl576/S25继承;git f43756c);**FLEET评期结案=管道PASS**(verdict=config._exp.prior_fleet_verdict);**低波适配三值包@04:1x(owner直令)**: minatr0.35+adt0.5+scp0.049[#65修]=梯度防护(≥0.5全分/0.35-0.5需raw0.75/<0.35拦);**_exp=open·中间裁决@21:2x=延评一次(落空档;verdict_n8全文config._exp+git 8a797ec;终裁n≥16或08-24 12Z二元[段净>0→keep否则回滚三值]不再延;劣化线n≥8∧≤−4U+带级护栏−3U+6h保险−6U站岗)**;**进度@08-23 15:2x: 段n13/16毛−0.885净≈−1.19 wr31%(4W/9L);⚠️12:1x轮COLLECT'±0.00'实结−0.72=中轮结算瞬态教训(hunger_sl 45m准点,跨12:02重启计时无损=DB open_time锚实证);带累计4笔≈−1.05 vs −3U护栏未触;超时簇5/13最大死法;MELANIA fade era n5/−0.86出池线未触;在飞2仓(COLLECT14:45/MELANIA15:09,均重启后≤2min开→§6新观察)→结算后n15,终裁大概率08-24 12Z,当前趋势=回滚**;程序=stop[poll1]-PATCH-start✓(嵌套体二次实锤史§3+git 8a797ec);保险=6h≤−6U停(继承) |
...
| 计数项 | 读数 | 更新轮 | 备注 |
|---|---|---|---|
| 重启后速开仓观测(crash loop伴生) | **n=2@08-23: 14:44重启→63s后v2开COLLECT;15:07重启→106s后开MELANIA**;机制候选=重启清cd_sec态+回填后立即重评(LastEntryAt闸只管同币再入) | 08-23 15:2x | ≥5例且亏单≥3→议重启后静默期;n<5只计数 |
