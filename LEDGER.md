...
| 载具 | id | 原型 | 池 | 状态 | 门/备注 |
|---|---|---|---|---|---|
| main 通才 | 8eb182b6 | 通才(S18-S23栈) | auto·**feed83@08-27 04:0x(03:5x重启重播种直出83零泄漏=bl19种子免疫首证;前史17:26泄漏14热清见git)** | running | pct0.08 sides=[buy,sell] **bl19@08-26 17:1x(+VELVET ROUTE#52)** **scp=0.049@08-26(提频直令收0.600档)** **long_conf_premium=0.35(#61c rollback@08-19,门0.90=多头事实关闸)** tpl567;**trailing(act1.0/cb1.2%)+BE(1.0)=基线几何(#49KEEP)**;**_exp=open·EXP提频0.600档@08-26 17:1x(评08-27 18Z或档n≥15**[评期顺延预注册@08-27 17:3x=待落#74:eval_after+=挂死时长,metric/劣化线不变]**;劣化线档净≤−3U即滚回scp0.05;档n2@08-27 04:0x:ARIA空−0.830+TAC空00:17 conf0.6000 24m TP+0.532=档净−0.298(1W1L);#61c史git 0940c62)**;RECOVER刹线6h空≤−8U继承;#11降级观察(§4) |
...
## 3. 已确认机制
- **[新增 08-27 04:4x] 后端DB层周期性全局阻塞(平台事实,n=2)**: 签名=进程活/静态+鉴权中间件秒回/一切触DB端点无限挂(login L104首查+public/templates双证);生产DB=MySQL(deploy脚本);机制=40连接池被卡死事务/锁等待耗尽,Go池等待无超时→全站排队;重启清态但08-27实证仅续命~30min即复发=有周期性拖死源(候选:定时任务/锁堆积/DB主机磁盘,需owner侧PROCESSLIST定位)。影响面(源码): ROI/TPSL/饥饿守护tick全首行触DB=冻结,信号链冻结=无新敞口,交易所侧algo TP/SL不依赖后端=已开仓有硬保护。止血=#73驱动级超时补丁(候部署)。探测口径: /api/health/db部署后2s定判;未部署用public/templates 15s超时探针。
...
| # | 类型 | 内容（含完整意图） | 登记轮 | 状态 |
|---|---|---|---|---|
| #74 | _exp管理 | **EXP提频评期顺延(不可抗力;18Z评期到点前登记@08-27 17:3xZ)**: DB挂死04:31起信号链冻结=治疗零表达(档n=2冻结),原评期08-27 18Z既不可执行也无新证据可裁→非事后换指标,系评期钟顺延。动作=恢复后首个空仓窗 PATCH main _exp: eval_after+=挂死时长(04:31→恢复时刻,小时圆整),metric(触发线conf0.600逐笔)/劣化线(档净≤−3U即滚回scp0.05;或档n≥20且净<0)/档口径全不变;恢复时若档n≥15可即评。阻塞=平台挂死(login不可用) | 08-27 17:3x | open |
...
|---|---|---|---|
| 后端DB层挂死watch(平台事故→已升§3@08-27 04:4x) | **连续挂死中@08-27 18:16,自04:31复发起≥13h45m(n=9观测段)**: 03:14首发→03:5x重启→04:01复活~30min→04:31复发;观测段06:28+07:12×16探+09:1x+10:38+12:37+13:56+17:28+**18:15-16例行轮四层(login再证60s满挂;新观测:/healthz 200=反代层存活端点,不在backend源码,非引擎信号,判读不变)**=签名恒定(鉴权401/1.5s+路由404/0.7s=进程活;login挂=DB死;health/db 404=#73未部署);owner零动作三证@18:16(main头09f76cf不变+ed0c160未并[FF零冲突复核✓@17:2x]+TG getUpdates空,自03:5x重启后零动作~14h) | 08-27 18:2x | 判据不变:部署#73后再现→health/db秒判+readTimeout日志定位卡死query;owner待办=重启前SHOW FULL PROCESSLIST/INNODB STATUS/df -h;恢复后首轮必做=active核对+closed since04:00盲窗重建+feed漂移复核+#74落地 |
...
| 08-27 18:1x-18:4xZ(18:12Z触发,投递正常) | 例行轮(max)·**全轮阻塞:DB挂死持续(连6轮HOLD)** | 五窗不可拉(login 60s满挂再证) | FGI71 Greed/BTC79946+1.78%(冻结中仅记录) | **被迫HOLD 0原子包;四层探针18:15-16Z签名恒定(401/1.5s+404/0.7s活;login 60s满挂;health/db 404=#73未部署;/healthz 200判为反代存活端点非引擎信号);自04:31复发≥13h45m(n=9段);owner零动作三证@18:16(09f76cf不变+ed0c160未并+TG空,~14h);TG第8报=简报+阶梯重申;守望链:18:49Z(前会话,有让位协议)+新arm~20:10Z(本会话)+21:09Z例行** | 敞口≈0推理不变(最后实测04:31=0持仓+信号链冻结;盲窗开仓=入场即挂括号有界);ROUTE无从跑;#74维持open(被阻);恢复后首轮必做=active核对+closed since04:00盲窗重建+feed漂移复核+#74落地 |
| 08-27 17:2x-17:5xZ(15:15Z触发,投递延迟~2h) | 例行轮(max)·**全轮阻塞:DB挂死持续(连5轮HOLD)** | 五窗不可拉(login 60s挂) | 宏观未采(交易冻结中无操作含义) | **被迫HOLD 0原子包;四层探针17:28-29Z签名恒定(401/1.4s+404/1.4s活;login 60s上限挂;health/db 404=#73未部署);自04:31复发≥12h58m(n=8段);owner零动作三证@17:2x(09f76cf不变+ed0c160未并[FF零冲突复核✓]+TG空);#74评期顺延预注册@18Z到点前;TG第7报=三档行动阶梯(①裸重启2min②并ed0c160+部署5min③重启前PROCESSLIST/INNODB STATUS/df -h取证)** | 敞口≈0推理不变(最后实测04:31=0持仓+信号链冻结;盲窗开仓=入场即挂括号有界);ROUTE无从跑;守望re-arm~18:50Z(下轮18:09Z触发,防同延迟);恢复后首轮必做=active核对+closed since04:00盲窗重建+feed漂移复核+#74落地 |
| 08-27 12:3x-12:5xZ | 例行轮(max)·**全轮阻塞:DB挂死持续(连4轮HOLD)** | 五窗不可拉(login挂×3) | FGI71 Greed/BTC79289+1.35%/mcap−1.29%无降压(挂死中只记录) | **被迫HOLD 0原子包;四层探针12:37Z签名恒定(401/0.9s+404/1.3s活;health/db 404=#73仍未部署;templates 15s挂);自04:31复发起≥8h06m(n=6段);owner零动作三证(main头09f76cf+ed0c160未并+TG空);TG第6报=状态+部署/取证重申;守望回访re-arm~13:55Z** | 敞口≈0推理不变(信号链冻结,§3机制;盲窗若瞬时复活开仓=入场即挂括号有界);EXP提频档18Z评顺延候议(挂死窗零积累);ROUTE无从跑;待落0;恢复后首轮必做=active核对+closed since04:00盲窗重建+feed漂移复核 |
| 08-27 09:1x-09:3xZ | 例行轮(max)·**全轮阻塞:DB挂死持续(连3轮HOLD)** | 五窗不可拉(login挂) | 宏观未采(专注事故) | **被迫HOLD 0原子包;四层探针09:17Z签名不变(401/0.9s+404/1.3s活;login 30s挂;health/db 404);自04:31复发起连续≥4h46m;#73双证未部署(ed0c160已base main可fast-forward);main头09f76cf实为08-25预事故提交(时区换算纠偏,owner自03:5x重启后无新动作);TG第5报=状态令+部署/取证重申** | 敞口≈0推理不变(04:31实测0持仓+挂死中信号链冻结,§3机制);残余不确定窗04:37-06:28+07:12-09:14未观测→恢复后closed since04:00盲窗重建仍首务;EXP提频档18Z评顺延候议;ROUTE无从跑;待落0;下轮优先=恢复验证 |
| 08-27 03:1x-04:4xZ | 例行轮(max)·**事故+恢复+复发轮** | income:24h净−0.14(rows n9 wr55.6 毛均−0.053组🔴) | FGI71 Greed无降压 | **HOLD 0原子包;DB挂死03:14发现→TG🆘→03:5x重启→04:01复活(重试28次/47min)→04:31复发n=2→watch升§3+#73补丁push候部署(TG7749/7750);ROUTE#56零差分(连4)** | 三查全过;bl19种子写回重启免疫首证;DB=MySQL定案;EXP提频档n1→n2(TAC空TP+0.532,档净−0.298,评18Z);磁铁era n2/−2.83无新;钱包168.93;复发窗敞口≈0;**全文git 9b9c39d/401efb2** |
| 08-27 06:2x-07:1xZ | 例行轮(max)·**全轮阻塞:DB挂死持续** | 五窗不可拉(login/income全挂) | FGI71 Greed/BTC78638−0.3%/mcap−2.7%无降压 | **被迫HOLD 0原子包;探针06:28四层=签名不变(鉴权401/1.4s活+静态404/0.4s;templates 15s挂;health/db 404=#73未部署,git双证未并main);守望16探/45min(06:30-07:12,150s间隔)全挂=连续挂死,自04:31复发起≥2h41m;mm做市线排除(marketmaker包零DB引用);TG7751升级令(部署#73+PROCESSLIST取证重申)** | 敞口≈0推理:挂死中信号链冻结=无新仓+存量仓交易所侧TP/SL独立护+最后实测04:31=0持仓;残余不确定窗04:37-06:28未观测(若瞬时复活开仓,入场即挂括号=有界);EXP提频档评期18Z顺延候议(挂死窗零积累);ROUTE本轮无从跑(需closed48);待落0;下轮优先=恢复验证(active+closed since04:00盲窗重建) |
| 08-27 00:1x-00:4xZ | 例行轮(max) | 24h净−0.79(rows n12 wr50.0/be54.1=−4.1pp;毛均−0.086组🔴连2) | FGI71 Greed | **HOLD;ROUTE#55零差分(连3);磁铁era n1→n2(ONG多22:56 conf0.95 4m SL毛−1.442,era累−2.83距门一步);围栏实证S22×3拦+CB+EMA解体;S30设计定稿挂§6** | stub@08-27 07:1x;全文git 9b9c39d(死法TP6+5.77/SL6−6.80;载具拆分main−1.54[ex-ONG+1.29]/v2+0.52/trend零;钱包168.41) |
