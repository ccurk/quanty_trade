...
| 载具 | id | 原型 | 池 | 状态 | 门/备注 |
|---|---|---|---|---|---|
| main 通才 | 8eb182b6 | 通才(S18-S23栈) | auto·**feed91@08-24 09:2x(crash loop 9重启05:58-08:32Z各次重播种100→ROUTE#34 purge9→91,18bl币零残留✓;#33/#29史git b505652/f171d49)** | running | pct0.08 sides=[buy,sell] **long_conf_premium=0.35(#61c终裁rollback@08-19 15:3x,门0.90=多头事实关闸,#61族基线)** tpl567;**trailing(act1.0/cb1.2%)+BE(1.0)=基线几何(#49KEEP)**;**_exp=closed rollback(#61c;史§4#61/#63+git 0940c62)**;RECOVER刹线6h空≤−8U继承;#11降级观察(§4) |
| qt-trend-long | 827ffe8c | 趋势动量多 | **COTI/TUT**(种子2=feed2✓#26重启后重建正确;ACE隔离@08-22 12:22;aging=COTI(24h−0.78/48h n14净+0.29未触出池线@08-24 09:2x);破荒史git f43756c) | running | **tpl575(S24)** pct0.05 mcp3 buy trailing on;**_exp=open·#69三值包重申请@08-23 09:18Z**(adt0.5+lcp0.149+mces off;终裁二元=新段净>0→keep否则回滚[adt1.0/lcp0.15/mces3];eval=n≥16或08-25 09:18Z;劣化线n≥8∧≤−4U;**进度@08-24 09:2x段n6净+1.23由负转正**(新3笔全W:TUT 06:28+0.46快TP4m/TUT 06:55+1.47 mv+6.2%7m trailing骑赢/COTI 08:39超时+0.045;劣化线远离;feed2在流);否决窗空置+程序史git 492779a);原三值段终裁rollback(n18净−0.40)+ex-ACE归因(n14净+4.11 wr86%)全文git 5df52d2+config史;**常驻保险=滚动6h净≤−5U→stop(继承)**;三值包全史git 8a1d995/f71d825;S24 keep史git f43756c |
| qt-fade-short-v2 | 7583727a | 冲高回落空(**tpl823/S28**;谱系tpl576/S25) | HANA/COLLECT/MELANIA(feed3=种子3✓;MELANIA晋升@08-22;#57-ROUTE病理活测第3例看护) | running | **S28衰竭签名直入上线@08-23 21:24Z**(#66修复:short∧RSI>75∧价≥布林上轨∧末bar涨<0.3×ATR%仅旁路EMA方向+趋稳一门,S19/S20/RSI<32/量能/ATR带/追空ext全保留;附随#65 epsilon永久化[ss≥thr−1e-9;0.60精确组合RSI0.30+BB0.15+MACD0.15=S28主通道解锁];apply字节复检✓零config泄漏✓feed3✓烟雾MELANIA18h task30;载具级重启实证→§6);**_exp=open·EXP:S28**(二元:段净[Σpnl−名义×0.1%]>0→keep否则rollback回tpl576;劣化线段n≥8∧≤−4U即回;评期段n≥16或08-25 21Z;崩溃立即回滚;**进度@08-24 09:2x段n0**@11.9h(零发单;09:14 COLLECT ss=0.560顶=0.60组合−0.04低波折扣,MELANIA RSI83.3但布林中部=S28签名未现·评估行正常管道健康;FGI73市顶稀少;若eval 08-25 21Z仍n0→rollback tpl576前查ss分布[零发单=市况非代码]));低波三值包终裁ROLLBACK@08-23 18:2x(低波解锁不付费=组级知识;全文git a2e845f+config史);保险=6h净≤−6U停(继承) |
...
下一个新锚点编号：**S29**(S23-S26/S28已用;S27预留#47突破复活版)。⚠️锚点自S24起分fork谱系: main=S4..S23;trend=+S24;fade-v2=+S25+S28;breakout=+S26(其均值回归核档案化)。apply前grep按该载具fork谱系核验。原注记:(历史上 S10 曾存在于注释引述，编号不复用)。
...
| 65 | 代码缺陷(组共病;v2/trend活性) | **信号门浮点边界:score与门槛精确等值被静默丢弃**——v2/main short_thr=0.55+0.05=0.6000000000000001>基础空头组合0.15+0.15+0.30=0.6(差2.2e-16);trend long_thr=0.55+0.15=0.7000000000000001>0.70精确组合(如MACD+多空比+EMA+单0.10加分);main长侧反向安全(组合0.9000000000000001>门0.55+0.35=0.9)不适用;main短侧同边界但**现阶段意外保护**(丢的是最弱0.60裸空,main空侧12h仍弥散亏)=不修 | 活体证据@08-22:v2 COLLECT 02:00-05六行 空头分=0.600 方向=None 无折扣(ATR1.01-1.09=今晚唯一过地板窗);实现损失0(RSI20-28<32超卖闸在后兜底);机制=python浮点算术直证(worktree验算) | n=6行/1夜 | **修复executed@08-22 04:1x(config路径)**:v2 scp0.049+trend lcp0.149(epsilon代码级方案留作下次apply窗永久化候选);main两侧均不修(短侧意外保护+长侧0.90反向安全);**main侧频率量化@04:3x=23行/21min**(owner'先这样'批准不修维持);**epsilon代码级永久化executed@08-23 21:24(v2域,随S28 apply:ss≥thr−1e-9;三值回滚曾使scp回0.05=边界复活,现代码层免疫)**;trend域仍靠lcp0.149 config;main两侧维持不修 |
| 66 | 代码候选(v2原型正确性;批2) | **S28-fade衰竭签名直入重写**:现继承逻辑要求EMA翻空+趋稳3bar才许开空→"冲高回落"载具永远不能在顶点开空,等确认完ATR尖峰已衰减回地板下;重写=衰竭签名入场(RSI>75+涨势失速[末bar涨幅<0.3×ATR]+布林上轨触碰,不等EMA),SL仍2.5×ATR+S22/RSI<32/保险线全保留 | 双标本同构(v2 COLLECT零发单链git f43756c+main MELANIA 04:04 EMA闸拒;7月反向血账−16.7→顶点确认要件;〔截断,全文git e49b3b7系〕) | n=2载具标本 | **executed@08-23 21:24Z=tpl823 apply✓**(排程条件全达:低波_exp终裁18:2x完成腾槽+带级知识[0.5地板keep,S28不动地板]参数化入设计;标本③MELANIA 15:24同型;评判随§1.5 v2行_exp二元;设计全文tpl823.py头注+git 7d6a27f) |
...
| 计数项 | 读数 | 更新轮 | 备注 |
|---|---|---|---|
| main断流观测(三闸交集关门) | **断流结束@08-23 18:24**(UAI空开仓,总时长23.0h[BTR 08-22 19:20→UAI 18:24];该笔60m超时−0.56;三闸交集设计性关门定性维持,18:1x首登全文git a2e845f);本轮main笔数=2+1零盈亏(TAC+1.13饥饿W/UAI−0.56超时/SCRT 0.00 BE) | 08-24 03:2x | 判据不变:再现6h+零笔且池ATR中位≥0.5→查管道;每轮记main笔数 |
| rotate has_open_position判定条件观测 | **n=2矛盾@08-23**: 15:16 COLLECT/MELANIA移除被拒(v2在押,当时判"账户级");18:3x TUT移除放行(trend在押,同为他载具仓)→"账户级"说法出反例;候选机制=按position.strategy_id比对被rotate载具?或时序/缓存 | 08-23 18:3x | 判据:再积2例分型(他载具仓被拒vs放行)→读strategy_symbols.go裁决;执行纪律不变=拒了下轮重试 |
| apply重启作用域观测 | **n=1@08-23 21:24**: apply@v2(tpl576→823)后main feed89/trend feed2均不变+main IDLE top计数连续=载具级重启实证(prompt"疑全局级"首反证;08-09全局重播种例疑属部署级路径) | 08-23 21:1x | 判据:再积1例同向(下次任意apply后查他载具feed)→定案入§3;反例(他载具feed跳回种子全集)即回全局级定性 |
| 重启后速开仓观测(crash loop伴生) | **n=4@08-24**(+TUT开06:24:30≈重启06:24窗内+0.46W;+COTI开07:39:30=07:38重启后~60-90s+0.045W;史14:44→63s/15:07→106s;4例亏单0);副证=SCRT仓07:33开跨07:36-38重启07:44平+0.43=持仓管理跨重启韧性;机制候选=重启清cd_sec态+回填后立即重评 | 08-24 09:2x | ≥5例且亏单≥3→议重启后静默期;n<5只计数 |
| closed平仓行消失观测(≤48h短窗) | 瞬时缺行首例反转@08-23(2行重现=拉取瞬态,史git 8a1d995);**新分型@08-24 03:2x零盈亏行窗口不对称(SCRT案)**: 01:43短@0.01744→01:54同价平,毛=0→Binance不生成零值REALIZED_PNL income→行无realized_pnl字段(貌似幻影行实为真实成交,佣金−0.0262在income by_symbol实证);48h窗持续含该行(无pnl字段),4h窗两拉均不含=系统性非瞬态 | 08-24 03:2x | 判据:瞬时缺行复现≥3例→读源码找竞态;零盈亏亚型再现1例→升§3定案;滤行纪律维持(毛=0不失真)但其佣金只在income侧,费覆计算记得含 |
...
| 08-24 09:1x-09:3xZ | 例行轮(max) | 1h+0.04(1)/3h+2.40(4)/6h+2.40(4)/12h+2.77(6,wr83/be52=+31pp)/24h净+1.52(13,毛+1.77佣−0.35资+0.10,wr62/be47=+15pp) | trend大转:TUT 06:28+0.46/06:55+1.47(mv+6.2%,7m trailing)+COTI+0.045→#69段n6净+1.23由负转正;v2段n0@11.9h(ss顶0.560,MELANIA RSI83布林中部=签名未现);main SCRT+0.43跨重启仓存活 | crash loop爆发:05:58-08:32Z 9重启(最短6min)后43min静,TG续报;§6速开仓n2→4(新2例均W) | ROUTE#34 purge9→feed91零残留 | **费覆首绿:24h毛均+0.137≥3×费0.077**(12h毛均+0.462);充值信号三条件全过(12/24h双正+12h wr−be+31pp)→TG建议充值250-300U | 0原子包HOLD(两_exp在飞未到门,队列空,刹车全绿) | 钱包170.7 |
| 08-24 06:2x-06:4xZ | 例行轮(max) | 1h0/3h0/6h净−0.81(1,trend COTI超时)/12h净−0.32(4)/24h毛−1.42净−1.63(income权威20笔wr50/be66🔴;closed48仅10行=窗掉行,聚合信income) | FGI73/BTC+1.3%/市值−0.7%无降压 | **HOLD 0原子包**;ROUTE=#33 plan.main.remove8(post-restart 05:58/06:24Z重播种feed100→purge92:隔离5 ACE/BEAT/BTW/H/龙虾+专家3 COLLECT/MELANIA/TUT;churn3;零差分连3被restart drift打断;bl_sync18=desired18无drift;aging trend.COTI+fade.COLLECT/HANA无新隔离);两_exp未到门[trend#69段n3净−0.90(COTI−0.06/−0.76超时+TUT+0.08;劣化n≥8∧≤−4U未触;eval n≥16或08-25 09:18Z)·v2 S28段n0@9h(ss顶0.560<0.60门·0 Traceback·eval 08-25 21Z)];刹车全绿(6h−0.81/24h−1.63 vs 8%12.5U/20%31.4U@钱包157)载具保险绿(trend6h−0.81>−5·fade6h0>−6·main空0>−8);费覆🔴净均−0.082 | **crash loop再燃**:16:05Z后延至05:58Z共13.9h零重启(top1033)→05:58+06:24Z连2重启(26min间隔=回08-23峰频);冷却证伪升级观察TG续报owner;24h死法慢衰亡(快SL0/15-60m 7笔−1.53/>60m 3笔−0.64超时);最大失血COTI 24h−1.62(trend,aging_watch);main开TUT多现浮盈+0.65骑赢家(trailing on);income窗含TRANSFER−50U=**08-23 18:1x owner提款(已录非新,24h窗内故复现)**充值信号moot;钱包156.8(=18:1x后稳态,03:4x报168.38疑total口径异常一次性);audit零无主;trending∩pool=TUT/MELANIA/TRUMP(brk锁) |
...
| 08-23 18:1x-18:4xZ | 例行轮(max) | 24h毛−0.98净−1.24(20,wr50/be57) | FGI66 | **v2三值包终裁=ROLLBACK(n16段净≈−1.00 wr38;低波解锁不付费=组级知识);ROUTE=#29热清11/11 feed100→89;⚠️owner TRANSFER−50U出金→钱包156.86** | stub;全文git a2e845f |
| 08-23 15:1x-15:4xZ | 例行轮(max) | 24h毛−2.75净−2.88(21,wr48/be63) | FGI66 | **HOLD 0原子包;ROUTE=#28热清10/12;crash loop法医学升级33h15次报owner** | stub;全文git a2e845f |
| 08-23 12:1x-12:4xZ | 例行轮(max) | 24h净+1.48(13,wr54/be48) | FGI66 | **HOLD 0原子包;ROUTE=#27热清9;重启2.7-4.8h疑crash loop报owner(15:1x轮法医学修正:真频率高5倍);COLLECT带级捕获±0.00(15:1x轮实结−0.72)** | stub;全文git f63a060 |
| 08-23 09:1x-09:3xZ | 例行轮(max) | 24h净+0.93(31,wr61/be57) | FGI66 | **#69执行=trend三值包重申请(1原子包;新段09:18Z起)+ROUTE=#26热清13/13** | stub;全文git 492779a |
| 08-23 06:1x-06:4xZ | 例行轮(max) | 24h毛+3.19净+1.67(51,wr63/be59) | FGI66 | **trend原三值段终裁=ROLLBACK(n18段净−0.40;ex-ACE n14+4.11→#69候选)+ROUTE=#25热清13/13** | stub;全文git 492779a |
|---|---|---|---|---|---|
