...
| main 通才(平台名 Meme_合约信号计算引擎_1) | 8eb182b6 | 通才(S4..S23+S30+S31+S33;S32关guard) | auto·feed184@09-18 10:2x(rotate remove BR+哈基米;隔离∩feed=∅复核✓) | running | **09-18 10:2xZ RECOVER(owner直令 lev10 在位): sides=[buy,sell] mcp10 lev10 pct0.05(DS锁至12:37Z→CC-8 12:40Z 升0.075) tp/sl pct=0(ATR口径 2.5/4.5×ATR;Go clamp SL≤0.3/lev=3%价) hunger45m 40%/12.5%ROI cs_mult[0.6,1.0] max_hold240(DS watch) trailing1.0/1.2% BE1.0 cd180/min_conf0.35(=进程值) bl24 feed184; 新增 sl_ratio0.03/tp_ratio0.06(重启兜底,Go零引用); leverage留frozen_keys(=DS勿动)** _exp_cc=P3 lev10 in-flight(eval 14:10Z或n≥30;首读n20 +4.27) carry cc8(pct0.075 scheduled 12:40Z) prev_verdict=P1/P2 KEEP 3/3@10:2x; 刹车链史=git e94a8b0版行; 09-18 08:2x 态=git 5d2c77a版行 |
...
- 保证金: Σ_running=main **0.05×10=0.50**@09-18 10:2x(CC-8 12:40Z→0.075×10=0.75顶格;史0.15×10=1.5越界87min@09-16 14:30-15:57 admin)
...
| DS-10 | owner确认 | 15:16:07Z admin PATCH max_hold_minutes 60→240(无_exp,DS节拍外)是DS还是owner手改;watch_ds线: 段open≥15:16Z hold>60m子集n≥8∧子集净≤−2U 或 段净≤−4U→回60;eval 09-18 15:16Z或子集n≥8先到;**读数@10:2x: 段n48 +3.37;子集n7 +1.22(AVA+0.83/AIN−0.57/LSK−0.50/AIN−0.25/VTHO+0.85/BR+0.27/LSK+0.58);24h硬超时类n7/+0.70→线未触,预读KEEP 240;14:15Z 自查点复核** | 09-17 | open |
...
| CC-8 | 定时(12:40Z send_later 已排 trig_01VGxuQ3) | DS breaker锁到期(12:37:15Z)→order_amount_pct 0.05→0.075(顶格0.075×10;owner 09-14/09-18令);**预注册已写 _exp_cc.carry.cc8_pct_0075@10:2x**(expect 均净≥+0.10∧6h>−8%∧笔/日≥38.7;worsen 段净≤−6U或6h≤−8%→回0.05;eval 执行+6h或n≥30);DS续锁→按owner令仍升+cc_note+TG;门=6h钱包净>−8% | 09-18 08:1x | open(scheduled) |
...
|---|---|---|---|
| **09-18 P3 lev10 段首读(RECOVER)** | @10:2x 段(open≥08:12:09Z) n20 226笔/日 净+4.27 均+0.213 wr50% 赢均+0.766 亏均−0.339; 死法: 交易所SL 3/−2.11(COTI/牛来/哈基米;SL距1.46%中位) 饥饿1/−0.76(T 45m) guard_sl 1/−0.33 其它亏5/−0.18 ‖ TP 2/+4.30(AKE +2.96 4m/CROSS +1.33) trail/早赢 7/+3.03 sl_crossed 1/+0.33; 名义中位53.5U; 多n11/+3.61 空n9/+0.66; 6h钱包净+4.39(+3.9%)/24h −2.44(−2.2%); 费覆 P3🟢(0.213 vs 0.158) 24h🔴(0.027 vs 0.092); 24h tape n66 wr58% be55%(+3pp) 12h n40 wr57% be45%(+12pp)。史: 09-17 刹车基线/post-BRAKE/禁多段/FIX段首读=git 5d2c77a版§6 | 09-18 10:2x | 终裁@14:15Z自查点(eval 14:10Z或n≥30) |
