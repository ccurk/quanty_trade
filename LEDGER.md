...
### 双执行体(09-17 起)
- **[新增 09-19 06:3x] REST 轮询地图(源码)**: strategy_position.go:896 每持仓 2s ticker→USDMPositionInfo(/fapi/v2/positionRisk 权重 5)=150 权重/分/仓(mcp6 满仓 900/分)+manager.go:980 每 owner 2s FetchPositions(150/分)+tpsl_monitor 15s+quick_trade 10s+roi_monitor 5s/1m+开仓/撤单/tpsl 设置; IP 上限 2400/分→429 伤出场(OP 饥饿平仓失败/ROBO 平仓失败/守护整轮跳过/开仓 tpsl_setup_failed 回滚)。**补丁 claude/dev-rest-budget 61631b0**: 仅 runPositionTPStopMonitor 的 REST 判定降到 10s(150→30/分/仓), K 线判定仍 2s; go build/vet 过; 部署权 owner。
- **[新增 09-19 06:3x] 本地 TP/SL 监控陈旧价假触发(源码+实证)**: lastCandleClose 只在新 1m K 线首 tick 更新(dataflow.go:441 时间戳不 After 即丢)=最多陈旧 59s; 入场前一分钟急跌后开空, 陈旧收盘价≥sl→"本地止盈止损触发 reason=sl"(ROBO 05:58:35 成交→05:58:38 触发, 429 挡下, 06:02 TP +5.52); 日志史 n=1(01:17 另一条 tp 为正常); 同补丁加 seenAt>startedAt 免疫。
...
| CC-11 | 巡检 | pct 配置下每轮报: 保证金占用峰值/最大单仓名义/最大单笔SL亏/6h,24h钱包%;DS breaker 若砍pct→按owner令处置并TG(裸改簿) | 09-18 13:1x | open(读数史 git df539c9版; 最新@06:3x: 最大单仓名义 479U(pct0.25), 最大单亏 OP −5.48, 6h ≈−16%/24h ≈−31%(DB 逐笔+费估)) |
...
| CC-9 | DS越界簿 | admin mcp 未声明改动=协议7越界 #1~#5(09-16 22:10/09-17 13:15/09-18 06:30/09-18 15:22/09-18 21:22, 全文 git df539c9版)+#6 09-19 03:22 pct→0.0625; 裁决=不硬抢(RECOVER 门关/mcp6 不绑定/乒乓), owner 04:49 自行解锁 pct0.25(=A 路径), mcp 仍 6; owner 令恢复即恢复+TG | 09-18 08:1x | open(mcp 6→10 候 owner; breaker 阈值改钱包比例候 owner) **@06:3x 预期 07:09 breaker 再砍→不硬抢只 TG; owner 13:4x"按 owner 值恢复"与 CC-9 不硬抢裁决冲突→候 owner 一字令(恢复即恢复)** |
...
| CC-21 | 评判 | **p9 ATR 括号**(_exp_cc 03:41:02Z; 重启 03:41:14Z 首单起): atr_sl 2.0×/atr_tp 2.5×/45m, tp,sl pct 0, hunger 0.30/0.30 兜底, trailing 关, BE 关, max_atr_pct 6; n≥40 或 12:00Z: 费后均净≥0 ∧ wr≥42% ∧ TP 命中≥35% ∧ 首5分钟SL占比<20%; worsen(只报警) n≥20∧费后≤−8U 或 首5分钟SL≥40%→备选 2×/3× 或 2.5×/2.5×; 回滚集 _exp_cc.rollback | 09-19 03:4x | open(自查点 2/3 @04:36Z 读数=git df539c9版; **@05:5x 段被 04:53 hunger_sl 0.04 污染(SL 腿失效): p9a(03:41→04:53) n11 −5.30 wr18% 空 8/9 触 2×ATR SL(现货 MAE −3~−11%); p9b(04:53→) n6 −12.25 全空 5/6<5m 饥饿SL 名义 375U; 反事实括号 −16.2 vs 实际 −17.5; 12:00Z 评判改为分段报读, 不裁 p9 本体; 06:38Z 自查点 3/3 在旧会话仍会跑(只报)** **@05:5x(并行会话) 1h 读: p9b n5 −10.37 名义中位 325U 饥饿触发 roi −5.3/−6.0/−6.1/−7.0/−10%(过冲 1.3~6pp); 首5分钟亏 7/16=44%≥40% 但 n16<20 不触 worsen; 12:00Z 分段报读不变** **@06:3x n20 费后 −19.07 触 worsen 线(n≥20∧≤−8U)→只报警(owner 13:4x); 归因 p9b 7/9 死于饥饿 0.04 刀非括号本体, 备选括号在 0.04 在位时无意义; 裁决仍等 CC-24 ①; 12:00Z 分段报读不变** **@06:4x 自查点 3/3(旧会话,只报): 03:41→06:39 DB n21 毛 −16.69 费后 −21.51 wr19% 空 19/21(空 wr16% 净 −23.8); 出场日志 饥饿 12/本地SL 1/超时 2=04:53 起 0.4% 刀再成主刀, ATR 括号未在工作; 分段 p9a(03:41→04:53 ATR括号+hunger0.30): n=9 毛-3.69 费1.20 费后-4.89 wr22% 空7/9 空净-6.23 最大亏-2.17 ‖ p9b(04:53→ hunger0.04 主刀): n=12 毛-13.00 费3.62 费后-16.62 wr17% 空12/12 空净-13.00 最大亏-5.48; worsen 线(n≥20∧费后≤−8U) 触=只报警, 归因 04:53 hunger_sl 0.04(admin/owner)+空头崩, 非括号本体; 6h income 净 −38.1 24h −83.9(费 −53.8); DS 04:53 后 0 PATCH; feed 272 ∩=∅; 自查点用尽, 交下一定时轮** |
...
| CC-23 | 管道(owner/M) | **REST 限频伤及避险动作**: 05:10:39 OP 饥饿平仓 429 失败(触发 −3.74→实平 −5.48), 05:10:49 TP/SL 守护整轮跳过, 01:11:37 ORDER 开仓因 -1003 回滚; 03:41 后 429 行 12(150 行窗); 病根=276 币 REST kline 回退+持仓监控叠加 2400 req/min; 选项 ①owner: select_limit 300→150(启动键) ②M: 后端 kline 回退限速/合并(claude/dev-rest-budget 候写) | 09-19 05:5x | open(候 owner 一句话) **@06:3x 轮询地图见 §3(每仓 2s positionRisk=主因); 补丁 claude/dev-rest-budget 61631b0 候 owner 部署(与 select_limit 300→150 可并行)** |
...
## 7. 运行日志（每轮一行，新行追加在表首）
| 2026-09-19 06:1x-06:4x | 定时轮(06:15 触发): 管道活(REST 200; 429 仍见: 05:58 ROBO 平仓失败/06:16 SWARMS TPSL 查询失败);钱包≈177(avail 131.6+SWARMS 保证金 43);DB 逐笔: 1h n4 +0.65 / 6h n63 毛 −20.6 费≈12.4 净≈−33(≈−16%) 报警 / 24h n253 毛 −20~−25 费≈53 净≈−78(≈−31%)🆘(owner 令不缩表, 只报);**p9 n20 费后 −19.1 触 worsen(n≥20∧≤−8U)→只报警**; p9b(04:53 后) n9 −9.72 全空: 饥饿 0.04 刀 7/9(−17.8) vs TP 2/9(+8.06);新证: 03:41 后空头 conf<0.45 n10 +1.10 vs ≥0.45 n9 −18.30(置信度反向), S31 risk_off 自≈03:40 起挡多头 <0.45 桶(仅 0.55 n2 过);有效门槛 0.36✓;本地监控陈旧 K 线价假触发 n=1(ROBO 05:58:38, 429 挡下);隔离规则 0 命中(M −9.05/PROM −7.57 均 n3) ∩=∅ feed272 bl39;宏观 FGI71 BTC +4.4% 市值 +1.3% trending∩池 5(ZEC/PONS/AR/TAO/HYPE);执行: 0 config; **M 补丁 claude/dev-rest-budget 61631b0**(positionRisk 轮询 2s→10s + 陈旧价免疫, 候 owner 部署);留言板 06:27Z ‖ DS: 05:47 后 0 PATCH, frozen [], _ai_task_ds 空;预期 07:09 breaker 再砍 pct(24h≪−4U)→不硬抢只 TG;下轮=07:1x 对账 DS 节拍+p9 分段读+CC-24 候 owner |
...
| 2026-09-19 05:1x-05:5x | 全文=git 4c56565版(owner 解锁后首轮+两条 live 直令: REST 限频伤避险 CC-23; 6h −14.7%; 空 164 腿 −31.5 wr28%; ROUTE AKE/MYX/POWER/EVAA→bl39; owner 裁决包 CC-24; 桥接未运行 DS-3)
