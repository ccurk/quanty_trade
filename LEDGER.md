...
| main 通才(平台名 Meme_合约信号计算引擎_1) | 8eb182b6 | 通才(S4..S23+S30+S31+S33;S32关guard;tpl998 hash 3e311e8b) | **owner 11:06Z 重置: symbols ""+max_price 1e12 ⇒ filter 路径 300 池(select_limit 300)**; feed 265(CC 11:4x rotate remove 35: bl∩feed=∅, ∩majors=∅) | running(pid 58521 START 11:08:43Z) | **09-19 13:1xZ 复核同值(lev5/pct0.125/mcp3/tp,sl0/max_hold45/bl45/feed265, 13:26 _exp_cc 更新复读✓); 11:4x 现值: 注入 min_conf0.35/cd180/atr 2.0/2.5/warmup50; pct0.125(DS 07:08, owner 值 0.25, lock-1789810634 至 09-20 09:37Z) lev5(DS 10:09 裸改, owner 值 10, watch_ds) mcp3(DS 09:37 未声明, owner 令 10; 11:16 起绑定) sides 双向 hunger OFF(owner 11:06) tp,sl pct0(交易所腿=信号 ATR 括号, 钳 0.3/lev) trailing/BE 关 max_hold45 sl_ratio0.03/tp_ratio0.06 bl45 conf_sizing 0.6-1.0 auto_optimize enabled=false(apply=true/dry_run=false 上膛)**; 7 币段(07:21→11:08) 终读 n12 +17.34 wr50% |
| majors(平台名 Majors_BTC_ETH_BNB_SOL) | e725e31a | main fork → **tpl1056 = tpl998 + S35 bar聚合 15m**(CC apply 09-19 08:39:38Z, hash 766b576a; 11:08 owner 重启后模板保持) | manual BTC/ETH/BNB/SOL/XRP(bl0; main 侧已 bl+rotate remove SOL/XRP 防撞) | **running@08:01:47Z(owner UI start); 11:08:41Z owner 重启(mcp 5→2 admin 11:06)** | 11:4x 现值: lev10 pct0.125 mcp2 cd900 max_hold720 hunger off atr 2.0/2.5 min_atr 0.03 reentry 15m bar_agg 15 sl_ratio0.03 auto_optimize dry_run=true; 1m 段(08:03-08:20) 3 空 SL −0.78; **S35 段: SOL/BNB/BTC 多 08:46/09:01/10:16 开, 11:08 重启时 'manual' 路径平仓 −0.63/−0.15/+0.03(段 n3 −0.75, 未经 TP/SL 检验); XRP 多 11:16→11:51 TP +0.88(SL 距 0.84%); BNB 多 12:46→12:55 TP +0.49(SL 距 0.44%); S35 有效段 n2 +1.37, SL 距中位 0.67%≥0.3✓; 腿 8/8✓; 12:16 SOL '保证金≤0' 丢单(429 族)**; CC-28 eval 09-20 08:40Z/n≥15 |
...
### 双执行体(09-17 起)
- **[新增 09-19 13:1x] DS 改动簿 #10 + lev5 watch 终裁(不执行)**: 11:06 后 DS 0 PATCH(main/majors; 13:09 节拍静默=6h income 转正 +13.1 未触熔断; lock-1789810634 冻 pct 至 09-20 09:37; _ai_task_ds 仍 null). lev5 watch 线(post 段 open≥10:09 n12 毛 −2.69 费后≈−0.30/笔 < pre +1.43)**形式上破**, 但 ①post 段被 owner 11:06 重置(7 币→300 池+hunger off)污染: 同池 lev5 样本仅 n2(DRIFT/G 皆饥饿刀), 归因 lev5 证据不足 ②回 lev10=升敞口: RECOVER 门关(24h −99.5)+v5.1 事前闸; 机制: 钳 SL≤0.3/lev=3%(300 段 3/10 括号 3.8-4.3% 会被剪)+单笔 SL 亏翻倍(B2 −3.13→≈−6.3 越 owner 3-5U 帽)+费翻倍 ⇒ 不执行, 候 owner 一字令(10/认可 5/阶梯 5→8→10 每档以 300 段费后均净≥0 为门); 全文=config _exp_cc.watch_ds.leverage@13:26Z. **源码复核: strategy_execution.go:277-282 hasEffectiveTPSL/:305-307 resolveTPSLFromROI 与 §3 10:3x 条一致(tp/sl pct=0 ⇒ 信号 tp/sl 原样, 腿照挂), 仓库=部署行为(日志 10/10 腿)**.
- **[09-19 11:4x] owner 11:06Z 批签名 + symbols "" 语义 + 跨实例槽位干扰(源码)**: 要点=11:06:38Z admin 三 PATCH(main hunger off/symbols ""/majors mcp2)+双壳 11:08 重启 归属 owner(候认领 CC-30); strategy_start.go:224-241 symbols 空∧max_price>0 ⇒ filter 路径 300 池; manager.go:287-313 isAllowedSymbol symbols 空 ⇒ 恒 true → strategy_signal.go:611-627 把 majors 仓计入 main openCount('已持仓4个≥3' 11:19/12:46 再现); DB 同仓双写去重键=symbol+direction+close 分钟+pnl 2dp; 429→tpsl 回滚也 429→裸奔 28s 后守护补设自愈(ZAMA 11:11); 全文=git 7e0e2fe 版§3。
- **[新增 09-19 10:3x] tp/sl pct=0 ≠ 裸奔(源码+日志实证; 纠正 prompt v5.1 协同协议 11①/硬边界 11/引擎速查的前提)**: strategy_execution.go resolveTPSLFromROI :305-307 当 take_profit_pct/stop_loss_pct 均 ≤0 时**原样返回信号自带 tp/sl**(非 0/0); hasEffectiveTPSL :278-283 信号 tp>0∧sl>0 即 true → 交易所 algo 腿照挂. 实证: main 07:21 后 7/7 开仓均有 '已设置止盈止损 … sl order_id'(SL 距 1.28-3.0% 价=2×ATR, 钳 0.3/lev), majors 08:46/09:01/10:16 同. 真裸奔条件=信号 tp/sl 也为 0(Python 兜底 sl_ratio0.03/tp_ratio0.06>0 在位即不可能, 见 09-18 陷阱条). 5.5 核对口径改为: 每 running 实例最近开仓有 '已设置止盈止损' 行 ∧ (sl_ratio,tp_ratio>0 ∨ pct>0) ∧ max_hold>0. **@12:4x 复核: main 11:08 后 5/5 腿✓, majors XRP 腿✓; 反例=ZAMA 11:11 挂腿 -1003 → 事务回滚查仓/撤主单亦 429 失败 → 仓留交易所无腿 28s 至守护补设(唯一裸奔路径=429 级联, 非 tp/sl=0; CC-23)**
...
| CC-9 | DS越界簿 | admin mcp 未声明改动=协议7越界 #1~#5(09-16 22:10/09-17 13:15/09-18 06:30/09-18 15:22/09-18 21:22, 全文 git df539c9版)+#6 09-19 03:22 pct→0.0625; 裁决=不硬抢(RECOVER 门关/mcp6 不绑定/乒乓), owner 04:49 自行解锁 pct0.25(=A 路径), mcp 仍 6; owner 令恢复即恢复+TG | 09-18 08:1x | open(mcp 6→10 候 owner; breaker 阈值改钱包比例候 owner; 读数史 06:3x/07:3x/08:4x=git 2d9c837版; **@09:2x 读数/预警=git 7e0e2fe 版**) **@10:3x #7: 09:37 breaker mcp 6→3 未声明 + 10:09 optimizer lev 10→5 裸改; 均不硬抢, watch_ds 登记(lev 劣化线 post 段 n≥10, eval 18:00Z); owner 一字令(恢复 lev10/mcp10 or 认可 DS 值)仍候** **@13:1x lev5 watch 终裁=线破不执行(§3 13:1x), 候 owner 一字令(lev 10/5/阶梯 5→8); mcp3 跳过 8 次/50min 含跨实例 '4个'** **@11:4x: mcp3 绑定实证(11:16/11:19/11:23/11:30 跳过 G/COTI/B2/ZAMA; majors XRP 计入 main 计数 manager.go:312); 仍不硬抢——理由=两壳共用 IP 配额, 5 仓已触 429→tpsl_setup_failed 裸奔窗; TG owner 三选一: 留 3 至 REST 预算补丁部署 / 恢复 10 / symbols 显式列表重启** **@12:4x: 11:33 后 DS 0 PATCH; 13:09 节拍预警(6h −13.1≤−4U)→只 TG 不硬抢** |
...
| CC-23 | 管道(owner/M) | 全文=git 6b44db7版§5(要点不变) | 09-19 05:5x | open(候 owner; **@13:1x 429 12:24 后仅 1 次(3-4 仓, 配额够); 12:16 majors SOL '保证金≤0' 丢单=同族** **@09:2x 429 复现 3 次: 08:56:49/09:10:49 positionRisk 守护跳过 + 09:13:34 查 TPSL 失败, main(owner=1)与 majors(owner=2)同秒=同账户配额, 3 仓即触; 61631b0 部署优先级↑**) **@11:4x 429 簇再现(11:08-11:25): FetchCandles history/ZAMA tpsl_setup_failed(裸奔 28s 自愈)/查仓失败→B/USDT 保证金≤0 跳过=直接丢单; 300 池重启预热+两壳=峰值; 61631b0 部署=第一优先** **@12:4x 429 实害计数(11:08 起): 主单丢 4(NAORIS 11:45/B 11:25/AGLD 11:31 main + SOL 12:16 majors '保证金≤0')+守护跳过 9+ZAMA 11:11 回滚失败裸奔 28s; 预热簇 282 次/min FetchCandles** |
...
| CC-28 | 评判(在飞) | **EXP S35 bar聚合 15m on majors**(tpl1056 @09-19 08:39:38Z; 预注册在 majors 壳 _exp_cc; metric=段 n/费后均净/wr/tpsl 日志 SL 距中位/笔日/首单时刻; expect n≥15 费后均净≥0∧SL 距中位≥0.3%∧wr≥45%; worsen 段净≤−4U ∨ n≥10∧wr<35% ∨ SL 距中位<0.2%→ROLLBACK(rollback→998); 24h 零信号无异常→只报) | 09-19 08:4x | open(**@13:1x S35 有效段 n2 +1.37: XRP 11:16 TP +0.88/BNB 12:46 TP +0.49, SL 距 0.84/0.44% 中位 0.67%≥0.3✓, 腿 8/8; worsen 未触** **@09:2x 平仓 n0; 在飞 SOL 08:46 多/BNB 09:01 多, SL 距 0.88/0.67% 中位 0.77%≥0.3 ✓聚合生效; worsen 未触; 滑点 watch=§4 H-0919e**; eval 09-20 08:40Z 或 n≥15) **@10:3x 平仓仍 0; 在飞 3(+BTC 10:16 SL 距 0.33%); SL 距中位 0.67%≥0.3 ✓; worsen 未触; admin 10:03/10:06 atr 改回净零未重启=段未污染** **@11:4x: S35 段 SOL/BNB/BTC 多 11:08 被 owner 重启 'manual' 平仓 −0.75(n3, 未经 TP/SL 检验, 不计 worsen); XRP 多 11:16 腿✓ SL 距 0.84%≥0.3% ✓; mcp 5→2(owner); 继续至 09-20 08:40Z/n≥15** **@12:4x XRP 11:51 TP +0.88(段 n1); 12:16 SOL 候选因 429 '保证金≤0' 丢单; 继续** |
...
| CC-30 | owner 认领+裁决 | **11:06Z admin 批(main hunger off + symbols ""→300 池 + majors mcp2 + 双壳重启)**: 请 owner 认领(若非 owner=未知 admin 写路径🆘); 若有意回 300 池: 建议 symbols 写显式 265 币列表(重启键)使 isAllowedSymbol 不再把 majors 仓计入 main 槽位; 若无意: 回 7 币 symbols 列表+重启(CC 可执行, 空仓窗) | 09-19 11:4x | open(@13:1x 仍候认领; '4个' 跨实例计数 12:46-12:53 再现, 8 次跳过/50min) |
| CC-31 | M候选(括号重锚) | strategy_position.go:476 前按 (成交价−信号 entry) 平移 tp/sl(或 ×成交/信号比), 阈值 |滑点|>0.25×ATR 才平移; 依据 H-0919f; 部署=owner; 回滚=不平移(默认关) | 09-19 12:4x | open(候 n≥20 或 owner 令; **@13:1x n11 逆向滑点中位 +0.06%, >1% 仅 2/11(ZAMA/SWARMS 追涨) ⇒ 非系统性偏移, 优先级降**) |
...
| 计数项 | 读数 | 更新轮 | 备注 |
| **09-19 13:1x 读数(DB 去重; income 括号)** | 1h n6 −0.18(income −0.54) / 3h n17 −2.07(−3.37) / 6h n29 +16.08 wr45%(income +13.1: 毛 +16.1 费 −3.0) 多 +16.4 空 −0.4 / 12h n88 −28.2 wr34% 多 +7.7 空 −35.9 / 24h n230 −30.1 wr44%(income −99.5: 毛 −42.9 费 −56.6; 入金 +142 另列) 多 −11.6 空 −18.5; 24h 死法(main n222): <2m 亏 n40 −81.8, 2-5m 亏 n37 −67.0, 5-44m 亏 n37 −64.8, 45m 超时 n10 −10.5, 赢<5m n48 +81.2, 赢≥5m n50 +112.9; **300 池段(main ≥11:08) n10 毛 −1.62 wr40%: SL 3/−4.99, 45m 超时 4/−2.59, TP 3/+5.96; hold<2m 亏 0(刀死法消失)**; SL 距中位 1.5%(3/10 >3%); 滑点 n11 中位 +0.06%; 成交 conf 0.36-0.55(门槛 0.35✓); 48h 规则新增 0(M n3 −9.05/VVV n3 −5.71 watch); 12:4x 读数全文=git 7e0e2fe 版§6 | 09-19 13:1x | 判据不变 |
...
## 7. 运行日志（每轮一行，新行追加在表首）
| 2026-09-19 13:1x-13:3x | 定时轮: 管道活(REST 200; 429 12:24 后 1 次); 钱包 162.3(空仓→13:20 COTI 空 mv79); income 1h −0.54/3h −3.37/6h +13.1/12h −40.1/24h −99.5(入金 +142 另列)🆘(owner 令不缩表, 只报); 300 池段(main 11:08 起) n10 毛 −1.62 wr40%: SL 3(−4.99; B2 4.33%=2×ATR 第一波 6m −4.3%)/45m 超时 4(−2.59)/TP 3(+5.96), hold<2m 刀死法 0✓; 止损完整性: main 11:08 后新开 10/10 腿✓(VVV 补设路径)/majors 8/8✓, tp/sl=0≠裸奔 源码复核 :277-282/:305-307; 隔离: feed265∩bl45=∅ ∩majors5=∅ bl∩majors=SOL/XRP(有意), 无同 symbol 跨实例仓, 但 '已持仓4个' 跨实例计数 12:46-12:53 再现(跳过 8 次/50min); DS: 11:06 后 0 PATCH(13:09 节拍静默), lock-1789810634 冻 pct, _ai_task_ds null; **lev5 watch 终裁: 线破(post n12 费后 −0.30<+1.43)但不执行(重置污染+升敞口闸+钳 3%/3-5U 帽), 候 owner 一字令**; OBS 在飞 n10/30 worsen 未触; majors S35 有效段 n2 +1.37; **改动: 0 交易键; _exp_cc +reads_1316/watch_ds.lev 终裁, 留言板 13:26Z(7 条 8.1KB) 复读✓**; 宏观 FGI71 BTC +4.2% 市值 +1.2% trending∩feed=ZAMA/ZEC/PENGU/ENA/PONS/LIT/UNI/AVAX/SUI; 下轮=14:15 定时: OBS n≥30 或 18:00Z 评判, DS 16:09 节拍对账 |
