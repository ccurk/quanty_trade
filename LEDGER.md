...
| main断流观测(三闸交集关门) | 本轮main笔数=15/24h@09-01 21:1x(末笔20:17;判据未触) | 08-30 00:1x | 判据不变:再现6h+零笔且池ATR中位≥0.5→查管道;每轮记main笔数 |
...
| S31·regime动态方向偏置(**升队首候选@08-29 17:0x owner动态化直令**) | v1谱:宽度regime三档(池内EMA-up占比<0.35=RISK_OFF/0.35-0.60=中性/>0.60=RISK_ON)→动态premium加减(OFF:多头prem+0.10;ON:多头−0.05空头+0.05),阈值内联防漂移,S31_ENABLED False-guard回滚,叠加于config prem之上(config回滚路径保留);main先行 | 08-29 17:0x | **已上线@09-01 10:02=tpl988**(patcher断言过=基底零漂移/编译+6符号+锚8组✓/apply字节复检✓/零模板泄漏✓);_exp评09-04 09Z或S31段n≥20,到点n<5顺延48h一次(49h宕机后市场冷启动条款);劣化线=S31段多头净≤−3U或组wr−be劣化≥5pp@n≥20→S31_ENABLED=False;草案+验证史=ops/drafts/+git b12d7d9版行;**段累积表n11/9W2L@09-01 18:1x(毛+3.30;多n4净−0.49/空n7净+3.79;判别笔仍仅ONG+0.37)**:①-⑧全链git 497cdf2版行(要点:①ONG L conf0.650 w0.656 ON降门放行=唯一判别+0.37;④FF L conf恰0.70非判别边界;⑦STAR S N档SL穿刺−1.42);⑨-⑪细目git 20ecf1d版行(要点:⑨FF边界笔SL−1.30;⑪BTR OFF+0.70;81分钟ON→N→OFF全滑);**n15/11W4L@09-01 21:1x(毛+1.49;多n5净−0.045/空n10净+1.54)**:⑫UAI S18:23 conf0.700 w0.183 OFF −1.32 SL穿刺30m⑬MARSCOIN S18:42 conf0.750 w0.161 OFF −1.17 SL穿刺16m(18:53-58空头双SL=宽度翻转squeeze窗)⑭牛来S18:59 conf0.800 w0.301 OFF +0.23⑮USELESS L19:34 conf0.750 w0.742 ON非判别(静态0.70亦放行)+0.45;71分钟OFF0.161→ON0.742剧烈翻转=churn观察n3;判别笔仍仅ONG;劣化线双查:多头−0.045≫−3U✓wr73.3−be68.1=+5.2pp✓;宽度重建法:logs q=日期T分钟→评估行EMA up占比/93(不带日期=跨天污染);评期用本累积表 |
...
| 09-01 18:1x-18:5xZ | 例行轮(max) | 0/+0.02/n10 +2.93/+3.30/+3.30(毛;净+2.95;wr81.8%@n11 be67.0% wr−be+14.8pp) | FGI69/BTC77147−2.1%/mcap API缺值/trend∩池{ARB,UNI,PUMP} | **HOLD原子包0;ROUTE v1.5零差分三连(plan空,bl14期望态,TUT aging第5日,want层退役壳原型续挂);S31段+3笔→n11/9W2L(⑨FF L边界笔非判别−1.30/⑩MARS S N+0.62/⑪BTR S OFF+0.70;81分钟ON→N→OFF全滑;劣化线双查绿);**#79双行升级(#77级)**:era BTR×2+今BTR均DB双行PnL双写;08-30 03:34壳饥饿档真接管main仓−0.61=有界风险首兑现;binance_only空sid=视图伪影且dedup保壳行=归因陷阱;n=15→TG催部署dev-adoption-attribution+#77;充值线三条件续达(承前轮不重发)** | 钱包164.04U零持仓;死法:SL穿刺2(STAR−1.42 26m/FF−1.30 11m)+TP/trailing收9;费覆🟢三连(均毛+0.300 vs 3×费0.097);trend TUT n0第5日;audit零无主(main1791/trend1790);feed93/1零漂移 |
