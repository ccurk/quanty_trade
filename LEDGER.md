...
| main 通才(平台名 Meme_合约信号计算引擎_1) | 8eb182b6 | **tpl1057 = Meme_v40_crypto_perp_20260919-133457(hash deb8795a 自 14:3x 未变; 0 个 S 锚点; 候认领 CC-32; 归档 confident-fermi strategies/quicktrade-8eb182b6/tpl1057_v40_*.py)**; 前代 tpl998(S4..S23+S30+S31+S33; hash 3e311e8b)=回滚目标; 史=git 063b9e8 版§1.5 | symbols ""+max_price 1e12 ⇒ filter 路径 300 池; feed 269(17:2x rotate remove 31 after 17:01:58Z 重启; bl∩feed=∅, ∩majors=∅; **每次重启后必 rotate**: 20 项截断已修但 symbols "" 重播种仍含 bl) | running(runtime 1789837318608=17:01:58Z **owner 部署 fd01fae..f0f8a62(S42 连亏熔断+预热限流+日志清理) 全实例重启**; 前 16:04:12Z e07f3d7+3e1e467; 今日重启 11:08/13:42/14:04/14:11/16:04/17:02) | **17:2x 现值: lev4(DS 16:09 裸改, 前 5@10:09, owner 值 10) pct0.125(DS 07:08, owner 值 0.25, lock-1789833131 至 09-20 15:52Z) mcp3(DS 09:37, owner 令 10; 被 majors 仓吃槽=实际 1-2) tp,sl pct0(腿=信号 ATR 括号, 部署后锚到成交价) max_hold45 cd180 reentry_cd0(owner 15:36) min_conf0.35 atr 2.0/2.5 hunger off trailing/BE off bl46(+BNB@16:20) sides 双向 loss_streak on(3 连亏/48h, owner 16:47, 引擎自算不写 bl); v40 只读 min_confidence/tp_ratio/sl_ratio/atr_tp_mult/atr_sl_mult/cooldown_sec/warmup_bars/max_bars/cost_mult/fee_round_trip/long_bias/short_bias/max_candidates/log_every/log_trace**; v40 段 @17:2x n28 −1.90 wr57%(DB 已补齐 income-only 行; 费后≈−4.0=−0.14/笔) |
| majors(平台名 Majors_BTC_ETH_BNB_SOL) | e725e31a | main fork → **tpl1056 = tpl998 + S35 bar聚合 15m**(CC apply 09-19 08:39:38Z, hash 766b576a; 13:44:29/16:04:12 重启后模板保持) | manual BTC/ETH/BNB/SOL/XRP, **bl [BNB,SOL,XRP]@16:23:48 admin ⇒ 有效池 BTC/ETH**(候认领; main 侧 bl SOL/XRP/BNB+rotate 防撞; BTC/ETH 未入 main bl=靠组内互斥兜底) | **running@08:01:47Z(owner UI start); 重启 11:08:41(owner)/13:44:29(v40 批次)/16:04:12/17:01:58(owner 部署×2)** | 现值: lev10 pct0.125 mcp2 cd900 reentry_cd0(owner 15:36) max_hold720 hunger off atr 2.0/2.5 min_atr 0.03 bar_agg 15 sl_ratio0.03 loss_streak on auto_optimize dry_run=true; S35 段 @17:2x DB n6 +0.14 wr50%(XRP TP +0.88/BNB +0.49/BTC +0.03 vs SOL 142m −0.63/BNB 127m −0.15/BNB 78m −0.47; 3 亏全为长持超时型), SL 距中位 ≥0.67%✓, 腿 ETH 15:46✓(在飞; 17:02 重启后交易所腿存活=SSV 同型实证 found=2); 史=git 063b9e8 版§1.5; CC-28 eval 09-20 08:40Z/n≥15 |
...
### 双执行体(09-17 起)
- **[新增 09-19 17:2x] owner 部署 fd01fae..f0f8a62(author black, 谱系 claude/hopeful-lamport-k1ytj4) → 17:01:58Z 两实例重启**: ①**S42 真实 PnL 连亏熔断** strategy_loss_streak.go: 某 symbol 最近连续 ≥loss_streak_threshold(3) 笔真实平仓(strategy_positions realized_pnl, exchange_income 优先, 孪生行去重 3e6d02d)全亏且末笔在 loss_streak_quarantine_hours(48) 内 ⇒ 信号闸(strategy_signal.go:868)/下单口(strategy_position.go:83)/收养闸拒开; 无状态、不落库、**不写 symbol_blacklist**、重启存活; 键缺省=开; 日志 "跳过信号：交易对连亏熔断中 symbol=X 连亏=N 隔离至=T"(APT 至 09-21 06:35Z 已见); 与 config bl 并行=两层隔离, 台账隔离区规则(≤−4U∧n≥4)与 bl 一致性核对不变, loss_streak 名单引擎自算不登记 ②f0f8a62 历史预热按请求节奏限流(manager.go)=CC-23 局部根修(17:02 重启后 429 仅 1 vs 16:04 首 12min 洪峰) ③日志清理: WS 链路逐 symbol 流水静音(a91cd0a/eed2a1d), 关键行(候选/置信度动态仓位/已设置止盈止损/锚到成交价/跳过本批)仍在; 16:47:02 admin 两实例同秒 PATCH loss_streak_*=owner 归属; 16:23:48 admin majors bl+[BNB,SOL,XRP]=有效池 BTC/ETH(候认领 CC-34).
- **[新增 09-19 17:2x] H-0919i 两闸口径不一致+批次竞态(源码+实证)**: 信号闸 strategy_signal.go:614-680 openCount=本策略 DB open+2min pending+交易所 FetchPositions 经 isAllowedSymbol(symbols ""⇒恒 true ⇒ 计入 majors ETH); 下单口闸 strategy_position.go:150-268 只数本策略 DB open+pending+Redis AcquireOpenSlot(inst.ID,maxPos)⇒不含 majors; processSignalBatch 由首信号后 500ms goroutine 触发(manager.go:1118 signalBatchWait, strategy_signal.go:419-452)无每实例串行锁, 各批 openCount 快照独立. 实证 17:18:00: BCH "已预留 3/3"(17:18:01) 与 WLD "已占用 3/3"(17:18:02) 同底数 2(COTI+ETH), WLD 批未见 BCH ⇒ 信号闸被竞态绕过, 下单口闸按本策略 3/3 放行 ⇒ 交易所 4 仓=main 3(COTI/BCH/WLD)+majors ETH, **main 自身=mcp3 未超**; WLD 17:23 SL 腿 −0.48 仅 income(DB 行未生成, §3 15:2x 丢行族). 结论: main 平时有效并发=mcp−majors 持仓数(CC-30), 仅竞态时拿满 mcp; 修(低优先候 owner)=信号闸只数本策略仓(与下单口一致) 或 main 显式 symbols 或 mcp 回 10; 批次串行化非必需(Redis 槽兜底).
...
- **[09-19 16:3x] backend 部署 e07f3d7+3e1e467 → 16:04:12Z 重启**: 全文=git 7f55bde 版§3(要点: reanchorTPSLToFill 上线(CC-31 结案); 排序 score=rr×1.25 盈利优先, rr 仍恒 1.25(CC-33); parseSymbolsValue 20 项截断修=H-0919g 真根因; UpdateStrategyConfig running 放行=部署版≠仓库版结案; symbol_reentry_cd 0=owner; strategy_signal.go:691 majors 仓计入 main openCount 仍在=CC-30)
- **[新增 09-19 16:3x] DS 改动簿 #12**: 15:36:18 admin symbol_reentry_cooldown_minutes→0(main+majors)=owner 归属(不评判); 15:52:13 breaker lock-1789833131 续锁 24h(pct 0.125→0.125 no-op; why 净 −8.00U); **16:09:14 optimizer lev 5→4 裸改(无 _exp)→watch_ds 线: post 段(open≥16:09Z) n≥10 段净≤−4U ∨ 费后均净<−0.19/笔 → DS-ROLLBACK 5(不回 10: RECOVER 门关), eval 22:00Z**; 16:20:15 admin symbol_blacklist +BNB/USDT(46) 合规(防撞 majors)候认领; 裁决=均不硬抢(降敞口不问); 留言板 16:3x 条已论证 mcp3 吃槽+请补 _exp.changed。
- **[新增 09-19 15:2x] 槽位分配丢弃置信度(源码)**: 全文=git 063b9e8 版§3(要点: strategy_signal.go rr=(tp−px)/(px−sl), v40 tp=1.25×sl ⇒ rr 恒 1.25 平局 ⇒ 置信度不参与槽位; 部署后排序见 16:3x 条; 行动=CC-33)。
- **[新增 09-19 15:2x] DB closed 行丢交易所腿平仓 + LSK SL 腿消失亚型**: 全文=git 063b9e8 版§3(要点: ENA 14:20 SL 腿成交无平仓日志无 DB 行, 仅 income by_symbol 有; ZAMA 2 笔亦消失) ‖ @16:3x 累计再 +5(RONIN SL/WAL/ALCH/ZK/ZIL TP 均只在 income; MITO 45m 超时有日志亦无 DB 行) ⇒ **逐笔 n/净 必须 DB∪income by_symbol, DB 只作下界**。
...
| **H-0919h 交易所 SL 腿触发前消失(priceProtect EXPIRED?)** | 09-19 15:2x | 证据=LSK 14:51 撤单 -2011 found=2 canceled=1 + 当日 8 例 "止损失效"; 机制候选=binance.go:2728-2730 STOP_MARKET closePosition+MARK_PRICE+priceProtect=TRUE 触发时 mark/last 偏离超阈→EXPIRED→仓位裸露至 15s 巡检; 全文=git 063b9e8 版§4 | 门=owner 查 Binance 条件单历史; @16:3x 14:51 后 0 新例 |
...
| CC-9 | DS越界簿 | admin mcp 未声明改动=协议7越界 #1~#7 + lev 10→5 裸改 10:09; 裁决=不硬抢(RECOVER 门关/429 预算/乒乓); owner 一字令候(mcp 10/5/3, lev 10/5/阶梯); v40 下 mcp3=频率上限(跳过 7+候选失败 23/1.2h); 全文=git 4d0e7da 版§5 | 09-18 08:1x | open (@16:3x +lev 5→4 16:09 裸改→watch_ds eval 22:00Z; mcp3 被 majors 仓吃槽=实际 1-2 槽) |
...
| CC-23 | 管道(owner/M) | 429 级联=唯一裸奔路径; REST 预算补丁 claude/dev-rest-budget 61631b0 候部署=第一优先; v40 自带 REST 拉取≈150 调用/min(首 12min)与下单同 IP; 全文=git 063b9e8 版§5 | 09-19 05:5x | open(候 owner; @16:3x since v40 5: 守护跳过 2/查 TPSL 3; +SOLV 16:09:37 tpsl_setup_failed 429→开仓事务回滚(非裸奔; 部署重启后 v40 首 12min REST 洪峰再现); @17:2x f0f8a62 历史预热限流已部署=局部根修(17:02 重启后 429 仅 1); 61631b0 每仓 2s positionRisk 部分仍候) |
...
| CC-30 | owner 认领+裁决 | 11:06Z admin 批(main hunger off + symbols ""→300 池 + majors mcp2 + 双壳重启) 请 owner 认领; symbols "" ⇒ isAllowedSymbol 恒 true ⇒ majors 仓计入 main 槽位 + 重启重播种隔离币; 全文=git 063b9e8 版§5 | 09-19 11:4x | open(@16:3x 部署后仍在: strategy_signal.go:691 循环未改; 16:16 ZIL 开仓即 3/3=ZIL+majors ETH/BNB, 16:18-16:23 跳过本批 ≥100 次 ⇒ 20 项上限已修, symbols 300 显式列表可写(重启键)+mcp 回 owner 10 二选一请 owner 令; @17:2x H-0919i: 信号闸计入 majors 仓但下单口闸/Redis 槽只数本策略, 批次竞态(500ms goroutine 无串行锁)时 main 拿满 mcp3(17:18 COTI/BCH/WLD+ETH=4 仓, main 自身未超), 平时=mcp−majors 仓数; 修=信号闸只数本策略仓, 候 owner) |
| CC-31 | M候选(括号重锚) | 全文=git 063b9e8 版§5 | 09-19 12:4x | **closed@16:3x: owner 部署 e07f3d7 reanchorTPSLToFill(ZIL 滑点 −0.03%/XMR +0.007% 实证)** |
| CC-32 | owner 认领+拍板(v40) | main 代码被换为 v40(tpl1057, 13:42:49 起, 零 audit): 请 owner 认领+拍板 A 保留 v40 评判(CC 推荐) / B 回滚 tpl998; 未拍板前按 _exp_cc EVAL 线执行(劣化→TG+rollback tpl998); 全文=git 063b9e8 版§5 | 09-19 14:3x | open(@16:3x v40 段 n≈21 −1.97 wr55% 费后≈−3.9; worsen 未触; 必修①bl 闸已由 3e1e467 部署解决, ②reanchor 已部署, ③REST 预算 f0f8a62 预热限流已部署) (@17:2x v40 n28 −1.90 wr57% 费后 −0.14/笔; n=30 预判不达 expect(费后≥0∧wr≥60%) 但非劣化(优于 tpl998 300 段 −0.25/笔 wr43%) → 封段时呈 owner A/B, 不自动 rollback) |
...
| CC-34 | owner 认领 | 三项 admin/无 audit 动作请 owner 认领: ①16:04 部署 e07f3d7+3e1e467(本分支) ②15:36 symbol_reentry_cooldown_minutes→0(main+majors) ③16:20 main bl +BNB; 附建议: BTC/ETH 一并入 main bl(现靠 strategy_signal.go 组内互斥兜底); @17:2x 追加 ④16:47:02 loss_streak_* 三键(main+majors) ⑤16:23:48 majors bl+[BNB,SOL,XRP] ⑥17:01:58 部署 fd01fae..f0f8a62(谱系 claude/hopeful-lamport-k1ytj4) | 09-19 16:3x | open |
...
| 计数项 | 读数 | 更新轮 | 备注 |
| **09-19 17:2x 读数(income 口径; DB 只作下界)** | 1h −0.44(毛 +0.16 费 −0.59) / 3h −3.8(毛 −1.64 费 −2.18) / 6h −6.0(毛 −2.61 费 −3.41 ≈−3.8% 钱包≈157) / 12h −16.9(毛 −6.32 费 −10.58) / 24h −66.2(毛 −26.60 费 −39.60; 入金 +58.5 另列 ≈−42%)🆘; paired 24h 腿 254 wr37% 多 151/−24.0 空 103/−9.9; DB 24h(main) n189 −27.27 wr44% 多 117/−1.93 空 72/−25.34, 死法 SL<2m 35/−66.1, SL2-5m 29/−43.1, SL5-44m 34/−42.8, 45m 超时 7/−5.7, 赢 84/+130.4; **v40 段(14:11 起) n28 −1.90 wr57%(DB 已补齐: SL<2m 0, SL2-5m 3/−1.82, SL5-44m 9/−6.81, 45m 超时 2/+0.28, 赢 16/+6.72; 多 20/−2.89 空 8/+0.99), 费≈−2.1 ⇒ 费后≈−4.0(−0.14/笔); lev4 post 段 n8 +0.63 wr62%; 成交 conf n59 中位 0.82(<0.5:1/0.5-0.7:14/≥0.7:44), 重启后 n10 中位 1.00 mult 全 1.0; 笔/日≈130-190(24h DB 189) vs 基线 38.7 但均净<0=无效频率; 跳过本批 16:30-16:50 135 次后 ≤2/5min(槽位随 majors ETH 在飞)**; majors S35 段 DB n6 +0.14; 16:3x 读数全文=git 7f55bde 版§6 | 09-19 17:2x | 判据不变 |
...
## 7. 运行日志（每轮一行，新行追加在表首）
| 2026-09-19 17:1x-17:3x | 定时轮: 管道活(REST 200; 17:02 重启后 429 仅 1); 钱包≈avail 113.8+保证金≈42≈157(在飞 COTI 多/BCH 多 + majors ETH 多); income 1h −0.44/6h −6.0(≈−3.8%, 未触 8%)/12h −16.9/24h −66.2(≈−42%; 入金 +58.5 另列)🆘 只报(owner 令不缩表); **17:01:58Z 两实例再重启=owner 部署 fd01fae..f0f8a62(S42 连亏熔断 3/48h+预热限流 429 修+日志清理; 16:47 owner PATCH loss_streak_* 两实例)** → 重播种 300 含 bl 31 → rotate remove 31(feed 269, bl∩feed=∅, ∩majors=∅); v40 段 n28 −1.90 wr57% 费后 −0.14/笔, worsen 未触, n<30 EVAL 继续(预判 n=30 不达 expect 但非劣化→呈 owner A/B); lev4 watch n8 +0.63(未到期); 止损完整性: main 16:28 后 opens 8/8✓ 锚到成交价 8/8, SSV 重启收养后 17:17 超时平仓撤腿 found=2=腿存活, majors ETH 15:46✓, max_hold 45/720 ⇒ 两实例非裸奔; 隔离: 无同 symbol 跨实例仓; majors bl+[BNB,SOL,XRP]→有效池 BTC/ETH(admin 16:23 候认领); DS: 16:28 后 0 交易键 PATCH(19:09 节拍未到), admin 动作全归 owner; H-0919i 两闸口径不一致+批次竞态(17:18 BCH/WLD 同底数 "3/3" → main 3+ETH=4 仓, main 自身未超)→CC-30 追加; 改动: rotate 31 + _exp_cc(reads_1720/deploy_1702/h0919i/watch_ds 3 项; 14:30-16:30 读数折入 ops/exp_archive/exp_cc_20260919T1720Z.json)+_ai_task_cc(两次 PATCH 复读✓; 留言板 1012 字仍超 600 限, 下轮硬裁); 宏观 FGI71 BTC +1.1% ETH +2.2% 市值 −1.2% trending∩feed=AVAX/FIL/PONS/ZAMA/ZEC; 下轮=18:1x: v40 n≥30 封段裁决呈 owner; lev4 watch n≥10; DS 19:09 节拍对账; CC-34 ④⑤⑥ 认领 |
| 2026-09-19 16:1x-16:3x | 全文=git 7f55bde 版§7(要点: owner 部署 e07f3d7+3e1e467 → 16:04 重启 → rotate remove 32; v40 n≈21 −1.97 EVAL 继续; 止损完整性 14/14+4/4; DS lev 5→4 watch; 留言板超 600 限) |
| 2026-09-19 15:1x-15:3x | 全文=git 063b9e8 版§7(要点: v40 n15 −1.28 wr53%; 腿 18/18+9/9; rotate remove LSK; DS 0 PATCH; 新发现 rr 恒 1.25/DB 丢腿行/LSK SL 腿消失) |
