...
| 计数项 | 读数 | 更新轮 | 备注 |
| backend→Binance REST DNS断连 | **n=1: 09-09 16:12Z起持续,09-12 09:13仍在(≥65.0h;probe史22miss+cron直证十一轮=git fc2c6d4/1e6270a/9e8dc48/01d79a0/adfc233→dc081bf链;09:1x直证=main/trend ctx income双载具+active positionRisk同串fapi SERVFAIL+balance伪零;ws活证main评分批recv183/183@09:13+trend recv3/3;feed183/3无重播种)**;特征=fapi SERVFAIL(resolver 100.100.100.100=Tailscale MagicDNS,修复在宿主机);伪影族: ctx balance=0+income{}伪零/skip文案误导/closed窗过滤失效升级@00:1x(hours=48回2847行最早07-19,末真实close不变=close_time过滤判n0不受扰) | 09-12 11:1x | probe#1@10:12+probe#2@11:12双miss(同串SERVFAIL≥67.0h,active+ctx双证;本session probe额度止,静默交接12:1x cron);恢复后复核:①income补记完整性②feed有无重播种③若平台重启于断连窗=ws也死须立报;再现→升§3;⚠️断连窗禁stop/start(重订阅可能失败) |
...
| 2026-09-12 06:1x | 断连轮#18(≥62.1h,锚09-09 16:12Z): 双直证同串(active positionRisk+main ctx income@06:13-16 fapi SERVFAIL on 100.100.100.100;balance=0伪零);ws活证(main评分KNC0.36/M0.40@06:14池183 recv183/183+trend评分TUT0.24@06:17 recv3/3);feed183/3无重播种;audit尾1808零无主;main/trend config全键零漂移(bl16/pct0.08/lcp0.15/scp0.05/S33 _exp open原封);真窗n0(末close TAC 09-09 14:56维持;2847行窗伪影维持);ROUTE v1.6 plan={}30连零,quarantine19,desired_bl22=#88一致,aging_watch trend三币;S31宽度本轮无触发行可采(断连窗评分行短格式无detail;零成交=判据零影响);S33冻结KEEP-armed零动;宏观FGI63 Greed/BTC−0.08%@77204/mcap−2.29%未触;刹车未触(零成交);保证金0.55✓;HOLD 0原子包;#88续候;TG催resolver(≥62h) |
<!-- §7瘦身史指针集=git 8729d22版本注(逐hash在内) -->
| 时间(UTC) | 档位 | 五窗净额 1h/3h/6h/12h/24h | 世界 | 决策 | 备注 |
> 瘦身注(指针@09-12 00:1x归并): 断连#11=git fc2c6d4;#12=git 0207b9f;#13(15:1x)=git 1e6270a版行;#14(18:1x)/#15(21:1x)=git 9e8dc48版行;#16(00:1x)=git 01d79a0版行;#17(03:1x)=git dc081bf版行。
