...
| 载具 | id | 原型 | 池 | 状态 | 门/备注 |
|---|---|---|---|---|---|
| main 通才 | 8eb182b6 | 通才(S18-S23栈) | auto·**feed89@08-18 15:4x**(#61b窗重启#18重播种100→补刀rotate清11→89泄漏0[补刀即打不等heatclean];STAR等2币auto全集自然新入=发现漏斗,无running载具冲突;bl15种子✓) | running | pct0.08 sides=[buy,sell] **long_conf_premium=0.20(EXP#61b@08-18 15:40Z)** tpl567;**trailing(act1.0/cb1.2%)+BE(1.0)=基线几何(#49终裁KEEP@08-16 18:2x)**;**_exp=open EXP(#61b) prem0.25→0.20 门0.80→0.75**(#61空臂修正[§3天花板条]:0.80门不可达段永n=0,零结果数据原位换臂;eval=08-21 12:30Z或段n≥15/刹车段6h≤−5U或载具6h≤−8U/metric段多头逐笔净+费覆+快SL≤30%全不变;rollback=prem0.35);#49 verdict存git;#49终数与裁由存_exp.verdict+git+§4#49(段n30赔率0.43=统计失真keep;ex-BICO n26/+0.04);RECOVER刹线6h空≤−8U继承;#11降级观察(§4);后续实验以本段为基线;**BICO已隔离@08-17 03:2x**(§6结案) |
...
- 互斥不变式: feed层@08-18 12:3x=main87/trend3(fade停机无live feed),main∩trend=∅✓;种子层=main bl**15**(12隔离+trend3)=desired**无缺口**;重播种**#17=08-18 12:2x EXP#61窗重启100→heatclean-v11未现(轮询6×15s)→补刀rotate清13→87泄漏0**(⚠️heatclean非确定性/慢周期实证:#16=15:19自动清12,#17须补刀——补刀rotate自此为重启后标准程序;#8-#16史git);引擎互斥闸✓live;**fade壳seed5与main auto理论重叠=停机态无违例**(不变式限定running载具;若owner UI复活fade须先重划池,互斥闸+TG示警兜底);隔离币零成交✓(复核@15:2x,BEAT/XNY隔离后零新笔)
...
| 61 | E7类观察(main) | **main多头溢价墙机制观察**:22h零笔分解=空侧无setup+多头候选0.72撞门0.90(min0.55+prem0.35);7min评分抽样n1331:86次≥0.55全多头分,max0.72(PROM/KAITO),空侧最高0.24量级;墙系历史多头失血期所砌,市场温和上行期(FGI41/BTC+1.3%)墙把载具憋成零频 | 09:1x轮日志实拉;与#55(trend prem)同族但main费覆24h avg−0.573🔴=红线锁死 | n=2轮快照 | **执行→在飞@08-18 12:30Z**:门达成=快照②12:2x(n659,多头max0.80 HOME×2,≥0.70仅5,0.90门全拦;空侧max0.60)+main24h费覆🟢(n=1 VELVET+1.04收养,薄口径诚实注记);EXP全序=真空仓窗stop(poll1沉降)-PATCH(prem0.25+_exp原子)-start-复检零泄漏;裁决按_exp预注册(eval 08-21 12:30Z或段n≥15);**#61b流量观察@08-18 18:2x**:15:32后0.76上尾6次(HOME/EDEN/KODEX200/CSOP×2)全被ATR%<0.5硬滤拦(硬滤先于门),段多头n=0@2.8h;但#61b前EDEN曾4次(13:27-15:19)过硬滤仅败于旧0.80门→**臂非结构死,属流量薄**(0.76×ATR≥0.5交集存在);判据:若eval期段多头持续n=0且0.76全ATR拦→下臂议低波折扣校准而非再降门 |
...
| 55 | E7类(trend) | **EXP:trend long_conf_premium 0.15→0.10**(gate 0.70→0.65;config-only零代码;回滚=回0.15) | 分数墙源码级:ACE多头0.650×9h+顶门0.70反复拒发(03:21实证);S24终裁链n33/+0.024=36h零笔载具空转;ACE 24h 4W+1.73=池内信号质量在场,门下0.05徘徊非无信号 | 机制级(gate) | 候选@06:35;**执行门=费覆红线解禁(组24h均净≥3×费)或owner裁量**(降prem=提频,红线内只登记@03:11轮既定);执行时走EXP _exp预注册(trend槽已空闲);**12:00Z审议补充反证@08-18 06:2x**:①trend 48h毛额n8/−0.48为负(24h+1.53全靠近段,拉长即水下=降门扩量的EV基础存疑)②当轮conf日志实拉:近6入场ACE0.75-0.85/COTI0.72=墙今日不咬合(0.65徘徊是08-17 03:21时点现象非常态)③24h组费覆首🟢(avg+0.385=3.4×实费0.115)但幽灵占+4.58,ex-ghost avg+0.038🔴——三点连同正方证据一并上12:00Z审议台;**09:1x补**:trend24h n6/+1.47 wr50全正牌=近段EV回暖;**12:00Z审议裁决@08-18 12:3x=续锁**:硬边界#1按载具判压过组口径——组24h费覆双🟢(实测:全口径avg+0.765=7.6×实费0.034;ex-ghost avg+0.229=7.8×实费0.0295)但trend载具自身avg+0.027<3×0.0855🔴;window滑动后trend24h回落n4/+0.11 wr25(ACE+0.59/−0.38/−0.06+COTI−0.04)=降门扩量EV基础仍未证;留候选,重审门=trend载具24h费覆🟢 |
...
| 08-18 09:13Z | 例行轮 | 24h+4.15(13) | FGI41 | HOLD;新登记#61溢价墙观察(7min抽样n1331 max0.72撞0.90门) | stub;全文git d19c710系+a477307 |
| 08-18 06:17Z | 例行轮 | 24h+5.00(13) | FGI41 | HOLD;#55三反证登记留12:00Z审议 | stub;全文git d19c710 |
| 08-18 03:12Z | 例行轮 | 24h+1.04(14) | FGI41 | HOLD;#60首拉13行;"幽灵醒STAR+2.55"→18:2x翻案为main开 | stub;全文git fa6e0e0 |
| 08-18 00:13Z | 例行轮 | 24h−1.80(14) | FGI41 | HOLD;main 8h零笔选择性实证(maxconf0.64) | stub;全文git fa6e0e0 |
|---|---|---|---|---|---|
