...
| main 通才(平台名 Meme_合约信号计算引擎_1) | 8eb182b6 | 通才(S4..S23+S30+S31+S33;S32关guard;tpl998) | auto select300·feed283@09-18 14:2x(owner 13:08 重启重播种→rotate remove 17;∩=∅) | running | **09-19 07:3xZ 现值: manual 7 币池 G/BULLA/MAGMA/UAI/HEI/DRIFT/ARB(owner 06:5x 直令 8 币, POWER 在隔离区未入; pid 55085 START 07:21:21Z 注入 min_conf0.35/cd180/atr 2.0/2.5/warmup50/max_price1e12) pct0.125(DS breaker #7 07:08 砍, owner 值 0.25, 不硬抢) lev10 mcp6(owner 令 10) sides双向 hunger 1m/tp0.30/sl0.04(04:53 admin, 候 owner 认领=CC-24①) tp,sl pct0 trailing/BE 关 max_hold45 bl42 feed7 ∩=∅; _exp_cc=OWNER-DIRECTIVE 多策略化(eval 18:00Z 或 n≥30; rollback=filter 300 池); 史 05:5x: pct0.25 lev10 mcp6 hunger 0.04 atr 2.0/2.5 bl39 feed272;  09-18 14:2xZ owner 进攻态: pct0.25(UI) lev10 mcp10 sides双向 max_init_margin500 tp/sl pct=0(ATR口径;Go clamp 3%) hunger 1m/tp0.30/sl0.075(ROI) max_hold45 trailing 0.5×ATR/0.5% BE0.5 cs_mult[0.6,1.0] cd180/min_conf0.35 warmup50 max_price1e12 max_consec100 sl_ratio0.03/tp_ratio0.06 bl24; Python 侧 atr 2.0/1.5 已入config候 owner 重启(进程现 4.5/2.5,CC-12);自动缩表已由 owner 去掉(13:4x)** _exp_cc=p5_pct(eval 20:00Z或n≥30)+p6_exit_tight(eval 09-19 00:00Z或n≥40;worsen 已触@14:2x→owner 拍板/CC-14 条件杠杆 15:30Z);史=git 91e4749/56a5311 版行 |
| majors(平台名 Majors_BTC_ETH_BNB_SOL) | e725e31a | main fork tpl998(候 S34 bar_agg_minutes 15 只 apply 到本壳) | manual BTC/ETH/BNB/SOL | **stopped**(owner 06:5x 直令; 启动门=15m 5d 回测 ≥2 币费后>0) | 09-19 07:1x live 会话建壳: lev10 pct0.125 mcp2 cd900 max_hold720 hunger 1m/0.30/0.30 atr 2.0/2.5 sides 双向 auto_optimize false; 1m 上 BTC/BNB 2×ATR≈0.10%=来回费→结构性负, 须 15m bar; 非退役壳(owner 新建), start/删除须 owner |
...
- **[新增 09-19 05:5x] DS 改动簿 #7(admin, 归属=owner 直令文本)**: 04:49:12Z _exp lock-1789793352-owner-unlock(closed, frozen []) + pct 0.25; 04:53:09Z hunger_stop_loss_pct 0.30→0.04(同 actor, 与 owner 09-18 17:59 自落同键同值→按 owner 值, 候认领); 后果=p9 交易所 2×ATR SL 腿被 0.4% 刀覆盖(quick_trade_monitor.go:112); 全文=git d19a237版§3。
