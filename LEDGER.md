...
| 计数项 | 读数 | 更新轮 | 备注 |
| backend→Binance REST DNS断连 | **n=1: 09-09 16:12Z起持续,04:01Z仍在(≥12h)**;特征=fapi.binance.com SERVFAIL(resolver 100.100.100.100);ws K线流存活189/183 active;开仓尝试14次全被skip(文案'当前市场不支持该交易对'实为exchangeInfo DNS err=误导性);ctx balance=0+income{}=伪影 | 09-10 04:1x | 恢复后复核:①income补记完整性②feed有无重播种③若平台重启于断连窗=ws也死须立报;再现→升§3;⚠️断连窗禁stop/start(重订阅可能失败) |
...
|---|---|---|---|
| main断流观测(三闸交集关门) | main笔数16/24h@09-10 04:1x(全部≤09-09 14:56Z;13:56后零开仓=DNS断连所致非三闸关门,开仓尝试14次全skip;史27@09-08git f9c8407) | 09-08 18:1x | 判据不变:再现6h+零笔且池ATR中位≥0.5→查管道;每轮记main笔数 |
