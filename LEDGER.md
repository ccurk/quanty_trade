...
|---|---|---|---|
| 后端DB层挂死watch(平台事故;新签名≠crash loop) | **n=1@08-27 03:1x**: 进程活/静态+中间件正常而一切触DB端点无限挂(login L104+public/templates双证);签名区别=crash loop是连接reset+重播种,本例是TCP通+永挂;上轮末00:45Z健康→03:14Z发现 | 08-27 03:1x | 判据: 再现(恢复后又挂)→升§3平台事实+候修根因(疑SQLite长事务/goroutine持锁,候选=WAL模式或busy_timeout);恢复轮必做三查(成交空窗/feed漂移/误复活)记本行 |
...
| 08-27 03:1x-03:2xZ | 例行轮(max)·**事故轮** | 数据不可得(后端DB层全局阻塞,登录挂死) | 未拉取 | **HOLD 0原子包;TG🆘 msg7747请owner重启后端;登录重试10+次全败(45s超时)** | 三层探针定位(源码级):非DB路径全正常(静态200/0.6s+401中间件0.5s+400绑定1.3s),触DB两端点(login handlers.go L104首查/public/templates)90s+零响应=SQLite疑长事务持锁;00:45Z上轮末尚健康→03:14Z发现,间窗引擎行为不可见;风险面(源码):ROI守护/TPSL补挂/饥饿超时tick全首行触DB=冻结,新开仓链冻结=无新敞口,已开仓交易所侧algo TP/SL不依赖后端仍有效=硬保护在;上轮末0持仓168.41U→大概率空仓冻结安全态;EXP提频评判(原18Z)与磁铁era门裁决顺延恢复轮;恢复后必查:①挂死窗内有无成交/幻影行②feed重播种漂移(重启预期)③stopped载具误复活;待落0 |
