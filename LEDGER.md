...
| 载具 | id | 原型 | 池 | 状态 | 门/备注 |
|---|---|---|---|---|---|
| main 通才 | 8eb182b6 | 通才(S18-S23栈) | auto·**feed90@08-18 21:2x**(#61c窗重启#19重播种100→补刀rotate清10→90泄漏0;bl15种子✓) | running | pct0.08 sides=[buy,sell] **long_conf_premium=0.19(EXP#61c@08-18 21:2x)** tpl567;**trailing(act1.0/cb1.2%)+BE(1.0)=基线几何(#49终裁KEEP@08-16 18:2x)**;**_exp=open EXP(#61c) prem0.20→0.19 门0.75→0.74**(#61b浮点边界修正[§3天花板条补记]:0.75原生档=多头主流量,因子和左结合0.7499999999999999<门0.55+0.20=0.75精确[L794-797 >=判],#61b后5.6h该档10行全拒[JCT/ALPINE/VELVET/1000RATS],0.76折扣档11行全ATR<0.5硬拦,0.78+零出现;门0.74恰放0.750原生档,下档0.72不放;eval=08-21 12:30Z或段n≥15/刹车段6h≤−5U或载具6h≤−8U/metric段多头逐笔净+费覆+快SL≤30%全继承;rollback=prem0.35族基线);#49 verdict存git;#49终数存_exp.verdict+git+§4#49;RECOVER刹线6h空≤−8U继承;#11降级观察(§4);#61a/b史git 0940c62+95e4c9f;**BICO已隔离@08-17 03:2x**(§6结案) |
...
- **[速记·压缩@08-16 15:5x] DELETE不杀进程竞态→幽灵载具(brk 2111f5f9实证)**: DeleteStrategy先删DB行,RemoveStrategy仅Status==running才Kill→竞态窗进程存活成幽灵(API全404不可控,仍收feed开仓,三层出场+互斥闸仍罩);杀灭唯一通路=**后端重启**;幽灵账目单列不入载具归因(行带sid=2111可滤);#53修复fa93736候部署即根治;机制全文(源码行号+时间线)git 766c7d9系〔00:2x初登〕~~复活实证@08-16 21:2x~~**⚖️翻案@08-18 18:2x(证据链三支)**: ①main日志流含全部5笔"幽灵STAR空"的触发开仓信号行(08-16 18:25/08-17 01:37/08-18 02:16+04:04+06:03,秒级吻合closed行开仓时点;且main自08-14起持续有STAR空信号史) ②对照实验:trend日志流仅含ACE/COTI/BICO@在池期=logs端点按sid过滤正确,main流的信号必main所发 ③brk buy-only(08-14铁判据)本就物理排除空单——"复活"叙事自带矛盾未察。18:22"巧合"实为因果:main重启重播种把STAR带回feed,3min后信号。⇒删除后幽灵可验证成交=0;closed行sid=2111=归因条③陈旧映射伪影同型;#53降卫生项;此案=**sid伪影污染叙事第3例**(BICO/收养/幽灵),#56部署优先级↑
...
- **[新增 08-10 18:2xZ,证据升级 08-11 06:4xZ] closed行strategy_id归因**: 专家载具closed行盖sid始于**08-09 19:05**(72h窗127行实证:19:05前全空串);main行至今全空(CTSI 08-09 20:59/MUBARAK 08-10 01:53均空)→主口径=sid,空且属专家池币=按池归属回填,其余=main。⚠️两陷阱实锤@08-11: ①**空sid≠必main**——后端断网重启窗重建的专家行sid丢失(COOKIE 08-11 01:16 +0.35,交易所侧TP成交+重建,按池归brk) ②**段起点污染**——trend段曾误含段前行TUT 19:05+5.13(S24 _exp ts=19:30前)致读数+7.48虚高,真值n5/+1.06;fade曾n9混1笔段前;段评判=sid∧close_time≥_exp.ts双滤。③**非空sid≠该载具开仓(08-16实锤)**——binance_only视图归因=enrichStrategyAttribution按(symbol,close±5min)反查DB行,镜像行双记账下同仓多行谁时间近谁赢:main开的BICO 04:02/07:57被trend镜像行抢归因;且f4848f8后DB main行已带正确sid=8eb182b6,视图main行sid仍空=main真行匹配失败旁证。判据链=开仓时点池归属+载具日志'触发开仓信号'行(ground truth)。根修=入场腿order_id精确反查strategy_orders(exchange_order_id由exec report回写),fa93736候部署(#56)。修正纪律(部署前每轮适用):专家sid行若该币开仓时不在其池→查开仓日志改归真凶。
...
| 18 | M通道 | closed窗口边界播种修复(幻影行族根因) | 幻影三形态全史+标本链见git 26eaf24 #18行(BTW隙5幻影/镜像行/ROBO整60m例) | n/a | **修复部署@08-15**(f4848f8封边界播种;隙型=另一路径未封,形态与矛盾链存git bb3b883);观察4翻案7隙行/观察5同窗0/观察6拉5全旧→观察8@21:2x拉0幻影(45/45全实);行集随重拉波动=懒生成方向;滤行纪律无限期保留;结案重置=隙型机制源码查明或#56部署后复核 →观察14@08-18 18:2x:0/22(连8轮,续至连11)→**观察15@08-19 06:2x:1/22隙型回归**(BEAT L 08-17 08:21-10:18旧窗行无realized_pnl+sid空,非新交易窗;破连11轮清零;懒生成方向一致,滤行纪律有效)→观察16@09:2x:0/16(BEAT隙行随48h窗滚出,新交易窗零幻影续) |
...
| 61 | E7类观察(main) | **main多头溢价墙→#61实验族**:22h零笔分解=多头候选撞门;族史=#61(prem0.35→0.25,门0.80快照口径错)→#61b(→0.20,门0.75,天花板0.76直查)→**#61c在飞@08-18 21:2x(→0.19,门0.74,浮点边界破案:0.750原生档=主流量被0.75精确门拒,§3天花板条补记)** | 三层机制全落源码+日志:0.80不可达/0.75浮点拒/0.76 ATR拦;**+06:18轮现场:低波折扣绑定观测**(9h段多头n=0;06:1x窗上尾0.72×6=HOME/HOLO 0.90因子和×0.8折扣<门0.74=回退路径b证据);**+09:2x轮:0.750档门开流量枯**(q=0.750直查:末次0.750多头分=08-18 21:17门前4连拒[JCT],门开21:24后12h全池零出现;段唯一closed=HOME 0.76档−0.26硬超时=新旧门皆过,**因果带[0.74,0.75)仍n=0**——评期若维持,keep/rollback之辩转'门无害留置vs无效改动清理',路径b折扣校准为下步主候选);全史git d19c710系+ba346aa+0940c62+95e4c9f | 在飞 | 裁决按_exp预注册(eval 08-21 12:30Z或段n≥15;段6h≤−5U或载具6h≤−8U提前回滚;rollback=0.35);判据:eval期段n≥10净>0=keep并议下一步;段仍n=0→议低波折扣校准或epsilon代码修复(非再降门) |
...
| 58 | E9类(main扩仓) | **main pct 0.08→0.10**(config-only;Σ=0.10×5+0.05×3=0.65≤0.75✓;回滚=回0.08) | main载具费覆首次12h/24h双🟢@08-17 09:2x(24h n12/+4.74 avg+0.395=3.8×费 wr83全空侧;12h+0.167=1.6×);硬边界#1按载具判=允许,组口径🔴不锁单载具扩仓但#57在飞收养竞态=fade config接管main赢家仓(BEAT/BLUAI实证)→扩仓入缺陷不洁 | n=12段 | 候选@09:2x;**门=①a451d32部署+#57双行归零观察2轮 ②main 24h费覆🟢维持 ③执行窗现拉复核净为正**;执行走EXP _exp预注册(main槽空闲);**门②失守@13:0x轮**(main 24h avg+0.009🔴/12h−0.485🔴);**门②复得@18:1x**(main24h avg+0.203=1.4×🟢,n4薄);门①a451d32未部署续锁=不可执行;门②三失@08-19 09:2x(main24h avg−0.087🔴,双SL拖累)——门②已3次翻转,n薄窗口下费覆指标高频摆动=按'执行窗现拉'裁,不逐轮追写 |
...
| 08-19 03:11Z | 例行轮(max·补记) | 24h+3.24(8,wr75) | — | HOLD 0原子包(三连);新笔2=AIOT S+0.09/CLO S+0.86(夜班trailing赢);ROUTE plan={};刹车全绿;#61c段5.9h多头n=0,0.750档零现 | 补记@06:2x:该轮commit ceb2d31空提交(0 diff),日志行与§4#62登记均丢,以message为准最小重建 |
| 08-19 00:15-00:3xZ | 例行轮(max) | 24h+4.85(7,wr71) | FGI46 | 0原子包HOLD(零新笔,#61c n=0);ROUTE plan={};刹车全绿 | stub;#61c流量观测2.9h 0.750档零现;全文git 4ee1d38 |
...
| 08-18 15:17Z | 例行轮(max) | 24h+5.57(6,wr67) | FGI41 | 1原子包=EXP#61→#61b空臂修正 prem→0.20(门0.75;根因=天花板0.76);重播种#18清11→feed89;ROUTE plan={};刹车全绿 | stub;全文git 0940c62 |
| 08-18 12:21Z | 例行轮(max) | 24h+6.12(8,wr62) | FGI41 | 1原子包=EXP#61 prem0.35→0.25;重播种#17清13→feed87;ROUTE plan={};#55续锁;刹车全绿 | stub(0.80门口径15:1x证伪转#61b);全文git ba346aa |
|---|---|---|---|---|---|
