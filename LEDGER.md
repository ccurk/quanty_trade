...
| 载具 | id | 原型 | 池 | 状态 | 门/备注 |
|---|---|---|---|---|---|
| main 通才 | 8eb182b6 | 通才(S18-S23栈) | auto·feed87(09:44重播种;AKE/EDEN未入=auto选集未选非阻断;bl15=#46未落) | **running@08-14 09:44Z(3重启全有主=#46三连试,CB清零成本已计)** | pct0.08 sides=[buy,sell] tpl567;_exp=**RECOVER刹线常驻**(6h空侧净≤−8U→重latch[buy];本轮读+3.75✓;**首评08-14 19:00Z**);24h n23/−6.48 wr30:空侧+1.05(BEAT+3.25/AVAAI+2.77/SCRT+0.42 vs BLUAI SL−1.83/H−2.02)vs长侧−7.53=快SL穿刺簇主失血;⚠️sid伪影:STAR/BLUAI行挂brk/fade名,池归属纪律归main(§3) |
| qt-trend-long | 827ffe8c | 趋势动量多 | TUT/BICO/ACE/COTI(**种子同步4币✓@09:41窗**) | **running@08-14 09:41Z(评期裁决PATCH原子窗15s)** | **tpl575(S24)** pct0.05 mcp3 buy trailing on;**S24评期v2到点裁决@09:41Z(累计n16≥15先到)=keep未达(净−1.85<0)∧劣化未触(>−5U)→S24保留(owner定制直令,回滚需劣化证据),_exp转v3常驻监护**:keep结案(随时)=累计净>0∧费覆;劣化=累计净≤−5U→rollback tpl567;canary 6h≤−5U→stop(本轮−0.01✓);**终裁08-16 06:35Z(v2原限期不后移)**;结构=TUT nL2/−3.43(108s闪崩+11m SL)≈全部净亏,ex-TUT n14/+1.58·ACE 4W0L+1.38=模板正边际病根在池;TUT处置只走机械门(trend出=nL≥6∧L<0/隔离=48h≤−4U∧n≥4)不裁量降级;aging_watch=TUT/BICO/ACE;v1/v2全史见git+config._exp |
| qt-fade-short | 21519f1b | 冲高回落空 | 1000CAT/ARC(种子✓;aging_watch全2币) | running@08-13 12:45Z | **tpl576(S25)** pct0.04 mcp2 sell;_exp v2续:累计n8/−2.63零增(现池零成交第4轮,conf峰<0.55门=市况安静),评期=累计n≥15或**08-16 12:50Z**;自动保险6h≤−6U(0✓);⚠️09:04 BLUAI空行挂fade名=sid伪影非fade仓(main的,不计fade累计;§3),fade instance按行sid代管其TP/SL并平仓−1.83=DB驱动监控跨载具代管实证(全组参数恰同=无害) |
| qt-breakout-follow | 2111f5f9 | 突破追动量(S26) | 无(孤儿:COOKIE/DYM/BTR候#46;AKE/EDEN候auto选中) | **stopped@08-14 06:50Z(评判FAIL,verdict-gated勿auto-start)** | _exp closed=段n15/−3.23≤−3U宽容线;诊断=trail0.8%回调帽截赢payoff倒挂(全文config._exp+git);复活=S27新原型周期(#47,门=组费覆🟢+新预注册+重ROUTE灌池;种子3币旧值勿直启);⚠️STAR空3闭+1飞行挂brk名=sid伪影(brk buy-only物理不可能开空=铁判据,实为main空侧;§3) |
...
- 保证金: Σ_running=main0.08×5+trend0.05×3+fade0.04×2=**0.63≤0.75✓**(08-14 09:5x复核,无pct/mcp变更)
...
| #46 | 种子同步 | main blacklist 15→12(释COOKIE/DYM/BTR;12项与对账器desired_bl@09:36精确一致);main symbols保持auto""不写死(写死feed会冻结发现漏斗,原行'symbols顺带写feed'语义弃用) | 08-14 07:0x | open;09:41三连试失败(main stop异步>30s,§3);下轮=stop→轮询90s→PATCH→start单试;AKE/EDEN已不受黑名单阻,候auto选中自回 |
