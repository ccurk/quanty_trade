...
| main 通才(平台名 Meme_合约信号计算引擎_1) | 8eb182b6 | 通才(S4..S23+S30+S31+S33;S32关guard) | auto·feed188@09-17 15:2x(rotate remove FLNC;quarantine∩feed=∅✓) | running | **09-17 15:20Z 刹车态第二步: sides=[sell](CC BRAKE step2,07:52预注册worsen线触发;恢复=双转正后先回[buy,sell]再升lev)/lev2(CC BRAKE 07:52,史10;frozen)/pct0.05(DS熔断锁 lock-1789648635-breaker eval 09-18 12:37Z)/cd1800(DS,Python侧未生效=进程180s)/mcp2(admin 13:15裸改6→2;owner直令10;DS-1待裁,降敞口未纠回)/min_conf0.45(DS,未生效=进程0.35)/lcp0+scp0/tp0.25+sl0.12 ROI口径+use_exchange_tpsl=true/be_atr1.0(CC DS-ROLLBACK 15:20,admin 08:30曾改0)/trailing(1.0/1.2%)/hunger45m(0.08/0.05)/max_hold60/cs五键在位(全单21U地板:95.5×0.05×2=9.6<21)** bl20(隔离19+FLNC TradFi) select_limit200 etw"" tpl998(S33);**_exp=DS锁 lock-1789648635-breaker frozen=[order_amount_pct,leverage](leverage由CC重新append@15:21,12:37续期曾覆盖)**;**_exp_cc=BRAKE step2 禁多 eval 09-17 21:00Z(metric 空头段n/毛/净/wr/饥饿占比+6h/24h钱包净;expect 空头段均净≥0∧6h>−5%∧穿刺<20%;worsen 空头段n≥10∧净≤−3U∨6h≤−8%→TG请owner裁stop;watch_ds be-atr rolled_back+mcp watch-no-metric)**;_ai_task_cc 3条(08:02/10:22/15:21Z),_ai_task_ds空;**Python进程有效值(START 09-16 04:33Z)=min_conf0.35/cd180s(post-BRAKE成交conf 0.36-0.45=25/41单)**;史_exp_cc(BRAKE lev10→2 评判@15:20: expect达成/穿刺2/35,worsen触发)原文git 8705b00版 |
...
| # | 类型 | 内容（含完整意图） | 登记轮 | 状态 |
|---|---|---|---|---|
| DS-1 | owner裁决 | mcp现值6(DS 09-16 22:10)vs owner直令10——两边不动,候owner一句话;裁定后同步_exp/_exp_cc注记 | 09-17 | open(**13:15Z admin PATCH mcp 6→2 裸改无_exp;降敞口→CC未纠回(纠回=刹车态升敞口须RECOVER门);候owner一句话裁10/6/2;_exp_cc.watch_ds已记**) |
...
| DS-8 | owner确认 | 24h income TRANSFER −50U(12h窗无)是否=现货钱包划转;若是→回填BACKEND_SPOT/SPOT_ID起跑canary;钱包口径现≈121U估(avail78.8+保证金41.9) | 09-17 | open |
...
|---|---|---|---|
| **09-17 刹车基线读数(BRAKE lev2起点)** | 07:52起点(24h净≈−66.9U/6h−41.5U=36%钱包/6h逐笔穿刺43/70 −53.36/钱包114.07)全文=git 8705b00版§6。**post-BRAKE读数@15:2x(open≥07:52Z,n35)**: wr51% 毛−4.15 净≈−4.9 均−0.119 穿刺2/−2.58(6%价格SL:UAI/REZ) 饥饿45m 11/−7.41(roi−5.0~−10.1%=价格−2.5~−5%;主血) 硬超时60m 12/+0.87 饥饿赢2/+0.79 TP/trail 8/+4.19;多n20/−3.35 wr45%(饥饿死7) vs 空n15/−0.79 wr60%(饥饿死4);hold中位45.4m p25 29.6 <3m=0;全单21U地板;失血榜UAI−1.33/REZ−1.27/COTI−1.12/牛来−1.01/AIN−0.91;自10:22段 n17 毛−1.98(多9/−1.93 空8/−0.05);五窗income净 1h−1.04/3h−2.23/6h−6.71(=钱包123.2的5.4%)/12h−29.35/24h−78.5(+TRANSFER−50;24h≈−32%已防御→TG🆘);钱包≈116.4(avail95.55+保证金20.9) | 09-17 15:2x | 判据: 空头段(open≥15:20Z)n≥10∧净≤−3U∨6h≤−8%→两侧皆劣TG请owner裁stop;双转正→sides回双向 |
...
| 终态归档集2(4项瘦身@09-17) | 平仓撤单竞态error观测(判据复现≥3独立窗)/热点∩池内盈利连续轮数(n<30轶事级)/S19顺涨拒空veto(KEEP终态;非应拦≥4且净损≥2U复评)/short双窗负(0维持) | 09-17 | 全文=git e9d1b17版§6;判据不变,触发即复活行 |
...
## 7. 运行日志（每轮一行，新行追加在表首）
| 2026-09-17 15:1x-15:3x | **BRAKE第二步·禁多轮**: 管道活(active2空头AIN/LAB 21U;fe=none;avail95.55 钱包≈116.4);五窗income净 1h−1.04/3h−2.23/6h−6.71/12h−29.35/24h−78.5U(+TRANSFER−50;24h≈−32%∧已防御→TG🆘);逐笔48h n361 wr49% −45.42;**post-BRAKE(open≥07:52)n35 wr51% 毛−4.15/净≈−4.9≤−4U=07:52预注册worsen线触发→既定动作 allowed_sides→[sell]**(多n20/−3.35 wr45% vs 空n15/−0.79 wr60%);病根(lev2态)=饥饿45m收割11/−7.41主血(§4新行),穿刺归零2/35;有效门槛反推=成交conf 0.36×1/0.40×2/0.44×18/0.45×4→进程min_conf0.35未变(未重启);**DS段: PATCH 2(12:37熔断续期lock-1789648635 pct0.05→0.05 合规keep,但frozen_keys丢leverage→CC重新append@15:21复读✓;13:15 mcp 6→2 裸改无_exp→降敞口未纠回,watch记,DS-1候owner)/越界0/留言板_ai_task_ds空(桥接未装)/_ai_task_cc第3条**;执行=①PATCH sides=[sell]+be_atr 0→1.0(DS-ROLLBACK:watch线破 open≥08:30 n27毛−6.26)+bl+FLNC(TradFi −4411直证×2)+_exp_cc新预注册(eval 21:00Z)复读✓他键零漂移 ②_exp.frozen_keys+leverage(读改写,他字段一字不动)✓ ③rotate remove FLNC feed189→188 ∩=∅✓;刹车=6h −5.4%未触8%线,24h≥30%已防御→🆘;宏观FGI50/BTC+0.9%/mcap−1.2%未触;trending∩feed=PONS,LIT,NEAR,ONDO,UNI(TRUMP不在);现货候部署(DS-6/DS-8 TRANSFER−50待认);下轮=21:00Z评禁多(空头段)+mcp/DS-9/DS-8催owner+读_ai_task_ds+复核DS续期是否再丢leverage |
