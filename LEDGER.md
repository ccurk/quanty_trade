...
| main 通才(平台名 Meme_合约信号计算引擎_1) | 8eb182b6 | 通才(S4..S23+S30+S31+S33;S32关guard;tpl998) | manual 7 币 G/BULLA/MAGMA/UAI/HEI/DRIFT/ARB(owner 06:5x 直令 8 币, POWER 隔离未入; feed7 ∩bl=∅) | running | **09-19 08:4xZ 现值: pid 55085 START 07:21:21Z 注入 min_conf0.35/cd180/atr 2.0/2.5/warmup50/max_price1e12; pct0.125(DS #7 07:08 砍, owner 值 0.25, 不硬抢; _exp_cc 直令=新段起点 eval 18:00Z 前不动) lev10 mcp6(owner 令 10) sides双向 hunger 1m/tp0.30/sl0.04(owner 09-18 17:59 ROI 4%; 04:53 admin 同键同值候认领) tp,sl pct0 trailing/BE 关 max_hold45 bl42; 新段(07:21 起) n4 毛 +5.26; _exp_cc=OWNER-DIRECTIVE 多策略化(eval 18:00Z 或 n≥30; rollback=filter 300 池); 史 07:3x/05:5x/09-18 14:2x 全文=git 6b44db7 版行** |
| majors(平台名 Majors_BTC_ETH_BNB_SOL) | e725e31a | main fork → **tpl1056 = tpl998 + S35 bar聚合 15m**(CC apply 09-19 08:39:38Z, hash 766b576a, 基线 3e311e8b 复读一致, config diff=NONE) | manual BTC/ETH/BNB/SOL/XRP(XRP=owner UI 加, 无 audit) | **running@08:01:47Z(owner UI start, 无 audit; 越过 07:15 登记的三门)** | 08:4x 现值: lev10 pct0.125 mcp5 cd900 max_hold720 hunger off(0.30/0.30 键在) atr 2.0/2.5 min_atr 0.03 symbol_reentry 15m bar_agg_minutes 15 auto_optimize dry_run; 1m 段(08:03-08:20) 3/3 空 SL 毛 −0.78 费 0.48(SL 距 0.082/0.085/0.216%=费尺度); S35 段首单 08:46:31 SOL 多 conf0.44 名义 156U SL 距 0.88%/TP 1.01%(=首个 15m 封口, 种子预热实证); _exp_cc=EXP S35(CC-28: eval 09-20 08:40Z 或 n≥15; worsen 段净≤−4U ∨ n≥10∧wr<35% ∨ SL 距中位<0.2%); 回滚=POST rollback→998 或 bar_agg_minutes=1+重启; 归档 confident-fermi 92327c4 strategies/quicktrade-e725e31a/; DS 目标 id=main 不碰本壳 |
...
| CC-11 | 巡检 | 全文=git 6b44db7版§5(要点不变) | 09-18 13:1x | open(读数史 git df539c9版; 最新@06:3x: 最大单仓名义 479U(pct0.25), 最大单亏 OP −5.48, 6h ≈−16%/24h ≈−31%(DB 逐笔+费估)) **@07:3x: 最大单仓名义 VTHO 354/ZEST 204U(pct0.25 段尾), 最大单亏 CROSS −5.51/OP −5.48/AR −5.11(均 0.4% 刀 1-2m 内过冲 1.3-1.6%); 6h ≈−27%/24h 净 −109.2U(入金 +142 另列)🆘; DS #7 07:08 砍 pct→0.125 未锁, 不硬抢** **@08:4x: 钱包 160.6(全平); 6h 净 −39.6(−24.7%)/24h 净 −95.9(入金 +142 另列)🆘; 最大单仓名义 majors SOL 156U(pct0.125×mult0.78); 最大单亏 XRP −0.49** |
...
| CC-26 | 备用池机制 | _standby_cc 8 币; 每轮 POST /backtest(sandbox, 1m 18h, 现行 config+sim_clock+taker_fee 0.0005); 连续 2 窗费后>0∧≥5 笔→晋升; core 48h ≤−4U∧n≥4→降回; 前提 S34 验证✓(#56); 阻塞=回测器 #54/55/57 挂起>1h 需重跑 | 09-19 07:1x | open(**@08:4x 校准门: 同窗结果不一致(§3 回测引擎事实)→晋升前须 sim-clock+同窗复现 2 次一致; 本轮未排新回测**) |
| CC-28 | 评判(在飞) | **EXP S35 bar聚合 15m on majors**(tpl1056 @09-19 08:39:38Z; 预注册在 majors 壳 _exp_cc; metric=段 n/费后均净/wr/tpsl 日志 SL 距中位/笔日/首单时刻; expect n≥15 费后均净≥0∧SL 距中位≥0.3%∧wr≥45%; worsen 段净≤−4U ∨ n≥10∧wr<35% ∨ SL 距中位<0.2%→ROLLBACK(rollback→998); 24h 零信号无异常→只报) | 09-19 08:4x | open(首单 08:46:31 SOL 多 conf0.44 SL 距 0.88%; eval 09-20 08:40Z 或 n≥15) |
...
| 计数项 | 读数 | 更新轮 | 备注 |
| **09-19 08:4x 7 币池新段(07:21 起)读数** | n4 毛 +5.26: DRIFT +2.48(交易所 TP, 7m)/HEI +3.55(饥饿 TP roi 30.7%, 6m48s)/G −0.46(饥饿 SL roi −6.9% 过冲 2.9pp, 1m5s)/G −0.30(1m); 刀 2/4; 成交 conf 0.36/0.40/0.48/-; 速率 4/83min≈69/日 ‖ majors 1m 段 n3 −0.78 全空 SL(SL 距 0.08-0.22%) ‖ S35 段首单 08:46:31 SOL 多 conf0.44 SL 距 0.88% | 09-19 08:4x | 复读线不变: n≥8∧刀占比≥60%∧段净≤−4U→TG 再荐 B(hunger_sl 0.30); majors 见 CC-28 |
...
| 2026-09-19 06:1x-06:4x / 05:3x-05:5x / 05:1x-05:5x / 03:3x-03:5x / 00:1x-00:4x / 09-18 20:1x-20:3x | 全文=git 6b44db7 版§7(逐行 git d19a237/4c56565/4c56565/f48d043/c06ab72/c06ab72) |
