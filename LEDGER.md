...
| 55 | E7类(trend) | **EXP:trend long_conf_premium 0.15→0.10**(gate 0.70→0.65;config-only;回滚=回0.15) | 分数墙源码级:ACE多头0.650×9h顶门0.70反复拒发;S24链n33/+0.024空转。**08-20 06:2x双向更新:分带上移(ACE 05:46-47多头0.680含S24续势+0.10,距门0.02;vs 03:18Z的0.28-0.40)但反事实转负——同例10min后−1.4%必触SL(2.5ATR≈1.4%),且trend影子多累计AL0/1+WL3/8=3W/9L(25%wr<影子be≈36%)⇒门0.70当前保护性**。前史git ba346aa+d19c710 | 机制级(gate) | 候选**已降级@08-20**(影子反事实负=解锁预期亏损);执行门不变=trend费覆解禁+影子多wr回到be上;08-18/08-20两审议=续锁;**三重锁死结构注@09:2x**:池陈旧→7.7h零候选连影子0→影子证据无法累积→解锁门不可评估;打破口=①main重开多头产样本(需全新假设)②换池(费覆锁:trend 48h n0无费据=禁扩)③零成本挂机等regime自证=**现行选项,BTC+11.4%日仍零候选实证池死非门死** |
