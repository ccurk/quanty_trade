...
|---|---|---|---|
| 后端DB层挂死watch(平台事故→已升§3@08-27 04:4x) | **连续挂死中@08-27 17:29,自04:31复发起≥12h58m(n=8观测段)**: 03:14首发→03:5x重启→04:01复活~30min→04:31复发;观测段06:28+07:12×16探+09:1x+10:38+12:37+13:56各四层+**17:28-29例行轮四层(login 60s上限挂死实证=最强L3证据)**=签名恒定(鉴权401/1.4s+路由404/1.4s=进程活;login挂=DB死;health/db 404);owner零动作三证@17:2x(main头09f76cf不变+ed0c160未并[**FF零冲突当轮复核✓**105行5文件]+TG getUpdates空);mm线已排除 | 08-27 17:3x | 判据不变:部署#73后再现→health/db秒判+readTimeout日志定位卡死query;owner待办=重启前SHOW FULL PROCESSLIST/INNODB STATUS/df -h;恢复后首轮必做=active核对+closed since04:00盲窗重建+feed漂移复核 |
