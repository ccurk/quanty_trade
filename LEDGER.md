...
|---|---|---|---|
| 2026-09-20 02:1x-02:4x(v6.0 #5) | 上轮(01:27)= [攻-1] owner UI main mcp 1→3(+majors bl 去 BNB/SOL/XRP) / [攻-2] 重发 DS hunger_mode_enabled true | **DS 01:27 后 0 改动**(audit admin 最新仍 23:54:08; 01:27:45/01:28:01 两行=CC 留言板写; config mcp=1 hunger=false max_hold=90 lev=2 pct=0.125 max_mult=0.8; majors bl 仍 [BNB,SOL,XRP]); owner 未动. lev 锁 01:55Z 已到期(DS 可改 lev); mcp/max_hold/pct 冻至 05:54Z. **DS 停摆定案**: MIN_TRADES=30 对 6h pair_count(ops/deepseek_optimize.py:72/439), 6h=31@02:16, DB 20:16-21:16 平仓 16 笔将滚出 ⇒ ≤~02:30 起每轮 [skip]; mcp1+90 钟下 6h 上限 ~4 ⇒ 05:54 解冻后 DS 也不会动 mcp(自锁→样本饥饿→永不回头 = 死锁, 只 owner 能破) | **效果**: 无法评(0 改动; 01:27 后 0 平仓 0 开仓). 北极星 15m 段(21:16→) DB n7 +0.50 均 +0.072 wr71% 7/7 多; mcp2 子段(21:16-23:31) 7 开/2h15m=3.1/h vs mcp1 子段(23:39→02:16) 1 开/2h37m=0.38/h(PUNDIX 01:01:40, SUI 平 01:01:35 后 5s 补位); mcp1 子段已平 n2 +0.68 均 +0.34. 槽位=瓶颈实证: 15m 段候选被槽拒 6 笔(21:16 SUI 1.216 / 22:01 A 1.314+JOE 1.044 / 23:31 ARK 1.341+TRB 1.258+EUL 0.748), 每 bar ★信号 107/146/147(01:46/02:01/02:16) maxconf 1.000 全弃. 五窗(交易所含费 02:16Z): 1h 0(费 −0.08) 3h +1.21(n5 wr80%) 6h −0.47(n31 费 0.82) 12h −9.53(n150) 24h −49.05(n487 费 18.07; 空 227/−37.74 wr25% vs 多 260/−11.37 wr47%); avail 116.1; 持仓 2(main PUNDIX 多 01:01 +0.26 / majors ETH 空 01:16 −0.13). **新病根(源码+日志)**: strategy_signal.go:660-679 批级 openCount 把交易所全账户持仓计入(isAllowedSymbol manager.go:319 对 symbols="" 恒 true, 不看 bl) ⇒ majors ETH 空(720 钟, 至 13:16Z)占 main 唯一槽: 01:16:34 "已持仓2个，达到最大并发仓位1"=PUNDIX+ETH 实锤; 02:16:54 "已持仓1个"=FetchPositions 恰 429(02:16:55 守护跳过)漏计. PUNDIX 90 钟到期 02:31:40 后 main 0 自有仓仍 openCount=1≥mcp1 ⇒ 0 开仓直至 ETH 出场. **02:31 实测**: 02:31:01 "已持仓2个"(PUNDIX+ETH) → 02:31:45 PUNDIX max_hold_timeout 平 roi +1.04% pnl +0.20(DB +0.18; 撤腿 canceled=2 found=2) → 02:31:50 "跳过本批信号：当前已持仓1个，达到最大并发仓位1"=main 自有仓 0, 1=majors ETH ⇒ **main 自 02:31 锁死至 ETH 出场(腿/13:16Z)**, 02:31 bar PUNDIX 1.000/RAYSOL 0.748 弃; ETH 空 02:36 +0.84. 429 澄清: q=429 命中多为数字子串, 真 429(守护跳过) 3 天 37 行≈1/h, 非慢性; 02:16:55 一次与 CC 24h ctx 拉取同秒(下轮 24h 窗只拉一次). 止损核对: PUNDIX 腿 tp 0.11773/sl 0.11431(order 4000001911725447)+90 钟; ETH 腿 tp 2605.35/sl 2631.58(order 4000001911744015)+720 ⇒ 均非裸奔; 隔离: 同币互斥闸在(position.go:201 accountSymbolOpen + signal.go:630 otherHeld), 无撞车. majors 01:16 后 0 信号(bl 拦 SOL/BNB/XRP 空 6 次). **本轮建议(板 02:37:30Z, 3 条 8.2KB; TG 10806)**: [攻-1] 重发 owner UI main mcp 1→3(实际 main 2 自有槽+ETH 占 1) 验 02:31-03:31 开仓数≥2·"达到最大并发"行 已持仓≥3·15m 段 n≥15 均净≥0; [攻-2] owner 改 ops/deepseek_optimize.py: MIN_TRADES 30→10 + 事实#4 改写("mcp 非瓶颈 7-8/10"=1m 时代旧事实, 现 1/1 满占+每 bar 100+ 信号) 验 DS TG 由 "跳过" 变 "apply" + audit admin 行重现; 携带项: hunger_mode_enabled true(DS 恢复后首改). 0 交易键写入(PATCH 仅 _ai_task_cc, 其余键 diff NONE). FGI 71; 市值 24h −3.7%; BTC 81113; trending∩池 TRUMP/AVAX/ENA/PENGU/ZEC/NEAR |
