...
| 载具 | id | 原型 | 池 | 状态 | 门/备注 |
|---|---|---|---|---|---|
| main 通才 | 8eb182b6 | 通才(S18-S23栈) | auto·feed89(种子bl13=期望态✓@08-15 03:20换CAP入H;CAP回发现漏斗;overlap0) | running | pct0.08 sides=[buy,sell] **long_premium=0.35(FIX@08-14 18:47Z)** tpl567;_exp=**FIX长溢价0.15→0.35**(long_thr0.70→0.90;证据=48h长侧n20 wr30/−11.79过20笔门,病根=逆势栈0.70恰等长门,S12先例;keep=08-15 19:00Z长侧24h净≥−2U∧组均净较−0.216改善;劣化=长侧≤−6U或组净恶化→回0.15再议[sell]latch;全文config._exp)+**继承RECOVER刹线**(6h空侧≤−8U→重latch[buy];03:2x读−1.29未触);**FIX段读@T+8.4h(03:2x轮)**:长侧n2/−1.44持平(节流在效),空侧n9/−7.60wr11(+ALICE饥饿±3.95/−1.23+AIO#30型−2.96)=**未触路径(空侧)仍是段主刀**;长侧劣化线−6U未触;19:00Z评判提示:keep门两臂判读按侧拆臂说理,03:2x参考=24h组均净−0.181已优于基线−0.216∧长侧24h−1.44≥−2U(两臂现值双过,以19:00Z当值为准);种子同步03:20落地(10s窗+重播种热rotate−11自愈,§7);sid伪影纪律照旧(§3) |
| qt-trend-long | 827ffe8c | 趋势动量多 | ACE/TUT/BICO/COTI(feed4=种子4✓@08-15 03:20 CAP种子卸载;aging_watch=TUT/BICO advisory) | running | **tpl575(S24)** pct0.05 mcp3 buy trailing on;_exp v3常驻:**累计n24/−0.61**(+BICO快TP+0.41@08-15 01:37);keep门净>0✗∧费覆🟢24h+0.149 vs 0.091=**1.64×首次达标**;劣化−5U/canary−5U(现6h+0.55)未触;**终裁08-16 06:35Z当值净>0=keep**;ACE 48h+3.44/COTI+3.56双扛旗∧ACE trending;全史git+config._exp |
| qt-fade-short | 21519f1b | 冲高回落空 | 1000CAT/ARC(种子✓;aging_watch全2币) | running@08-13 12:45Z | **tpl576(S25)** pct0.04 mcp2 sell;_exp v2续:累计n8/−2.63零增(现池零成交**第10轮**,want.fade=[]连续;非载具故障[03:23统计行在读,ARC空头分0.48近0.55门]);评期=累计n≥15或**08-16 12:50Z**(明日到点n8<15将再触证据不足格→届时议池补灌或停驶取舍);自动保险6h≤−6U(0✓) |
...
- 互斥不变式: feed层✓(overlap=0三running:main89/trend4/fade2@08-15 03:2x种子同步+自愈后复核)>**种子层=期望态✓@08-15 03:20**(main bl13含H无CAP;trend种子4币;CAP释回main发现漏斗;重启免疫达成)>⚠️引擎互斥闸未部署;aging=trend2/fade2 advisory;重播种规律第3例再实证:start后feed播回auto100含bl币(信号层bl在防零交易风险)→热rotate−11自愈;pre-restart IDLE行93 vs API91=频道层订阅残留伪影(重启自清,观测勿当违规)
...
| 30 | S21类(代码gate) | 追跌空反弹拒空veto候选(gate7e,暂缓中): short且跌势陈旧拒;draft单维300m阈值族**证伪@08-07**(误杀24%>10%门,收紧复审仍12%,−10~−14带无分离力);下一迭代=加第二判别维(距60m低点反弹幅/自低点时间) | 追跌型累计10例/−24.28+晨午簇候审15(vision T+1定标待做)〔审计全史见git 121e975 #30行〕 | **n=13/−30.68**+候审15·暂缓(+1@08-15 03:2x CG5m定标:AIO 00:35空,60m低点6m前已出∧入场已反弹+2.41%∧前120m实为+5.8%拉升中回调→2m后+9%反抽轧死−2.96=双维判别[距低点反弹幅+自低点时间]正面实证;+2@08-14 20:1x AIO/DOLO案全文git 42e4fc8 #30行) | 执行门=确认型≥20或owner核准;影子通道S23 WS vs AS已随#37冻结;误杀审计先行原则保留 |
...
| main无端重启重播种观测 | **1@08-13(22:5x-00:18窗feed89→100;rotate源码排除非重启回加路径[strategy_symbols.go:feedSymbols仅rotate/start改写]→必经进程重启;窗内零apply零start API;audit盲区=start/stop不写;领先假设=owner后端重启/部署BOOT RESTORE)** | 08-13 00:2x | 复现→升§3候证平台事实+评估counter清零对评期锚影响;黑名单闸在防=零实仓风险;每轮复核feed数(08-14 21:2x复核92→91=有主H隔离;08-15 00:2x复核91✓无新无主重启) |
...
> 瘦身注 08-15(03:4x轮更新)：更老运行日志行随轮裁剪(08-14 09:28轮S24评期v2+14:38轮CAP晋trend/#46落地要点存git a3804f3;08-14 06:41轮brk评判FAIL要点存brk _exp+§1.5+#47;08-14 00:32轮v1.3对账器要点存§1.5对账器行),全史见 git 历史。