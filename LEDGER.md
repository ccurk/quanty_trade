...
|---|---|---|---|
| 后端DB层挂死watch(平台事故→已升§3@08-27 04:4x) | **n=3连续中@08-27 07:12**: 03:14首发→03:5x重启→04:01复活仅~30min→04:31复发;06:28四层探针(鉴权401/1.4s+静态404/0.4s活;templates 15s挂;health/db 404)+06:30-07:12守望16探/150s间隔全挂=**连续挂死≥2h41m非间歇**;#73未部署双证(health/db 404+git未并main);mm做市线排除(marketmaker包零DB/gorm引用=纯内存+Gate API) | 08-27 07:1x | 后续判据不变: 部署#73后再现→health/db秒判+readTimeout日志定位卡死query;owner待办=重启前SHOW FULL PROCESSLIST/INNODB STATUS/df -h(TG7749);恢复后首轮必做=active核对+closed since 04:00盲窗重建 |
...
| 08-27 03:1x-04:4xZ | 例行轮(max)·**事故+恢复+复发轮** | income:1h0/3h0(挂死空窗)/6h−0.18/12h−2.20/24h净−0.14(REAL−0.48佣−0.22资费+0.56;rows n9 wr55.6 毛均−0.053组🔴) | FGI71 Greed/BTC78830−0.2%/mcap−2.66%无降压 | **HOLD 0原子包;事故=后端DB层全局阻塞(进程活,login+一切触DB端点永挂):03:14发现→三层探针定位→TG🆘7747→03:5x重启(疑owner响应)→04:01 API复活,重试28次/47min守到;ROUTE#56零差分(连4)** | 三查全过:①窗内零成交②feed83/5/1零泄漏=**bl19+种子写回重启免疫首证(免热清)**③旧壳未误活;audit1774无新;**DB=MySQL**(首报SQLite假设撤回;机制/影响面全文§3新条);EXP提频档n1→n2:**TAC空00:17 conf0.6000 24m TP毛+0.532**(资费正向空bonus,对照ONG负资费多bonus=病理不对称,S30仅治多侧方向正确),档净−0.298远劣化线,评18Z;磁铁era n2/−2.83无新;v2段n6签名域n0(auto-C~41h);钱包168.93(168.41+TAC✓);warmup 8/35@04:00→04:27完;**04:31回访:挂死复发(n=2,重启仅续命~30min,04:37仍挂)→watch升§3+M通道#73补丁(claude/dev-db-timeouts:DSN超时+health/db探针,build/test✓)推送候部署,TG7749取证清单+7750部署令**;复发窗敞口≈0(挂前0持仓+warmup恰完);待落0 |
...
|---|---|---|---|---|---|
> 瘦身注(压缩@08-27 03:2x): 行裁全文git指针: 08-26 00:1x=4fc0468/03:1x=1ae847c/06:3x=e00be39(sid纠偏BOOK行滚出@08-27); 08-25 21:1x=5da0604/06:2x=12699f5; 08-24 18:1x=17251a9/15:1x=eaf1bcd/09:1x=040d80d^链+12699f5/06:2x=b505652/00:1x=02d3fb5+c1f6f24; 08-23前压缩史见git(a2e845f/cb2fc25等)。
