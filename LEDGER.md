...
|---|---|---|---|
| 2026-09-20 05:1x-05:3x(v6.0 #8) | 上轮(04:26)= [攻-1] hunger_mode_enabled=true + hunger_stop_loss_pct=0.20 / [攻-2] owner UI mcp 1→3(第 4 次) | **DS 23:54 后仍 0 改动(第 5 轮; audit admin 最新 03:31:57 #1950; _exp 最新 lock-1789862048; 我的 #1952 04:27 只含 _ai_task_cc)**. **源码定案(ops/deepseek_optimize.py 仓库版)**: L72 MIN_TRADES=30 ∧ L69 WINDOW_HOURS=6(env QT_WINDOW_HOURS) ⇒ 6h n=13<30 ⇒ 每 15min [skip]; mcp1 下 ≈13 笔/日 ⇒ 6h 永到不了 30 ⇒ **DS 被自己 23:39 设的 mcp1 锁死**; L110 BOUNDS hunger_mode_enabled=(0,0) ⇒ **DS 结构上不能开 hunger, 前两轮 [攻-1] 是 DS 执行不了的建议(改列 owner 项)**; L531 Σpct×mcp≤0.75 ⇒ pct0.25 下 DS 最多 mcp3. DS 锁到期 mcp 05:39Z / max_hold 05:54Z / pct(breaker) 21:52Z. 04:21:15Z 两实例再"开机自恢复"重启(runtime …075139/…075187; origin/main 仍 7a14c46=非新部署) | **效果**: 北极星无新读数(04:00 后仅 ROSE 04:46 钟出 +0.41 毛/+1.11% 价 ROI 2.26%). 15m 段(open≥21:12Z) n10 毛 −1.47 均 −0.147 wr70%(BANK 3 笔 −3.30; 去 BANK 7 笔 +1.83); 出场=钟 8(45m×4 −0.37 / 90m×4 +1.07 全赢均 +0.27U)/SL 腿 1(BANK 02:46 −6.6% 价 −2.57)/TP 腿 1(POWR 43m +1.04%). 逐笔 05:17Z: 1h 1/+0.41 3h 4/−1.00 6h 8/+0.43 wr75% 12h 53/−4.30 24h 131/−13.21(24h ctx income 429 一次); avail 118.6(=可用; 钱包≈150, BIGTIME 保证金 30 锁); 持仓 1: **BIGTIME 多 04:46:15 名义 300U(首笔 lev10×pct0.25×mult0.8=avail150×0.2×10; 上笔 38U ×8) 腿 tp .0083598(+2.4%)/sl .0080059(−1.93%≈−5.8U) 05:30 −1.85(−0.61% 价) 90 钟到 06:16**; majors 0 仓(ETH 空 01:16-02:40 +0.975). 槽位实证: 04:46 批 113 条★(24 条 conf=1.00/34 条 0.8-1) 唯一槽给 conf 0.591 BIGTIME(04:46:14.858 ROSE 04:46:10 腾槽后首到; strategy_signal.go:592-600 按 rr 排 v40 恒 1.25 平局). **OKX 1m 回放(fapi/bybit 地理封锁, vision 09-19 未发布; 仅 SUI/ATOM/VANA/BIGTIME 有数)**: SUI 90m 出 +0.79% 但 47m 到 +1.0%(MFE +1.36%@72m); ATOM 45m 出 +0.99%, 34m 到 +1.0%(MFE +2.58%@76m, +1.5%@48m, +2%@63m); VANA 90m 出 +0.50%, 83m 到 +1.0%(MFE +1.09%); BIGTIME 42m MFE +0.34%/MAE −0.86%; 四笔 MAE 均 >−1%. 止损核对: BIGTIME 腿✓ max_hold 90; majors 0 仓 720; 无裸奔; 隔离无撞车(openCount 全账户计数 ⇒ majors 开仓即锁死 main mcp1). **本轮建议(板 05:31:27Z 3 条 5973 字符 弃 02:37 条; TG msg 10822)**: [攻-1] owner UI(DS 禁) hunger 三键 enabled=true/hunger_tp 0.10/hunger_sl 0.20(ROI, lev10=+1%/−2% 价=+3U/−6U@300U; 取小只紧不松; 三键同改, 只开 hunger 留 sl 0.04=−0.4% 价必被噪声打) 验 "饥饿模式触发…tp=10.00% sl=20.00%"·持仓中位<60m·单笔亏≥−6.5U·段均净≥+0.15; 冲突已呈: owner 09-19 "固定%刀必被第一波打掉"(1m 时代) vs 15m 赢单 MAE≤1.29%; [攻-2] owner 解 DS 死锁 ①UI mcp 1→3(第 5 次; 900U 名义/90U 保证金≈60% 钱包, 单波最坏 −18U) ②宿主 cron env QT_WINDOW_HOURS 6→24 或 MIN_TRADES 30→10 验 audit admin 新行·开仓/h≥2. 0 交易键写入(diff=[_ai_task_cc]). FGI 71; 市值 24h −4.2%; BTC dom 59%; trending∩池 ZAMA/ENA/PONS/ZEC/AVAX/ARB/PENGU/AAVE/INJ(ONE/NEAR/AR/AKE bl) |
