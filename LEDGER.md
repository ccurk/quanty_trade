...
| 载具 | id | 原型 | 池 | 状态 | 门/备注 |
|---|---|---|---|---|---|
| main 通才 | 8eb182b6 | 通才(S18-S23栈) | auto·**feed86@08-17 12:57**(BEAT/XNY隔离热移88→86;bl种子仍13,bl15同步被§3全账户空仓铁律挡窗→§5#59) | running | pct0.08 sides=[buy,sell] long_conf_premium=0.35 tpl567;**trailing(act1.0/cb1.2%)+BE(1.0)=基线几何(#49终裁KEEP@08-16 18:2x)**;_exp=closed(verdict 628字存config._exp.verdict+git);**实验槽空闲**;#49终数=段n30/−3.81/均净−0.127(5.7×)/SL率20%/赔率0.43裁=统计失真keep(裁由全文_exp+git+§4#49);段亏全额=BICO×4−3.85(§6),ex-BICO n26/+0.04;RECOVER刹线6h空≤−8U继承;#11降级观察(§4);后续实验以本段为基线;**BICO已隔离@08-17 03:2x**(§6结案;隔离后零新笔✓复核09:2x) |
| qt-trend-long | 827ffe8c | 趋势动量多 | ACE/TUT/COTI(feed3=种子3✓;**aging_watch=COTI/TUT advisory@08-17 13:0x**,ACE重达标出列[48h L n7/+1.66 wr71%];S24 keep后漂移观察→24h n3/+0.18转正,COTI多在飞12:40) | running | **tpl575(S24)** pct0.05 mcp3 buy trailing on;**归因修正@09:2x**:BICO 04:02(+0.57)/07:57(−1.90)两行盖trend sid实为main开仓(trend日志零开仓行,main日志全链;S24 keep判据'36h零新笔'修正后仍真);正牌08-16=ACE+0.02@06:46+COTI多在飞@09:30;**_exp=closed@06:35:12Z: S24终裁keep结案**(预注册当值格'净>0=keep':链n33/+0.024>0;末笔08-15 18:17 BICO−0.91,终裁前36h零新笔=03:11后零笔复核@06:35;程序=真空仓窗stop[poll1沉降]-PATCH-start,种子3币完好);**常驻保险(继承canary)=滚动6h净≤−5U→stop本载具**;实验槽空闲;候选EXP降prem=§4#55(费覆红线内只登记) |
...
| # | 类型 | 内容（含完整意图） | 登记轮 | 状态 |
|---|---|---|---|---|
| 59 | ROUTE种子同步 | main symbol_blacklist 13→15(+BEAT/XNY;全表=4/ACE/AIO/APR/BEAT/BICO/BMT/BTW/COTI/CYS/H/TST/TUT/XNY/龙虾,各/USDT形态)。隔离已feed级生效(rotate 12:57),此项只补重启免疫 | 08-17 13:0x | **被§3铁律挡窗**:main stop需全账户空仓,COTI(trend)在飞→2次尝试不沉降(§3第4例);重试=每轮查**全账户**空仓窗一气呵成,单发不连发;若先遇重启重播种→heatclean补刀+对账器自愈 |
