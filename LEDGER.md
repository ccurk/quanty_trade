...
| 2026-09-18 | **止损 ROI 4% 直令(live 17:5x-18:0x)**: owner "止损在收紧 调整到 3%"→"变成 4%"→"我看现在的单子都是在 10% 亏损的时候才平掉"; **口径=ROI(UI)**; owner 17:59:41Z 自落 hunger_stop_loss_pct=0.04(v59); CC 误读价格距离(v57/58)已纠; 三条路 ①ROI 4% ②pct 减半 ③看 30 笔; 引擎钳 0.3/lev 与 4% 无关; 全文=git c06ab72版§1 |
...
- **反事实数据源@09-18 00:2x**: data-api.binance.vision /api/v3/klines 1m 现货作永续代理(容器可达; fapi/api.binance.com 451; 无现货的 meme 币 400→Bitget/Gate 永续); 归一化以入场分钟前一根现货收盘为 p0; 方法 cf2.py/preentry(scratch); 全文=git c06ab72版§3。
...
| H-0919c | 空头信号本窗口无正期望且机制未定位: 6h 空 164 腿 −31.5 wr28% / 24h 现货可核 n45 −24.7; 入场前动量赢输无差、BTC 平、括号反事实无改善 ⇒ 候选机制=(a)空头评分组件(RSI>70/上轨)在 meme 池的均值回归失效(b)多空比/资金费项反向(c)单向持仓下空头被多头回补挤压; 需 detail 组件级归因(signal detail 不落后端日志→需 apply 加 emit 详情或离线重算) | 09-19 05:5x | 门=组件级归因 n≥30 后再写门; 期间 owner 裁 sides/短侧溢价 |
...
| CC-11 | 巡检 | pct 配置下每轮报: 保证金占用峰/最大单仓名义/最大单笔SL亏/6h,24h 钱包%; DS breaker 砍 pct→按 owner 令处置并TG | 09-18 13:1x | open(**@09-19 05:5x: 并发峰 3 仓/824U 名义/保证金 82U=46% 钱包@05:13; 最大单仓 480U(XTZ); 最大单亏 OP −5.48(429 平仓失败); 6h −14.7%; 史读数=git c06ab72**) |
