...
| 2026-09-17 | **现货载具直令(live会话,08:2x)**: owner原话"增加现货策略。下单，写到 promt 中。"。源码取证: market进程级(exchange/binance.go:129-141从conf/env读一次;Manager.GetExchange()单例manager.go:1867)→现货=第二后端进程(独立DB/Redis);现货可用=市价买/api/v3/order(binance.go:1339-1425)+市价卖平仓closeSpotPosition(strategy_execution.go:478-540)+本地逐仓TP/SL监控(strategy_position.go:859-973,2s轮询1m收盘);不可用=饥饿/max_hold(quick_trade_monitor.go:41)/ROI(strategy_roi_monitor.go:72)/交易所侧TPSL·追踪·保本(strategy_tpsl_monitor.go:105)/WS守卫/收养/余额比例sizing(strategy_position.go:107-112);"数量<10跳过"地板(:118-124);信号无反转平仓+allowed_sides门在前(strategy_signal.go:821)→出场只有TP/SL;python amount=cfg.trade_amount静态300→S34改名义/现价。交付: prompt v5.1【现货载具】节(部署配方env覆盖BINANCE_MARKET/BASE_URL/WS_BASE_URL+DB_NAME/REDIS_DB;载具规格qt-spot-long buy-only/12U名义/mcp3/cd300/mc0.60/atr3.0-1.5;S34规格;超时缺口M候选dev-spot-maxhold;劣化线段净≤−3U∨n≥10∧wr<35%→stop;现货刹车6h≥5%)。状态=候部署(owner部署第二进程+划转后由cron建壳canary)。 |
...
| main 通才(平台名 Meme_合约信号计算引擎_1) | 8eb182b6 | 通才(S4..S23+S30+S31+S33;S32关guard) | auto·feed188@09-17 20:2x(quarantine∩feed=∅✓复核) | running | **09-17 15:20Z 刹车态第二步: sides=[sell](CC BRAKE step2,07:52预注册worsen线触发;恢复=双转正后先回[buy,sell]再升lev)/lev2(CC BRAKE 07:52,史10;frozen)/pct0.05(DS熔断锁 lock-1789648635-breaker eval 09-18 12:37Z)/cd1800(DS,Python侧未生效=进程180s)/mcp2(admin 13:15裸改6→2;owner直令10;DS-1待裁,降敞口未纠回)/min_conf0.45(DS,未生效=进程0.35)/lcp0+scp0/tp0.25+sl0.12 ROI口径+use_exchange_tpsl=true/be_atr1.0(CC DS-ROLLBACK 15:20,admin 08:30曾改0)/trailing(1.0/1.2%)/hunger45m(0.08/0.05)/**max_hold240(admin 15:16裸改60→240,无_exp;watch_ds线=段open≥15:16Z hold>60m子集n≥8∧净≤−2U∨段净≤−4U→回60,eval 09-18 15:16Z;DS-10候owner认)**/cs五键在位(全单21U地板:95.5×0.05×2=9.6<21)** bl20(隔离19+FLNC TradFi) select_limit200 etw"" tpl998(S33);**_exp=DS锁 lock-1789648635-breaker frozen=[order_amount_pct,leverage](leverage由CC重新append@15:21,12:37续期曾覆盖)**;**_exp_cc=BRAKE step2 禁多 eval **顺延 09-18 01:00Z**(20:2x预读n6<10样本不足,指标不换;metric 空头段n/毛/净/wr/饥饿占比+6h/24h钱包净;expect 空头段均净≥0∧6h>−5%∧穿刺<20%;worsen 空头段n≥10∧净≤−3U∨6h≤−8%→TG请owner裁stop;watch_ds be-atr rolled_back+mcp watch-no-metric+max_hold watch)**;_ai_task_cc 4条(08:02/10:22/15:21/20:2xZ),_ai_task_ds空;**Python进程有效值(START 09-16 04:33Z)=min_conf0.35/cd180s(post-BRAKE成交conf 0.36-0.45=25/41单)**;史_exp_cc(BRAKE lev10→2 评判@15:20: expect达成/穿刺2/35,worsen触发)原文git 8705b00版 |
...
### 双执行体(09-17 起)
- **DeepSeek管线事实**: 宿主cron ops/deepseek_optimize.py(experiment_hold冻结键)+ops/qt_breaker.py(熔断: 净≤−4U 或 wr<45%∧净<0→单向砍pct,锁24h每触发续期,自到期);admin#1;节拍≈3h(audit 16:12/19:09/22:10/04:09)+熔断(06:18/06:23)+探针(05:04 _probe_running_check 写+null删);只动config不动代码;改动史09-16 13:33起: lev10×2/pct0.15(越#8 87min→0.075自纠)+sl0.12+tp0.25(owner令"一次性修改完毕上线")/cd300→600→1200→1800/mcp10→6/min_conf0.45/pct0.05(熔断)/**09-17节拍外单键裸改三连: be_atr1→0(08:30)/mcp6→2(13:15)/max_hold60→240(15:16),均无_exp(DS或owner手改待认);09-17 16:12/19:09节拍无PATCH(锁态experiment_hold或无改动,不可见)**。_exp schema: id(lock-*/exp-*)/status/changed/frozen_keys/eval_after/prior_exp(嵌套保留上一实验)/mechanism/why。
...
- **现货引擎事实@09-17(源码)**: market进程级(binance.go:129-141;GetExchange单例manager.go:1867)→现货须第二进程;可用=市价买(binance.go:1339-1425,LOT_SIZE/NOTIONAL过滤,市价单自动抬到最小名义)+市价卖平仓(strategy_execution.go:478-540)+本地逐仓TP/SL(strategy_position.go:859-973);不可用=饥饿/max_hold(quick_trade_monitor.go:41)/ROI(roi_monitor:72)/交易所侧TPSL·追踪·保本(tpsl_monitor:105)/WS守卫(:42)/收养(lifecycle:251)/余额比例sizing(position:107-112)/杠杆;开仓要求tp/sl(position:90);"数量<10跳过"(position:118-124);无反转平仓且allowed_sides门在前(signal:821)→python发不出卖出平仓;amount=cfg.trade_amount静态(python _emit_signal:1505)→现货当基础币数量;funding非usdm返0→S30/S33/S28失活;env覆盖conf.go:522-530(BINANCE_MARKET/BASE_URL/WS_BASE_URL),deploy脚本BACKEND_PORT/DB_NAME/REDIS_DB/REDIS_PREFIX。
...
| **lev2态饥饿45m收割=主血(候反事实)** | 09-17 15:2x | post-BRAKE n35: 45m时roi≤−5%(价格−2.5%)的11笔被饥饿SL收割−7.41U(均−0.67,含roi−7.5~−10%即价格已−4~−5%),45m时带内的12笔60m硬超时净+0.87(持平),trailing赢8/+4.19;6%价格初始SL仅触2笔→lev2态出场结构=45m前几乎无SL,45m一刀。候选杠杆: hunger_stop_loss_pct 0.05→0.03(价格−1.5%)或hunger_after 45→30;**门槛=T+1 vision 1m K线反事实(11笔在30m/−1.5%处是否已破+带内12笔是否被误杀)**,无反事实不动;禁多先行(空头饥饿死4/15 vs 多头7/20) | 逐笔n35 | 候证据 |
...
| # | 类型 | 内容（含完整意图） | 登记轮 | 状态 |
|---|---|---|---|---|
| DS-1 | owner裁决 | mcp现值6(DS 09-16 22:10)vs owner直令10——两边不动,候owner一句话;裁定后同步_exp/_exp_cc注记 | 09-17 | open(**13:15Z admin PATCH mcp 6→2 裸改无_exp;降敞口→CC未纠回(纠回=刹车态升敞口须RECOVER门);候owner一句话裁10/6/2;_exp_cc.watch_ds已记;20:2x仍2,TG提醒#2**) |
...
| DS-6 | owner部署(现货) | 第二进程quanty-spot: server_deploy_docker.sh env覆盖 BACKEND_PORT=8081 DB_NAME=quanty_spot REDIS_DB=1 REDIS_PREFIX=qts BINANCE_MARKET=spot BINANCE_BASE_URL=https://api.binance.com BINANCE_WS_BASE_URL=wss://stream.binance.com:9443;API key开现货权限;现货钱包划转(≤期货钱包30%);反代域名;建claude_cron;自检dashboard account.market=spot;填prompt环境节BACKEND_SPOT/SPOT_ID | 09-17 | open |
...
|---|---|---|---|
| **09-17 刹车基线读数(BRAKE lev2起点)** | 07:52起点(24h净≈−66.9U/6h−41.5U=36%钱包/6h逐笔穿刺43/70 −53.36/钱包114.07)全文=git 8705b00版§6。**post-BRAKE读数@15:2x(open≥07:52Z,n35)**: wr51% 毛−4.15 均−0.119 穿刺2 饥饿45m 11/−7.41主血 硬超时12/+0.87;多n20/−3.35 vs 空n15/−0.79;全文=git 6c1bd6a版§6。**禁多段读数@20:2x(open≥15:20:37Z,n6 全空头=sides生效)**: wr67% 毛−0.16 均−0.026 穿刺1/−1.23(AVA 16:38 −5.92%价=6%SL) 饥饿SL 1/−0.57(BULLA 49.7m) 饥饿TP 1/+0.83(AVA 115m roi8.0%,赖max_hold240) 早TP/trail 3/+0.81;全单≈21U;成交conf 0.44×2/0.52×2/0.56/0.72/0.75(进程min_conf0.35仍有效);五窗income净 1h+1.05/3h+0.45/6h−0.94(=钱包116.7的0.8%)/12h−7.17/24h−43.06(≈−27%,TRANSFER已出窗;24h rows n161 wr45% 毛−31.84 均−0.198 估费13.1);钱包≈116.7(avail106.38+保证金10.34);post-BRAKE全段(open≥07:52)n43 wr56% 毛−3.93 | 09-17 20:2x | 判据: 空头段(open≥15:20Z)n≥10∧净≤−3U∨6h≤−8%→两侧皆劣TG请owner裁stop;双转正→sides回双向;eval顺延09-18 01:00Z |
...
| 终态归档集2(4项瘦身@09-17) | 平仓撤单竞态error观测(判据复现≥3独立窗)/热点∩池内盈利连续轮数(n<30轶事级)/S19顺涨拒空veto(KEEP终态;非应拦≥4且净损≥2U复评)/short双窗负(0维持) | 09-17 | 全文=git e9d1b17版§6;判据不变,触发即复活行;**平仓撤单竞态新亚型·粉尘残量 n=1@09-17 19:52 AVA**(饥饿TP平仓后残量pnl0.0012仍在,19:52:34 TPSL监控"已补设"委托,19:54二次饥饿TP归零;无实害;≥3例→升§4/M通道) |
...
## 7. 运行日志（每轮一行，新行追加在表首）
| 2026-09-17 20:1x-20:3x | **BRAKE step2续·禁多评判预读轮**: 管道活(active1空头AIN 20.7U;fe=none;avail106.38 钱包≈116.7);五窗income净 1h+1.05/3h+0.45/6h−0.94(−0.8%)/12h−7.17/24h−43.06(≈−27%;TRANSFER出窗);48h n361 wr50% 毛−43.76;**禁多段(open≥15:20:37Z)n6 全空头✓ wr67% 毛−0.16 均−0.026 穿刺1/−1.23 饥饿SL1/−0.57 饥饿TP1/+0.83(AVA 115m) 早TP3/+0.81→expect 2/3(均净边际✗) worsen未触 n<10→eval顺延09-18 01:00Z 指标不换**;成交conf 0.44×2/0.52×2/0.56/0.72/0.75→进程未重启 min_conf0.35仍有效;**DS段: PATCH 1(15:16 max_hold 60→240 裸改无_exp 节拍外→watch_ds 线=hold>60m子集n≥8∧≤−2U∨段≤−4U→回60;16:12/19:09节拍无PATCH)/越界0/lock frozen=[pct,leverage]保留✓/_ai_task_ds空/留言#4(772字,下轮压缩)**;执行=PATCH _exp_cc+_ai_task_cc 复读✓零漂移;刹车无新触发,RECOVER门未到;宏观FGI50/BTC+0.6%/mcap−1.5%;trending∩feed=ONE,TRUMP,NEAR,UNI,PONS,PUMP,COTI,LIT;feed188 ∩=∅✓;code=tpl998一致;AVA粉尘残量二次平仓n=1(§6);现货候部署;下轮=01:00Z评禁多+max_hold watch+mcp/DS-9/DS-10催owner |
