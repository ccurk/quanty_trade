...
| 计数项 | 读数 | 更新轮 | 备注 |
| backend→Binance REST DNS断连 | **n=1(episode): 09-09 16:12Z起持续,09-13 18:11仍在(≥98.0h;probe史54miss+cron直证廿二轮=git链→7e776b7→f8cf733;18:1x直证=active positionRisk+main/trend ctx income同串fapi SERVFAIL+balance伪零双载具;ws活证main评估SENT@18:11+trend TUT评估@18:11;feed183/3无重播种=无重启=#89未部署;S31宽度0.639=断连窗首ON直采上界更新)** | 09-13 18:1x | 特征=fapi SERVFAIL(resolver 100.100.100.100=Tailscale MagicDNS,修复在宿主机);伪影族=ctx balance/income伪零+skip文案+closed窗过滤失效(close_time过滤免疫)+active回error(全谱git ecec3b0);**已升§3复发向量n=2+#89补丁c49c555候部署(§4)**;本轮re-arm×3@18:57/19:57/20:57→恢复即落#88;恢复后复核:①income补记完整性②feed重播种③断连窗内重启(ws也死须立报);⚠️断连窗禁stop/start(重订阅可能失败) |
...
## 7. 运行日志（每轮一行，新行追加在表首）
| 2026-09-13 23:5x | 注记: 断连probe#3 miss(同串SERVFAIL@23:57,≥103.8h;ws活证main评估流@23:57;末次);session probe预算尽,交接00:1x cron |
| 2026-09-13 22:5x | 注记: 断连probe#2 miss(同串SERVFAIL@22:57,≥102.8h;ws活证main评估流@22:57);probe#3@23:57候(末次) |
| 2026-09-13 21:5x | 注记: 断连probe#1 miss(同串SERVFAIL@21:58,≥101.8h;ws活证main CLANKER评估行@21:58);probe#2@22:57候 |
