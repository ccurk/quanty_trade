...
| main 通才 | 8eb182b6 | 通才(S18-S23栈) | auto·**feed83@08-29 15:2x(ROUTE v1.5摘BAS,NIL→孤儿态待挂brk;前史feed85@14:5x=apply重播种热清15/15,HANA回池,4币→breakout池)** | running | **pct0.10@09-03(E9扩仓rider,owner直令;史0.08)** sides=[buy,sell] **bl15@09-03(#81:隔离13+TUT+USELESS,重启免疫闭合;史git)** **select_limit150@09-03 06:2x(FREQ扩面rider)** feed138@06:2x(重播种150→热清12[含新面捞进的APR]残留0;当日三重启窗03:3x/03:4x同型) **scp=0.05@08-28 09:33(EXP提频终裁rollback)** **lcp0.15门0.70@08-29(提频直令+S30;史#61c 0.90系git)** tpl989(+S32)@09-03;**trailing(act1.0/cb1.2%)+BE(1.0)=基线几何(#49KEEP)**;**_exp=open·EXP S32接刀冷却+E9扩仓rider@09-03(不稳定窗[10m内S18veto或RSI>70]+A-bar拒多,W-bar放行;#80升门n=4/−5.21;劣化线=段多头≤−3U或拦截>25/72h或误杀审计≥50%@n≥4;eval 09-06T04Z或段多头n≥15;进度@09-03 15:2x:拦截无新观测仍1=EDGE08:23误杀#1(1/1,回滚线n≥4∧≥50%)/段多头n1闭(HEMI−2.02)+CAP15:14 W-bar多在飞(active行sid=""空归属=#52案2,交易所TP/SL已挂+main在管实害0);E9段n10/−4.04(口径=开仓≥04:07,12:2x轮已裁排除CLO04:01边界行;回滚钟~42/48h,到点09-03 21:15Z归21:1x轮:费覆四窗🔴+段负=双条件现指回滚pct0.08,劣化线A 24h−6.09>−8U未触不提前;回滚需stop窗,CAP sid=""行不卡stop[#52洞①],重启后收养闸+交易所TP/SL兜底))**;S31=基线常规监控(KEEP verdict全文移档ops/exp_archive/s31_verdict_20260902.json,原config._exp已被S32覆写);前_exp S30 KEEP@09-01(史git 0940c62);RECOVER 6h空≤−8U继承;#11观察(§4) |
...
- 隔离区(规则≤−4U∧n≥4)【13】: 4/CYS/TST/龙虾/BMT/BTW/H/APR/AIO/BICO/BEAT/XNY/ACE(逐条血统/日期全文git bb3b883/8a797ec/bb2e系)。**池列值=快照,权威=每轮对账器输出**。**对账器v1.5权威副本=quanty-ledger分支ops/route_pools.py@08-29**(v1.5=roster增补breakout复活壳3b646bf4,阈值零改动;谱系v1.4←v1.3←61b1bb4;v1.0默认分支禁跑)。**arg4隔离表必须X/USDT形态**(08-15实证:裸名norm不match feed键→漏移)。出池只认头注规则或隔离,失格=aging_watch advisory;brk/lowvol出池阈值未注册(#42)前无自动出口。**MAGMA网:复发@09-02 03:1x(S −1.17 n1)→网复挂,追踪48h币净≤−4U∧n≥4隔离线;读数@09-03 00:2x=n2/−1.08距线2.92(22:22空+0.095回拉)**(09-01曾窗空解除;史git bf52ca3/ed40296);对账史压缩@09-01: **09-01泄漏清扫7币feed93零残留(bl14=期望态,隔离13无新增,aging=TUT续,MAGMA网解除)**;08-30 00:1x零差分/08-29斜杠二发史与更早链=git b12d7d9版行;#51-#58史git 9957041/1ae847c/91b3276/830a46c/188af33/66048a9
- 保证金: Σ_running=main0.10×5+trend0.05×3=**0.65≤0.75✓**(09-03 E9扩仓+0.10;前史0.55@brk退役/更早见git)
- 互斥不变式: feed层@09-02 21:2x复核=main89/trend2(TUT,USELESS)**∩∅**(bl待+USELESS§5#81)(v2退役壳+老壳双RETIRED占位=交易/feed层收养封闭,**记账归属层仍漏@09-01(#79双行升级,末代持仓币绑定绕RETIRED闸,详§6)**;重播种漏9热清9/9;隔离13币零在feed✓);种子层bl=18(desired达成@#70);**退役壳自bl5=收养向量封闭**;重播种法医学详§3(签名=top回落~202;bl闸零违例);#20=bl币可直漏feed(交易闸级非feed级);#24/#23及#8-#19史git;引擎互斥闸✓live;**fade壳seed5与main auto理论重叠=停机态无违例**(不变式限定running载具;若owner UI复活fade须先重划池,互斥闸+TG示警兜底);隔离币零成交✓(复核@08-21 09:2x,48h窗隔离12币零平仓行)
...
| 11 | E8 类 | short跨币修补:`breadth_max_for_short` null→0.50设计定稿@08-15(config-only;宽度门L1713 veto;回滚=null;设计全文git d19c710前史) | 依据链git+§1.5#49终数;短侧n26/+0.04已打平 | 2/2达成→病灶消退 | **降级观察@08-16 18:2x**:08-09依据(24h空−15.20 burst)在#49基线几何下未再现→撤出候选队列;**重升门=main空侧24h≤−8U复现**(届时以#49段为基线重算keep线再上_exp);watch@09-03 15:2x:**空侧多币化扩散预警·逆转恶化**——24h空n17/−4.60距门3.40(链−5.56→−3.93→−3.41→−4.60;增量=MARSCOIN12:31+0.98/牛来14:22−1.27穿刺;BTC+4.6%反弹日24h穿刺8笔中7空=市况型反弹收割续,多币分散未变,06:2x细目git 46bd2fc版行;同期多头n3/−1.49含HEMI W-bar案);过门即重启#11(breadth_max_for_short null→0.50,以#49段为基线重算keep线上_exp);史读−2.41@08-21 |
...
| 58 | E9类(main扩仓) | **兑现@09-03=E9扩仓rider(pct0.08→0.10)落地**: owner live直令[增加开仓数量]覆盖原自设门(费覆双绿/#79双行归零/main行归属恢复,全未达但直令优先);预注册=S32 _exp内rider_e9(劣化线24h组净≤−8U或费覆连🔴48h∧rider段净<0→回0.08);下一档0.12=Σ顶0.75须新直令或rider段证据 | 直令+rider全文config._exp;门史与三翻转史git f3c6469 | n=12段(史) | **结案→rider评判随_exp**;史全文git 64ede74版行 |
| 52 | 引擎候修(M通道) | **stop校验双洞+复活路径**:①validateStrategyCanStop把sid=""仓当不存在+auto池跳过交易所侧校验→带仓stop可过闸(修法=exchange侧校验在auto池用全量active仓判);②stop后≤40s自动回running复现机制未定(requestRestart候选);顺手项=stop回执改真沉降或带async标注 | 18:27全录grab50.log+源码strategy_lifecycle.go L233-270/strategy_runtime.go L135;XNY sid=""行实证;+CAP09-03 15:14案2(active行sid=""空归属,main开仓链日志实锤W-bar多conf0.68,交易所TP/SL已挂+main在管=实害0) | n=2(main) | 候选;dev分支claude/dev-*走M通道,部署权owner;先复现②再动手;运维注=sid=""行不卡stop(洞①反向便利,E9回滚stop窗适用),重启后收养闸+交易所TP/SL兜底 |
...
| EXP提频0.600档watch(**终裁rollback结案@08-28 09:33**) | 档n9闭毛−0.76=边际档负EV定谳,浮点修复机械成立;漏斗史全文git 11d9c50 | 08-28 09:3x | 结案;#75部署验证轨独立继续(签名=最小手数凑整日志);重开=#75部署+池ATR中位回升,从零预注册新_exp |
| main断流观测(三闸交集关门) | 本轮main笔数=23/24h@09-03 12:2x(判据未触) | 09-03 12:2x | 判据不变:再现6h+零笔且池ATR中位≥0.5→查管道;每轮记main笔数 |
...
## 7. 运行日志（每轮一行，新行追加在表首）
<!-- §7瘦身@09-03 15:3x: 09-02 21:1x/09-03 00:1x两stub删除,全文git 558543d/a3dbdcc版行 -->
| 时间(UTC) | 档位 | 五窗净额 1h/3h/6h/12h/24h | 世界 | 决策 | 备注 |
| 09-03 15:1x-15:4xZ | 例行轮(max) | −1.27(n1)/−0.29(n2)/−0.98(n4 wr50)/−4.83(n12 wr50 be69 −19pp)/−6.09(n20 wr55 be71 −16pp)(毛) | FGI65/BTC80529+4.6%/市值+1.34%/trending∩feed=MARSCOIN,ARB,LIT,UNI,PUMP,PENGU+USELESS(trend池续榜) | **HOLD原子包0**(双_exp评期未到[S32段多头n1+CAP W-bar多在飞,拦截无新观测仍1;E9段n10/−4.04回滚钟~42/48h,21:15Z到点归21:1x轮:费覆四窗🔴+段负=双条件现指回pct0.08,劣化线A −6.09>−8U不提前];刹车全距[6h−0.98vs12.3/24h−6.09vs30.8];ROUTE零差分十六连bl15期望态) | 钱包≈153.9(avail124.41+margin29.4+浮0.20);**CAP15:14 active行sid=""=#52案2登记**(main链实锤实害0,E9回滚stop窗不受卡);空侧24h n17/−4.60恶化(#11距门3.40,BTC+4.6%反弹日穿刺8笔7空=市况收割,详§4);死法24h=穿刺8(7空1多)+超时1+BE微5+赢6;费覆四窗🔴(24h−0.304vs0.112);UAI−2.29(首行18:53滚出→−0.97)/MAGMA n2−1.08(09-04 00:52滚出候);牛来48h n4/−1.45;trend USELESS首开15:13多在飞21.7U地板✓(#65段候1);audit 1794-1798全claude_cron零无主;feed138/2∩∅;ΣM0.65✓;充值线未触 |
| 09-03 12:2x-12:5xZ | 例行轮(max) | +0.52(n1)/−0.69(n2)/−2.26(n5 wr60)/−5.84(n12 wr50 be74 −24pp)/−3.01(n23 wr61 be67 −6pp)(毛) | FGI65/BTC77939+1.26%/trending∩feed=MARSCOIN,ARB,PENGU,LIT,UNI,ZEC,PUMP+USELESS(trend池) | **HOLD原子包0**(双_exp评期未到[S32段多头仍n1/−2.02拦截仍1;E9段n8/−3.75回滚钟~45/48h,21:15Z到点归21:1x轮:费覆仍红∧段负→回pct0.08];刹车全距[6h−2.26vs12.2/24h−3.01vs30.4];ROUTE零差分十五连bl15期望态) | 钱包≈152.0(avail131.67+MARSCOIN空margin21.1浮−0.79,main12:13 W-bar空在飞);**#79第9案=COLLECT11:56双行fade-v2壳首案**(main全链开仓+trailing出场+0.519,壳行dedup陷阱再证,§6 n33,TG重申#77部署);#11空侧−3.41距4.59续改善;死法24h=穿刺8(6空2多)+超时1+BE3+赢11;费覆三窗🔴(24h−0.131vs0.107);UAI−2.29(首行18:53滚出候)/MAGMA n2−1.08(09-04 00:52滚出候);trend双币评估活跃零过门(TUT ATR0.38死市/USELESS0.44<0.699);#62 emit veto×3带戳;audit零无主;feed138/2互斥✓;ΣM0.65✓;充值线未触 |
| 09-03 09:1x-09:4xZ | 例行轮(max) | 0(n0)/−1.57(n3)/−3.85(n8 wr50)/−4.46(n14 wr57 be72 −15pp)/−3.52(n23 wr61 be68 −7.5pp)(毛) | FGI65/BTC77858+1.3%/市值−2.22%/trending∩feed=MARSCOIN,ARB,AKE,CHIP+USELESS(trend池续榜) | **HOLD原子包0**(双载具_exp槽满评期未到[S32@09-06或段多头15,今n1;trend FIX@09-07或n5,今n0];刹车全距线6h−3.85vs12.4/24h−3.52vs30.9;ROUTE v1.5零差分十四连bl15期望态;无新隔离) | 钱包154.75U全空仓;**S32首日双样本均逆假设**:拦截1=EDGE08:23 A-bar→60m+4.54%=误杀#1(1/1,线n≥4∧50%);段多头n1=HEMI W-bar放行−2.02穿刺(距劣化线0.98U);**W-bar残余型n2/−3.31登§4#80**(S33候选判据n≥4∧≤−4U);E9段n7/−4.85(回滚钟费覆红连~36/48h,~21:15Z到点);#11空侧n17/−3.93距门4.07窗滚改善;死法24h=穿刺8(6空2多)+超时1(AGT)+BE3+赢11,最大失血=市况型空头反弹收割非池错配;费覆三窗🔴(24h−0.153vs0.107);UAI−2.29距1.71/MAGMA n2−1.08(−1.17行09-04 00:52滚出=网解除候);trend32h静默(TUT ATR0.27死市/USELESS RSI71.5未过门)载具活;#79零新增;audit零无主;ΣM0.65✓;充值线未触;§7瘦身3行(09-02 09:1x/12:2x/15:1x) |
...
| 09-03 06:2xZ | owner直令轮2 | (无新平仓;05:49 emit待归因) | 同03:1x | **FREQ扩面**: ①main select_limit100→150(rider_freq;重播种150热清12→feed138=信号面+55%) ②trend lcp0.149(#65,独立_exp) ③拒拧三杠杆(证据§1直令行);双stop-PATCH-start全绿复读 | 新面cohort≤−4U→回100;150币回放预热健康;store三连冷启自暖 |
| 09-03 03:5x-04:0xZ | owner直令轮 | (同03:1x轮,无新平仓) | 同上 | **E9扩仓rider执行**: owner live[增加开仓数量]→pct0.08→0.10一步档(Σ0.65✓),stop poll1→PATCH(pct+rider_e9原子)→start✓复读落库;重播种100→热清11→feed89残留0;证据答复(wr63<be68负期望窗,扩仓放大非创造期望)TG+会话送达 | #58让渡;§1直令登记;S31/S32 store重启再冷启(自暖) |
| 09-03 03:1x-03:5xZ | 例行轮stub: 原子包1=S32接刀冷却EXP上线tpl989(#80升门ZKC第4实锤;6例4死2赢A/W-bar分离;stop-PATCH-apply-start三验全绿);ROUTE十二连;费覆24h🔴;全文git 8c39ae5版行 |
| 09-02 18:1x-18:3xZ | 例行轮stub: HOLD;ROUTE零差分十连;AGT S硬超时−0.71+PROM BE保本;S31 verdict后多头4笔+1.96;费覆24h🔴八连;+AGT主-主同笔双行1例(#18隙型非#79);全文git 4a7b0ba版行 |
