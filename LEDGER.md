...
| 载具 | id | 原型 | 池 | 状态 | 门/备注 |
|---|---|---|---|---|---|
| main 通才 | 8eb182b6 | 通才(S18-S23栈) | auto·**feed91@12:52**(bl12种子=9隔离+trend3,重播种#13+heatclean-v8;5 fade币回池) | running | pct0.08 sides=[buy,sell] long_conf_premium=0.35 tpl567;_exp=**FIX-GEO(#49)@15:32:13Z**: trailing(act1.0/cb1.2%)+BE(1.0),TP4.5/SL2.5/premium不动;基线=72h赔率0.88/SL率40%/均净−0.73;**段@12:50=n21/−2.49/赔率0.36/wr67%/均净−0.12/SL率14%**(段口径=池归属+日志修正:BICO两笔归main[§3陷阱③]+VELVET17:18归main;均净/SL率大改善,赔率腿0.36仍远)→回滚线(赔率<1**∧**净≤−10)未触=continue;keep=n≥30或48h先到时赔率≥1.2∧段均净>−0.73(**注:eval_after=15:30Z[24h]与expect'48h先到'并存,当值轮按预注册原文裁,禁事后换标**);eval 08-16 15:30Z(order_id口径候fa93736部署,未部署则沿池归属口径);RECOVER刹线6h空≤−8U继承(空6h 0✓);#11空侧gate=下一槽;段@09:26旧读数git 766c7d9 |
...
| ~~qt-fade-short~~ | 21519f1b | **停机@08-16 12:26Z 劣化线**(预注册'累计n≥10∧净≤−5U→rollback或stop'触发:累计**n12/−5.44**=v1承前8/−2.63+v2池内4笔sid确证−2.81[BEAT+0.17/−1.28/−0.88,VELVET−0.82];BEAT 10:08Z−0.88落线,评期12:50Z前劣化线先至;终裁全文=config._exp verdict=degraded_stop) | — | stopped(**gated劣化,勿auto-start**) | stop舍rollback四由:①两代模板两池n12零正期望(v2赔率0.14) ②0.60墙放行的最高分空单仍3/4亏=解墙只放更低分同型(prem→0候选随停机取消) ③池内挣钱行为全在main几何下(VELVET nS5/+7.03,BULLA+1.06)=原型病非仅选币病 ④tpl567回滚=冗余sell-only main克隆;**ROUTE:5币回main(bl17→12)**;tpl576+S25+设计稿存git,复活=新壳走FLEET(brk先例);壳留seed5+auto_symbols=false硬停;自动保险随停机失效 |
...
- **[新增 08-16 00:2x] DELETE不杀进程竞态→注册表驻留幽灵载具(源码+行为双证)**: DeleteStrategy(strategy_handlers.go L359)先删DB行再stratMgr.RemoveStrategy(manager.go L1496)——后者仅当内存Status==running才Process.Kill,删除瞬间处start/stop竞态窗则进程存活,且start单worker队列/requestRestart可把实例重插注册表→【DB无行+注册表在册】幽灵:API全404(detail/logs/audit/stop皆DB先查)不可控,但仍收feed/发单/受三层出场监控(quick_trade_monitor按DB行遍历+inst=registry查得)——brk(2111f5f9)23:11Z STAR空开仓+整60m max_hold平仓实证。互斥闸兜底仍有效(闸=DB行级owner_id+sid<>本id,幽灵仓行带sid双向可见);残余风险=main与幽灵同:34s批口同击TOCTOU窗→已检疫(STAR/AKE/EDEN出main feed)。杀灭唯一通路=**后端重启**(DB无行→boot不复活;孤儿进程失stdin/stdout管道饿死/崩死)。时间线:brk复活链起点08-14 01:13恰在全局重启#1(00:18终)后=DB status=running被boot拉起;穿越重启#2存活;owner UI删15:5x未斩进程;后7h仍开仓。幽灵账目单列不入载具归因(行带sid=2111可滤)。
...
| # | 类型 | 内容 | 依据 | 复现计数 | 状态 |
|---|---|---|---|---|---|
| 34 | S23候选 | 长侧首腿穿刺节流→S23影子观测版(逆bar cohort审计;enforcement门=A组n≥20且AL/WL wr分离≥15pp) | **结案@08-16 12:39 观测债清偿+裁决**:q=S23bar&limit=2000回补(达08-15 02:52),重置点法切10段,无歧义段(03:21后)新增A组6(AL0/1+AS1/4);A组累计19+6=**25≥20门成**;分离度**AL=1胜4负(20%) vs WL=1胜4负(20%)=0pp<<15pp门**(且08-13曾方向反)→**enforcement不采纳**;S23影子零交易路径留场,不再追账;WS线索(新段12/21)归#30;前史git 121e975/766c7d9 | 25·**结案** | 副产物:重置点法揭08-15 03:21/11:03两次台账外main重启(§3计数+2);歧义段(02:52前跨08-13计数)弃计,禁回头重算 |
...
| 08-15 21:13-21:4xZ | 例行轮(max) | 24h−7.43(39) | FGI34 | 0原子包;ROUTE双出池:AIO隔离(48h n4/−6.53)+BICO出trend→真空窗程序v2×2+heatclean-v6+bl16v2落库;#49段n6/−5.15续飞;fade新池首单BEAT+0.17=0.60墙非全封;刹线全绿〔全文git 2aa822b〕 | 费覆24h🔴 |
| 08-15 19:05-19:2xZ | 金丝雀唤醒(trig_014KQE) | 6h+0.32(16) | 静默窗 | #49健康复核PASS;#50落地结案(bl16/VELVET种子免疫)+heatclean-v5;#49段初读n5/−5.45轶事级;刹线全绿〔全文git 2aa822b〕 | 无TG夜间免噪 |
| 08-15 18:12-18:5xZ | 例行轮(max) | 24h−12.22(42,wr38 be49) | 无降压 | 0原子包;heatclean-v4×2+#51落地(fade池4币);#50抢窗失败=stop乐观回执+校验双洞机制发现(§3+#52+程序v2);归因修正(VELVET17:04/BEAT三笔=main)+0.60墙发现;刹线全绿〔全文git ffebcb4〕 | 钱包≈223.5;SL穿刺13/−24.52主刀 |
|---|---|---|---|---|---|

> 瘦身注 08-16 12:5x：15:12/16:06两行随轮裁剪(#49上线轮+VELVET晋升轮,要点存§1.5/_exp)〔另 08-15 21:4x：12:18/14:3x/15:2x/15:1x四行裁剪〕(质疑轮结论=era对比部分证实部分证伪+owner择A打#49,全文git 7a6729c前后;其余要点存§1.5/§3/§4/_exp),全史见 git 历史。