...
| # | 类型 | 内容（含完整意图） | 登记轮 | 状态 |
|---|---|---|---|---|
| 81 | bl种子同步 | main symbol_blacklist 14→15(+"USELESS/USDT"斜杠形态)=blacklist_sync@09-02 21:18 plan(隔离13+TUT+USELESS);执行窗=下一个main自然重启/PATCH窗(bl=重启免疫互斥层;当前互斥由feed不相交+引擎闸兜底,trend种子已落库无缺口) | 09-02 21:1x | open |
...
| main断流观测(三闸交集关门) | 本轮main笔数=21/24h@09-02 18:1x(末close16:52/末open16:29;判据未触) | 08-30 00:1x | 判据不变:再现6h+零笔且池ATR中位≥0.5→查管道;每轮记main笔数 |
...
| #79·closed行sid按symbol旧绑定继承(#77族新亚型) | 两形态:①**双行**(main行+末代壳行同笔PnL双写,壳行tp/sl null裸奔,出场治理权=两行落库序竞态,08-30 03:34壳饥饿档45m真接管收割−0.61实锤;判别特征=末代持仓币BTR/NIL/COLLECT类,泛池币牛来/USELESS零双行)②**空sid**=binance_only视图伪影(全量视图sid=main齐);binance_only对双行币dedup保壳行=池归因陷阱 | 09-01 12:5x | 归因纪律:sid冲突以载具日志开仓链为准,逐笔归因用全量视图+日志链;机制修复并入#77 ownership单一化(候owner部署),部署后复验消失;**n=31@09-02 15:2x**(+BTR12:36双行第6案=12:24在飞多单平仓计入[main行+壳行同PnL+2.3595;上轮DB仅壳行可见,平仓后main行现身=回填时序证据]+BTR13:35双行第7案;同拉取T12:42/ZKC/牛来单行sid=main=泛池币无双行再证);史读n=29@12:3x(+BTR11:01第5案+UAI空sid)/n=27@09:1x/n=26@06:3x(细目git ed1ba85版行);史:MAGMA第2币种@03:1x/BTR三案升级链@09-01 18:1x(#77级实锤)细目git 1417e31/558c235版行 |
...
| 09-01 18:1x | 例行轮stub: HOLD;S31段n11/9W2L;ROUTE零差分三连;#79双行升级#77级实锤(壳饥饿档接管案);费覆🟢三连;充值线三条件续达;全文git 558543d版行 |
