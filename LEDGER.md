...
| 载具 | id | 原型 | 池 | 状态 | 门/备注 |
|---|---|---|---|---|---|
| main 通才 | 8eb182b6 | 通才(S18-S23栈) | auto·feed87(00:37对账@08-14:AKE/EDEN→brk,H候移[在飞空仓];bl15种子滞后=期望18[+AKE/EDEN/H候main重启窗];重启重播种→对账自愈,自发计数2) | **running@08-13 19:03Z(RECOVER unlatch原子窗30s)** | pct0.08 **sides=[buy,sell]双向恢复@19:03(#43第12读达门6h+0.97∧24h+1.06;latch史08-09 18:49→08-13 19:03≈4天)** tpl567;_exp=**RECOVER刹线常驻**(6h main空侧净≤−8U≈钱包3.1%→重latch[buy],brake_20260809键转recovered续用;首评08-14 19:00Z)＋s23_closure归档(#45 main✓);段(自08-12 22:20)n9/−3.41 wr33;unlatch后首夜4连亏−6.32 wr0(H空SL14m/AKE SL15m/EDEN SL6m/BR超时−0.04)=夜盘接刀n4轶事;空侧6h−2.02 vs 刹线−8U未触 |
...
| qt-breakout-follow | 2111f5f9 | 突破追动量(顺bar,S23 W组实盘化) | COOKIE/DYM/BTR/COTI(晋升@08-13)/**AKE/EDEN(入池@08-14 00:37,maxmv3.6%/4.4%≥3.5%)**;种子落后3币(COTI/AKE/EDEN)候12:36Z评判PATCH补写;aging_watch=COTI/DYM(COTI trend-want被互斥拦=留池非病) | **running@08-12 22:20Z(全组复飞令;自动保险在力)** | **tpl577(S26新内核)** pct0.04 mcp2 buy hunger30 max_hold45 trail(1.0/0.8);_exp EXP在飞,段真值**n11/−2.75 wr55**=前9/−2.10+BTR−1.02(快SL13.2m@15:19)+COTI+0.38(快TP3.5m@15:41,晋升后首笔即盈);COOKIE段3胜2负+0.14仍组内最佳币;**策略自防护在工作@06:2x日志:BTR策略内隔离中+COOKIE连败2计数+BTR追多被RSI超买硬拦(82.7>68,多头分0.95=blowoff帽实证n=1)**;评期**累计33.73h/72h→≈08-14 12:36Z或段n≥15**;自动保险段滚动6h≤−5U(现−1.05未触);TUT入池提名被v1.2封死(在trend池) |

- 隔离区(规则≤−4U∧n≥4): 4/CYS/TST/龙虾/BMT/BTW; 孤儿4=AIO/BLUAI/DODOX/KAITO(黑名单内待main重启窗释,黑名单18=期望超集安全)。**池列值=快照,权威=每轮对账器输出**。**对账器现版v1.3权威副本=quanty-ledger分支ops/route_pools.py@08-14(=334dae1内容;谱系61b1bb4←377c567←f970704;默认分支工作树只有v1.0初版——禁直接跑,实证2例错误plan)**:出池只认头注规则或隔离(失格币aging_watch);隔离表arg4;add互斥只排他专家池(v1.2误把main feed入排除集=晋升封死,COTI案实证修复);晋升币同轮移出main(封v1.1单轮重叠窗,执行序=先main remove后专家add);brk/lowvol出池阈值未注册前无自动出口(#42)
- 保证金: Σ_running=main0.08×5+trend0.05×3+fade0.04×2+brk0.04×2=**0.71≤0.75✓**(08-12 22:2x全组态复核)
- 互斥不变式: feed层✓(overlap=0全组,08-13 19:04 main重启+ROUTE−11后复核)>种子层**blacklist15=期望精确态达成@19:03**(COTI含,孤儿4释)>⚠️引擎互斥闸未部署,部署升急;**v1.3@08-14 00:37运行:plan=AKE/EDEN→brk已执行(churn2,main87/brk6/overlap0),H被has_open_position拒候下轮**;COTI trend-want续拦=enforcement实证第3例;aging=trend3/fade2/brk[COTI,DYM] advisory
...
| #45(缩@08-13 19:0x) | _exp补写(余brk) | brk config._exp补写resume_note(评期重锚累计口径)+symbols种子补COTI/AKE/EDEN——候08-14 12:36Z评判窗顺带;main部分已落地@08-13 19:03(s23_closure入_exp槽+RECOVER同PATCH)✓;trend 06:35✓;fade 12:45✓ | 08-12 18:2x扩22:4x缩06:3x缩19:0x | open,阻塞=brk running;落地窗=brk评判PATCH(明12:36Z) |
...
|---|---|---|---|---|---|
| 08-14 00:32-00:5xZ | 例行轮(max) | close: 1h0(0)/3h−2.44(1)/6h−6.32(4)/12h−7.35(9,wr22)/24h−5.27(13,wr31) | FGI29/BTC−0.24%/mcap−0.06%无降压;trending∩池=AKE(brk新入)/ETHFI/AVNT(main) | ①ROUTE:默认分支v1.0误plan弃用(漂移实证2例)→v1.3重跑=AKE/EDEN→brk已执行(maxmv3.6/4.4%,churn2,main87/brk6/overlap0),H拒候下轮;COTI续拦=实证3例②**修根:v1.3权威副本入台账分支ops/**③0原子包(四_exp未到期:brk12:36Z/RECOVER19:00Z/trend/fade08-16) | 钱包≈247.6(avail178.5+3仓保证金69.1);持仓3=CAP多/H空/ESPORTS空(main,unlatch后空侧首开);**unlatch首夜main4连亏−6.32 wr0**(3×SL穿刺6-15m+BR超时)n4轶事只登记;main段n9/−3.41;组6h=钱包2.6%<8%,四保险未触;费覆🔴−0.41/笔禁提频;brk评期12:36Z下轮裁(段n11/−2.75贴宽容线−3U,新池6币) |
