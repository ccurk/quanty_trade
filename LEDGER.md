...
|---|---|---|---|
| 2026-09-20 07:1x-07:3x(v6.0 #10) | 上轮(06:25)= [攻-1] owner UI hunger 三键 true/0.10/0.20(第 3 次) / [攻-2] owner mcp 1→3(第 6 次)+备选 MIN_TRADES 30→10 | **DS 0 交易键改动(第 7 轮)**: audit admin 最新仍 06:22:16 #1954(breaker 0.25→0.25 空改, 冻 pct 至 09-21 06:22Z); 07:17Z config: main lev10 pct0.25 mcp1 tp/sl pct0 hunger false(sl0.04/tp0.30) max_hold90 max_mult0.8; majors lev10 pct0.25 mcp2 max_hold720. 样本门口径订正: window_line L256 取 binance.paired_trades.pair_count(6h=20; 300U 单在薄币拆多笔 fill ⇒ pair_count≠仓数) <MIN_TRADES 30 ⇒ [skip]. _ai_task_ds 仍 null. | **效果**: 无改动 ⇒ 无前后. 逐笔 07:17Z: 1h 0 / 3h 2/−1.72 / 6h 5/−3.13 wr60% / 12h 38/−1.97 / 24h 115/+8.81(窗口滚出 09-19 06-07Z 亏段; 1-24h income_complete=true); avail 119.1; 持仓 1: main CTR 空 06:16:23 295U conf0.859 腿 tp .011177(−1.53%=+4.5U)/sl .011490(+1.23%=−3.6U) 现 +0.78U → 07:46 钟; majors 0 仓. 北极星: 15m 段全程(21:12Z 起) 28.5/日×−0.22=−6.3U/日 → mcp1 段(23:39Z 起 7.7h) 15.6/日×−0.63=−9.8U/日(变差; mcp1 低于 owner 09-18 频率地板 19/日). 本小时 3 bar 269 信号(≥0.9 82) 0 开(CTR 独占槽). **病根(12 笔 1m 回放 OKX/Bitget/Gate 首次 12/12; 币安 fapi 451)**: 7/12 曾到 +1%(ROSE 2m/BANK 2m/NIL 30m/VANA 40m/POWR 43m/SUI 48m/BANK-21:16 1m) 仅 3 笔以 ≥+1% 出场; 亏损全 BANK 型: 2 笔 −10.69% 抵掉其余 10 笔 +6.48% ⇒ 均 −0.35%/笔; 非 BANK MAE 最深 −1.27%(PUNDIX) ⇒ 帽不碰赢单. 反事实同 12 笔(分钟先到先算, 同分钟算 SL): tp0.10/sl0.20 均 +0.25%/笔, tp0.10/sl0.15 +0.335%/笔; 均持仓 62m→36m. 止损核对: main 腿✓ max_hold✓; majors 0 仓 机制同; 无裸奔; 隔离无撞车. **管道注**: 05:17:55/06:17:40/07:17:55 各 1 次 429(止盈止损守护跳 1 轮 + v40 premiumIndex 失败)与本 cron 五窗拉数同分钟=自伤(腿在交易所不受影响) ⇒ 下轮五窗分批拉(间隔 ≥60s); v40 资金费率全宇宙刷新 908 币/30s 吃 IP 权重(Python 侧). **本轮建议(板 07:28:18Z; 首写 10208B 超 8KB(jq length 按码点误判) ⇒ 二次按 utf8bytelength 裁为 2 条 6726B, audit #1957/#1958 均仅 _ai_task_cc; TG msg 10827)**: [攻-1] 重发第 4 次 owner UI hunger_mode_enabled=true + hunger_tp 0.10 + hunger_sl 0.20(金额令取 0.15) | 12 笔反事实 −0.35%→+0.25%/+0.335%/笔, 6 笔 2-48m 提前腾槽 | 验: 饥饿模式触发 tp=10.00% sl=20.00% 行·亏单 ≥−6.5U·持仓中位 <45m·段均净 ≥+0.15. [攻-2] 重发第 7 次 owner UI mcp 1→3(先 攻-1 后落; 有帽单波最坏 −18U, 无帽 Go 钳 3%@lev10=−27U) | 15.6→~47/日; 6h pair_count 20→~60 过样本门=DS 自解锁; 备选宿主 MIN_TRADES 30→10 或 QT_WINDOW_HOURS=24 | 验: audit admin 新行·开仓/h ≥2·跳过本批信号 行减少. 0 交易键写入(diff 非板键=none ✓). FGI 71 Greed; 市值 24h −4.5%; BTC 80297(−1.1%) ETH 2574(−2.2%); dom 58.9%; trending 已见信号 PRL/G/ZAMA/AVAX(ONE/AKE/NEAR bl) |
