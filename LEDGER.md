...
| 载具 | id | 原型 | 池 | 状态 | 门/备注 |
|---|---|---|---|---|---|
| main 通才 | 8eb182b6 | 通才(S18-S23栈) | auto·feed83@06:3x(**bl17种子落库=#54结案@06:23**;heatclean-v7热清17币[bl在册14+僵尸STAR/AKE/EDEN3]→残留0;zombie检疫重固化;heatclean史git 26eaf24) | running | pct0.08 sides=[buy,sell] long_conf_premium=0.35 tpl567;_exp=**FIX-GEO(#49)@15:32:13Z**: trailing(act1.0/cb1.2%)+BE(1.0),TP4.5/SL2.5/premium不动;基线=72h赔率0.88/SL率40%/均净−0.73;**段@09:26=n20/−5.36/赔率0.34/wr60%/均净−0.268/SL率20%**(新增XNY−0.66+BICO−1.90;段含BICO两笔+0.57/−1.90=**main开仓被镜像行盖trend sid,按开仓日志修正归main**[§3归因条陷阱③];病理不变=trail锁小赢 vs 亏满SL,赔率腿仍远)→回滚线(赔率<1**∧**净≤−10)未触=continue;keep=n≥30或48h先到时赔率≥1.2∧段均净>−0.73(**注:eval_after=15:30Z[24h]与expect'48h先到'并存,当值轮按预注册原文裁,禁事后换标**);eval 08-16 15:30Z(现n20,15:30轮请用order_id归因口径复核段);RECOVER刹线6h空≤−8U继承(现+1.00✓);#11空侧gate=下一槽 |
...
| qt-fade-short | 21519f1b | 冲高回落空 | 1000CAT/ARC/BEAT/VELVET/BULLA(live=种子=5✓@06:3x;aging_watch=1000CAT/ARC/BEAT/BULLA advisory[对账器@06:30],VELVET持续合格) | running | **tpl576(S25)** pct0.04 mcp2 sell;_exp v2续:**累计n11/−4.56@09:26**(+VELVET−0.82@06:41开14m;距劣化线n≥10∧净≤−5U仅0.44U);**0.60墙在案**:BEAT空头分0.600整×20次顶死门(min0.55+short_prem0.05需**>**0.60)系统性拒发,处置候选=EXP:short_prem 0.05→0(评期时预注册执行,不中途改污染段;BULLA入池后墙对其同样适用=天然对照);评期=累计n≥15或**08-16 12:50Z**,到点n<15按证据不足格裁;自动保险6h≤−6U(6h 0✓) |
| ~~qt-breakout-follow~~ | 2111f5f9 | **owner删除@08-15 15:5x**(UI删,承15:36复活事件后清理;评判FAIL史+S26谱系+tpl577存git可重建;#47 S27复活=建新载具走FLEET流程,同lowvol先例;**⚠️幽灵运行时@08-16 00:1x发现**:删除未斩进程,曾在册交易STAR/AKE/EDEN[23:11Z开仓实证],机制§3,候修#53,已检疫+TG请重启;00:11后零新笔(复核@09:2x),检疫由heatclean-v7重固化;**#53修复代码已推@09:5x=claude/dev-closed-attrib-delete-kill(fa93736,RemoveStrategy无条件Kill+stopping压制复活),owner部署+重启即根治**) | — | deleted | — |
