...
- 隔离区(规则≤−4U∧n≥4)【13】: 4/CYS/TST/龙虾/BMT/BTW/H/APR/AIO/BICO/BEAT/XNY/ACE(逐条血统/日期全文git bb3b883/8a797ec/bb2e系)。**池列值=快照,权威=每轮对账器输出**。**对账器v1.5权威副本=quanty-ledger分支ops/route_pools.py@08-29**(v1.5=roster增补breakout复活壳3b646bf4,阈值零改动;谱系v1.4←v1.3←61b1bb4;v1.0默认分支禁跑)。**arg4隔离表必须X/USDT形态**(08-15实证:裸名norm不match feed键→漏移)。出池只认头注规则或隔离,失格=aging_watch advisory;brk/lowvol出池阈值未注册(#42)前无自动出口。**MAGMA网:解除@09-04 03:1x(复挂判据=磁铁型死法再现;史git bf52ca3);MARSCOIN promo_hold@09-06 12:2x:对账器首提名trend晋升(规则A达标:48h L n3/+1.92 wr67%)但拆向空侧n6/+1.61 wr83%=更强一面(占48h总盈+3.53的46%),晋升到long-only池+main拉黑=空侧流归零→#57病理家族(晋升证据不可移植/机制同型)执行层暂缓;解除→照晋判据=滚动48h空侧衰减(S净<+1.0或nS<4)∧L侧仍达标;持续判据=hold跨窗滚动仍双向皆盈→v1.6空侧否决阈值走ROUTE _exp固化(校准锚:BULLA空+0.46放行/MARSCOIN空+1.61 hold);触隔离或L失格=hold自然失效;隔离线远离(48h+3.53);KOMA watch维持:48h n3/−2.68零新笔(再1亏≤−1.32触隔离n≥4∧≤−4;史git 6a3383a);PORTAL watch:48h n2/−1.78(窗滚解压);牛来watch:48h n5/+0.17(18:15决胜空已闭+0.09)**;对账史(09-01清扫/08-30/08-29链)=git b12d7d9版行;#51-#58史git 9957041/1ae847c/91b3276/830a46c/188af33/66048a9
...
|---|---|---|---|
| main断流观测(三闸交集关门) | 本轮main笔数=25/24h@09-06 12:2x(main19+壳6;09:24后零新笔~3h+12:1x活性直证:2000行/3.5min全池138评估,池ATR%中位0.18/91%<0.5硬滤线=安静市非管道;判据未触) | 09-06 09:1x | 判据不变:再现6h+零笔且池ATR中位≥0.5→查管道;每轮记main笔数 |
...
| S31·regime动态方向偏置(**结案KEEP@09-02 06:3x**) | v1谱=宽度三档(EMA-up<0.35 OFF多+0.10/0.35-0.60 N/>0.60 ON多−0.05空+0.05)阈值内联+False-guard,上线09-01 10:02=tpl988;评判KEEP@09-02(段n22/15W7L毛−0.11,ON档多n3均+0.73 vs N/OFF n5净−1.99,劣化线双绿);verdict全文ops/exp_archive/s31_verdict_20260902.json+git 1417e31/eea7cb7/558c235系 | 08-29 17:0x | 常规监控;复核判据:多头累计≤−5U或ON档均净转负→议关guard;verdict后多头4笔+1.96@09-02判据远离；+09-05 09:2x宽度首直采=0.65(138币eval行EMA-up90/138)=ON档实证@−3.2%大盘日；门反证=当日多头死亡簇入场分0.75-0.90均非OFF窗产物且ON降门带[0.65,0.70)零死亡入账=S31非本轮失血向量,残差定性(原型固有方差)成立；宽度直采纪律入轮程(eval行EMA趋势聚合)；regime翻转速度=单日内;直采史(N五连+OFF首翻0.333+N回翻0.413+ON回翻0.652)=git ff561cc/933abbd/044689a版行;**+09-06 12:2x直采0.493(68/138)=N回落**(两日内OFF/N/ON多摆=振荡市;三态直采全命中维持) |
| funding磁铁·他币再现watch(重开@08-26;BICO条引用) | era史n2/−2.83+BICO/ACE史=git 9b9c39d/86e771d/8a797ec/bb3b883;**S30深档减半实证@09-02(+0.85良性收束)** | 08-27 00:2x | **S30已上线@08-29(owner提频直令跳磁铁门,设计按定稿字面实现)**;watch=疫苗效果验证(磁铁死法计数);era史n2/−2.83留档;疫苗期n1-n6细目+源码定案(L820/823)+排除案全文git 8729d22/ad3c8d4版行;判据n≥3达成@09-05 09:2x→**下修议案呈owner(温和+0.20→+0.10,深+0.10→+0.05,极深−0.10不动)+升级门预注册:决胜cohort累计n≥10∧净≤−3U→S33下修_exp自动执行(劣化线=下修后多头段n≥15且净劣于下修前同长窗→回滚),owner否决即关闭**;**多头cohort n6/−4.37@09-05 12:2x(死4/−5.34+赢2/+0.97;升级门6/10轨迹−3.11→−4.37;成员史git ad3c8d4)**;**空侧镜像(≥等值同口径,资金费率>0.0003:+0.20;独立观测不入多头cohort;判据=空侧决胜累计n≥6∧净≤−3U→议短侧入S33包)**:方向不对称结构化=空侧+0.20在赚不动(史n11/+0.84细目git ff561cc链);空侧批史(03:1x五决胜批净−0.10→n16/+0.74;06:2x XAN0317+0.117→n17/+0.86;逐笔细目)=git 933abbd版行;逐轮决胜扫描纪律=新入场评分行含资费bonus且减bonus后<门即计入;**+09-06 09:1x MARSCOIN0831空决胜闭+0.290(0.64−0.20=0.44<0.60门)→镜像n18/+1.15**,判据(n≥6∧净≤−3U)反向远离;多头cohort零新增(AKE0705空腿无资费bonus非决胜)维持6/−4.37=升级门6/10 |
...
| 09-06 09:1x-09:4xZ | 例行轮(max) | +0.29(n1)/+0.53(n2)/+0.65(n3 wr100)/+0.19(n13 wr62 be59)/+0.79(n36 wr58 be57)(毛) | FGI73 BTC79870+0.2% 市值−2.75% trending∩池=ARB,UNI,MARSCOIN,SUI | **HOLD 0原子包5连(队列无达门候选:S33攒死例距15差4[vision T+1无新档];S30下修6/10;#11远−0.84;select_limit→200扩面主动缓行=FREQ150 cohort段净−2.85未证正期望,先正期望后提频不满足)** | **24h组净转正+0.79(昨−1.85)且五窗全非负**;死法24h:SL穿刺9/−8.74 vs WIN21/+11.66 中段3/−1.35 超时2/−0.32 饥饿1/−0.47;归因main n18/−0.86(wr67<be70;6h n3/+0.65全胜)·trend n11/+2.55(wr55 be39)·壳n7/−0.90;新笔自06:2x仅2(AKE0705空+0.24/MARSCOIN0831空+0.29决胜);S31宽0.652=ON回翻(三态振荡日);镜像n18/+1.15;#79 n45零新增;KOMA n3/BULLA段3/6零新笔;ROUTE零差分4连(aging=CAP/TUT/USELESS);feed138(+1良性)三交集∅;钱包118.33全avail;费覆main24h🔴32连(6h🟢+0.216)/trend🟢+0.232;充值信号未触(12h wr−be+3pp<6pp);保证金0.55;待落0 |
| 09-06 08:4x-08:5xZ | 自查点stub: S33精测11案兑现,扫描64/73%<80%线不发货,距15死差4,RSI口径Cutler SMA14;全文git 044689a版行 |
| 09-06 06:1x-06:5xZ | 例行轮stub: HOLD 0原子包4连;24h−1.85(n36);S31宽0.413 N回翻;空侧镜像n17/+0.86(XAN0317决胜);费覆main红31连;全文git 933abbd版行 |
| 09-06 03:1x-03:4xZ | 例行轮stub: HOLD 0原子包;S31宽0.333 OFF首翻;空侧镜像n16/+0.74;#79 n45(案19-21);ROUTE零差分2连;24h−1.80(n42 wr55);钱包117.76;费覆main红30连;全文git 610475d版行 |
> 瘦身注(指针集@09-06 08:5x归并): 09-06 00:1x stub(trend种子写回)=git 14eca0b;09-05 21:1x stub(#84落地bl17)=git ff561cc;06:1x stub=git 2af6466;09-05五stub(15:1x/18:1x/08:1x/09:1x/12:1x)=git ff561cc+6a3383a/ad3c8d4版§7;09-04..09-05三stub+09-02..09-04六条压缩注=git 6a3383a/1195a37版§7(内含指针6d82e95/8c39ae5/f3c6469/558543d/3954cea/497cdf2)。
|---|---|---|---|---|---|
