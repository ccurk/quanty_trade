...
| 载具 | id | 原型 | 池 | 状态 | 门/备注 |
|---|---|---|---|---|---|
| main 通才 | 8eb182b6 | 通才(S18-S23栈) | auto·**feed88@08-17 15:2x**(#59落地:bl15种子✓@15:19;重启重播种100→heatclean-v11清12→88泄漏0) | running | pct0.08 sides=[buy,sell] long_conf_premium=0.35 tpl567;**trailing(act1.0/cb1.2%)+BE(1.0)=基线几何(#49终裁KEEP@08-16 18:2x)**;_exp=closed(verdict 628字存config._exp.verdict+git);**实验槽空闲**;#49终数=段n30/−3.81/均净−0.127(5.7×)/SL率20%/赔率0.43裁=统计失真keep(裁由全文_exp+git+§4#49);段亏全额=BICO×4−3.85(§6),ex-BICO n26/+0.04;RECOVER刹线6h空≤−8U继承;#11降级观察(§4);后续实验以本段为基线;**BICO已隔离@08-17 03:2x**(§6结案;隔离后零新笔✓复核09:2x) |
| qt-trend-long | 827ffe8c | 趋势动量多 | ACE/TUT/COTI(feed3=种子3✓;**aging_watch=ACE/COTI/TUT advisory@08-17 15:2x**[ACE 48h L n5/+0.80回落<1.5失格线,保留;COTI n2/−0.89;TUT 48h零笔];S24 keep后漂移观察→24h n5/+0.73 wr60%) | running | **tpl575(S24)** pct0.05 mcp3 buy trailing on;**归因修正@09:2x**:BICO 04:02(+0.57)/07:57(−1.90)两行盖trend sid实为main开仓(trend日志零开仓行,main日志全链;S24 keep判据'36h零新笔'修正后仍真);正牌08-16=ACE+0.02@06:46+COTI多在飞@09:30;**_exp=closed@06:35:12Z: S24终裁keep结案**(预注册当值格'净>0=keep':链n33/+0.024>0;末笔08-15 18:17 BICO−0.91,终裁前36h零新笔=03:11后零笔复核@06:35;程序=真空仓窗stop[poll1沉降]-PATCH-start,种子3币完好);**常驻保险(继承canary)=滚动6h净≤−5U→stop本载具**;实验槽空闲;候选EXP降prem=§4#55(费覆红线内只登记) |
| ~~qt-fade-short~~ | 21519f1b | **停机@08-16 12:26Z 劣化线**(预注册'累计n≥10∧净≤−5U'触发:n12/−5.44;终裁=config._exp verdict=degraded_stop;stop舍rollback四由+逐笔链全文git 766c7d9后继) | — | stopped(**gated劣化,勿auto-start**) | 要点:原型病非仅选币病(池内挣钱行为全在main几何下,VELVET nS5/+7.03);5币已回main(bl17→12);tpl576+S25存git,复活=新壳走FLEET;壳留seed5+auto_symbols=false硬停;自动保险随停机失效;**停机壳=收养错标磁铁实证@08-17**(main开BEAT/BLUAI落fade sid+fade config接管,#57修复a451d32候部署;账按开仓者归main) |
| ~~qt-breakout-follow~~ | 2111f5f9 | owner删除@08-15 15:5x(评判FAIL史+S26+tpl577存git;#47复活=新壳FLEET) | — | deleted | 幽灵运行时00:1x发现→检疫+heatclean固化;**幽灵仍活实证(最新08-17 02:37)**:STAR空01:37→02:37超时−0.29=删除后第3笔,累计n3/−0.15(18:25/19:25史+0.32;API404复核不可控;"沉寂"=信号荒假象);**#53修复fa93736候部署**(RemoveStrategy无条件Kill+stopping压制复活;同分支#56 closed归因order_id反查),owner部署+重启即根治=当前唯一通路;详史git 766c7d9+§3 |

- 隔离区(规则≤−4U∧n≥4): 4/CYS/TST/龙虾/BMT/BTW/H/APR/AIO/BICO/**BEAT/XNY**(+BEAT+XNY@08-17 13:0x对账器v1.3双触线:BEAT 48h n9/−4.19**混合血统**[fade era 4笔−3.43+main era 5笔−0.76,rehab评估时须知main era仅−0.76]、XNY 48h n6/−4.23全main空侧;rotate热移88→86✓,bl15种子同步挡窗§5#59;+BICO@08-17 03:2x对账器v1.3:48h n8/−4.05全多头≤−4∧n≥4[24h口径n3=窗口差,08-15五笔在册],资金费磁铁§6结案,rotate热移+bl13种子同轮闭环;+AIO@08-15(n4/−6.53全空)/APR(n5/−7.54)/H等史git)。**池列值=快照,权威=每轮对账器输出**。**对账器现版v1.3权威副本=quanty-ledger分支ops/route_pools.py@08-14(=334dae1内容;谱系61b1bb4←377c567←f970704;默认分支工作树只有v1.0初版——禁直接跑,实证2例错误plan)**:出池只认头注规则或隔离(失格币aging_watch);隔离表arg4;add互斥只排他专家池(v1.2误把main feed入排除集=晋升封死,COTI案实证修复);晋升币同轮移出main(封v1.1单轮重叠窗,执行序=先main remove后专家add);brk/lowvol出池阈值未注册前无自动出口(#42)。**arg4输入纪律(08-15 12:2x实证):隔离表必须传X/USDT形态——裸名norm后不match feed键,5隔离币漏出remove计划,当轮补刀rotate清除;代码零改动,属输入格式纪律**。最近跑@08-17 15:2x: **plan={}**,desired bl15=actual✓(#59闭环),aging_watch=ACE/COTI/TUT,want.fade=SPORTFUN仅存档
...
| 35 | M通道 | **S14 CB状态持久化**（restart清内存计数株连修复；方案候选=①引擎侧enforcement+计数DB落库[LastEntryAt播种先例L315-316] ②cron落地前CB快照→config种子字段策略init读回） | 方案细节+先例链见git(08-14轮commit);要点=CB计数DB落库,restart不清 | n=2币/实现成本−7.66 | 候选(候部署窗,deploy队列积压4支;缓解=restart节流纪律在用;反事实双向入账纪律自此适用;经owner部署后生效) |
...
| 15 | M通道 | 脱管仓sweeper(实仓∩DB无行→重收养/告警) | KOMA 29h/−15.13+源码L48;FOLKS粉尘watch史git 26eaf24 | 0/2 | **结案@08-16 21:2x**——部署后≥5轮零新漂移;复现即重开 |
| 18 | M通道 | closed窗口边界播种修复(幻影行族根因) | 幻影三形态全史+标本链见git 26eaf24 #18行(BTW隙5幻影/镜像行/ROBO整60m例) | n/a | **修复部署@08-15**(open_key唯一索引+entry去重入f4848f8);**观察第4轮@08-17 09:2x:未归零翻案**——48h窗7隙型幻影(BEAT×4/VELVET×3;形态=开时刻=前仓平点/平时刻=后仓开点/方向翻转long/sid空/零pnl;最新close=08-17 08:08=部署后仍生成;ROBO例已出窗)。与21:2x轮'0新增'矛盾=检测器口径差(missing-key vs null)或懒生成,未定。f4848f8只封边界播种,隙型=另一路径;滤行纪律无限期保留;结案重置=隙型机制源码查明或#56部署后复核;**观察5@08-17 13:0x:同窗重拉隙型=0**;**观察6@15:15:又见5隙行全旧**(≤08-16 14:03,新增0;13:0x同窗0↔15:15拉5=行集随重拉波动,懒生成再证;滤行纪律续)  |
...
| 58 | E9类(main扩仓) | **main pct 0.08→0.10**(config-only;Σ=0.10×5+0.05×3=0.65≤0.75✓;回滚=回0.08) | main载具费覆首次12h/24h双🟢@08-17 09:2x(24h n12/+4.74 avg+0.395=3.8×费 wr83全空侧;12h+0.167=1.6×);硬边界#1按载具判=允许,组口径🔴不锁单载具扩仓但#57在飞收养竞态=fade config接管main赢家仓(BEAT/BLUAI实证)→扩仓入缺陷不洁 | n=12段 | 候选@09:2x;**门=①a451d32部署+#57双行归零观察2轮 ②main 24h费覆🟢维持 ③执行窗现拉复核净为正**;执行走EXP _exp预注册(main槽空闲);**门②失守@13:0x轮**(main 24h avg+0.009🔴/12h−0.485🔴,BEAT/XNY隔离或改善样本;门重挂=双🟢再现) |
...
| 57 | 引擎候修(M通道) | **收养幂等闸(账户级)+守护唯一化**:收养前现查共享账户任意owner名下同symbol open行,有即跳过;守护循环每(账户,symbol)每tick单行驱动TP/SL(owner升序+行序恒定) | §3#57条(BEAT双行互搏全链);BLUAI同型;根在roi_monitor快照竞态+per-strategy查重 | n=2 | **修复已推@08-17 07:0x a451d32**(fa93736同分支后继);候owner部署;部署后验证=开仓3s窗零收养行+'重复止盈止损单'日志归零,观察2轮结案 |
...
| 08-17 15:15-15:4xZ | 例行轮(max) | @15:16: 1h 0(0)/3h+0.29(1)/6h−1.42(7,wr57)/12h−2.18(11,wr55 be66)/24h−1.55(17,wr59 be64) | FGI31/BTC+1.2%/市值+0.9%无降压;trending∩池=GPS/PORTAL/PUMP(main) | ①**§5#59落地**(15:19全账户空仓窗程序v2:stop→poll1沉降→PATCH bl15→start;重播种100→heatclean-v11清12→feed88泄漏0;对账器复核desired=actual闭环) ②0原子包HOLD(组24h费覆🔴avg−0.091 vs 3×RT≈0.13;main 24h avg−0.230🔴 #58门②续失守;trend 5.0×🟢但n5;#55#47续锁;双_exp槽空闲) ③ROUTE plan={};aging_watch=ACE/COTI/TUT(ACE回落+0.80<1.5) ④刹车全绿(6h−1.42/24h−1.55 vs −17.5/−43.9;trend6h+1.91>−5;main空6h−3.33>−8;Σ0.55✓) | 死法24h:TP6/+4.16 SL穿刺4/−6.51=最大失血(全在已隔离币);**隔离刀验证:main 6h−3.33全=BEAT−1.88+XNY−1.74,12:57后main仅ACU+0.29,ex-iso 24h n4/+0.20**;trend 6h+1.91(ACE 3W+1.95,COTI超时−0.04,TUT零笔);ghost STAR 01:37后13h静默(累计n3/−0.15,#53候部署);幻影行=5隙型全旧(新增0,§4#18观察6);钱包219.43空仓;充值信号未触(12h/24h负);⏭隔离后首个干净24h窗→#58门②复评/#53+#57部署提醒 |
| 08-17 12:53-13:4xZ | 例行轮(max) | @12:54: 1h+0.29(1)/3h−2.86(4)/6h−1.65(6,wr67)/12h−3.01(10,wr50 be67)/24h−1.29(18,wr61 be65) | FGI31/BTC+0.9%/市值+0.6%无降压;trending∩池=ACE(trend在飞) | ①0原子包HOLD(组24h费覆🔴avg−0.072 vs 0.134;#58门②失守[main 24h avg+0.009🔴/12h−0.485🔴];#55#47续锁;双_exp槽空闲) ②**ROUTE隔离BEAT+XNY**(48h BEAT n9/−4.19+XNY n6/−4.23双触≤−4∧n≥4,v1.3 plan;rotate热移88→86✓;bl15种子同步2次stop不沉降→**§3铁律纠错:main stop需全账户空仓,COTI在飞挡窗,非新异常**,§3第4例+§5#59排队) ③aging_watch=COTI/TUT(ACE 48h L n7/+1.66 wr71%重达标出列) ④刹车全绿(6h−1.65/24h−1.29 vs −17.5/−43.8;trend6h+1.36>−5;main空6h−3.01>−8;Σ0.55✓;宏观无降压) | 死法24h:WIN8/+6.85,twin3/+1.03,**fastSL5/−8.12=最大失血机制**,tloss2/−1.05;载具拆分=main n12/+0.11(wr67全空侧;EMPTY sid29行=#56缺口按池归main)/trend n3/+0.18(wr67,ACE 1L2W转正;COTI多在飞12:40)/ISO1/−1.61(BICO末笔,隔离后零新笔✓)/ghost2/+0.02(STAR,02:37后10h+静默);幻影行=0(§4#18观察5);a451d32未部署(BEAT行10:27仍fade sid);钱包≈219(avail203.5+COTI仓);充值信号未触(12h/24h净负);⏭#59全账户空仓窗重试/a451d32部署观察/#58门重挂 |
| 08-17 09:2x-09:5xZ | 例行轮(max) | @09:20: 1h+0.32(1)/3h+1.19(2)/6h−0.75(4)/12h−0.80(6)/24h+1.13(17,wr65 be61) | FGI31无降压 | 0原子包HOLD(组费覆🔴锁#55#47);**#58 main扩仓候选登记**(main首次12h/24h双🟢0.167/0.395,门见§4#58);ROUTE plan={};#18翻案7隙型行;刹车全绿 | 死法:WIN11/+7.53 fastSL3/−4.51;main n12/+4.74(wr83全空侧)/trend n2/−2.03;钱包≈222;全文git 0db226f |
|---|---|---|---|---|---|
