...
| 载具 | id | 原型 | 池 | 状态 | 门/备注 |
|---|---|---|---|---|---|
| main 通才 | 8eb182b6 | 通才(S18-S23栈) | auto·**feed87@08-23 06:2x(#25重播种热清13/13;bl18种子完好)**(#24清11/#23清10零挡;#22以前+宇宙换血史git 18a2dbf) | running | pct0.08 sides=[buy,sell] **long_conf_premium=0.35(#61c终裁rollback@08-19 15:3x,门0.90=多头事实关闸,#61族基线)** tpl567;**trailing(act1.0/cb1.2%)+BE(1.0)=基线几何(#49终裁KEEP@08-16 18:2x)**;**_exp=closed rollback(#61c终裁;史§4#61/#63+git 0940c62/95e4c9f)**;#49终裁KEEP史git;RECOVER刹线6h空≤−8U继承;#11降级观察(§4);**BICO已隔离@08-17 03:2x**(§6结案) |
| qt-trend-long | 827ffe8c | 趋势动量多 | **COTI/TUT**(种子2=feed2✓#26重启后重建正确;ACE隔离@08-22 12:22;aging=COTI+TUT;破荒史git f43756c) | running | **tpl575(S24)** pct0.05 mcp3 buy trailing on;**_exp=open·#69三值包重申请@08-23 09:18Z执行**(adt0.5+lcp0.149+mces off;终裁二元=新段[09:18Z起open_time]逐笔净>0→keep否则回滚[adt1.0/lcp0.15/mces3];eval=n≥16或08-25 09:18Z先到;劣化线n≥8∧段净≤−4U;程序stop[poll1]-PATCH-start✓running✓feed2✓;否决窗06:2x TG→09:1x空置+反转检查=回滚后段仅n1[COTI 07:02L−0.79]轶事);原三值段终裁rollback(n18净−0.40)+ex-ACE归因(n14净+4.11 wr86%)全文git 5df52d2+config史;**常驻保险=滚动6h净≤−5U→stop(继承)**;三值包全史git 8a1d995/f71d825;S24 keep史git f43756c |
| qt-fade-short-v2 | 7583727a | 冲高回落空(tpl576/S25继承) | HANA/COLLECT/**MELANIA**(feed3=种子3@08-22 18:2x #68写回落库✓;MELANIA晋升自main@15:2x[48h S+2.29 nS3 wr100%];⚠️#57-ROUTE病理活测第3例[VELVET/BULLA先例],看护=隔离线/出池线/保险线既有三层) | running | 建壳@08-21 16:5x(tpl576/S25继承;config史+首笔+三层拦链git f43756c);**FLEET评期提前结案@04:1x**(管道域PASS/盈利域不可判n1;verdict=config._exp.prior_fleet_verdict);**低波适配三值包@04:1x(owner直令)**: minatr0.35+adt0.5+scp0.049[#65修]=梯度防护(≥0.5全分/0.35-0.5需raw0.75/<0.35拦);**_exp=open·中间裁决@21:2x=延评一次(落空档;verdict_n8全文config._exp+git 8a797ec;终裁n≥16或08-24 12Z二元[段净>0→keep否则回滚三值]不再延;劣化线n≥8∧≤−4U+带级护栏−3U+6h保险−6U站岗)**;**进度@08-23 06:2x: 段n12/16毛−0.16净−0.43 wr33%(4W+2.27/8L−2.43);新笔MELANIA 03:47空+0.20(22m,band未捕获=03:25开仓已滚出logs窗[2000行buffer仅回溯至04:29,带级捕获须当轮纪律],带级护栏为亏损触发不受影响,地板带累计仍3笔−0.33);MELANIA fade era n5/−0.86,全账户nS8/+1.43出池线(nS≥6∧S净<0)未触,aging_watch维持;标本④⑤→§4#66不变**;程序=stop[poll1]-PATCH-start✓(嵌套体二次实锤史§3+git 8a797ec);保险=6h≤−6U停(继承) |
...
- 隔离区(规则≤−4U∧n≥4)【13】: 4/CYS/TST/龙虾/BMT/BTW/H/APR/AIO/BICO/BEAT/XNY/**ACE**(+ACE@08-22 12:22[n4/−4.40闪崩滑点主因;榜上仍隔离=失血压过信号;详git 8a797ec])(+BEAT[n9/−4.19混合血统,main era仅−0.76 rehab须知]+XNY[n6/−4.23全main空]@08-17 13:0x;+BICO[n8/−4.05资金费磁铁§6结案]@08-17 03:2x;+AIO/APR/H等史git;逐条全文git bb3b883)。**池列值=快照,权威=每轮对账器输出**。**对账器现版v1.4权威副本=quanty-ledger分支ops/route_pools.py@08-22**(v1.4=roster遮蔽修复:ROSTER同角色双壳时stopped旧壳[21519f1b]覆盖running新壳[7583727a]的running态→fade差分v2上线以来静默跳过,MELANIA晋升条达标无plan实证@15:18;修复=running壳优先;头注阈值零改动;谱系v1.3@08-14←61b1bb4←377c567←f970704;v1.0默认分支禁跑;演进史git bb2e)。**arg4输入纪律(08-15实证):隔离表必须X/USDT形态**(裸名norm不match feed键→漏移)。出池只认头注规则或隔离,失格=aging_watch advisory;brk/lowvol出池阈值未注册(#42)前无自动出口。最近跑@08-23 09:15(v1.4): **plan=main.remove13(重播种#26泄漏,同#25名单:隔离9[4/ACE/APR/BEAT/BTW/CYS/H/TST/龙虾]+专家4[COTI/TUT/HANA/MELANIA])→rotate热清13/13✓feed100→87残留0**;aging_watch=trend.COTI+TUT/fade.HANA+MELANIA;无新隔离;want.fade=COLLECT(已在池);want.breakout=HANA/MELANIA/MOVE/PUMP/TRUMP/TUT(#47锁);前跑06:21(#25热清13/13);零差分连4史止于#25;15:18 MELANIA晋升/12:20 ACE隔离详git 8a797ec;前史git d0e40d9/2ce0fb4/f623b98/044907d
- 保证金: Σ_running=main0.08×5+trend0.05×3+fadev2 0.04×2=**0.63≤0.75✓**(08-21 16:5x FLEET建壳重算;前史0.55见git)
- 互斥不变式: feed层@08-23 09:2x复核=main87/trend2/fadev2 3**两两∩∅**(热清#26后);种子层=main bl**18**=desired**18**零缺口✓;重播种**#26=08-23 06:21-09:15窗(无audit非配置类重启**连2、~3h节律疑外部部署/崩溃**;窗内main于泄漏13币零成交=bl交易闸零违例;trend/v2种子重建完好;fadeV1未误复活✓)清13/13;#25=03:18-06:21窗清13/13;#24=08-22 18:2x计划内清11零挡;#23清10;#20=首例实证bl币可直漏feed('重启免疫'=交易闸级非feed级)**(#22及#8-#19史git);引擎互斥闸✓live;**fade壳seed5与main auto理论重叠=停机态无违例**(不变式限定running载具;若owner UI复活fade须先重划池,互斥闸+TG示警兜底);隔离币零成交✓(复核@08-21 09:2x,48h窗隔离12币零平仓行)
...
| 08-23 09:1x-09:3xZ | 例行轮(max) | 1h 0(0)/3h−0.82(1)/6h−0.14(6)/12h−0.44(10,wr60/be64)/24h毛+1.17净+0.93(31,wr61/be57=+4pp) | FGI66 Greed/BTC76.6k−0.9%/市值−3.9%无降压;trending∩池=PUMP/TRUMP(main) | **#69执行=trend三值包重申请**(1原子包;adt0.5/lcp0.149/mces off;新段09:18Z起,eval=n≥16或08-25 09:18Z,劣化n≥8∧≤−4U,保险6h−5U继承;程序stop[poll1]-PATCH-start✓);**ROUTE=#26重播种热清main13币**(feed100→87;窗06:21-09:15无audit连2~3h节律疑外部;bl闸零违例);刹车全绿[6h−0.14/24h+0.93];载具保险绿[trend6h−0.26/v2+0.20];费覆24h毛均+0.038<0.090🔴禁提频(6h−0.008🔴);充值不发[12h负] | 死法24h:WIN19/+7.73,硬超时9/−3.53最大簇,SL中2/−2.24,饥饿1/−0.79;载具:main9/+0.32 wr56(超时簇主载体),trend14/+1.72 wr79毛均+0.123🟢,v2 8/−0.87 wr38🔴;v2段n12/16零新笔(终裁n≥16或08-24 12Z);MELANIA v2era n5/−0.86距出池线1笔;audit三车零无主;幻影0 |
| 08-23 06:1x-06:4xZ | 例行轮(max) | 1h−0.53(1)/3h+0.59(5)/6h净+1.15(7)/12h净−0.07(14,wr57/be58)/24h毛+3.19净+1.67(51,wr63/be59=+4pp) | FGI66 Greed/BTC76.2k−1.4%/市值−5.2%未达线无降压;trending∩池=PUMP/TRUMP(main持有) | **trend _exp终裁=ROLLBACK执行**(n18门到,段净−0.40≤0;三值回adt1.0/lcp0.15/mces3;程序stop-PATCH-start✓;ex-ACE n14净+4.11 wr86%→**#69重申请候选队首,TG报owner否决窗至09:1x**);**ROUTE=#25重播种热清main13币**(feed100→87互斥恢复;窗03:18-06:21无audit疑外部重启,bl闸零违例);0新原子包;刹车全绿[6h+1.15/24h+1.67双正];两保险绿[trend6h+0.53/v2+0.62];费覆24h毛均+0.063<0.090🔴禁提频(6h+0.193🟢);充值不发[12h−0.07] | 死法24h:WIN32/+13.28,超时12/−4.13最大簇,SL中3/−3.31,饥饿3/−1.92,fastSL1/−0.73;载具:main24/+0.94毛全短(6h零笔=夜静+多头门0.90),trend17/毛+3.79净+3.30 wr82%,v2 10/毛−1.54 wr30%;audit三车零无主;v2新笔03:47 band未捕获(logs窗滚出);幻影0去重0 |
