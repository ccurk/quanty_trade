...
| CC-11 | 巡检 | pct0.25 配置下每轮报: 保证金占用峰值/最大单仓名义/最大单笔SL亏/6h,24h钱包%;DS breaker 若砍pct→按owner令恢复0.25并TG(裸改簿) | 09-18 13:1x | open(**读数@20:2x: 最大单仓名义 557U(USELESS)/最大单亏 M −7.81(−1.43%价 1m03s)/6h income 净 −49.3U=−31%(基 159)/24h −43.6=−20.7%/1h −8.4/avail 168.3(入金 +58.5 在 3h 窗,+83.56 在 12h 窗);DS 15:22 后 0 PATCH**) |
| CC-12 | 候owner重启 | atr_tp_mult 2.0/atr_sl_mult 1.5 已入config,进程仍4.5/2.5→owner重启(或CC stop?force=true→start,v5.3准许)后生效;重启后10分钟自查: feed重播种→rotate隔离币、首单tp/sl距离≈2.0/1.5×ATR(logs q=止盈止损) | 09-18 13:4x | open(@20:2x 进程仍 4.5/2.5:tpsl 日志 tp距/4.5 反推 ATR 自洽;owner 下次重启即生效,重启后 10 分钟自查) |
...
| CC-15 | owner裁决 | **p6 出场几何 D 方案(推荐)**: 只放宽赢单侧 trailing_callback_pct 0.5→1.2 / trailing_activation_atr 0.5→1.0 / breakeven_trigger_atr 0.5→1.0(亏单侧 hunger_sl0.075/max_hold45 不动);证据=6 笔可取赢单 45m 现货 MFE vs 实收: G +11.49/+2.07, VTHO +5.22/+2.39, LPT +3.72/+0.03(BE 出场), JUP +1.69/+0.08(BE), STRK +0.55/+0.14, HEI +0.93/+0.03;代价 STRK/HEI 转 0.75% 止损(MAE45 −1.21/−4.52);粗估 6 笔 +17→+35~40U(现货代理小样本);备选 A(反事实否决)/C 维持;owner 表键 CC 不擅动,每轮 TG 提醒 | 09-18 15:2x | open(**20:2x 第 3 次提醒: D 赢单侧仍候;p7 赢 7 笔均 +0.9%价 全被 0.5×ATR 追踪/BE 截断,MFE 证据未变;lev5/关交易所TPSL 议题因 ROI 口径落地(0.4%价<3% 钳)已失效→撤**) |
| CC-17 | 评判 | p7(owner hunger_sl **0.04**=ROI 4%,v59@17:59:41Z) 段(open≥17:59:41Z): 段均净(毛)≥0 ∧ 止损占比 ∧ 单次均亏 ∧ 赢单是否被 0.4% 噪声提前杀 ∧ 6h/24h 钱包%;eval 09-19 00:00Z 或 n≥30;劣化(段净≤−10U 或 24h≤−20%)只 TG 建议;DS 砍 pct→按 owner 值恢复 | 09-18 18:0x | open(**读数@20:2x: DB n15(+STRK/SAGA 漏行+AR 20:14=n18) 172笔/日 毛+1.59→≈−3.3 wr47% be45% 赢均+3.49 亏均−2.85 hold中位1.7m;饥饿SL 11/−27.2 触发 roi −4.0~−11.6%(首查第 1 分钟整点过冲);赢 7/+24.4 全 trailing/BE;费/笔 0.38U 🔴;worsen 24h≤−20% 已触(−20.7% 含入金基/−62% 入金前基)→TG🆘 建议 D/pct 减半/维持,不擅动**) |
| CC-18 | ROUTE 落地 | 48h 币净≤−4U∧n≥4 机械规则: SYN 17/−8.23, 牛来 9/−5.81, PIEVERSE 5/−5.47, USELESS 13/−5.31, AIN 22/−4.60, MARSCOIN 8/−4.12 + CYPH(-4411 TradFi) → blacklist 26→33(v62)+rotate remove 7→feed 281→274 ∩=∅ 复读✓;隔离出=7 天无交易∧画像翻转或 owner 令;注记: −4U 阈值在 300~500U 名义下=1 次止损即触(过敏),候 owner 改钱包比例 | 09-18 20:2x | **落地@20:2x** |
| CC-9 | DS越界簿 | admin 06:30:06 mcp 10→4=协议7越界#3(史: 09-16 22:10 10→6 / 09-17 13:15 6→2);已纠回;**第4次@15:22:01Z: breaker lock-1789744921 砍 pct 0.25→0.125(声明)+mcp 10→6(未声明)→按 owner 13:0x/13:4x 直令恢复 pct0.25/mcp10(v56 复读✓,触碰其 frozen_keys 已 TG 明示);再犯=不硬抢,TG owner 裁;建议 owner 在 qt_breaker.py 冻结 mcp/pct 并把 −4U 阈值改钱包比例** | 09-18 08:1x | open(@20:2x: 15:22 后 DS 0 PATCH,lock 至 09-19 15:22 未再砍;1h 净 −8.4<−4U 阈却未触=锁期内不重复砍(观测 n1)) |
> 结案集@09-18 20:2x(DS-1/DS-10/CC-3/CC-7/CC-8/CC-10/CC-16/CC-14/DS-11/CC-10/CC-6 终态全文=git 2fcc0da版§5)。
...
| **09-18 p5(pct0.25) 终裁 + p6(紧出场) 读数** | @15:2x **p5 段(open≥13:09:22Z) n31 395笔/日 净+8.40 均+0.271 wr58% be53% 赢均+2.47 亏均−2.77 名义中位291U(max429) → KEEP 3/3(owner 值);诚实注记 G 2笔+21.62 撑段,去 G −13.2** ‖ **p6 段(open≥13:39:15Z) n23 全多 净−11.46 均−0.498 wr48% be58% 穿刺13%;饥饿SL 11/−28.7;微赢(<0.4%价)7/+2.95;跑赢 3/+18.29;conf0.44 桶 n10 −15.73**;24h 逐笔 n100 均+0.119 wr54% be49%;income 24h 毛+14.86 手续费−12.86(费吞 87%)净+2.19;6h +1.63;1h −3.88;入金 +83.56 已在 24h 窗;费覆 p5 0.271 vs 3×费0.87 🔴 | 09-18 15:2x | **p6 封段终读@17:56Z(open 13:39:15→17:56:55) n71 398笔/日 净−34.75 均−0.490 wr44% be55% 多51/−41.69 空20/+6.94;饥饿SL 33/−91.8 vs 赢 31/+61.8;hold<10m 60/71;conf0.44 桶 n25 −27.89, ≥0.56 n15 +19.46;15:25 后子段 n41 −30.99 饥饿SL 22/−54.72** → owner 自落 hunger_sl 0.04(ROI 4%);**p7 读数@20:2x n15(+3 漏行) 毛≈−3.3 wr47% be45% 饥饿SL 11/−27.2 roi −4~−11.6% hold中位1.7m 费/笔0.38 🔴;6h income −49.3(−31%) 24h −43.6(−20.7%/−62%);ATR>1% 币 n4 −9.66 wr25% vs 0.5~1% n9 +4.88(方向性,小样本)**;D 候 owner(CC-15) |
