...
| 载具 | id | 原型 | 池 | 状态 | 门/备注 |
|---|---|---|---|---|---|
| main 通才 | 8eb182b6 | 通才(S18-S23栈) | auto·feed89(黑名单18在库⊃期望14;第2次无端重播种00:25已自愈,**06:3x复核feed89稳定6h+无第3次**;孤儿3[KAITO/DODOX/BLUAI]留feed待种子同步) | **running@08-12 22:20Z(全组复飞令;[buy]latch维持)** | pct0.08 sides=[buy](刹车latched) tpl567;段(自22:20)**n2/+0.07**(HOME+0.62硬超时60m+COTI−0.55硬超时60m,均空sid行按池归main);**S23评期已裁@08-13 00:33Z=证据不足·结案**(跨段A组19<20,AL/WL噪声方向暂反;代码在场纯观测;重开判窗=#37;本段计数@06:29统计行AL1/0 AS1/1 WL1/0 WS0/1;_exp槽名义空,清写入#45) |
| qt-trend-long | 827ffe8c | 趋势动量多 | TUT/BICO/ACE(种子同步✓;aging_watch=全3币维持@08-13 06:31 v1.2对账[48h窗仅TUT/BICO各n1+ACE 0笔],保留在池) | **running@08-13 06:35Z(评期裁决PATCH原子窗stop→PATCH→start;06:33首次stop后误保护start多一次重启,无害已记)** | **tpl575(S24定制)** pct0.05 mcp3 buy trailing on;**S24评期v1已裁@08-13 06:35Z=证据不足·期满续观测**(48h到点段n7/−2.13 wr43[段1 n5/+1.06@31.4h+复飞段n2/−3.19=TUT闪崩穿刺−2.26+BICO饥饿−0.93],keep门n≥15与劣化线n≥10∧净≤−5U均未触→**S24保留零代码动作**);_exp v2已注册:累计口径承前,keep=累计n≥15∧净>0∧费覆,劣化=累计n≥10∧净≤−5U→rollback tpl567,评期=**累计n≥15或08-16 06:35Z先到**;canary保险滚动6h≤−5U维持(已随本PATCH写入config=#45 trend部分落地;重启清内存,cd由LastEntryAt播种,feed3重建✓) |
...
| qt-breakout-follow | 2111f5f9 | 突破追动量(顺bar,S23 W组实盘化) | COOKIE/DYM/BTR(种子同步✓;aging_watch=BTR/DYM@06:31对账) | **running@08-12 22:20Z(全组复飞令;自动保险在力)** | **tpl577(S26新内核)** pct0.04 mcp2 buy hunger30 max_hold45 trail(1.0/0.8);_exp EXP在飞,段真值**n9/−2.10 wr56**=前8/−1.05+COOKIE−1.05(08-13 02:03快SL,hold9.5m,mv−3.7%);COOKIE段3胜2负+0.14仍组内最佳币;**策略自防护在工作@06:2x日志:BTR策略内隔离中+COOKIE连败2计数+BTR追多被RSI超买硬拦(82.7>68,多头分0.95=blowoff帽实证n=1)**;评期**累计33.73h/72h→≈08-14 12:36Z或段n≥15**;自动保险段滚动6h≤−5U(现−1.05未触);TUT入池提名被v1.2封死(在trend池) |
...
- 互斥不变式: feed层✓(overlap=0,08-13 06:3x复核)>种子层blacklist18⊃期望14(孤儿4待重启窗blacklist_sync释放)>⚠️引擎互斥闸未部署,部署升急;对账器**v1.2已落本会话分支21lcmn@61b1bb4**(谱系6fe64eb←rv5cwj 377c567;出池只认头注规则+隔离表arg4+add互斥;08-13 06:31运行:plan={}零rotate,aging_watch=trend3+fade2+brk2,期望bl14与台账一致)
...
| 30 | S21类(代码gate) | 追跌空反弹拒空veto候选(gate7e,暂缓中): short且跌势陈旧拒;draft单维300m阈值族**证伪@08-07**(误杀24%>10%门,收紧复审仍12%,−10~−14带无分离力);下一迭代=加第二判别维(距60m低点反弹幅/自低点时间) | 追跌型累计10例/−24.28+晨午簇候审15(vision T+1定标待做)〔审计全史见git 121e975 #30行〕 | n=10+候审15·暂缓 | 执行门=确认型≥20或owner核准;影子通道S23 WS vs AS已随#37冻结;误杀审计先行原则保留 |
...
| #43(冻结@02:38Z停机直令) | RECOVER执行包 | 门=组逐笔6h≥0∧24h≥0(**≥0含0,n=0视为0**——latch下零成交是结构性上限,止血即达门;此解释先于数据登记防移门柱;组口径承12:3x轮先例)→步=main sides=[buy]→[buy,sell]空仓窗stop→PATCH→start原子,同PATCH写回刹线:unlatch后6h main空侧净≤−8U(≈钱包3.1%,镜像brake_20260809原触发型短burst)→重latch sides=[buy]续用同键;执行轮先复核无新刹车触线∧宏观降压未触发 | 08-10 18:2x | open,冻结维持;门读数: 12:38Z首读→18:14Z第2读→08-12 00:17Z第3读→06:26Z第4读→**12:34Z连续第5读达标**(第4/5读6h=0✓∧24h=0✓[双窗n=0=预注册视0];第3读6h=0✓∧24h=+0.35✓;无移门柱)→**18:2x第6读不达标**(6h−3.19<0=canary段亏,达标链重断,unlatch候双转正重来)→**22:19复飞令到冻结解除,执行前复核(第7读)FAIL**(24h−3.19<0)→**08-13 00:3x第8读FAIL**→**06:3x第9读FAIL**(6h−0.98✗∧24h−4.18✗;新3笔COOKIE−1.05/COTI−0.55/HOME+0.62入窗,TUT/BICO 15:22滚出后24h仍−0.98,复位窗后移至新3笔滚出≈明日01:00-02:13,除非新增盈利抵消);执行窗=门成立∧全组空仓窗(=main空仓窗,回声行自愈已证) |

| #45(缩@08-13 06:3x) | _exp补写×3(main/fade/brk) | 四载具config._exp补写resume_note(评期重锚累计口径;trend另含canary_insurance):trend原PATCH 13:54:27Z被500拒(running锁,audit1717载荷留痕);fade/brk/main 22:20复飞后同样running锁。**重锚权威=§1直令条(08-12 22:19+补录)+§1.5各行**,约束力不受config缺载影响;**main payload追加@08-13 00:3x**: s23_closure速记(证据不足结案00:33Z)+_exp槽清空;**trend部分已落地@08-13 06:35(S24评期verdict PATCH原子窗顺带,canary_insurance+resume_note入库✓)** | 08-12 18:2x扩22:4x缩06:3x | open,阻塞=三载具running;落地窗=自然stopped窗或eval verdict动作同PATCH带上(fade评期今12:28Z=下轮首选窗) |
...
| 计数项 | 读数 | 更新轮 | 备注 |
|---|---|---|---|
| S23逆bar影子观测(main _exp 08-08起) | **裁决=证据不足·结案@08-13 00:33Z**(评期=累计48h先到;跨段求和[#34行纪律]可观测A组n=**19**<20门=seg1 15[AL1/3+AS4/7@08-09 12:30]+seg2 3[@15:50抢救]+本段1[AS0/1两拍一致,00:18/00:26];seg3+22:20段未捕=+0;主判据AL vs WL=1胜4负wr20% vs 0胜3负wr0%→n5v3纯噪声且方向暂反假设→不可裁不升enforcement零事后改指标;S23代码原样在场=纯观测零交易路径,摘除不值一次apply) | 08-13 00:3x | 重开判窗=#37部署后重启观测段(DB可回补`?q=统计汇总`跨段真值);_exp物理清写入#45;逆bar族假设(#30/#34)冻结至可观测 |
| main无端重启重播种观测 | **1@08-13(22:5x-00:18窗feed89→100;rotate源码排除非重启回加路径[strategy_symbols.go:feedSymbols仅rotate/start改写]→必经进程重启;窗内零apply零start API;audit盲区=start/stop不写;领先假设=owner后端重启/部署BOOT RESTORE)** | 08-13 00:2x | 复现→升§3候证平台事实+评估全组counter清零对评期锚影响;黑名单闸全程在防(窗内唯一开仓HOME不在bl18)=零实仓风险;每轮复核feed数 |
...
| 统计汇总行未捕获连续轮数 | **08-13 00:1x轮罕见2拍成功**(00:18:01/00:26:03安静分钟捕获=当前段读数;评估爆发期100行窗仍仅覆~2s,跨段累计依旧不可得;**?q&limit第7验未部署**[limit300返100行,q过滤无效]);前史结构性不可捕结论维持〔压缩@08-13,全文见git 121e975〕 | 08-13 00:2x | 跨段求和纪律不变;#37七催(S23判窗重开的解锁条件) |
...
|---|---|---|---|---|---|
| 08-13 06:25-06:5xZ | 例行轮(max) | close: 1h0/3h0/6h−0.98(3)/12h−0.98(3)/24h−4.18(5,wr20) | FGI29/BTC+0.29%/mcap+0.24%无降压;trending∩池=APR(main内) | ①**S24评期v1裁决=证据不足·期满续观测**(48h到点段n7/−2.13,keep门n≥15与劣化线n≥10∧净≤−5U均未触→S24保留;_exp v2注册=累计n≥15或08-16 06:35Z,原子stop→PATCH→start@06:35,#45 trend部分顺带落地;06:33首stop后轮询用错端点误保护start,多一次无害重启已记)②ROUTE v1.2对账plan={}零动作(aging_watch=trend3/fade2/brk2,失格源=48h证据窗萎缩仅5笔;无新隔离)③对账器v1.2落本会话分支21lcmn@61b1bb4④#43第9读FAIL(6h−0.98∧24h−4.18)⑤1原子包 | 钱包251.9空仓;段内新3笔:COOKIE−1.05快SL9.5m+COTI−0.55硬超时+HOME+0.62硬超时;main段n2/+0.07;brk段n9/−2.10(BTR策略内隔离+RSI超买拦血证n=1);四漏斗健康零信号=市况安静(置信中位0.36-0.48<0.55门);费覆🔴24h均净−0.84禁提频;载具保险全未触;fade评期今12:28Z下轮裁 |
| 08-13 00:17-00:4xZ | 例行轮(max) | close@00:30(3真48h): 1h0/3h0/6h0(n0)/12h−3.19(2)/24h−3.19(2,wr0)/48h−2.84(3) | FGI29/BTC+0.12%/mcap−0.22%无降压;trending∩池=∅(BTW隔离上榜≠rehab) | ①**S23评期裁决=证据不足结案**(跨段可观测A组19<20,AL/WL n5v3噪声方向暂反;槽释放,清写入#45)②**发现main第2次无端重播种**(feed89→100)→ROUTE−11自愈@00:25复feed89层1互斥✓③对账器v1.2自rv5cwj恢复推j6xm1s分支(容器v1.0误提议拆trend/fade池,被§1.5语义拦截=版本漂移教训)④#43第8读FAIL(24h−3.19,候~15:22天然复位18:1x轮首验)⑤0原子包HOLD(复飞段2h组0新平仓,评期在途trend06:19/fade12:2x/brk08-14) | 钱包253.0(avail224.5+HOME保证金28.6);组持仓1(HOME多+0.24在飞,main池[buy]latch内合规);费覆🔴24h毛−1.60/笔禁提频;载具保险全未触(trend滚6h=0/−5U,fade/brk段0笔);#37第7验未部署 |
