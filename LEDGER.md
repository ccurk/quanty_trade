...
| #79·closed行sid按symbol旧绑定继承(#77族新亚型) | 宕机前段08-30 02:04-06:38共6笔全=main自开(main日志完整触发/下单链:KOMA/BTR×2/COLLECT/ZKC/SKR),但DB closed行归属=该币末代载具(BTR→brk 3b64,COLLECT→fade-v2 7583)或空sid(KOMA/ZKC/SKR无前史);n=6/6一致零反例→sid解析疑走symbol历史绑定非下单载具;+空sid亚型恢复后2例(ONG 10:45第7/MARSCOIN 12:40第8,均无前史币=零绑定→空sid,main日志开仓链实锤,累计n=8) | 09-01 12:5x | 归因纪律即刻生效:sid与池归属冲突时以载具日志(触发开仓信号行)为准;机制修复并入#77 ownership单一化(候部署);#77部署后复验此亚型消失 |
| S31·regime动态方向偏置(**升队首候选@08-29 17:0x owner动态化直令**) | v1谱:宽度regime三档(池内EMA-up占比<0.35=RISK_OFF/0.35-0.60=中性/>0.60=RISK_ON)→动态premium加减(OFF:多头prem+0.10;ON:多头−0.05空头+0.05),阈值内联防漂移,S31_ENABLED False-guard回滚,叠加于config prem之上(config回滚路径保留);main先行 | 08-29 17:0x | **已上线@09-01 10:02=tpl988**(patcher断言过=基底零漂移/编译+6符号+锚8组✓/apply字节复检✓/零模板泄漏✓);_exp评09-04 09Z或S31段n≥20,到点n<5顺延48h一次(49h宕机后市场冷启动条款);劣化线=S31段多头净≤−3U或组wr−be劣化≥5pp@n≥20→S31_ENABLED=False;草案+验证史=ops/drafts/+git b12d7d9版行;**段n2/2胜@09-01 12:5x**:ONG L 10:45=判别笔(宽度实测0.656>0.60=RISK_ON,降门0.65放行conf0.650,静态0.70必拒;3m TP+0.34)+MARSCOIN S 12:40=非判别笔(宽度0.312<0.35=RISK_OFF,多头门抬0.80防御态,空门0.60静态放行conf0.650;trailing收割+0.87峰roi8.98%落袋6.15%);2h内ON→OFF翻转实录=设计场景;宽度重建法固化:logs q=带日期分钟(如2026-09-01T12:40:0)→评估行EMA趋势up占比/93币,不带日期=跨天污染(203币双态实证);逐轮采集因日志滚出,评期用台账累积段表 |
