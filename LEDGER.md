...
| main 通才(平台名 Meme_合约信号计算引擎_1) | 8eb182b6 | 通才(S4..S23+S30+S31+S33;S32关guard;tpl998) | manual 7 币 G/BULLA/MAGMA/UAI/HEI/DRIFT/ARB(owner 06:5x 直令 8 币, POWER 隔离未入; feed7 ∩bl42=∅) | running | **09-19 10:3xZ 现值: pid 55085 START 07:21:21Z 注入 min_conf0.35/cd180/atr 2.0/2.5/warmup50/max_price1e12; pct0.125(DS 07:08 砍, owner 值 0.25, 冻结 lock-1789810634 至 09-20 09:37Z) lev5(DS optimizer 10:09 裸改, owner 值 10, watch_ds 劣化线) mcp3(DS breaker 09:37 未声明, owner 令 10) sides双向 hunger 1m/tp0.30/sl0.04(owner 09-18 17:59 ROI 4%; lev5 下=0.8% 价) tp,sl pct0(交易所腿仍挂=信号 ATR 括号, §3 10:3x) trailing/BE 关 max_hold45 bl42; 7 币段(07:21 起) n7 毛 +10.89 wr57% 全多, 刀 3/3 亏单(1.1/1.1/5.1m), 赢 4 均 +3.05, 末单 G 09:48 TP +2.80 无引擎日志, 09:41 后日志静默(无新信号); _exp_cc=OWNER-DIRECTIVE 多策略化(eval 18:00Z 或 n≥30; rollback=filter 300 池)+watch_ds lev/mcp; 史 09:2x/08:4x/07:3x/05:5x 全文=git c75a1c3/2d9c837 版行** |
| majors(平台名 Majors_BTC_ETH_BNB_SOL) | e725e31a | main fork → **tpl1056 = tpl998 + S35 bar聚合 15m**(CC apply 09-19 08:39:38Z, hash 766b576a, 基线 3e311e8b, config diff=NONE) | manual BTC/ETH/BNB/SOL/XRP(XRP=owner UI 加, 无 audit) | **running@08:01:47Z(owner UI start, 无 audit)** | 10:3x 现值: lev10 pct0.125 mcp5 cd900 max_hold720 hunger off(0.30/0.30 键在) atr 2.0/2.5(admin 10:03 改 11/14→10:06 回 2/2.5 净零, Python 键未重启本就无效, 归属待认) min_atr 0.03 symbol_reentry 15m bar_agg 15 auto_optimize dry_run bl0 pid 56316 START 08:39:40Z 未重启; 1m 段(08:03-08:20) 3/3 空 SL 毛 −0.78; **S35 段: 平仓 n0, 在飞 SOL 多 08:46(SL 距 0.88%)/BNB 多 09:01(0.67%)/BTC 多 10:16:32(conf0.44 名义 81U SL 距 0.33% TP 0.48%; 信号仍在 15m 边界后 ≈92s)**; 429 两壳同秒 3 次(08:56/09:10/09:13; 10:16 后 0); _exp_cc=EXP S35(CC-28: eval 09-20 08:40Z 或 n≥15; worsen 段净≤−4U ∨ n≥10∧wr<35% ∨ SL 距中位<0.2%); 回滚=POST rollback→998 或 bar_agg_minutes=1+重启; 归档 confident-fermi 92327c4; DS 目标 id=main 但 10:03 admin 触碰本壳 atr(待认); 史 09:2x=git c75a1c3 |
...
- **[新增 09-19 10:3x] DS 改动簿 #9**: 09:37:15Z admin breaker lock-1789810634(至 09-20 09:37Z) _exp.changed 声明 pct 0.125→0.125(地板), **同秒 mcp 6→3 未声明**(协议7 #3 第 7 次; 史 09-16 22:10/09-17 13:15/09-18 06:30/15:22/21:22/09-19 03:22); 10:09:04Z admin leverage 10→5 无 _exp(optimizer 3h 节拍 04:09/07:08/10:09 坐实; 裸改→watch_ds 劣化线 post 段 n≥10); majors 10:03:46Z atr_sl/tp 2/2.5→11/14, 10:06:04Z 回 2/2.5(净零; Python 键未重启本就无效; 两分钟改回=人手迭代 or LLM 回退签名, 归属待 owner 认). 裁决=均不硬抢(CC-9), 登 watch_ds, TG owner 一字令. 机制注: lev5 使 hunger_sl 0.04 ROI 刀=0.8% 价(lev10 0.4%), 名义/费减半, 交易所 ATR 腿不随 lev 变.
...
- **[09-19 06:3x] REST 轮询地图(源码)+本地 TP/SL 陈旧价假触发**: 全文=git 6b44db7 版§3(要点: 每仓 2s positionRisk 权重 5=150/分/仓, mcp6≈900/分→429 伤出场; 补丁 claude/dev-rest-budget 61631b0 候 owner 部署; 陈旧价假触发 n=1 ROBO 05:58, 同补丁免疫)。
...
- **[新增 09-19 08:4x] apply 契约(optimize_handlers.go 实证)**: guards=4 符号(on_market_message/_emit_signal/_append_bar/self.pub.publish)→py_compile→baseline_hash(服务端 sha256 对 code 原字节; jq -r 多一尾部换行→本地 hash 不等, 复检用 JSON 原串)→param_drift(TP_RATIO/SL_RATIO/ATR_*_MULT/MIN_CONFIDENCE/WARMUP_BARS/VOLUME_RATIO_MIN/MAX_BARS 漂移>2x 拒); 建新 template+换绑→running 则 stop(force)+2s+start 异步; 响应 new_template_id/previous_template_id/new_code_hash; 09-19 08:39 majors 998→1056 config diff=NONE(本次未见模板泄漏)。
- **[新增 09-19 09:2x] 回测引擎非确定性实证(取代 08:4x"同窗不一致")**: sandbox MAGMA 1m 18h 同请求 3 次: #56 10 笔 −12.28(首入场 20:44)/#59 12 笔 +3.57(20:05)/#60 12 笔 +7.64(20:06); 首分歧=首入场时刻 ⇒ 信号随喂送节奏漂移(根因 §5 CC-29); **#51/#53 0 笔=overrides 缺 max_price(=0 全拒), 非引擎不一致**. 推论: 单次回测符号不可信 ⇒ CC-26 晋升暂停至 lockstep 修复或临时门(同窗 3 次符号一致∧中位>0, 候 owner); #54/55/57 running>2h=挂死株勿叠加.
- **[新增 09-19 09:2x] 饥饿触发价≠成交价亚型**: G 08:39:39 饥饿触发 roi −4.30%(引擎估 pnl −0.71)→市价成交 mv −0.18%(realized −0.30): 触发用价与成交差 0.25% 价(65s 持仓, 08:56 前无 429); 同批 DRIFT 09:15 触发 −4.19%/实 −0.554 一致 ⇒ 陈旧价/插针族(§6 本地 TP/SL 陈旧价行 +1), 判据=触发 roi 与实 roi 差≥2pp.
- **[新增 09-19 08:4x] 回测引擎事实**: POST /strategies/:id/backtest 支持 timeframe(原生 N 分钟 K 线)+config_overrides(注入 backtest=true, 身份/redis 键不可覆盖); GET /backtests 列表/ :id 详情(result.equity_curve); 任务并发不限(54/55/57 running 1h+ 未阻塞 58 秒级完成); **结果暂不可作晋升门**: G 1m 18h 同窗 task49 2 笔 −32.6 vs task51 0 笔; 15m 5d 四主流币(43-46) 0 笔=旧门 min_atr 0.5 全挡, 无信息。
...
| CC-11 | 巡检 | 全文=git 6b44db7版§5(要点不变) | 09-18 13:1x | open(读数史 git 2d9c837版; **最新 @10:3x: 钱包≈164.2(avail 127.4+保证金 37.5+浮盈 −0.7); income 1h +6.18/3h +18.15/6h 净 −24.3(≈−14.8%)报警/24h 净 −89.9(REALIZED −33.2 费 −56.6; 入金 +142 另列)🆘; 最大单仓名义 majors SOL 155U; 最大单亏 DRIFT −0.55(饥饿 0.4% 刀)**) |
...
| CC-28 | 评判(在飞) | **EXP S35 bar聚合 15m on majors**(tpl1056 @09-19 08:39:38Z; 预注册在 majors 壳 _exp_cc; metric=段 n/费后均净/wr/tpsl 日志 SL 距中位/笔日/首单时刻; expect n≥15 费后均净≥0∧SL 距中位≥0.3%∧wr≥45%; worsen 段净≤−4U ∨ n≥10∧wr<35% ∨ SL 距中位<0.2%→ROLLBACK(rollback→998); 24h 零信号无异常→只报) | 09-19 08:4x | open(**@09:2x 平仓 n0; 在飞 SOL 08:46 多/BNB 09:01 多, SL 距 0.88/0.67% 中位 0.77%≥0.3 ✓聚合生效; worsen 未触; 滑点 watch=§4 H-0919e**; eval 09-20 08:40Z 或 n≥15) **@10:3x 平仓仍 0; 在飞 3(+BTC 10:16 SL 距 0.33%); SL 距中位 0.67%≥0.3 ✓; worsen 未触; admin 10:03/10:06 atr 改回净零未重启=段未污染** |
| CC-29 | M候选(回测确定性) | **根因(源码, 子代理 09:3x)**: strategy_backtest.go:426 每根 K 线仅 sleep 10ms 开环喂送(无 ack), :473-479 非阻塞 select 每根最多吸 1 单且 default 跳过, :475 takeOrder 按喂送端"当前" K 线成交而非信号所在 K 线 ⇒ Python 单根处理>10ms 即错位 1 根, 累积落后→首入场漂 40 分钟; 信号丢弃 :326-329/backtest_fakeredis.go:159-164 静默; 出场模拟本身按 K 线确定(:433-471). 修法=lockstep: _emit_signal 回显 K 线 ts, :426 改为阻塞等 ack/信号(含 action=none), takeOrder 按回显 K 线收盘成交; 分支 claude/dev-backtest-lockstep 候写→owner 部署; 前置=CC-26 晋升门 | 09-19 09:2x | open |
...
| 计数项 | 读数 | 更新轮 | 备注 |
| **09-19 10:3x 7 币池新段(07:21 起)读数** | n7 毛 +10.89 wr57% 均 +1.56(费后≈+1.43) 全多: 赢 DRIFT +2.48(交易所 TP 7.7m)/HEI +3.55(饥饿 TP 6.8m)/HEI +3.39(guard_tp 7.0m)/G +2.80(09:48 TP 7.0m 无引擎日志); 亏 G −0.46(饥饿 SL roi −6.9% 过冲 1.1m)/G −0.30(饥饿触发 −4.3% 实 −1.8% 陈旧价亚型 1.1m)/DRIFT −0.55(饥饿 SL −4.2% 5.1m); **亏单 3/3=0.4% 刀; 赢单 3/4 hold=7m0s±1s(疑 7 分钟节拍, 候查)**; 交易所 SL 腿距 1.28-3.0% 从未轮到; 成交 conf 0.36-0.48(门 0.35 在位); 速率 7/185min≈54/日 ‖ majors S35 段 平仓 0 在飞 3(SL 距 0.88/0.67/0.33%) | 09-19 10:3x | 复读线不变: n≥8∧刀占比≥60%∧段净≤−4U→TG 再荐 B(hunger_sl 0.30); 10:09 起 lev5 段单列(watch_ds); majors 见 CC-28 |
...
| **09-19 06:3x 空头置信度反向(03:41 后)** | 空 conf<0.45 n10 +1.10 vs ≥0.45 n9 −18.30(OP0.56 −5.48/G0.50 −3.40/SOLV0.48 −2.15/XTZ0.70 −1.85/CROSS0.65 −1.61); 多头 03:41 后仅 n2(0.55: XTZ −2.17/G +4.71) | 09-19 06:3x | 行动=不抬 scp(留最差桶); 门=n≥30 再议 S31 OFF 偏置对称化(S34 候选) |
...
## 7. 运行日志（每轮一行，新行追加在表首）
| 2026-09-19 10:1x-10:3x | 定时轮: 管道活(REST 200; 429 自 09:13 后 0); 钱包≈164.2(avail 127.4+3 仓保证金 37.5+浮盈 −0.7); income 1h +6.2 / 3h +18.2 / 6h 净 −24.3(≈−14.8%)报警 / 12h 净 −46.4 / 24h 净 −89.9(REALIZED −33.2 费 −56.6; 入金 +142 另列)🆘(较上轮 −99.1 收敛); main 7 币段(07:21 起) n7 毛 +10.89 wr57% 全多, 刀 3/3 亏单, 赢 4 均 +3.05, ≈54 笔/日, 09:41 后无新信号; majors S35 段 平仓 0, 在飞 SOL/BNB/BTC 多(BTC 10:16 SL 距 0.33%); **DS #9: 09:37 breaker mcp 6→3 未声明(第 7 次)+10:09 optimizer lev 10→5 裸改+majors 10:03/10:06 atr 11/14→2/2.5 净零 ⇒ 均不硬抢, watch_ds 登 lev/mcp 劣化线, TG owner 一字令**; **止损完整性核对: 两实例均挂交易所 ATR 腿(源码 :305-307 tp/sl pct=0 时返信号 tp/sl, 非 0/0)⇒ prompt v5.1 裸奔前提纠正入 §3**; 隔离撞车 0(7 币 ∩ 5 币=∅, 持仓 symbol 零重叠); **0 交易键改动**(两在飞未到期: main 18:00Z/n≥30, majors 09-20 08:40Z/n≥15); 留言板 10:30Z(v+1, 7 条 7.8KB 近上限→下轮裁最旧); 宏观 FGI71 BTC +3.9% ETH +5.1% SOL +5.1% 市值 +0.9% trending∩池 ∅; 下轮=DS 13:09 节拍对账+lev5 段首读(post n≥10 裁)+main 日志静默复核(>2h 无信号且池 ATR≥0.5→查管道)+_ai_task_cc 瘦身 |
| 2026-09-19 09:1x-09:3x / 08:1x-08:5x / 08:2x(检查轮) / 07:1x-07:3x / 06:1x-06:4x / 05:3x-05:5x / 05:1x-05:5x / 03:3x-03:5x / 00:1x-00:4x / 09-18 20:1x-20:3x | 全文=git c75a1c3(09:1x-09:3x 行)/2d9c837(08:1x-08:5x/08:2x/07:1x-07:3x 三行)/6b44db7 版§7(逐行 git d19a237/4c56565/4c56565/f48d043/c06ab72/c06ab72) |
