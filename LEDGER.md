...
| 55 | E7类(trend) | **EXP:trend long_conf_premium 0.15→0.10**(gate 0.70→0.65;config-only;回滚=回0.15) | 分数墙源码级:ACE多头0.650×9h顶门0.70反复拒发;S24链n33/+0.024空转。**08-20 06:2x双向更新:分带上移(ACE 05:46-47多头0.680含S24续势+0.10,距门0.02;vs 03:18Z的0.28-0.40)但反事实转负——同例10min后−1.4%必触SL(2.5ATR≈1.4%),且trend影子多累计AL0/1+WL3/8=3W/9L(25%wr<影子be≈36%)⇒门0.70当前保护性**。前史git ba346aa+d19c710 | 机制级(gate) | 候选**已降级@08-20**(影子反事实负=解锁预期亏损);执行门不变=trend费覆解禁+影子多wr回到be上;08-18/08-20两审议=续锁;**三重锁死结构注@09:2x**:池陈旧→7.7h零候选连影子0→影子证据无法累积→解锁门不可评估;打破口=①main重开多头产样本(需全新假设)②换池(费覆锁:trend 48h n0无费据=禁扩)③零成本挂机等regime自证=**现行选项**;**池苏醒首证@08-20 12:2x修正"池死"判**:ACE垂直拉升RSI95.2多头分0.85被反追高硬过滤×4拒(RSI>68×3/放量不足×1),COTI 0.88被ATR地板0.15<0.5拦=池活了,门按设计拒追高;**watch=ACE RSI回落<68+EMA up+放量≥1.0成形而trend仍零发→复核S24硬过滤参(届时走trend _exp)**;watch判定@08-21 00:1x=**未成形三缺一**(ACE RSI61.5✓/EMA up✓/量比0.83✗;TUT RSI81.7量1.97=追高域;影子多累计3W/10L恶化,门0.70保护续证) |
...
| 08-19 21:13-21:3xZ | 例行轮(max) | 24h−0.55(10,wr40/be53) | FGI46 | **HOLD 0原子包**(#30 JCT出列19→18详§4#30;5h流量枯=漏斗定量泊松正常;ROUTE plan={};刹车全绿) | stub;集中度注记CLO+STAR11笔+9.09;全文git 7e2b925 |
|---|---|---|---|---|---|

> 瘦身注 08-20 18:3x：08-20 15:16行转stub(全文git bf6c431);08-19 15:15行裁(#61c ROLLBACK全文§4#61/#63+git 9d0dc94)。
> 瘦身注 08-21 00:4x：08-19 18:14行随轮裁(β快SL环境伤要点已入#30/#61族;全文git 82e6cf6前史);08-19 09:3x/08-17 13:4x两旧维护注并归本注(其git指针链在git历史)。
> 瘦身注 08-20 06:4x：08-19 21:13行转stub(全文git 7e2b925);§3四超长条(854/883/929/941B)+评分天花板条压缩为速记(全文git 0980231前);§1.5 main池列压缩(bl直漏feed语义归一互斥行)。
