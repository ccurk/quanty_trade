...
| 载具 | id | 原型 | 池 | 状态 | 门/备注 |
|---|---|---|---|---|---|
| main 通才 | 8eb182b6 | 通才(S18-S23栈) | auto·feed83@03:5x(**−BULLA晋升fade@03:57**;zombie检疫−STAR/AKE/EDEN维持中,owner重启后自动恢复;bl16v2重启免疫,**bl17=#54待落**;heatclean史git 26eaf24) | running | pct0.08 sides=[buy,sell] long_conf_premium=0.35 tpl567;_exp=**FIX-GEO(#49)@15:32:13Z**: trailing(act1.0/cb1.2%)+BE(1.0),TP4.5/SL2.5/premium不动;基线=72h赔率0.88/SL率40%/均净−0.73;**段@03:5x=n13/−5.33/赔率0.32/wr54%/均净−0.41**(BULLA收官−0.67硬超时60m整入段;BULLA晋升出池=段内赢源+1.73随行,与AIO隔离对称登记;段病理=trail锁小赢[7赢均+0.44]而亏满SL[6亏均−1.41])→n<30且回滚线(赔率<1**∧**净≤−10)未触=continue;keep=n≥30或48h先到时赔率≥1.2∧段均净>−0.73(**注:eval_after=15:30Z[24h]与expect'48h先到'并存,当值轮按预注册原文裁,禁事后换标**);eval 08-16 15:30Z;RECOVER刹线6h空≤−8U继承(现+0.48✓);#11空侧gate=下一槽 |
| qt-trend-long | 827ffe8c | 趋势动量多 | ACE/TUT/COTI(feed3=种子3✓;aging_watch=**全池3币** advisory[对账器@03:17]) | running | **tpl575(S24)** pct0.05 mcp3 buy trailing on;_exp v3常驻:**累计链n33/+0.024不变**(18:17 BICO后33h零笔;链=终裁权威口径,72h重拉丢行不可用[§3]);**分数墙实证@03:21**=ACE多头0.650顶门0.70(min0.55+long_conf_prem0.15)反复拒发→冷案破:零笔≠无信号,是门下0.05徘徊;终裁后候选EXP:降prem(费覆红线先行,禁提频窗内只登记);**终裁今08-16 06:35Z**当值:净>0=keep,毫厘正(+0.024)——零新笔则keep结案铁定;劣化−5U/canary未触;两格皆留S24,仅净≤−5U才rollback tpl567 |
| qt-fade-short | 21519f1b | 冲高回落空 | 1000CAT/ARC/BEAT/VELVET/**BULLA**(live=种子=**5**✓ **ROUTE:BULLA晋升@03:57**[进池条48h nS3/+1.73≥1.5;main内收官−0.67后其净+1.06/nS4];程序v2一气呵成[stop try1即沉降-PATCH种子5-start],_exp完好,重启免疫) | running@03:58 | **tpl576(S25)** pct0.04 mcp2 sell;_exp v2续:**累计n10/−3.74**(+BEAT−1.28@23:20;新池首单BEAT+0.17@18:43);**0.60墙在案**:BEAT空头分0.600整×20次顶死门(min0.55+short_prem0.05需**>**0.60)系统性拒发,处置候选=EXP:short_prem 0.05→0(评期时预注册执行,不中途改污染段;BULLA入池后墙对其同样适用=天然对照);评期=累计n≥15或**08-16 12:50Z**,到点n<15按证据不足格裁;自动保险6h≤−6U(6h−1.28✓) |
| ~~qt-breakout-follow~~ | 2111f5f9 | **owner删除@08-15 15:5x**(UI删,承15:36复活事件后清理;评判FAIL史+S26谱系+tpl577存git可重建;#47 S27复活=建新载具走FLEET流程,同lowvol先例;**⚠️幽灵运行时@08-16 00:1x发现**:删除未斩进程,仍在册交易STAR/AKE/EDEN[23:11Z开仓实证],机制§3,候修#53,已检疫+TG请重启) | — | deleted | — |
...
- 互斥不变式: feed层@03:5x=main83/trend3/fade5,**overlap0全绿**(rotate后四方python实核:∩fade/∩trend/∩隔离/∩幽灵3币全空)>种子层=fade5币✓/trend3币✓/main bl16(**BULLA未入bl→#54前owner重启会使BULLA回main feed=预期漂移**,互斥闸兜底+对账器下轮自愈)>引擎互斥闸✓live;aging=trend3全池/fade3(1000CAT/ARC/BEAT) advisory;重播种#8-#11史git;隔离币15:32后零成交✓
...
| # | 类型 | 内容（含完整意图） | 登记轮 | 状态 |
|---|---|---|---|---|
| 54 | 种子同步 | main symbol_blacklist bl16→bl17(+BULLA/USDT;权威=对账器blacklist_sync@03:17=17项)。需 main 空仓窗 stop-PATCH-start(程序v2)——main mcp200 常有仓,耐心等真空仓窗;未落地前 owner 重启会使 BULLA 回 main feed(预期漂移:引擎互斥闸兜底一仓一主,对账器下轮 rotate 自愈) | 08-16 03:5x | open |
