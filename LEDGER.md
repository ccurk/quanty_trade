...
| #79·closed行sid按symbol旧绑定继承(#77族新亚型) | 宕机前段08-30 02:04-06:38共6笔全=main自开(main日志完整触发/下单链:KOMA/BTR×2/COLLECT/ZKC/SKR),但DB closed行归属=该币末代载具(BTR→brk 3b64,COLLECT→fade-v2 7583)或空sid(KOMA/ZKC/SKR无前史);n=6/6一致零反例→sid解析疑走symbol历史绑定非下单载具;+空sid亚型恢复后8例(ONG/MARSCOIN×3/牛来/FF/USELESS/STAR@09-01,均无前史币零绑定→空sid,main日志开仓链全实锤,累计n=14;MARSCOIN三现同型) | 09-01 12:5x | 归因纪律即刻生效:sid与池归属冲突时以载具日志(触发开仓信号行)为准;机制修复并入#77 ownership单一化(候部署);#77部署后复验此亚型消失 |
| S31·regime动态方向偏置(**升队首候选@08-29 17:0x owner动态化直令**) | v1谱:宽度regime三档(池内EMA-up占比<0.35=RISK_OFF/0.35-0.60=中性/>0.60=RISK_ON)→动态premium加减(OFF:多头prem+0.10;ON:多头−0.05空头+0.05),阈值内联防漂移,S31_ENABLED False-guard回滚,叠加于config prem之上(config回滚路径保留);main先行 | 08-29 17:0x | **已上线@09-01 10:02=tpl988**(patcher断言过=基底零漂移/编译+6符号+锚8组✓/apply字节复检✓/零模板泄漏✓);_exp评09-04 09Z或S31段n≥20,到点n<5顺延48h一次(49h宕机后市场冷启动条款);劣化线=S31段多头净≤−3U或组wr−be劣化≥5pp@n≥20→S31_ENABLED=False;草案+验证史=ops/drafts/+git b12d7d9版行;**段累积表n8/7W1L@09-01 15:3x(毛+3.28;多n3全胜+0.81/空n5四胜+2.47;判别笔仍仅ONG)**:①ONG L10:45 conf0.650 w0.656 ON降门0.65放行判别+0.37②MARSCOIN S12:40 conf0.650 w0.312 OFF+0.89③牛来 S13:45 conf0.750 w0.495 N,TP达成+2.09④FF L13:54 conf0.700 w0.667 ON(降门0.65;conf恰=静态门0.70按≥判据静态亦放行=非判别边界笔)+0.24⑤MARSCOIN S13:58 conf0.950 w0.688 ON,trailing 28m+0.63⑥USELESS L14:31 conf0.720 w0.559 N+0.20⑦STAR S14:41 conf0.800 w0.559 N,SL穿刺26m−1.42⑧MARSCOIN S15:04 conf0.750 w0.452 N+0.28;70分钟内N→ON→N二次翻转实录;劣化线双查:段多头净+0.81≫−3U✓组wr87.5%−be67.9%=+19.6pp无劣化✓;宽度重建法:logs q=日期T分钟→评估行EMA up占比/93(不带日期=跨天污染);评期用本累积表 |
...
| 09-01 12:4x-13:1xZ | 例行轮(max) | +0.89/n2 +1.26/+1.26/+1.26/+1.26(毛;净≈+1.21;wr100%@n2 be不适用) | FGI69/BTC77918+0.1%/mcap−2.9%/trend∩池{ARB,UNI,CRV,MARSCOIN,PUMP,SKR} | **HOLD原子包0;ROUTE v1.5零差分(plan空,bl14期望态,TUT aging续,无新隔离);S31段n2/2胜全链归因:①ONG判别笔=宽度0.656实测RISK_ON降门0.65放行(静态0.70必拒)+0.34②MARSCOIN非判别笔=宽度0.312 RISK_OFF(多头门0.80防御)空门0.60放行trailing收割+0.87;regime 2h内ON→OFF翻转=S31设计场景首实录;宽度重建法固化入§4行;#79空sid第8例(MARSCOIN);充值线形式第7触(双正+wr形式达标)n=2判未熟不行动** | 钱包162.31U全可用零持仓;死法:零亏损轮(TP 3m+trailing 6m47s);费覆🟢首绿(均毛+0.632 vs 3×费0.087,9连红破,n=2口径);trend TUT n0漏斗活(conf0.44<0.55);audit零无主(main1791/trend1790自书);feed93/1零漂移=宕机后零重启 |
| 09-01 09:4x-10:2xZ | 例行轮(max) | 五窗全0笔(49h宕机) | FGI69/BTC77973−0.9%/mcap−2.9%/trend∩池{ARB,UNI,SKR} | **①宕机窗定谳≈49h(§3;cron14轮零写入,两_exp评期过期)②trend FIX评判PASS-KEEP(n1+0.329零拦截,mces=0维持)③main多头闸+S30评判=未劣化KEEP(段n3≈−1.2>−3U磁铁<3,lcp0.15+S30留,expect未达非成功)④S31接棒上线tpl988(断言/编译/字节复检/零泄漏全✓;_exp评09-04 09Z或n≥20,n<5顺延一次)⑤ROUTE清扫7币(隔离6+TUT)feed93零残留,bl14期望态,MAGMA网解除⑥#79登记sid继承误归n6/6(main日志实锤)** | 钱包161.09U零持仓;宕机前段6笔净−2.89(SKR−1.19/COLLECT−1.39);自查点10:52✓:烟雾[backtest strategy]START吃K线不崩;首笔ONG L+0.37@10:45-10:48(3m TP,main日志conf0.650<静态门0.70=S31 RISK_ON降门0.65实证候验);其sid空=#79第7例 |
