...
| 载具 | id | 原型 | 池 | 状态 | 门/备注 |
|---|---|---|---|---|---|
| main 通才 | 8eb182b6 | 通才(S18-S23栈) | auto·feed92(~17:49疑似全局重启重播种100→18:28热移除9✓→FIX窗start再播种再自愈;bl13=desired不变) | running | pct0.08 sides=[buy,sell] **long_premium=0.35(FIX@08-14 18:47Z)** tpl567;_exp=**FIX长溢价0.15→0.35**(long_thr0.70→0.90;证据=48h长侧n20 wr30/−11.79过20笔门[现池子集wr14],空侧n19/+0.83;病根=逆势栈RSI+BB+资金费+多空比=0.70恰等长门,S12先例同修法;keep=08-15 19:00Z长侧24h净≥−2U∧组均净改善;劣化=长侧≤−6U或组净恶化→回0.15再议[sell]latch;全文config._exp)+**继承RECOVER刹线**(6h空侧≤−8U→重latch[buy];首评19:00Z=**KEEP**,空侧24h+3.60含TAKE+2.77饥饿收割✓);24h n34/−12.47wr35:长n15/−13.30wr20主失血vs空+0.83;主死法快SL;⚠️sid伪影纪律照旧(§3) |
| qt-trend-long | 827ffe8c | 趋势动量多 | TUT/BICO/ACE/COTI/CAP(种子=feed=5✓@18:24,重启零漂移=种子同步实证) | running | **tpl575(S24)** pct0.05 mcp3 buy trailing on;_exp v3常驻:**累计n21/−0.38**(CAP首笔−1.10快SL@18:04;09:41后段n5/+1.47);keep门净>0✗∧费覆🟢0.150=4.9×;劣化−5U/canary−5U(现6h−2.27)未触;**终裁08-16 06:35Z当值净>0=keep**;ACE 48h+3.44组最佳∧trending;aging_watch=TUT/BICO;全史git+config._exp |
| qt-fade-short | 21519f1b | 冲高回落空 | 1000CAT/ARC(种子✓;aging_watch全2币) | running@08-13 12:45Z | **tpl576(S25)** pct0.04 mcp2 sell;_exp v2续:累计n8/−2.63零增(现池零成交**第7轮**,对账器want.fade=[]=市况无冲高回落型,非载具故障);评期=累计n≥15或**08-16 12:50Z**;自动保险6h≤−6U(0✓) |
...
|---|---|---|---|---|---|
| 08-14 18:14-19:0xZ | 例行轮(max) | close: 1h−3.21(5)/3h−5.17(9)/6h−9.13(13,wr38 be62=−24pp)/12h−1.55(31,wr52 be54=−2pp)/24h−10.99(51,wr47 be57=−10pp) | FGI29/BTC−0.33%/mcap−0.42%无降压;trending∩池=ACE+CAP(trend)+AKE/ETHFI/UNI(main) | ①RECOVER首评@19:00Z=**unlatch KEEP**(空侧24h n19/+0.83恢复成立;刹线6h空侧−1.65未触−8U)→刹线条款转入新_exp继承 ②**原子包1: FIX main长溢价0.15→0.35**(18:46:55空仓窗23s全序stop→PATCH验证→start;证据/病根见§1.5;评估08-15 19:00Z) ③ROUTE:~17:49疑似全局重启重播种(audit零无主,config零改动,trend/fade种子零漂移)→main热清9violations×2窗(18:28+start后)feed92 overlap0;H@18:07两拉pnl−1.42/−0.30不一致(候证不改结论) | 钱包收轮242.38全空仓(TAKE空+2.77饥饿收割@18:46终局);24h载具:main n34/−12.47wr35(长−13.30wr20主失血)/trend n13/+1.96wr69/fade零成交第7轮(want=[])/brk gated候S27;死法:快SL16/−28.11主刀,超时+饥饿+快TP全正;费覆组🔴−0.216vs0.125禁提频(main🔴/trend🟢4.9×);四保险未触;audit零无主;?q第8验未部署 |
...
> 瘦身注 08-14：更老运行日志行随轮裁剪(08-13 18:20轮#43落地要点存§1 08-12直令+§5维护注),全史见 git 历史。