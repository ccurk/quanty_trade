...
|---|---|---|---|
| 2026-09-20 09:1x-09:3x(v6.0 #12) | 上轮(08:27 板#1959)= [攻-1] DS Go 热键 take_profit_pct 0.10+stop_loss_pct 0.20 / [攻-2] DS mcp 1→3 | **DS 0 交易键改动(第 9 轮)**: audit admin 最新仍 06:22:16 #1954(breaker 仅冻 order_amount_pct 至 09-21 06:22Z); 09:27Z config: main lev10 pct0.25 mcp1 tp/sl pct0 hunger false max_hold90 max_mult0.8; majors lev10 pct0.25 mcp2 max_hold720 bl BNB/SOL/XRP tpl1056 hash 766b576a 未变. **为何 7 次 mcp 建议不可能被 DS 执行(源码定案)**: ops/deepseek_optimize.py validate() MAX_DRIFT=2.0 ⇒ mcp 1→3 ratio 3>2 直接拒"单次漂移超过2.0x"; 可执行=1→2. tp/sl 0→非0 跳过漂移检查(注释"两端非0才算比率"), BOUNDS tp[0.05,2] sl[0.02,0.5] ⇒ 0→0.10/0.20 可执行. bool 键(hunger_mode_enabled 等)走 isinstance(bool) 早返回, BOUNDS(0,0) 不生效 ⇒ 旧注"BOUNDS 禁开 hunger"作废(DS 可置 true). **样本门再开**: 09:20Z ctx 6h pair_count=31≥30(ON 7+CTR 10+BIGTIME 12+ROSE 2, fills 配对口径), 估持续至 ~10:46Z(ROSE 对滑出) ⇒ DS 09:22(未动, 09:25 audit 无 admin)/09:37/09:52/10:07/10:22 可动. **留言板可见性订正**: main() 先 n<MIN_TRADES 即 return, 门开时 build_prompt 以 json.dumps(cfg) 整段 dump config(ctx.config 含 _ai_task_cc 5556B+_exp) ⇒ 门开轮 DS 的 LLM 能读到板; 此前 8 轮均在门关期 return ⇒ "DS 不读板"实为"没到读板那一步". 我方写: 板 09:28:37 audit #1960 仅 _ai_task_cc(复读 mcp1/tp0/sl0 未变 ✓; 2 条 7096B, 07:28 条按 8KB 字节口径裁掉). | **效果**: 无改动 ⇒ 无前后. 逐笔(DB 09:20Z 全账户): 1h 1/+0.07 / 3h 2/+1.06 / 6h 4/−0.66 wr75% / 12h 14/−1.57 wr71% / 24h 107/−7.01 wr49%; 交易所配对: 6h 31 对 毛−0.66 费−1.00(多 14/−2.04 空 17/+0.38) / 12h 49 对 −1.57(多 31/−4.24 空 18/+1.19) / 24h 242 对 毛−7.01 费−7.62 净−14.6(多 167/−15.96 空 75/+1.33); avail 130.5(钱包≈149). 北极星: 15m 段 main 13 笔 27.7/日×−0.196=−5.4U/日(12 钟+1 SL 腿+0 TP 腿; 多 11/−3.61 wr64% 空 2/+1.06); lev10/300U 段 3 笔全钟 BIGTIME −0.71%/CTR +0.34%/ON +0.02%(09:16:40 到钟) 净−1.07, ON 毛+0.072 费−0.149=费吃. OKX 1m 回放: BIGTIME MFE +0.28% MAE −0.98%(+1%/+0.5% 皆未到); ON(OKX 同名异币 ref 70.1 ⇒ 无效)/CTR/ROSE 无 OKX 合约; Binance fapi 451 地理拒. **病根**: ①main 09:16:43「当前已持仓1个，达到最大并发仓位1」=majors ETH 占槽 ⇒ main 0 开仓直至 ETH 出场(tp 2561.87 −0.64%/sl 2592.81 +0.56%/钟 20:16Z); 08:46-09:17 265 信号(多 160 均 conf0.83/空 105 均 0.81; 多理由 89/160 裸"EMA多头") 0 开 ⇒ 笔/日→0. ②多头三窗全负 vs 空头全正(上); 市值 24h −4.9% BTC 80233(−1.4%) ETH 2575(−2.8%) SOL −3.6%; 48h 口径多空皆负(多 229/−31.1 空 120/−14.9)=方向优势系 24h 下跌段现象. v40 定案: cfg 只读 min_confidence/cooldown_sec/atr_tp_mult/atr_sl_mult/bar_agg_minutes/warmup_bars/max_bars/cost_mult/fee_round_trip/long_bias/short_bias/tp_ratio/sl_ratio/trade_amount/base_trade_usdt/max_candidates(+symbols/redis/log); long_conf_premium 不读 ⇒ #11 "lcp 0→0.10"作废, 方向杠杆=long_bias/short_bias(spawn 读, cfg 现无键, DS 白名单外). 止损核对: main ON 腿 07:46:36 ✓ 钟 90 ✓; majors ETH 腿 08:16:32 ✓ 钟 720 ✓; 无裸奔; 无撞车. **本轮建议(板 #1960; TG 10835)**: [攻-1] DS 可执行 mcp 1→2(漂移 2.0x 不超; Σ0.5≤0.75; 非冻结) | 占槽锁死 13.9→≥28 笔/日 | 验: audit admin mcp=2·ETH 在持期 main 开仓≥1·开仓/h≥1.2·6h 净≥−0.66. [攻-2] DS 可执行 take_profit_pct 0→0.10(单独替换 TP 腿, SL 腿仍 ATR ≈−1.9%) | 13 笔 0 TP 腿/12 钟均净 −0.196; 300U 下 +1%=+3U vs 费 0.30U | 验: 新腿距 1.00%·5 笔内 TP 成交≥30%·均净≥+0.15; 5 笔 MFE 全<1% ⇒ 改 0.05. 候 owner: long_bias 0.85(新增键+重启) 或 allowed_sides=["sell"](与 09-18 禁方向直令有张力). FGI 71 Greed; dom 58.9%; trending ONE/FIRO/NEAR/BP/ZEC/PONS/STRK/AKE/PENGU/ARB **09:41Z 补(门开未动实锤)**: DS 09:37 拍(+≤120s)过后 audit 仍无 admin 写(最新 #1960 我方 09:28), 而 09:41 ctx 6h pair_count=31≥30 ⇒ 门开、板可读、0 改动. 源码归因=build_prompt 硬编码事实: #1+每轮注入「手续费占净亏损的 62%——提高交易频率会等比例放大它」(6h 毛−0.66 费−1.00) 否决提频; #4「mcp 通常不是瓶颈 槽位 7-8/10」(mcp10 旧数) 否决 mcp↑; #5「TP 0 命中…调 tp 无意义」否决 tp↑; 叠 MAX_DRIFT 2.0 ⇒ 两条攻建议结构性不可通过(不再是样本门). 建议 owner: A 改脚本 3 行(事实 4/5/1 更新) + B UI 直改 mcp2/tp0.10(TG 10836). 下轮验: 09:52-10:37 四拍 audit 有无 admin 写; 脚本是否改(宿主侧, 看 DS 行为反推). |
