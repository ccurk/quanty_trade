...
| # | 类型 | 内容（含完整意图） | 登记轮 | 状态 |
|---|---|---|---|---|
| 82 | _exp写入(trend) | #65裁决KEEP已定@09-04 06:3x,待trend空仓窗stop→PATCH config._exp{status:closed,verdict:KEEP=段n5/−0.314劣化线−2U未触+6h保险未触+06:10 USELESS入场conf=0.7000恰边界实锤(旧lcp0.15下被浮点等值静默丢弃),lcp维持0.149,混杂注=段活性主体来自USELESS新晋池}→start复读;自查点send_later已排(≤3retry后留下轮cron) | 09-04 06:3x | open |
...
|---|---|---|---|
| main断流观测(三闸交集关门) | 本轮main笔数=22/24h@09-04 06:2x(判据未触) | 09-04 06:2x | 判据不变:再现6h+零笔且池ATR中位≥0.5→查管道;每轮记main笔数 |
...
| #79·closed行sid按symbol旧绑定继承(#77族新亚型) | 两形态:①**双行**(main行+末代壳行同笔PnL双写,壳行tp/sl null裸奔,出场治理权=两行落库序竞态,08-30 03:34壳饥饿档45m真接管收割−0.61实锤;判别特征=末代持仓币BTR/NIL/COLLECT类,泛池币牛来/USELESS零双行)②**空sid**=binance_only视图伪影(全量视图sid=main齐);binance_only对双行币dedup保壳行=池归因陷阱 | 09-01 12:5x | 归因纪律:sid冲突以载具日志开仓链为准,逐笔归因用全量视图+日志链;机制修复并入#77 ownership单一化(候owner部署),部署后复验消失;**n=32@09-03 00:2x**(+MAGMA22:22双行第8案:main行+壳行同PnL+0.0949壳行tp/sl null,binance_only又dedup保壳行;同窗UAI22:01/BULLA23:39全量视图单行sid=main正常);n=33@09-03 12:2x(案9=COLLECT11:56 fade-v2壳首案,"末代持仓币"判别命中,细目git e97b10c版行);**n=34@09-04 00:2x(+COLLECT23:31案10=同币同壳复发:main开仓链conf0.70空mult1.4 A-bar+行带TP/SL非裸奔,active视图仅壳行可见无main行)**;案10 close行验证命中@09-04 03:1x(00:31 +0.209 sid=7583727a=dedup保壳行预测成立,跨壳收养开→管→平全环节首次闭环实证);史读链n=26..31细目git ed1ba85/1417e31/558c235版行;**n=35@09-04 06:2x(+BTR05:49案11=−0.326壳行sid=3b646bf4,BTR在main feed逐分钟评估∧brk壳stopped=排除法main开仓,末代持仓币判别再命中)** |
...
| 09-04 06:2x-06:5xZ | 例行轮(max) | −0.15(n3)/+0.54(n4)/+2.41(n9 wr56)/+0.19(n14 wr50 be49 +1pp)/−3.70(n27 wr52 be60 −8pp)(毛) | FGI74/BTC80945+4.1%/市值+1.0%/trending∩池=USELESS(trend) | **HOLD原子包0;trend #65裁决=KEEP**(段n5/−0.31劣化线−2U未触;06:10 USELESS入场conf=0.7000=边界修复因果直证;_exp写入排§5#82+自查点send_later);FREQ rider n13/−0.26距线3.74大回血(CAP02:52多03:13平+3.26=上轮预判决定笔反转,机械回100预案未触);#80候审证伪=CAP同笔+3.26入§4反例(零新增第4轮);牛来watch n6/−1.69退压(04:44+0.69/06:04+0.28两空赢=零赢行签名破);#79案11=BTR05:49壳行(n35) | 钱包≈151.9(avail141.22+USELESS证10.66+浮+0.02)较上轮+3.8=CAP大赢主导;死法24h=穿刺9/−11.88(空亏−7.2 vs 多亏−5.6均衡化)+赢14/+9.14(残赢8/14)+超时1/−0.43+刮擦3/−0.52;#11空侧24h−2.56距门5.44(恶化链彻底断);费覆24h🔴15连(均毛−0.137 vs 0.101)但6h🟢+0.268首绿/12h+0.013转正;ROUTE零差分21连(bl15期望态;USELESS aging新挂nL5/−0.31再1亏触出池条);audit末1800零无主;充值线未触(24h负) |
...
| 09-03 15:1x-15:4xZ | 例行轮stub: HOLD原子包0;CAP#52案2登记(细目§4#52);#11空侧−4.60;E9钟42/48h;ROUTE十六连;全文git f477a04版行 |
| 09-03 12:2x-12:5xZ | 例行轮stub: HOLD原子包0;#79第9案COLLECT双行=fade-v2壳首案跨壳泛化(n33,TG重申#77);#11空侧−3.41;E9段n8/−3.75;全文git 8ddf77f版行 |
| 09-03 09:1x-09:4xZ | 例行轮stub: HOLD原子包0;S32首日双样本均逆假设(EDGE拦截误杀#1+HEMI W-bar放行−2.02);#80残余n2/−3.31登§4;E9段n7/−4.85;全文git 8ddf77f版行 |
| 09-03 06:2x-07:1xZ | 例行轮stub(与直令轮2并轮): HOLD原子包0;空侧扩散预警登§4#11;ROUTE十三连;烟雾task40 completed=S32三验闭环;feed138=FREQ重播种定性更正;全文git 8c39ae5版行 |
> 瘦身注(压缩@09-04 06:4x): 09-03 03:5x/06:2x两owner直令轮stub删(E9+FREQ+lcp0.149注册全文=git 8ddf77f版行,E9/S32 verdict存main _exp);同轮09-04 00:1x/03:1x行stub化=git a317555版行。
