...
# S34 @2026-09-19 sim-clock: 回测时用K线时间代替墙钟(cooldown/急再入veto/影子/CB隔离/S31/S32/择优窗)。
#   否则回测把小时压成秒, 所有时间门永久关闭(09-19 实证: 18h 回测每币仅 1 笔, 其余全被"冷却中/急再入veto"挡)。
#   live 模式(config sim_clock 缺省/False)下 _ct()/_cm() 严格等于 time.time()/time.monotonic(), 零行为差异;
#   sim 模式只在 on_market_message 收到 candle 的 timestamp 时单调推进。备用池模拟(owner 09-19 直令)依赖此锚点。
_SIM_CLOCK = {"enabled": False, "t": 0.0}


def _ct() -> float:
    return _SIM_CLOCK["t"] if (_SIM_CLOCK["enabled"] and _SIM_CLOCK["t"] > 0.0) else time.time()


def _cm() -> float:
    return _SIM_CLOCK["t"] if (_SIM_CLOCK["enabled"] and _SIM_CLOCK["t"] > 0.0) else time.monotonic()


def _sim_clock_advance(ts_raw) -> None:
    if not _SIM_CLOCK["enabled"] or ts_raw in (None, "", 0):
        return
    try:
...
def _s31_note_ema(symbol: str, ema_trend) -> None:
    ...
    try:
        _s31_ema_state[str(symbol)] = (1 if ema_trend == "up" else 0, _cm())
    except Exception:
        pass
...
def analyze_with_detail(
    ...
    if S32_ENABLED and rsi_val > _S32_HOT_RSI:
        _s32_unstable_state[str(snapshot.symbol)] = _cm()  # S32: RSI热盖戳(冲顶接刀记忆源)
    ...
    if direction is not None:
        ...
        if reject_reason is None and direction == "long" and atr_pct > 0 and len(closes) >= 4:
            ...
            if reject_reason is not None and S32_ENABLED:
                _s32_unstable_state[str(snapshot.symbol)] = _cm()  # S32: veto开火盖戳(接刀记忆源)
...
class Strategy:
    ...
    def _load_config(self):
        ...
        _SIM_CLOCK["enabled"] = bool(self.cfg.get("sim_clock"))  # S34
    ...
    def _emit_signal(self, symbol: str, direction: str, entry_price: float, tp: float, sl: float, confidence: float):
        ...
        self.shadow_open[symbol] = {"entry": float(entry_price), "tp": float(tp) if tp else 0.0, "sl": float(sl) if sl else 0.0, "direction": direction, "ts": _ct(), "s23": _t23}
    ...
    def _update_shadow(self, symbol: str, high: float, low: float, close: float):
        ...
        if direction == "long":
            ...
            if sl > 0 and high >= sl:
                ...
                outcome = "win"
        basis = "触及"
        if not outcome and opened_ts > 0 and _ct() - opened_ts > Config.MAX_HOLD_MINUTES * 60.0:
            if direction == "long":
        ...
        self.last_shadow_close[symbol] = {"ts": _ct(), "direction": direction, "outcome": outcome}
        ...
        if Config.CB_ENABLED and n >= Config.CB_CONSEC_LOSSES:
            until = _ct() + Config.CB_QUARANTINE_MIN * 60.0
    ...
    def on_market_message(self, msg: dict, from_history: bool = False):
        ...
        if _lsc:
            _lsc_gap = _ct() - _f(_lsc.get("ts"), 0.0)
    ...
    def _offer_best_pick(self, symbol: str, direction: str, entry_price: float, tp: float, sl: float, confidence: float):
        ...
        if cur is None:
            self.best_pick = cand
            self.best_pick_deadline = _ct() + float(Config.BEST_PICK_WINDOW_SEC)
