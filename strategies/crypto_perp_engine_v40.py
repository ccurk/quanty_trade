...
class Strategy:
    ...
    def _emit_signal(self, symbol, direction, entry_price, tp, sl, confidence):
        ...
        msg = {
            ...
            # 信号生成时的评估价（= 上面算 tp/sl 用的同一根 price）。Go 侧用它把 tp/sl 的
            # 【相对距离】原样搬到实际成交价上（backend/internal/strategy/strategy_position.go
            # reanchorTPSLToFill）：这里的 tp/sl 锚在评估价上、不随成交漂，而市价单成交价会
            # 偏离评估价 0.01%~0.22%，偏离多少就按比例吃掉多少止损距离 —— 实测设计 1.25:1
            # 落到实盘变成 0.44~1.29:1（ROBO 那笔滑点吃掉 56% 的止损距离）。
            # 不发这个字段 Go 就不重锚（退回旧行为）。
