...
# S27 @2026-08-29 #47 breakout复活版: S26内核零逻辑改动复活于新壳。
# 依据: 旧壳FAIL已翻案@08-18(幽灵行伪证,可验证成交=0,内核从未获公平实测);
# want.breakout档案4币[BTR/LIGHT/MAGMA/我踏马来了]48h在main均值回归池合计毛≈-8.05
# =全组最大失血源=池错配证据(同档BAS顺势日+4.90为对照)。本锚仅标记谱系与复活依据。
def score_breakout_detail(
    price: float,
    closes: list[float],
    atr_pct: float,
    atr_abs: float,
    ...
    """S26 @2026-08-09 breakout-follow 原型评分核: N-bar收盘价破位+量能推力+动量对齐顺势入场。
    与均值回归核(score_confidence_detail,本fork弃用档案化)哲学相反: 不接刀、追延续。
    结构: base 0.40(破位事实) + 量能0.15 + 3bar动量0.15 + EMA对齐0.15 + 破位余量0.10 → 顶格0.95;
    过门需 base+2因子(long_thr=0.55+premium0.15=0.70 / short_thr=0.60 锚不动)。
    原生反追单: 破位余量>1.2×ATR=blowoff迟单直接弃(替代gate6追高门的原型化表达)。
    detail 键契约与 score_confidence_detail 一致(评估/评分明细日志与硬拦截日志直接复用)。"""
    ls = 0.0
    ss = 0.0
    ls_parts: list[str] = []
    ss_parts: list[str] = []
    N = 30
    ok = len(closes) >= N + 4 and atr_abs > 0 and price > 0
    if ok:
        window = closes[-(N + 1):-1]
        prior_high = max(window)
        prior_low = min(window)
        c0 = closes[-1]
        _r3 = (c0 - closes[-4]) / closes[-4] * 100.0 if closes[-4] > 0 else 0.0
        if c0 > prior_high:
            margin = (c0 - prior_high) / atr_abs
            if margin <= 1.2:
                ls = 0.40
                ls_parts.append(f"S26破位{N}bar高(余量{margin:.2f}×ATR):+0.40")
                if volume_ratio >= 1.5:
                    ls += 0.15
                    ls_parts.append(f"量能推力(×{volume_ratio:.2f}≥1.5):+0.15")
                if atr_pct > 0 and _r3 >= 0.6 * atr_pct:
                    ls += 0.15
                    ls_parts.append(f"3bar动量(+{_r3:.2f}%≥0.6×ATR):+0.15")
                if ema_trend == "up":
                    ls += 0.15
                    ls_parts.append("EMA对齐up:+0.15")
                if margin >= 0.25:
                    ls += 0.10
                    ls_parts.append("破位余量≥0.25×ATR:+0.10")
            else:
                ls_parts.append(f"blowoff弃单(余量{margin:.2f}>1.2×ATR)")
        elif c0 < prior_low:
            margin_s = (prior_low - c0) / atr_abs
            if margin_s <= 1.2:
                ss = 0.40
                ss_parts.append(f"S26破位{N}bar低(余量{margin_s:.2f}×ATR):+0.40")
                if volume_ratio >= 1.5:
                    ss += 0.15
                    ss_parts.append(f"量能推力(×{volume_ratio:.2f}≥1.5):+0.15")
                if atr_pct > 0 and _r3 <= -0.6 * atr_pct:
                    ss += 0.15
                    ss_parts.append(f"3bar动量({_r3:.2f}%≤-0.6×ATR):+0.15")
                if ema_trend == "down":
                    ss += 0.15
                    ss_parts.append("EMA对齐down:+0.15")
                if margin_s >= 0.25:
                    ss += 0.10
                    ss_parts.append("破位余量≥0.25×ATR:+0.10")
            else:
                ss_parts.append(f"blowoff弃单(余量{margin_s:.2f}>1.2×ATR)")
    long_thr = Config.MIN_CONFIDENCE + Config.LONG_CONF_PREMIUM
    short_thr = Config.MIN_CONFIDENCE + Config.SHORT_CONF_PREMIUM
    direction: Optional[str] = None
    if ls >= long_thr:
...
def analyze_with_detail(
    ...
    # S26 @2026-08-09 breakout-follow fork: 评分核整体替换为突破追动量;
    # 均值回归核 score_confidence_detail 弃用档案化(在场不可达), 其活跃演化在 main/trend/fade。
    confidence, direction, detail = score_breakout_detail(
        price, closes, atr_pct, atr_abs, ema_trend, volume_ratio, rsi_val
    )
    ...
    if direction is not None:
        ...
        elif False:  # S26: 趋势稳定门弃用(破位初段EMA常刚翻转,与原型冲突;档案化)
        ...
        # 3) 震荡过滤
        if False and Config.REJECT_ON_CHOP and is_choppy(closes, Config.CHOP_LOOKBACK):  # S26: 震荡门弃用(盘整后破位=原型标的;档案化)
            reject_reason = "震荡走势"
        ...
        if False and direction == "short" and rsi_val < Config.RSI_SHORT_REJECT_BELOW:  # S26: RSI极端拒弃用(破位单天然处极端区;档案化)
        ...
        if False and Config.MAX_ENTRY_EXT_ATR > 0 and atr_abs > 0:  # S26: 追单门由原生blowoff余量帽(≤1.2×ATR)替代;档案化
