...
class Strategy:
    ...
    def _log_stats_summary(self):
        ...
        self._log(
            ...
            f"冷却跳过={g('cooldown_skip', 0)} 择优批={g('pick_batch', 0)} 择优发出={g('pick_emit', 0)} 择优落选={g('pick_lose', 0)} "
    ...
    def _emit_signal(self, symbol: str, direction: str, entry_price: float, tp: float, sl: float, confidence: float):
        if self._symbol_denied(symbol):
            ...
            self._stat("bl_block")
            return
        # S17 @2026-08-01 BRAKE 临时禁short：逐笔证据 12h short n44 net-37.40 wr9.4% / 24h n65 -38.41,
        # 跨8+币非单币=short入场信号当前regime失效(宏观平稳独血)。6h刹车线触发的降险一步。
        # 与 config allowed_sides=["buy"] 双保险(config PATCH受持仓锁,本gate为保底执行点)。
        # RECOVER条件(6h/24h双转正)满足后经 apply 移除本gate并恢复config双向。
        if direction == "short":
            self._log(f"S17 BRAKE禁short拦截 sym={symbol} 置信度={confidence:.4f} 信号已丢弃 时间={_now()}")
            self._stat("s17_short_block")
            return
        ...
        self.shadow_open[symbol] = {"entry": float(entry_price), "tp": float(tp) if tp else 0.0, "sl": float(sl) if sl else 0.0, "direction": direction, "ts": time.time()}
