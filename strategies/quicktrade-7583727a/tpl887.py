...
def score_confidence_detail(
    ...
    if rsi_val < 35:
        ...
        # S25 @2026-08-09 fade-short定制: 深超买第二档(衰竭区). RSI维度与S19/S20的bar动量·
        # 拉升语境维正交=无gate冲突; 依据=fade池主盈形态为冲高回落空(48h S净 BTW+5.85/
        # 1000CAT+4.57/BLUAI+3.07/AIOT+1.50), 深衰竭区空单是其中最强亚型; 0.60锚不动。
        if rsi_val > 75:
            ss += 0.10
            ss_parts.append("S25 RSI>75衰竭区:+0.10")
    ...
    if False:  # S25 @2026-08-09 fade-short fork: 多头出口封闭(sides=[sell]死代码档案化; S18多侧veto原样保留但不可达, 活跃继承在 trend fork S24)
        direction = "long"
        conf = ls
    elif ss >= short_thr - 1e-9:  # S28 附随 @2026-08-23: #65浮点边界epsilon永久化——short_thr=0.55+0.05
        # =0.6000000000000001, 精确0.60组合(如RSI>75衰竭0.30+布林上轨0.15+MACD死叉0.15=S28主通道)
        # 差2.2e-16被静默丢弃(08-22活体六行实证)。epsilon=1e-9远大于双精度累积误差、远小于任何真实分差。
        direction = "short"
        conf = ss
    else:
        conf = max(ls, ss)
...
def analyze_with_detail(
    ...
    if direction is not None:
        # S29 @2026-08-24 S28签名域ATR地板降档(gate1对签名bar 0.5→0.30):
        #   依据=08-24 08:44-47Z MELANIA放量冲顶(ss0.80/RSI79.8→88.4/触上轨/量比3.03-4.71)
        #   4bar全灭于"ATR%过低(0.30-0.35<0.5)"——S28旁路在gate2的elif链里, 签名bar在gate1
        #   就死, S28机制性不可达(48h census: ATR%过低=ss≥0.60第一杀手378/791)。机制: ATR
        #   回看窗滞后于垂直拉升, 冲顶起步时ATR%必然还压在地板下=地板恰在S28猎物出现时挡道。
        #   降档仅限完整签名bar(RSI>75∧触/破上轨∧末bar失速<0.3×ATR%), 地板保留0.30:
        #   HANA/COLLECT 0.11-0.26阴跌区仍全拒——非低波解锁(低波解锁不付费=组级知识,
        #   tpl806三值包n16净−1.00已证伪), 是签名域的定向解锁。内联不接config防回退覆盖。
        _s29_sig = False
        if (direction == "short" and rsi_val > 75.0 and bb_upper > 0
                and price >= bb_upper and atr_pct > 0 and len(closes) >= 2):
            _c1p = closes[-2]
            _r1p = (closes[-1] - _c1p) / _c1p * 100.0 if _c1p > 0 else 999.0
            if _r1p < 0.3 * atr_pct:
                _s29_sig = True
        _s29_floor = 0.30 if _s29_sig else Config.MIN_ATR_PCT_FOR_TRADE
        # 1) 极端波动两端：太死或太疯都拒
        if atr_pct > Config.MAX_ATR_PCT:
            reject_reason = f"ATR%过高({atr_pct:.2f}>{Config.MAX_ATR_PCT})"
        elif atr_pct < _s29_floor:
            reject_reason = f"ATR%过低({atr_pct:.2f}<{_s29_floor})"
        # 2) EMA fast/slow 关系刚翻转：whipsaw 主要来源
        elif Config.TREND_CONFIRM_BARS > 0:
            # S28 @2026-08-23 衰竭签名直入(fade原型正确性修复, §4#66三标本):
            #   继承的 EMA-down+趋稳 门要求"下跌趋势已成型"才许开空 → 冲高回落原型永远无法在
            #   顶点开空——等确认完成 ATR 尖峰已衰减回地板下(v2黄金窗COLLECT 37行≥0.55零发单),
            #   或整单被拒(main MELANIA 0.80分/RSI87 死于"EMA非down")。三要件同时成立时仅旁路
            #   本 gate(EMA方向+趋稳)这一门:
            #   ①RSI>75(深衰竭区, 与 S25 第二档同锚) ②价触/破布林上轨(过度延伸确认)
            #   ③末完成bar涨幅<0.3×ATR%(失速要件=顶点确认, 严于 S19 第一条款的0.6×,
            #     7月拉升中裸逆势空9笔−16.7 是本要件的存在理由——不接飞刀, 只接失速顶)。
            #   ATR带[min_atr,max_atr]/量能地板/RSI<32/追空ext/S19 3bar条款/S20拉升语境
            #   全部保留在后续 gate 继续把关; 评分门0.60另要求至少一项独立空头确认
            #   (MACD死叉/资金费/多空比)叠加签名才成立。阈值内联不接 config 防回退覆盖。
            _s28_fire = False
            if (direction == "short" and rsi_val > 75.0 and bb_upper > 0
                    and price >= bb_upper and atr_pct > 0 and len(closes) >= 2):
                _c1s = closes[-2]
                _rise1s = (closes[-1] - _c1s) / _c1s * 100.0 if _c1s > 0 else 999.0
                if _rise1s < 0.3 * atr_pct:
                    _s28_fire = True
                    detail = dict(detail)
                    detail["s28_exhaustion_entry"] = (
                        f"S28衰竭直入(RSI{rsi_val:.1f}>75 触上轨 末bar涨{_rise1s:.2f}%<0.3×ATR{atr_pct:.2f}%)"
                    )
            if not _s28_fire:
