...
def score_confidence_detail(
    ...
    # S24 @2026-08-09 trend-long定制: 稳步爬升续势加分. 3bar累涨∈[0.2,1.0]×ATR%=趋势中继
    # (尖峰追高域仍由gate6反追高与S18看护; 上界1.0×ATR排除尖峰bar). 依据=trend池主盈形态
    # 为爬升续势长单(48h L净 TUT+13.39/KAITO+8.12/BICO+5.05), 0.45档干净续势单(EMA up+MACD)
    # 加此0.10后过0.55底线, 打开中继入场漏斗; 门槛0.70锚不动。
    if closes is not None and atr_pct > 0 and len(closes) >= 4 and closes[-4] > 0:
        _rise3_s24 = (closes[-1] - closes[-4]) / closes[-4] * 100.0
        if 0.2 * atr_pct <= _rise3_s24 <= 1.0 * atr_pct:
            ls += 0.10
            ls_parts.append(f"S24续势(3bar+{_rise3_s24:.2f}%∈[0.2,1.0]×ATR):+0.10")
    ...
    if ls >= long_thr:
        direction = "long"
        conf = ls
    elif False:  # S24 @2026-08-09 trend-long fork: 空头出口封闭(sides=[buy]死代码档案化; S19/S20空侧保护栈原样保留但不可达, 活跃继承在 fade fork S25)
        direction = "short"
        conf = ss
    else:
        conf = max(ls, ss)
